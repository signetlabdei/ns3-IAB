      ...
      if (info.flags & TypeId::ATTR_CONSTRUCT && info.accessor->HasSetter ())
        {
          ...
          os << "    " << listLineStart << "Initial value: " << value << listLineStop << std::endl;
        }
  ...
  for (GlobalValue::Iterator i = GlobalValue::Begin (); i != GlobalValue::End (); ++i)
    {
      ...
      os << indentHtmlOnly << listLineStart << boldStart << hrefStart << (*i)->GetName () << hrefMid
         << "GlobalValue" << (*i)->GetName () << hrefStop << boldStop << ": " << (*i)->GetHelp ()
 ...
 */
void
PrintAttributeValueWithName (std::ostream &os, const std::string &name, const std::string &type,
...
PrintAttributeImplementations (std::ostream &os)
{
  ...
  const AttributeDescriptor attributes[] = {
      ...
      {"OrganizationIdentifier", "OrganizationIdentifier", true, "vendor-specific-action.h"},

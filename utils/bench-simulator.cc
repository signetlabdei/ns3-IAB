...
public:
   ...
   */
  void
  SetRandomStream (Ptr<RandomVariableStream> stream)
  {
    m_rand = stream;
  }
   ...
   */
  void
  SetPopulation (const uint32_t population)
  {
...
Bench::RunBench (void)
{
  ...
  LOG (std::setw (g_fwidth) << init << std::setw (g_fwidth) << (m_population / init)
                            << std::setw (g_fwidth) << (init / m_population) << std::setw (g_fwidth)
                            << simu << std::setw (g_fwidth) << (m_count / simu)
...
main (int argc, char *argv[])
{
  ...
  LOG (std::left << std::setw (g_fwidth) << "Run #" << std::left << std::setw (3 * g_fwidth)
                 << "Inititialization:" << std::left << std::setw (3 * g_fwidth) << "Simulation:");

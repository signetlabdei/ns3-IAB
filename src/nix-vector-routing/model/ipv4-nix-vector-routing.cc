...
Ipv4NixVectorRouting::Ipv4NixVectorRouting () : m_totalNeighbors (0)
{
...
bool
Ipv4NixVectorRouting::BuildNixVector (const std::vector<Ptr<Node>> &parentVector, uint32_t source,
                                      uint32_t dest, Ptr<NixVector> nixVector)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
Ipv4NixVectorRouting::GetAdjacentNetDevices (Ptr<NetDevice> netDevice, Ptr<Channel> channel,
                                             NetDeviceContainer &netDeviceContainer)
{
  NS_LOG_FUNCTION_NOARGS ();
              ...
              if (!DynamicCast<LoopbackNetDevice> (device))
                {
                  int32_t interfaceIndex =
                      (ipv4)->GetInterfaceForDevice (node->GetDevice (deviceId));
                  if (interfaceIndex != -1)
                    {
                      ...
                      NS_LOG_LOGIC ("Adding IPv4 address "
      ...
      if (ndTest->IsBridge ())
        {
          ...
          NS_ABORT_MSG_UNLESS (bnd, "Ipv4NixVectorRouting::NetDeviceIsBridged (): GetObject for "
...
Ptr<Ipv4Route>
Ipv4NixVectorRouting::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif,
...
bool
Ipv4NixVectorRouting::RouteInput (Ptr<const Packet> p, const Ipv4Header &header,
...
Ipv4NixVectorRouting::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  ...
  *os << "Node: " << m_ipv4->GetObject<Node> ()->GetId () << ", Time: " << Now ().As (unit)
      << ", Local time: " << GetObject<Node> ()->GetLocalTime ().As (unit) << ", Nix Routing"
      << std::endl;
  ...
  if (m_ipv4RouteCache.size () > 0)
    {
      *os << "Destination     Gateway         Source            OutputDevice" << std::endl;
      for (Ipv4RouteMap_t::const_iterator it = m_ipv4RouteCache.begin ();
           it != m_ipv4RouteCache.end (); it++)
        {
...
bool
Ipv4NixVectorRouting::BFS (uint32_t numberOfNodes, Ptr<Node> source, Ptr<Node> dest,
                           std::vector<Ptr<Node>> &parentVector, Ptr<NetDevice> oif)
{
  NS_LOG_FUNCTION_NOARGS ();

...
template <typename T>
bool
NixVectorRouting<T>::BuildNixVector (const std::vector<Ptr<Node>> &parentVector, uint32_t source,
                                     uint32_t dest, Ptr<NixVector> nixVector) const
{
...
template <typename T>
void
NixVectorRouting<T>::GetAdjacentNetDevices (Ptr<NetDevice> netDevice, Ptr<Channel> channel,
                           ...
                           addressIndex++)
                        {
                          ...
                          NS_ABORT_MSG_IF (
                              ...
                              "Duplicate IP address ("
                                  << addr << ") found during NIX Vector map construction for node "
                                  << node->GetId ());
...
template <typename T>
uint32_t
NixVectorRouting<T>::FindNetDeviceForNixIndex (Ptr<Node> node, uint32_t nodeIndex,
...
template <typename T>
Ptr<typename NixVectorRouting<T>::IpRoute>
NixVectorRouting<T>::RouteOutput (Ptr<Packet> p, const IpHeader &header, Ptr<NetDevice> oif,
  ...
  if constexpr (!IsIpv4)
    {
      ...
      if (destAddress.IsLinkLocalMulticast ())
        {
          NS_ASSERT_MSG (
              oif, "Try to send on link-local multicast address, and no interface index is given!");
          rtentry = Create<IpRoute> ();
          rtentry->SetSource (
...
template <typename T>
bool
NixVectorRouting<T>::RouteInput (Ptr<const Packet> p, const IpHeader &header,
...
template <typename T>
void
NixVectorRouting<T>::NotifyAddRoute (IpAddress dst, Ipv6Prefix mask, IpAddress nextHop,
                                     uint32_t interface, IpAddress prefixToUse)
{
  g_isCacheDirty = true;
}
template <typename T>
void
NixVectorRouting<T>::NotifyRemoveRoute (IpAddress dst, Ipv6Prefix mask, IpAddress nextHop,
                                        uint32_t interface, IpAddress prefixToUse)
{
  g_isCacheDirty = true;
}
...
template <typename T>
bool
NixVectorRouting<T>::BFS (uint32_t numberOfNodes, Ptr<Node> source, Ptr<Node> dest,
...
template void NixVectorRouting<Ipv4RoutingProtocol>::PrintRoutingPath (
    Ptr<Node> source, IpAddress dest, Ptr<OutputStreamWrapper> stream, Time::Unit unit) const;

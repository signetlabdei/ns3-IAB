          ...
          for (int j = 0; j < nLANClients; ++j)
            {
              ...
              if (!useIpv6)
                {
                  ifs2LanRemoteAddress[z][i][j] = InetSocketAddress (
                      addressHelperv4.Assign (ndc2LAN[i][j]).GetAddress (0), 9999);
                }
              else
                {
                  ifs2LanRemoteAddress[z][i][j] = Inet6SocketAddress (
                      addressHelperv6.Assign (ndc2LAN[i][j]).GetAddress (0, 1), 9999);
                }
            }
        }
      // Create Net3
          ...
          for (int j = 0; j < nLANClients; ++j)
            {
              ...
              if (!useIpv6)
                {
                  ifs3LanRemoteAddress[z][i][j] = InetSocketAddress (
                      addressHelperv4.Assign (ndc3LAN[i][j]).GetAddress (0), 9999);
                }
              else
                {
                  ifs3LanRemoteAddress[z][i][j] = Inet6SocketAddress (
                      addressHelperv6.Assign (ndc3LAN[i][j]).GetAddress (0, 1), 9999);
                }
            }
        }

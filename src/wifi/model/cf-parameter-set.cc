...
uint8_t
CfParameterSet::DeserializeInformationField (Buffer::Iterator start, uint8_t length)
{
  NS_ASSERT (length == 6);
...
{
  os << (uint16_t) cfParameterSet.GetCFPCount () << "|" << (uint16_t) cfParameterSet.GetCFPPeriod ()

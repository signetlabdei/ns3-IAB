...
struct IdealWifiRemoteStation : public WifiRemoteStation
{
  ...
  uint16_t
      m_lastChannelWidthObserved; //!< Channel width (in MHz) of most recently reported packet sent to the remote station
  uint16_t
      m_lastNssObserved; //!<  Number of spatial streams of most recently reported packet sent to the remote station
  ...
  static TypeId tid =
      TypeId ("ns3::IdealWifiManager")
          ...
          .AddAttribute ("BerThreshold",
                         ...
                         DoubleValue (1e-6), MakeDoubleAccessor (&IdealWifiManager::m_ber),
...
IdealWifiManager::IdealWifiManager () : m_currentRate (0)
{
...
void
IdealWifiManager::DoReportRtsOk (WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode,
                                 double rtsSnr)
{
...
void
IdealWifiManager::DoReportAmpduTxStatus (WifiRemoteStation *st, uint8_t nSuccessfulMpdus,
...
IdealWifiManager::DoGetDataTxVector (WifiRemoteStation *st)
{
  ...
  if ((station->m_lastSnrCached != CACHE_INITIAL_VALUE) &&
      ...
      (channelWidth == station->m_lastChannelWidth))
    {
      ...
      NS_LOG_DEBUG ("Using cached mode = "
                    << maxMode.GetUniqueName () << " last snr observed "
                    << station->m_lastSnrObserved << " cached " << station->m_lastSnrCached
                    << " channel width " << station->m_lastChannelWidth << " nss " << +selectedNss);
    }
  else
    {
          ...
          for (uint8_t i = 0; i < GetNSupported (station); i++)
            {
              ...
              txVector.SetChannelWidth (channelWidth);
              double threshold = GetSnrThreshold (txVector);
              uint64_t dataRate = mode.GetDataRate (
                  txVector.GetChannelWidth (), txVector.GetGuardInterval (), txVector.GetNss ());
  ...
  if (m_currentRate != maxMode.GetDataRate (channelWidth, guardInterval, selectedNss))
    {
      NS_LOG_DEBUG (
          "New datarate: " << maxMode.GetDataRate (channelWidth, guardInterval, selectedNss));
      m_currentRate = maxMode.GetDataRate (channelWidth, guardInterval, selectedNss);
    }
  return WifiTxVector (
      maxMode, GetDefaultTxPowerLevel (),
      ...
      guardInterval, GetNumberOfAntennas (), selectedNss, 0,
      GetChannelWidthForTransmission (maxMode, channelWidth), GetAggregation (station));
}
...
IdealWifiManager::DoGetRtsTxVector (WifiRemoteStation *st)
{
  ...
  return WifiTxVector (
      ...
      GetPreambleForTransmission (maxMode.GetModulationClass (), GetShortPreambleEnabled (),
                                  UseGreenfieldForDestination (GetAddress (station))),
      800, GetNumberOfAntennas (), nss, 0, GetChannelWidthForNonHtMode (maxMode),
      GetAggregation (station));
}

double
IdealWifiManager::GetLastObservedSnr (IdealWifiRemoteStation *station, uint16_t channelWidth,
                                      uint8_t nss) const
{

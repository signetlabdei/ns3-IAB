  ...
  static TypeId tid =
      TypeId ("ns3::ThompsonSamplingWifiManager")
          ...
          .AddAttribute (
              ...
              DoubleValue (1.0), MakeDoubleAccessor (&ThompsonSamplingWifiManager::m_decay),
...
void
ThompsonSamplingWifiManager::DoReportRxOk (WifiRemoteStation *station, double rxSnr,
                                           WifiMode txMode)
{
  ...
  for (uint32_t i = 0; i < station->m_mcsStats.size (); i++)
    {
      ...
      double rate = mode.GetDataRate (station->m_mcsStats.at (i).channelWidth, guardInterval,
...
ThompsonSamplingWifiManager::DoGetDataTxVector (WifiRemoteStation *st, uint16_t allowedWidth)
{
  ...
  return WifiTxVector (
      ...
      GetModeGuardInterval (st, mode), GetNumberOfAntennas (), nss,
      0, // NESS
      GetChannelWidthForTransmission (mode, channelWidth), GetAggregation (station), false);
}

...
struct MinstrelHtRateInfo
{
  ...
  double
      prob; //!< Current probability within last time interval. (# frame success )/(# total frames)
  ...
  uint32_t
      numSamplesSkipped; //!< Number of times this rate statistics were not updated because no attempts have been made.
...
static const uint8_t MAX_HT_STREAM_GROUPS =
    4; //!< Maximal number of groups per stream in HT (2 possible channel widths and 2 possible GI configurations).
static const uint8_t MAX_VHT_STREAM_GROUPS =
    8; //!< Maximal number of groups per stream in VHT (4 possible channel widths and 2 possible GI configurations).
static const uint8_t MAX_HE_STREAM_GROUPS =
    12; //!< Maximal number of groups per stream in HE (4 possible channel widths and 3 possible GI configurations).
...
private:
   ...
   */
  Time CalculateMpduTxDuration (Ptr<WifiPhy> phy, uint8_t streams, uint16_t gi, uint16_t chWidth,
   ...
   */
  void UpdatePacketCounters (MinstrelHtWifiRemoteStation *station, uint16_t nSuccessfulMpdus,
   ...
   */
  double CalculateThroughput (MinstrelHtWifiRemoteStation *station, uint8_t groupId, uint8_t rateId,
                              double ewmaProb);
   ...
   */
  Time CalculateTimeUnicastPacket (Time dataTransmissionTime, uint32_t shortRetries,
  ...
  Time
      m_legacyUpdateStats; //!< How frequent do we calculate the stats for legacy MinstrelWifiManager.
  ...
  bool
      m_useLatestAmendmentOnly; //!< Flag if only the latest supported amendment by both peers should be used.
  ...
  Ptr<MinstrelWifiManager>
      m_legacyManager; //!< Pointer to an instance of MinstrelWifiManager. Used when 802.11n/ac/ax not supported.

  ...
  static TypeId tid =
      TypeId ("ns3::RraaWifiManager")
          ...
          .AddAttribute (
              ...
              BooleanValue (false), MakeBooleanAccessor (&RraaWifiManager::m_basic),
              MakeBooleanChecker ())
          .AddAttribute ("Timeout", "Timeout for the RRAA BASIC loss estimation block",
                         TimeValue (Seconds (0.05)), MakeTimeAccessor (&RraaWifiManager::m_timeout),
                         MakeTimeChecker ())
          .AddAttribute (
              "FrameLength", "The Data frame length (in bytes) used for calculating mode TxTime.",
              UintegerValue (1420), MakeUintegerAccessor (&RraaWifiManager::m_frameLength),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("AckFrameLength",
                         "The Ack frame length (in bytes) used for calculating mode TxTime.",
                         UintegerValue (14), MakeUintegerAccessor (&RraaWifiManager::m_ackLength),
                         MakeUintegerChecker<uint32_t> ())
          ...
          .AddAttribute ("Beta", "Constant for calculating the ORI threshold.", DoubleValue (2),
                         ...
                         MakeDoubleChecker<double> (1))
          .AddAttribute ("Tau", "Constant for calculating the EWND size.", DoubleValue (0.012),
...
RraaWifiManager::RraaWifiManager () : WifiRemoteStationManager (), m_currentRate (0)
{
...
void
RraaWifiManager::DoReportRtsOk (WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode,
                                double rtsSnr)
{

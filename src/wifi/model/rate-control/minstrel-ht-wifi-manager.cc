...
void
MinstrelHtWifiManager::DoReportAmpduTxStatus (WifiRemoteStation *st, uint16_t nSuccessfulMpdus,
  ...
  if (!station->m_isHt)
  ...
  else
    {
      ...
      return WifiTxVector (
          ...
          800, 1, 1, 0,
          GetChannelWidthForTransmission (rtsRate, GetPhy ()->GetChannelWidth (),
                                          GetChannelWidth (station)),
...
MinstrelHtWifiManager::GetVhtGroupId (uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
  ...
  groupId += (MAX_VHT_SUPPORTED_STREAMS * 2 * widthIndex) + (MAX_VHT_SUPPORTED_STREAMS * giIndex) +
             txstreams - 1;
  return groupId;
}

                              ...
                              Time availableTime) const
{
  ...
  Ptr<WifiMacQueueItem> amsdu =
      peekedItem->GetItem (); // amsdu points to the peeked MPDU, but it's non-const
  uint8_t nMsdu = 1;
  ...
  while (peekedItem && m_htFem->TryAggregateMsdu (peekedItem, txParams, availableTime))
    {
      ...
      queue->Transform (amsdu, [&msdu] (Ptr<WifiMacQueueItem> amsdu) { amsdu->Aggregate (msdu); });
  ...
  if (modulation >= WIFI_MOD_CLASS_VHT)
    {
      ...
      maxAmsduSize = std::min (maxAmsduSize,
                               static_cast<uint16_t> (vhtCapabilities->GetMaxMpduLength () - 56));
    }

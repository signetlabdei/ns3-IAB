...
void
ReducedNeighborReport::SetBssParameters (std::size_t nbrApInfoId, std::size_t index,
...
void
ReducedNeighborReport::SetMldParameters (std::size_t nbrApInfoId, std::size_t index, uint8_t mldId,
                                         uint8_t linkId, uint8_t changeCount)
{
  NS_ASSERT (nbrApInfoId < m_nbrApInfoFields.size ());
  NS_ASSERT (index < m_nbrApInfoFields.at (nbrApInfoId).tbttInformationSet.size ());
      ...
      for (uint8_t j = 0; j < ReadTbttInformationCount (neighborId); j++)
        {
          ...
          if (m_nbrApInfoFields.back ().hasMldParams)
            {
              m_nbrApInfoFields.back ().tbttInformationSet.back ().mldParameters.mldId =
                  i.ReadU8 ();

...
public:
   ...
   */
  void
  AccumulateFirstFragment (Ptr<const Packet> packet)
  {
   ...
   */
  Ptr<Packet>
  AccumulateLastFragment (Ptr<const Packet> packet)
  {
    NS_ASSERT (m_defragmenting);
    m_fragments.push_back (packet);
    m_defragmenting = false;
   ...
   */
  void
  AccumulateFragment (Ptr<const Packet> packet)
  {
    NS_ASSERT (m_defragmenting);
    m_fragments.push_back (packet);
  }
   ...
   */
  bool
  IsNextFragment (uint16_t sequenceControl) const
  {
   ...
   */
  uint16_t
  GetLastSequenceControl (void) const
  {
   ...
   */
  void
  SetSequenceControl (uint16_t sequenceControl)
  {

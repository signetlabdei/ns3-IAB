...
WifiMacQueue::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::WifiMacQueue")
          ...
          .AddAttribute ("MaxSize", "The max queue size", QueueSizeValue (QueueSize ("500p")),
          ...
          .AddAttribute ("MaxDelay",
                         ...
                         MakeTimeAccessor (&WifiMacQueue::SetMaxDelay), MakeTimeChecker ())
          .AddAttribute ("DropPolicy",
                         "Upon enqueue with full queue, drop oldest (DropOldest) or newest "
                         "(DropNewest) packet",
                         EnumValue (DROP_NEWEST), MakeEnumAccessor (&WifiMacQueue::m_dropPolicy),
...
WifiMacQueue::WifiMacQueue (AcIndex ac) : m_ac (ac), NS_LOG_TEMPLATE_DEFINE ("WifiMacQueue")
{
}
  ...
  if (Queue<WifiMacQueueItem>::DoEnqueue (pos, item, ret))
    {
      ...
      if (item->GetHeader ().IsQosData ())
        {
          WifiAddressTidPair addressTidPair (item->GetHeader ().GetAddr1 (),

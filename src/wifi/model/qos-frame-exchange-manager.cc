...
bool
QosFrameExchangeManager::StartFrameExchange (Ptr<QosTxop> edca, Time availableTime,
                                             bool initialFrame)
{
                                                  ...
                                                  Time ppduDurationLimit) const
{
  ...
  return IsWithinSizeAndTimeLimits (mpdu->GetSize (), mpdu->GetHeader ().GetAddr1 (), txParams,
                                    ppduDurationLimit);
}
...
Time
QosFrameExchangeManager::GetRtsDurationId (const WifiTxVector &rtsTxVector, Time txDuration,
                                           Time response) const
{
...
Time
QosFrameExchangeManager::GetCtsToSelfDurationId (const WifiTxVector &ctsTxVector, Time txDuration,
                                                 Time response) const
{
          ...
          if (hdr.IsQosData () && hdr.IsQosEosp ())
            {
              ...
              StaticCast<ApWifiMac> (m_mac)->SetBufferStatus (hdr.GetQosTid (), hdr.GetAddr2 (),
                                                              hdr.GetQosQueueSize ());
            }
        }
    }
      ...
      if (hdr.GetAddr2 () == m_txopHolder || m_navEnd <= Simulator::Now ())
        {
          ...
          Simulator::Schedule (m_phy->GetSifs (), &QosFrameExchangeManager::SendCtsAfterRts, this,
      ...
      if (hdr.GetAddr1 () == m_self && hdr.GetQosAckPolicy () == WifiMacHeader::NORMAL_ACK)
        {
          NS_LOG_DEBUG ("Received " << hdr.GetTypeString () << " from=" << hdr.GetAddr2 ()
                                    << ", schedule ACK");
          Simulator::Schedule (m_phy->GetSifs (), &QosFrameExchangeManager::SendNormalAck, this,
                               hdr, txVector, rxSnr);
        }

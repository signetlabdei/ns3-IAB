...
ConstantObssPdAlgorithm::ConstantObssPdAlgorithm () : ObssPdAlgorithm ()
{
...
ConstantObssPdAlgorithm::ConnectWifiNetDevice (const Ptr<WifiNetDevice> device)
{
  ...
  phy->TraceConnectWithoutContext ("EndOfHePreamble",
                                   MakeCallback (&ConstantObssPdAlgorithm::ReceiveHeSig, this));
      ...
      if (WToDbm (params.rssiW) < m_obssPdLevel)
        {
          NS_LOG_DEBUG ("Frame is OBSS and RSSI " << WToDbm (params.rssiW)
                                                  << " is below OBSS-PD level of " << m_obssPdLevel
                                                  << "; reset PHY to IDLE");
          ResetPhy (params);
        }
      else

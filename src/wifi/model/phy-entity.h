...
public:
   ...
   */
  virtual Time GetPayloadDuration (uint32_t size, const WifiTxVector &txVector, WifiPhyBand band,
                                   MpduType mpdutype, bool incFlag, uint32_t &totalAmpduSize,
                                   double &totalAmpduNumSymbols, uint16_t staId) const = 0;
   ...
   */
  virtual Ptr<WifiPpdu> BuildPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector,
                                   Time ppduDuration);
   ...
   */
  void Transmit (Time txDuration, Ptr<const WifiPpdu> ppdu, const WifiTxVector &txVector,
                 std::string type);
   ...
   */
  virtual Time CalculateTxDuration (WifiConstPsduMap psduMap, const WifiTxVector &txVector,
...
protected:
   ...
   */
  std::pair<bool, SignalNoiseDbm> GetReceptionStatus (Ptr<const WifiPsdu> psdu, Ptr<Event> event,
                                                      uint16_t staId, Time relativeMpduStart,
                                                      Time mpduDuration);
  /**
   ...
   * \param mpduDuration the duration of the MPDU
   */
  void EndOfMpdu (Ptr<Event> event, Ptr<const WifiPsdu> psdu, size_t mpduIndex, Time relativeStart,
                  Time mpduDuration);
   ...
   */
  virtual std::pair<uint16_t, WifiSpectrumBand>
  ...
  GetCurrentPreambleEvents (void) const;
  /**
   ...
   */
  Ptr<Event> CreateInterferenceEvent (Ptr<const WifiPpdu> ppdu, const WifiTxVector &txVector,
   ...
   */
  virtual Ptr<SpectrumValue> GetTxPowerSpectralDensity (double txPowerW, Ptr<const WifiPpdu> ppdu,
  ...
  using CcaIndication = std::optional<std::pair<
      Time,
      WifiChannelListType>>; //!< CCA end time and its corresponding channel list type (can be std::nullopt if IDLE)
  ...
  Ptr<WifiPhyStateHelper>
      m_state; //!< Pointer to WifiPhyStateHelper of the WifiPhy (to make it reachable for child classes)
  ...
  std::map<UidStaIdPair, std::vector<bool>>
      m_statusPerMpduMap; //!< Map of the current reception status per MPDU that is filled in as long as MPDUs are being processed by the PHY in case of an A-MPDU
  std::map<UidStaIdPair, SignalNoiseDbm>
      m_signalNoiseMap; //!< Map of the latest signal power and noise power in dBm (noise power includes the noise figure)

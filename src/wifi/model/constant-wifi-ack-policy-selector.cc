...
{
  static TypeId tid =
      TypeId ("ns3::ConstantWifiAckPolicySelector")
...
void
ConstantWifiAckPolicySelector::UpdateTxParams (Ptr<WifiPsdu> psdu,
                                               MacLowTransmissionParameters &params)
{
  ...
  uint16_t maxDistToStartingSeq =
      psdu->GetMaxDistFromStartingSeq (m_qosTxop->GetBaStartingSequence (receiver, tid));
  ...
  bool isResponseNeeded =
      (maxDistToStartingSeq >= m_baThreshold * m_qosTxop->GetBaBufferSize (receiver, tid) ||
       m_qosTxop->PeekNextFrame (tid, receiver) == 0 ||
       (m_qosTxop->GetTxopLimit ().IsStrictlyPositive () &&
        m_qosTxop->GetTxopRemaining () == m_qosTxop->GetTxopLimit () && !params.MustSendRts ()));

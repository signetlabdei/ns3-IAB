...
struct WifiRemoteStation
{
  ...
  std::pair<double, Time>
      m_rssiAndUpdateTimePair; //!< RSSI (in dBm) of the most recent packet received from the remote station along with update time
};
...
struct WifiRemoteStationState
{
   ...
   */
  enum { BRAND_NEW, DISASSOC, WAIT_ASSOC_TX_OK, GOT_ASSOC_TX_OK } m_state;
  ...
  uint16_t
      m_guardInterval; //!< HE Guard interval duration (in nanoseconds) supported by the remote station
...
public:
   ...
   */
  void ReportDataOk (Ptr<const WifiMacQueueItem> mpdu, double ackSnr, WifiMode ackMode,
...
private:
   ...
   */
  virtual bool DoNeedRetransmission (WifiRemoteStation *station, Ptr<const Packet> packet,
                                     bool normally);
  /**
   * \param station the station that we need to communicate
   * \param packet the packet to send
   ...
   */
  virtual bool DoNeedFragmentation (WifiRemoteStation *station, Ptr<const Packet> packet,
                                    bool normally);
  /**
   ...
   * \param rtsSnr the SNR of the RTS we sent
   */
  virtual void DoReportRtsOk (WifiRemoteStation *station, double ctsSnr, WifiMode ctsMode,
                              double rtsSnr) = 0;
  /**
   * This method is a pure virtual method that must be implemented by the sub-class.
   * This allows different types of WifiRemoteStationManager to respond differently,
   ...
   */
  virtual void DoReportAmpduTxStatus (WifiRemoteStation *station, uint16_t nSuccessfulMpdus,
  ...
  bool
      m_useNonErpProtection; //!< flag if protection for non-ERP stations against ERP transmissions is enabled
  bool
  ...
  ProtectionMode
      m_erpProtectionMode; //!< Protection mode for ERP stations when non-ERP stations are detected
  ProtectionMode

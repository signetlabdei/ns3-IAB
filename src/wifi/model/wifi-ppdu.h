...
public:
   ...
   */
  WifiPpdu (Ptr<const WifiPsdu> psdu, const WifiTxVector &txVector, uint16_t txCenterFreq,
   ...
   * \param uid the unique ID of this PPDU
   */
  WifiPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector, uint16_t txCenterFreq,
            uint64_t uid);
  /**
...
private:
  ...
  bool
      m_truncatedTx; //!< flag indicating whether the frame's transmission was aborted due to transmitter switch off
  uint8_t
      m_txPowerLevel; //!< the transmission power level (used only for TX and initializing the returned WifiTxVector)

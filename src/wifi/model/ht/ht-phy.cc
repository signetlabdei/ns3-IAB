...
/* *NS_CHECK_STYLE_OFF* */
const PhyEntity::PpduFormats HtPhy::m_htPpduFormats{{WIFI_PREAMBLE_HT_MF,
  ...
  switch (field)
    ...
    case WIFI_PPDU_FIELD_NON_HT_HEADER:
      ...
      case WIFI_PPDU_FIELD_TRAINING: {
        ...
        uint8_t nss =
            txVector
                .GetNssMax (); //so as to cover also HE MU case (see section 27.3.10.10 of IEEE P802.11ax/D4.0)
        if (nss < 3)
          {
            nDataLtf = nss;
          }
...
Time
HtPhy::GetTrainingDuration (const WifiTxVector &txVector, uint8_t nDataLtf,
                            uint8_t nExtensionLtf /* = 0 */) const
{
  NS_ABORT_MSG_IF (
      nDataLtf == 0 || nDataLtf > 4 || nExtensionLtf > 4 || (nDataLtf + nExtensionLtf) > 5,
      "Unsupported combination of data ("
          << +nDataLtf << ")  and extension (" << +nExtensionLtf
          << ")  LTFs numbers for HT"); //see IEEE 802.11-2016, section 19.3.9.4.6 "HT-LTF definition"
...
Time
HtPhy::GetPayloadDuration (uint32_t size, const WifiTxVector &txVector, WifiPhyBand band,
                           ...
                           double &totalAmpduNumSymbols, uint16_t staId) const
{
  ...
  uint8_t stbc =
      txVector.IsStbc ()
          ? 2
          : 1; //corresponding to m_STBC in Nsym computation (see IEEE 802.11-2016, equations (19-32) and (21-62))
...
Ptr<SpectrumValue>
HtPhy::GetTxPowerSpectralDensity (double txPowerW, Ptr<const WifiPpdu> /* ppdu */,
...
HtPhy::CreateHtMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      ...
      MakeBoundCallback (&GetHtCodeRate, index), MakeBoundCallback (&GetHtConstellationSize, index),
...
{
  return GetPhyRate (txVector.GetMode ().GetMcsValue (), txVector.GetChannelWidth (),
...
{
  return GetDataRate (txVector.GetMode ().GetMcsValue (), txVector.GetChannelWidth (),
                      txVector.GetGuardInterval (), txVector.GetNss ());
}
...
HtPhy::CalculateDataRate (Time symbolDuration, uint16_t usableSubCarriers,
                          uint16_t numberOfBitsPerSubcarrier, double codingRate, uint8_t nss)
{
  return nss * OfdmPhy::CalculateDataRate (symbolDuration, usableSubCarriers,
                                           numberOfBitsPerSubcarrier, codingRate);
}
...
HtPhy::GetCcaIndication (const Ptr<const WifiPpdu> ppdu)
{
  ...
  if (delayUntilCcaEnd.IsStrictlyPositive ())
    {
      return std::make_pair (delayUntilCcaEnd,

...
public:
  ...
  Time GetPayloadDuration (uint32_t size, const WifiTxVector &txVector, WifiPhyBand band,
                           MpduType mpdutype, bool incFlag, uint32_t &totalAmpduSize,
                           double &totalAmpduNumSymbols, uint16_t staId) const override;
   ...
   */
  virtual Time GetTrainingDuration (const WifiTxVector &txVector, uint8_t nDataLtf,
...
private:
  ...
  uint8_t
      m_maxSupportedNss; //!< Maximum supported number of spatial streams (used to build HT MCS indices)

...
EhtPhy::EhtPhy (bool buildModeList /* = true */) : HePhy (false) //don't add HE modes to list
{
...
EhtPhy::CreateEhtMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      "EhtMcs" + std::to_string (index), index, WIFI_MOD_CLASS_EHT, false,
...
EhtPhy::GetPhyRateFromTxVector (const WifiTxVector &txVector, uint16_t staId /* = SU_STA_ID */)
{
  ...
  return EhtPhy::GetPhyRate (txVector.GetMode (staId).GetMcsValue (), bw,
...
EhtPhy::GetDataRateFromTxVector (const WifiTxVector &txVector, uint16_t staId /* = SU_STA_ID */)
{
  ...
  return EhtPhy::GetDataRate (txVector.GetMode (staId).GetMcsValue (), bw,
                              txVector.GetGuardInterval (), txVector.GetNss (staId));
}

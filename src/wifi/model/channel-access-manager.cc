...
public:
   ...
   */
  PhyListener (ns3::ChannelAccessManager *cam) : m_cam (cam)
  {
  }
  virtual ~PhyListener ()
  {
          ...
          for (Txops::iterator j = i; j != m_txops.end (); j++, k++)
            {
              ...
              if (otherTxop->GetAccessStatus (m_linkId) == Txop::REQUESTED &&
                ...
                {
                  NS_LOG_DEBUG ("dcf "
                                << k << " needs access. backoff expired. internal collision. slots="
...
ChannelAccessManager::GetAccessGrantStart (bool ignoreNav) const
{
  ...
  if (ignoreNav)
    {
      accessGrantedStart =
          std::max ({rxAccessStart, busyAccessStart, txAccessStart, ackTimeoutAccessStart,

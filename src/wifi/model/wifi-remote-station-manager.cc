...
void
WifiRemoteStationManager::SetMldAddress (const Mac48Address &address,
...
WifiRemoteStationManager::GetCtsToSelfTxVector (void)
{
  ...
  return WifiTxVector (defaultMode, GetDefaultTxPowerLevel (), defaultPreamble,
                       ConvertGuardIntervalToNanoSeconds (defaultMode, m_wifiPhy->GetDevice ()),
                       GetNumberOfAntennas (), 1, 0,
...
void
WifiRemoteStationManager::ReportRtsOk (const WifiMacHeader &header, double ctsSnr, WifiMode ctsMode,
                                       double rtsSnr)
{
...
void
WifiRemoteStationManager::ReportRxOk (Mac48Address address, RxSignalInfo rxSignalInfo,
                                      WifiTxVector txVector)
{
...
void
WifiRemoteStationManager::ReportAmpduTxStatus (Mac48Address address, uint16_t nSuccessfulMpdus,
...
WifiRemoteStationManager::NeedRetransmission (Ptr<const WifiMacQueueItem> mpdu)
{
  ...
  return DoNeedRetransmission (Lookup (mpdu->GetHeader ().GetAddr1 ()), mpdu->GetPacket (),
                               normally);
}
...
WifiRemoteStationManager::NeedFragmentation (Ptr<const WifiMacQueueItem> mpdu)
{
  ...
  NS_LOG_DEBUG ("WifiRemoteStationManager::NeedFragmentation result: " << std::boolalpha
                                                                       << normally);
  return DoNeedFragmentation (Lookup (mpdu->GetHeader ().GetAddr1 ()), mpdu->GetPacket (),
                              normally);
}
...
uint32_t
WifiRemoteStationManager::GetFragmentSize (Ptr<const WifiMacQueueItem> mpdu,
...
uint32_t
WifiRemoteStationManager::GetFragmentOffset (Ptr<const WifiMacQueueItem> mpdu,
                                             uint32_t fragmentNumber)
{
  NS_LOG_FUNCTION (this << *mpdu << fragmentNumber);
  NS_ASSERT (!mpdu->GetHeader ().GetAddr1 ().IsGroup ());
...
bool
WifiRemoteStationManager::DoNeedFragmentation (WifiRemoteStation *station, Ptr<const Packet> packet,
                                               bool normally)
{
  return normally;
}

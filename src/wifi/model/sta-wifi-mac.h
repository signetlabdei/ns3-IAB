...
struct WifiScanParams
{
  ...
  enum : uint8_t { ACTIVE = 0, PASSIVE } type; ///< indicates either active or passive scanning
...
private:
   ...
   */
  void SetEdcaParameters (AcIndex ac, uint32_t cwMin, uint32_t cwMax, uint8_t aifsn,
                          Time txopLimit);
  /**
   ...
   */
  void SetMuEdcaParameters (AcIndex ac, uint16_t cwMin, uint16_t cwMax, uint8_t aifsn,
                            Time muEdcaTimer);
  /**

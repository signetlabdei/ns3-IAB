...
private:
  ...
  uint8_t
      m_numberOfSoundingDimensionsForSmallerOrEqualThan80Mhz; //!< # of sounding dimensions for < 80 MHz
  ...
  uint8_t
      m_rxFullBwSuInHeMuNonCompressedSigB; //!< RX full BW SU using HE MU PPDU with non-compressed SIGB
  uint8_t m_nominalPacketPadding; //!< Nominal packet padding
  uint8_t
      m_maxHeLtfRxInHeMuMoreThanOneRu; ///< max HE-LTF symbols STA can Rx in HE MU PPDU with more than one RU

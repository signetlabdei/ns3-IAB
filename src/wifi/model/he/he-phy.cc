...
const PhyEntity::PpduFormats HePhy::m_hePpduFormats{
    //Ignoring PE (Packet Extension)
    {WIFI_PREAMBLE_HE_SU,
     {WIFI_PPDU_FIELD_PREAMBLE, //L-STF + L-LTF
...
Time
HePhy::GetTrainingDuration (const WifiTxVector &txVector, uint8_t nDataLtf,
  ...
  if (txVector.IsDlMu ()) //See section 27.3.10.8 of IEEE 802.11ax draft 4.0.
    {
      ...
      if (bw <= 40)
        ...
        {
          commonFieldSize +=
              8 * (bw / 40) /* one allocation field per 40 MHz */ + 1 /* center RU */;
        }
       ...
       */
      std::pair<std::size_t, std::size_t> numStaPerContentChannel =
          txVector.GetNumRusPerHeSigBContentChannel ();
      std::size_t maxNumStaPerContentChannel =
          std::max (numStaPerContentChannel.first, numStaPerContentChannel.second);
      std::size_t maxNumUserBlockFields =
          maxNumStaPerContentChannel /
...
Time
HePhy::ConvertLSigLengthToHeTbPpduDuration (uint16_t length, const WifiTxVector &txVector,
                                            WifiPhyBand band)
{
  NS_ABORT_IF (!txVector.IsUlMu () || (txVector.GetModulationClass () < WIFI_MOD_CLASS_HE));
  uint8_t sigExtension = (band == WIFI_PHY_BAND_2_4GHZ ? 6 : 0);
...
HePhy::BuildPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector, Time ppduDuration)
{
  ...
  return Create<HePpdu> (psdus, txVector,
                         ...
                         ppduDuration, m_wifiPhy->GetPhyBand (), ObtainNextUid (txVector),
      ...
      if (m_currentMuPpduUid == ppdu->GetUid () && GetCurrentEvent ())
        {
          ...
          NS_ASSERT (m_beginOfdmaPayloadRxEvents.find (staId) ==
                     m_beginOfdmaPayloadRxEvents.end ());
      ...
      else
        {
          ...
          NS_LOG_INFO (
              "Consider OFDMA part of the PPDU as interference since device dropped the preamble");
...
HePhy::ProcessSigA (Ptr<Event> event, PhyFieldRxStatus status)
{
  ...
  NotifyEndOfHeSigA (
      params); //if OBSS_PD CCA_RESET, set power restriction first and wait till field is processed before switching to IDLE
  ...
  if (status.isSuccess)
    {
      ...
      if (ppdu->GetType () != WIFI_PPDU_TYPE_DL_MU &&
          !GetAddressedPsduInPpdu (
              ppdu)) //Final decision on STA-ID correspondence of DL MU is delayed to end of SIG-B
        {
...
HePhy::DoStartReceivePayload (Ptr<Event> event)
{
  ...
  Time payloadDuration = ConvertLSigLengthToHeTbPpduDuration (txVector.GetLength (), txVector,
...
HePhy::GetRuBandForRx (const WifiTxVector &txVector, uint16_t staId) const
{
  ...
  band = m_wifiPhy->ConvertHeRuSubcarriers (
      channelWidth, GetGuardBandwidth (m_wifiPhy->GetChannelWidth ()), range,
...
void
HePhy::NotifyCcaBusy (const Ptr<const WifiPpdu> ppdu, Time duration,
...
void
HePhy::NotifyCcaBusy (Time duration, WifiChannelListType channelType,
        ...
        if (flag == HePpdu::PSD_NON_HE_PORTION)
          {
            ...
            return WifiSpectrumValueHelper::CreateDuplicated20MhzTxPowerSpectralDensity (
                centerFrequency, channelWidth, txPowerW, GetGuardBandwidth (channelWidth),
                std::get<0> (txMaskRejectionParams), std::get<1> (txMaskRejectionParams),
                std::get<2> (txMaskRejectionParams), puncturedSubchannels);
          }
        else
          {
            const auto band = GetRuBandForTx (
                ppdu->GetTxVector (),
                GetStaId (
                    hePpdu)); // Use TXVECTOR from PPDU since the one passed by the MAC does not have PHY index set
            return WifiSpectrumValueHelper::CreateHeMuOfdmTxPowerSpectralDensity (
                centerFrequency, channelWidth, txPowerW, GetGuardBandwidth (channelWidth), band);
          }
      }
  ...
  if (ppdu->GetType () == WIFI_PPDU_TYPE_UL_MU || ppdu->GetType () == WIFI_PPDU_TYPE_DL_MU)
    {
      ...
      Simulator::Schedule (nonOfdmaDuration, &PhyEntity::Transmit, this, ofdmaDuration, hePpdu,
                           txVector, "OFDMA transmission");
    }
  else
    {
...
Time
HePhy::CalculateTxDuration (WifiConstPsduMap psduMap, const WifiTxVector &txVector,
...
HePhy::CreateHeMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      ...
      MakeBoundCallback (&GetCodeRate, index), MakeBoundCallback (&GetConstellationSize, index),
      MakeCallback (&GetPhyRateFromTxVector), MakeCallback (&GetDataRateFromTxVector),
      MakeBoundCallback (&GetNonHtReferenceRate, index), MakeCallback (&IsAllowed));
}
  ...
  switch (constellationSize)
    ...
    case 1024:
      if (codeRate == WIFI_CODE_RATE_3_4 || codeRate == WIFI_CODE_RATE_5_6)
        ...
        {
          NS_FATAL_ERROR ("Trying to get reference rate for a MCS with wrong combination of coding "
                          "rate and modulation");
        }
      break;
    default:

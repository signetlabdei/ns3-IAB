...
public:
  ...
  Ptr<WifiPpdu> BuildPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector,
  ...
  Time CalculateTxDuration (WifiConstPsduMap psduMap, const WifiTxVector &txVector,
   ...
   */
  static std::pair<uint16_t, Time>
  ConvertHeTbPpduDurationToLSigLength (Time ppduDuration, const WifiTxVector &txVector,
                                       WifiPhyBand band);
  /**
   ...
   */
  static Time ConvertLSigLengthToHeTbPpduDuration (uint16_t length, const WifiTxVector &txVector,
                                                   WifiPhyBand band);
  /**
   ...
   */
  static uint64_t GetPhyRate (uint8_t mcsValue, uint16_t channelWidth, uint16_t guardInterval,
                              uint8_t nss);
  /**
   ...
   * \return the data bit rate in bps.
   */
  static uint64_t GetDataRate (uint8_t mcsValue, uint16_t channelWidth, uint16_t guardInterval,
                               uint8_t nss);
  /**
...
protected:
  ...
  Ptr<SpectrumValue> GetTxPowerSpectralDensity (double txPowerW, Ptr<const WifiPpdu> ppdu,
  ...
  uint64_t
      m_previouslyTxPpduUid; //!< UID of the previously sent PPDU, used by AP to recognize response HE TB PPDUs
  uint64_t m_currentMuPpduUid; //!< UID of the HE MU or HE TB PPDU being received

  std::map<uint16_t /* STA-ID */, EventId>
      m_beginOfdmaPayloadRxEvents; //!< the beginning of the OFDMA payload reception events (indexed by STA-ID)

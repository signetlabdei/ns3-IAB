...
HeFrameExchangeManager::HeFrameExchangeManager () : m_triggerFrameInAmpdu (false)
{
...
bool
HeFrameExchangeManager::StartFrameExchange (Ptr<QosTxop> edca, Time availableTime,
                                            bool initialFrame)
{
...
bool
HeFrameExchangeManager::SendMpduFromBaManager (Ptr<QosTxop> edca, Time availableTime,
                                               bool initialFrame)
{
  NS_LOG_FUNCTION (this << edca << availableTime << initialFrame);
...
HeFrameExchangeManager::SendPsduMap (void)
{
      ...
      if (!m_triggerFrame)
      ...
      else
        {
          ...
          Time txDuration = m_phy->CalculateTxDuration (
              triggerPsdu->GetSize (), acknowledgment->muBarTxVector, m_phy->GetPhyBand ());
      ...
      for (auto &station : acknowledgment->stationsReplyingWithBlockAck)
        {
          ...
          std::vector<Ptr<WifiMacQueueItem>> mpduList (psduMapIt->second->begin (),
          ...
          m_trigVector.SetHeMuUserInfo (
              psduMapIt->first, station.second.blockAckTxVector.GetHeMuUserInfo (psduMapIt->first));
        }
   ...
   */
  else if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::UL_MU_MULTI_STA_BA)
    {
      ...
      Time tbPpduDuration = HePhy::ConvertLSigLengthToHeTbPpduDuration (
          m_muScheduler->GetUlMuInfo ().trigger.GetUlLength (), acknowledgment->tbPpduTxVector,
          m_phy->GetPhyBand ());
   ...
   */
           ...
           (*m_psduMap.begin ()->second->begin ())->GetHeader ().IsTrigger ())
    {
      ...
      acknowledgment->acknowledgmentTime +=
          ...
          HePhy::ConvertLSigLengthToHeTbPpduDuration (trigger.GetUlLength (), txVector,
                                                      m_phy->GetPhyBand ());
   ...
   */
           ...
           m_txParams.m_acknowledgment->method == WifiAcknowledgment::ACK_AFTER_TB_PPDU)
    {
      ...
      txVector = m_mac->GetWifiRemoteStationManager ()->GetBlockAckTxVector (
          m_psduMap.begin ()->second->GetAddr1 (), m_txParams.m_txVector);
      responseTxVector = &txVector;
    }
  /*
  ...
  if (m_txParams.m_txVector.IsUlMu ())
    {
      txDuration = HePhy::ConvertLSigLengthToHeTbPpduDuration (
          m_txParams.m_txVector.GetLength (), m_txParams.m_txVector, m_phy->GetPhyBand ());
    }
  else
    {
      txDuration =
  ...
  else
    {
      ...
      switch (timerType)
        ...
        case WifiTxTimer::WAIT_NORMAL_ACK_AFTER_DL_MU_PPDU:
          NS_ASSERT (mpdu);
          m_txTimer.Set (timerType, timeout, &HeFrameExchangeManager::NormalAckTimeout, this, mpdu,
                         m_txParams.m_txVector);
          break;
        case WifiTxTimer::WAIT_BLOCK_ACK:
          NS_ASSERT (psdu);
          m_txTimer.Set (timerType, timeout, &HeFrameExchangeManager::BlockAckTimeout, this, psdu,
                         m_txParams.m_txVector);
          break;
        case WifiTxTimer::WAIT_BLOCK_ACKS_IN_TB_PPDU:
          m_txTimer.Set (timerType, timeout, &HeFrameExchangeManager::BlockAcksInTbPpduTimeout,
                         this, &m_psduMap, &m_staExpectTbPpduFrom, m_staExpectTbPpduFrom.size ());
          break;
...
HeFrameExchangeManager::CalculateAcknowledgmentTime (WifiAcknowledgment *acknowledgment) const
{
   ...
   */
  else if (acknowledgment->method == WifiAcknowledgment::DL_MU_TF_MU_BAR)
    {
      ...
      for (const auto &stations : dlMuTfMuBarAcknowledgment->stationsReplyingWithBlockAck)
        {
          ...
          Time currBlockAckDuration = m_phy->CalculateTxDuration (
              GetBlockAckSize (info.baType), info.blockAckTxVector, m_phy->GetPhyBand (), staId);
      ...
      WifiTxVector &txVector =
          dlMuTfMuBarAcknowledgment->stationsReplyingWithBlockAck.begin ()->second.blockAckTxVector;
      ...
      dlMuTfMuBarAcknowledgment->acknowledgmentTime =
          m_phy->GetSifs () +
          m_phy->CalculateTxDuration (muBarSize, dlMuTfMuBarAcknowledgment->muBarTxVector,
                                      m_phy->GetPhyBand ()) +
   ...
   */
  else if (acknowledgment->method == WifiAcknowledgment::UL_MU_MULTI_STA_BA)
    {
      ...
      Time duration =
          m_phy->CalculateTxDuration (GetBlockAckSize (ulMuMultiStaBa->baType),
...
HeFrameExchangeManager::BlockAcksInTbPpduTimeout (
    WifiPsduMap *psduMap, const std::set<Mac48Address> *staMissedBlockAckFrom,
    std::size_t nSolicitedStations)
{
  ...
  if (staMissedBlockAckFrom->size () == nSolicitedStations)
    {
      ...
      m_mac->GetWifiRemoteStationManager ()->ReportDataFailed (
...
void
HeFrameExchangeManager::BlockAckAfterTbPpduTimeout (Ptr<WifiPsdu> psdu,
  ...
  for (const auto &userInfoField : trigger)
    {
      v.SetHeMuUserInfo (userInfoField.GetAid12 (),
                         {userInfoField.GetRuAllocation (),
                          HePhy::GetHeMcs (userInfoField.GetUlMcs ()), userInfoField.GetNss ()});
    }
  return v;
}
                                         ...
                                         Mac48Address triggerSender) const
{
  NS_ASSERT (
      triggerSender !=
      m_self); //TxPower information is used only by STAs, it is useless for the sending AP (which can directly use CtrlTriggerHeader::GetHeTbTxVector)
  NS_ASSERT (m_staMac);
  ...
  int8_t pathLossDb =
      ...
      static_cast<int8_t> (m_mac->GetWifiRemoteStationManager ()->GetMostRecentRssi (
...
HeFrameExchangeManager::SendMultiStaBlockAck (const WifiTxParameters &txParams)
{
  ...
  Ptr<WifiPsdu> psdu =
      GetWifiPsdu (Create<WifiMacQueueItem> (packet, hdr), acknowledgment->multiStaBaTxVector);
  ...
  Time txDuration =
      m_phy->CalculateTxDuration (GetBlockAckSize (acknowledgment->baType),
  ...
  for (const auto &tid : tids)
    {
      ...
      if ((mpdu = edca->GetBaManager ()->GetBar (false, tid, hdr.GetAddr2 ())) &&
          TryAddMpdu (mpdu, txParams, ppduDuration))
        {
          ...
          psdu =
              Create<WifiPsdu> (edca->GetBaManager ()->GetBar (true, tid, hdr.GetAddr2 ()), true);
          break;
        }
                                                   ...
                                                   const WifiMacHeader &hdr)
{
  ...
  while (tid < 8 &&
         IsWithinSizeAndTimeLimits (txParams.GetSizeIfAddMpdu (mpdu = Create<WifiMacQueueItem> (
  ...
  if (txVector.IsUlMu () && m_txTimer.IsRunning () &&
      m_txTimer.GetReason () == WifiTxTimer::WAIT_TB_PPDU_AFTER_BASIC_TF)
    {
      ...
      if (!acknowledgment->stationsReceivingMultiStaBa.empty () && !m_multiStaBaEvent.IsRunning ())
        {
          m_multiStaBaEvent =
              Simulator::Schedule (m_phy->GetSifs (), &HeFrameExchangeManager::SendMultiStaBlockAck,
  ...
  if (hdr.IsCtl ())
    {
      if (hdr.IsCts () && m_txTimer.IsRunning () &&
          m_txTimer.GetReason () == WifiTxTimer::WAIT_CTS && m_psduMap.size () == 1)
        {
          ...
          m_mac->GetWifiRemoteStationManager ()->ReportRtsOk (
              m_psduMap.begin ()->second->GetHeader (0), rxSignalInfo.snr, txVector.GetMode (),
              tag.Get ());
               ...
               m_txTimer.GetReason () == WifiTxTimer::WAIT_NORMAL_ACK_AFTER_DL_MU_PPDU)
        {
          ...
          SnrTag tag;
          mpdu->GetPacket ()->PeekPacketTag (tag);
          ReceivedNormalAck (*it->second->begin (), m_txParams.m_txVector, txVector, rxSignalInfo,
                             tag.Get ());
          m_psduMap.clear ();
        }
      ...
      // actually receiving a TB PPDU.
               ...
               m_txTimer.GetReason () == WifiTxTimer::WAIT_BLOCK_ACKS_IN_TB_PPDU)
        {
          ...
          m_mac->GetWifiRemoteStationManager ()->ReportAmpduTxStatus (
              hdr.GetAddr2 (), ret.first, ret.second, rxSignalInfo.snr, tag.Get (),
              m_txParams.m_txVector);
              ...
              if (blockAck.GetAckType (index) && tid < 8)
              ...
              else
                {
                  ...
                  std::pair<uint16_t, uint16_t> ret = GetBaManager (tid)->NotifyGotBlockAck (
                      blockAck, hdr.GetAddr2 (), {tid}, index);
                  m_mac->GetWifiRemoteStationManager ()->ReportAmpduTxStatus (
                      hdr.GetAddr2 (), ret.first, ret.second, rxSignalInfo.snr, tag.Get (staId),
                      m_txParams.m_txVector);
                }
          ...
          if (trigger.IsMuBar ())
          ...
          else if (trigger.IsBsrp ())
            {
              Simulator::Schedule (m_phy->GetSifs (),
                                   &HeFrameExchangeManager::SendQosNullFramesInTbPpdu, this,
                                   trigger, hdr);
            }
        }
      else

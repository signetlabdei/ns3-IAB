...
MinstrelHtWifiManager::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::MinstrelHtWifiManager")
          ...
          .SetGroupName ("Wifi")
          .AddAttribute ("UpdateStatistics", "The interval between updating statistics table ",
          ...
          .AddAttribute (
              ...
              UintegerValue (10), MakeUintegerAccessor (&MinstrelHtWifiManager::m_lookAroundRate),
              MakeUintegerChecker<uint8_t> (0, 100))
          .AddAttribute ("EWMA", "EWMA level", UintegerValue (75),
                         MakeUintegerAccessor (&MinstrelHtWifiManager::m_ewmaLevel),
                         MakeUintegerChecker<uint8_t> (0, 100))
          .AddAttribute ("SampleColumn", "The number of columns used for sampling",
                         UintegerValue (10),
          ...
          .AddAttribute (
              ...
              UintegerValue (1200), MakeUintegerAccessor (&MinstrelHtWifiManager::m_frameLength),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("UseVhtOnly", "Use only VHT MCSs (and not HT) when VHT is available",
                         BooleanValue (true),
                         MakeBooleanAccessor (&MinstrelHtWifiManager::m_useVhtOnly),
                         MakeBooleanChecker ())
          .AddAttribute (
              "PrintStats", "Control the printing of the statistics table", BooleanValue (false),
              MakeBooleanAccessor (&MinstrelHtWifiManager::m_printStats), MakeBooleanChecker ())
...
MinstrelHtWifiManager::MinstrelHtWifiManager () : m_numGroups (0), m_numRates (0), m_currentRate (0)
{
              ...
              for (uint8_t streams = 1; streams <= MAX_SUPPORTED_STREAMS; streams++)
                {
                  ...
                  if (!(!GetShortGuardIntervalSupported () &&
                        m_minstrelGroups[groupId].sgi) ///Is SGI supported by the transmitter?
                      && (GetPhy ()->GetChannelWidth () >=
                          m_minstrelGroups[groupId]
                      ...
                      for (uint8_t i = 0; i < MAX_HT_GROUP_RATES; i++)
                        {
                          ...
                          AddFirstMpduTxTime (groupId, mode,
                                              CalculateMpduTxDuration (GetPhy (), streams, sgi,
                                                                       chWidth, mode,
                                                                       FIRST_MPDU_IN_AGGREGATE));
                          AddMpduTxTime (groupId, mode,
                                         CalculateMpduTxDuration (GetPhy (), streams, sgi, chWidth,
...
bool
MinstrelHtWifiManager::IsValidMcs (Ptr<WifiPhy> phy, uint8_t streams, uint16_t chWidth,
                                   WifiMode mode)
{
...
void
MinstrelHtWifiManager::DoReportRtsOk (WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode,
                                      double rtsSnr)
{
  NS_LOG_FUNCTION (this << st);
}
...
void
MinstrelHtWifiManager::DoReportAmpduTxStatus (WifiRemoteStation *st, uint8_t nSuccessfulMpdus,
...
MinstrelHtWifiManager::DoGetDataTxVector (WifiRemoteStation *st)
{
  ...
  if (!station->m_isHt)
  ...
  else
    {
      ...
      if ((group.sgi && !GetShortGuardIntervalSupported (station)) ||
          ...
          group.streams > GetNumberOfSupportedStreams (station))
        {
          NS_FATAL_ERROR ("Inconsistent group selected. Group: ("
                          << +group.streams << "," << +group.sgi << "," << group.chWidth << ")"
      ...
      return WifiTxVector (
          ...
          GetPreambleForTransmission (mode.GetModulationClass (), GetShortPreambleEnabled (),
                                      UseGreenfieldForDestination (GetAddress (station))),
          group.sgi ? 400 : 800, GetNumberOfAntennas (), group.streams, GetNess (station),
  ...
  else
    {
      ...
      return WifiTxVector (
          ...
          GetPreambleForTransmission (rtsRate.GetModulationClass (), GetShortPreambleEnabled (),
                                      UseGreenfieldForDestination (GetAddress (station))),
          800, 1, 1, 0, GetChannelWidthForTransmission (rtsRate, GetChannelWidth (station)),
          GetAggregation (station));
    }
}

bool
MinstrelHtWifiManager::DoNeedRetransmission (WifiRemoteStation *st, Ptr<const Packet> packet,
                                             bool normally)
{
          ...
          station->m_groupsTable[sampleGroupId].m_ratesTable[sampleRateId].supported)
        {
          ...
          if (sampleIdx != station->m_maxTpRate && sampleIdx != station->m_maxTpRate2 &&
              sampleIdx != station->m_maxProbRate && sampleRateInfo.ewmaProb <= 95)
            {
              ...
              Time maxProbDuration =
                  station->m_groupsTable[maxProbGroupId].m_ratesTable[maxProbRateId].perfectTxTime;
              ...
              if (station->m_groupsTable[j].m_ratesTable[i].supported)
                {
                  ...
                  NS_LOG_DEBUG (+i
                                << " "
                                << GetMcsSupported (
                                       station, station->m_groupsTable[j].m_ratesTable[i].mcsIndex)
                                << "\t attempt="
                  ...
                  if (station->m_groupsTable[j].m_ratesTable[i].numRateAttempt > 0)
                    {
                       ...
                       */
                      tempProb = (100 * station->m_groupsTable[j].m_ratesTable[i].numRateSuccess) /
                                 station->m_groupsTable[j].m_ratesTable[i].numRateAttempt;
                      ...
                      if (station->m_groupsTable[j].m_ratesTable[i].successHist == 0)
                        ...
                        {
                          station->m_groupsTable[j].m_ratesTable[i].ewmsdProb = CalculateEwmsd (
                              station->m_groupsTable[j].m_ratesTable[i].ewmsdProb, tempProb,
...
double
MinstrelHtWifiManager::CalculateThroughput (MinstrelHtWifiRemoteStation *station, uint8_t groupId,
                                            uint8_t rateId, double ewmaProb)
{
  /**
      ...
      if (m_minstrelGroups[groupId].isSupported)
        {
          ...
          if (!(!GetVhtSupported (station) &&
                m_minstrelGroups[groupId].isVht) ///Is VHT supported by the receiver?
              && (m_minstrelGroups[groupId].isVht || !GetVhtSupported (station) ||
                  !m_useVhtOnly) ///If it is an HT MCS, check if VHT only is disabled
              && !(!GetShortGuardIntervalSupported (station) &&
                   m_minstrelGroups[groupId].sgi) ///Is SGI supported by the receiver?
              && (GetChannelWidth (station) >=
                  m_minstrelGroups[groupId].chWidth) ///Is channel width supported by the receiver?
              && (GetNumberOfSupportedStreams (station) >=
                  m_minstrelGroups[groupId].streams)) ///Are streams supported by the receiver?
            {
              NS_LOG_DEBUG ("Group " << +groupId << ": (" << +m_minstrelGroups[groupId].streams
                                     << "," << +m_minstrelGroups[groupId].sgi << ","
              ...
              station->m_groupsTable[groupId].m_ratesTable =
                  HtMinstrelRate (m_numRates); ///Create the rate list for the group.
              for (uint8_t i = 0; i < m_numRates; i++)
                {
              ...
              for (uint8_t i = 0; i < station->m_nModes; i++)
                {
                  ...
                  if ((m_minstrelGroups[groupId].isVht &&
                      ...
                      || (!m_minstrelGroups[groupId].isVht &&
                          mode.GetModulationClass () ==
                          ...
                          && mode.GetMcsValue () >= ((m_minstrelGroups[groupId].streams - 1) * 8)))
                    {
                      NS_LOG_DEBUG ("Mode " << +i << ": " << mode
                                            << " isVht: " << m_minstrelGroups[groupId].isVht);
...
void
MinstrelHtWifiManager::CalculateRetransmits (MinstrelHtWifiRemoteStation *station, uint8_t groupId,
                                             uint8_t rateId)
{
...
double
MinstrelHtWifiManager::CalculateEwmsd (double oldEwmsd, double currentProb, double ewmaProb,
                                       double weight)
{
...
void
MinstrelHtWifiManager::StatsDump (MinstrelHtWifiRemoteStation *station, uint8_t groupId,
                                  std::ofstream &of)
{
...
MinstrelHtWifiManager::GetHtGroupId (uint8_t txstreams, uint8_t sgi, uint16_t chWidth)
{
  ...
  return MAX_SUPPORTED_STREAMS * 2 * (chWidth == 40 ? 1 : 0) + MAX_SUPPORTED_STREAMS * sgi +
         txstreams - 1;
}
...
{
  NS_LOG_FUNCTION (this << +txstreams << +sgi << chWidth);
  return MAX_HT_STREAM_GROUPS * MAX_SUPPORTED_STREAMS +
         MAX_SUPPORTED_STREAMS * 2 *
             (chWidth == 160  ? 3
              : chWidth == 80 ? 2
              : chWidth == 40 ? 1
                              : 0) +
         MAX_SUPPORTED_STREAMS * sgi + txstreams - 1;
}

...
ApWifiMac::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::ApWifiMac")
          ...
          .AddAttribute (
              "BeaconInterval", "Delay between two beacons", TimeValue (MicroSeconds (102400)),
          ...
          .AddAttribute ("BeaconGeneration", "Whether or not beacons are generated.",
                         BooleanValue (true), MakeBooleanAccessor (&ApWifiMac::SetBeaconGeneration),
                         MakeBooleanChecker ())
          .AddAttribute ("EnableNonErpProtection",
                         "Whether or not protection mechanism should be used when non-ERP STAs are "
                         "present within the BSS."
                         "This parameter is only used when ERP is supported by the AP.",
                         BooleanValue (true),
                         MakeBooleanAccessor (&ApWifiMac::m_enableNonErpProtection),
                         MakeBooleanChecker ())
          .AddAttribute ("BsrLifetime", "Lifetime of Buffer Status Reports received from stations.",
                         TimeValue (MilliSeconds (20)),
                         MakeTimeAccessor (&ApWifiMac::m_bsrLifetime), MakeTimeChecker ())
          .AddTraceSource ("AssociatedSta", "A station associated with this access point.",
                           MakeTraceSourceAccessor (&ApWifiMac::m_assocLogger),
                           "ns3::ApWifiMac::AssociationCallback")
          .AddTraceSource ("DeAssociatedSta", "A station lost association with this access point.",
...
{
  NS_LOG_FUNCTION (this);
  if (GetErpSupported (SINGLE_LINK_OP_ID) && GetShortSlotTimeSupported () &&
      (m_numNonErpStations == 0))
    {
          ...
          if (GetWifiRemoteStationManager ()->GetHtSupported (sta.second))
            {
              ...
              for (uint8_t j = 0;
                   j <
                   (std::min (nMcs, GetWifiRemoteStationManager ()->GetNMcsSupported (sta.second)));
                   j++)
                {
                  WifiMode mcs = *itMcs++;
                  uint8_t nss = (mcs.GetMcsValue () / 8) + 1;
                  NS_ASSERT (nss > 0 && nss < 5);
                  uint64_t dataRate = mcs.GetDataRate (
                      GetWifiRemoteStationManager ()->GetChannelWidthSupported (sta.second),
                      GetWifiRemoteStationManager ()->GetShortGuardIntervalSupported (sta.second)
                          ? 400
                          : 800,
                      nss);
              ...
              if (GetWifiRemoteStationManager ()->GetNumberOfSupportedStreams (sta.second) <
                  maxSpatialStream)
                {
                  maxSpatialStream =
                      GetWifiRemoteStationManager ()->GetNumberOfSupportedStreams (sta.second);
                }
            }
        }
  ...
  if (GetVhtSupported ())
    {
      ...
      operation.SetChannelCenterFrequencySegment0 (
          (bssBandwidth == 160) ? GetWifiPhy ()->GetOperatingChannel ().GetPrimaryChannelNumber (
                                      80, WIFI_STANDARD_80211ac)
      ...
      for (uint8_t nss = 1; nss <= maxSpatialStream; nss++)
        {
          operation.SetMaxHeMcsPerNss (
              nss, 11); //TBD: hardcode to 11 for now since we assume all MCS values are supported
        }
          ...
          if (hdr->IsAssocReq ())
            {
              ...
              GetWifiRemoteStationManager ()->AddSupportedPhyPreamble (
              ...
              if (problem)
                {
                  NS_LOG_DEBUG ("One of the Basic Rate set mode is not supported by the station: "
                                "send association response with an error status");
                  ...
                  for (const auto &mode : GetWifiPhy ()->GetModeList ())
                    {
                      if (rates.IsSupportedRate (
                              mode.GetDataRate (GetWifiPhy ()->GetChannelWidth ())))
                        {
                          GetWifiRemoteStationManager ()->AddSupportedMode (from, mode);
                        }
                    }
                  if (GetErpSupported (SINGLE_LINK_OP_ID) &&
                      GetWifiRemoteStationManager ()->GetErpOfdmSupported (from) &&
                  ...
                  if (GetHtSupported ())
                    {
                      ...
                      if (htCapabilities.IsSupportedMcs (0))
                        {
                          GetWifiRemoteStationManager ()->AddStationHtCapabilities (from,
                                                                                    htCapabilities);
                        }
                    }
                  if (GetVhtSupported ())
                      ...
                      if (vhtCapabilities.GetRxHighestSupportedLgiDataRate () > 0)
                        {
                          GetWifiRemoteStationManager ()->AddStationVhtCapabilities (
                              from, vhtCapabilities);
                          for (const auto &mcs : GetWifiPhy ()->GetMcsList (WIFI_MOD_CLASS_VHT))
                            {
                              if (vhtCapabilities.IsSupportedTxMcs (mcs.GetMcsValue ()))
                                {
                                  GetWifiRemoteStationManager ()->AddSupportedMcs (hdr->GetAddr2 (),
                                                                                   mcs);
                  ...
                  if (GetHeSupported ())
                    {
                      ...
                      if (heCapabilities.GetSupportedMcsAndNss () != 0)
                        {
                          GetWifiRemoteStationManager ()->AddStationHeCapabilities (from,
                                                                                    heCapabilities);
                  ...
                  if (GetEhtSupported ())
                    {
                      ...
                      GetWifiRemoteStationManager ()->AddStationEhtCapabilities (from,
                                                                                 ehtCapabilities);
          ...
          else if (hdr->IsReassocReq ())
            {
              ...
              if (problem)
                {
                  NS_LOG_DEBUG ("One of the Basic Rate set mode is not supported by the station: "
                                "send reassociation response with an error status");
                  ...
                  if (it->second == from)
                    {
                      ...
                      if (GetWifiRemoteStationManager ()->GetDsssSupported (from) &&
                          !GetWifiRemoteStationManager ()->GetErpOfdmSupported (from))
                        {
                          m_numNonErpStations--;
                        }

...
public:
   ...
   */
  Time CalculateTransmissionTime (Ptr<const Packet> packet, const WifiMacHeader *hdr,
   ...
   * the next packet transmission if one was selected.
   */
  Time CalculateOverallTxTime (Ptr<const Packet> packet, const WifiMacHeader *hdr,
  ...
  virtual void StartTransmission (Ptr<WifiMacQueueItem> mpdu,
                                  MacLowTransmissionParameters parameters, Ptr<Txop> txop);
   ...
   */
  void ReceiveOk (Ptr<WifiMacQueueItem> mpdu, double rxSnr, WifiTxVector txVector,
                  bool ampduSubframe);
  /**
   ...
   *
   */
  void DeaggregateAmpduAndReceive (Ptr<WifiPsdu> aggregatedPacket, double rxSnr,
...
private:
   ...
   */
  Time GetResponseDuration (const MacLowTransmissionParameters &params, WifiTxVector dataTxVector,
   ...
   */
  void SendCtsAfterRts (Mac48Address source, Time duration, WifiTxVector rtsTxVector,
                        double rtsSnr);
  /**
   ...
   */
  void RxCompleteBufferedPacketsWithSmallerSequence (uint16_t seq, Mac48Address originator,
                                                     uint8_t tid);
  /**
   * \param originator Address of peer participating in block ack mechanism.
   * \param tid TID for which block ack was created.
   ...
   */
  void SendBlockAckResponse (const CtrlBAckResponseHeader *blockAck, Mac48Address originator,
  ...
  Ptr<WifiRemoteStationManager>
      m_stationManager; //!< Pointer to WifiRemoteStationManager (rate control)
  ...
  class PhyMacLowListener
      *m_phyMacLowListener; //!< Listener needed to monitor when a channel switching occurs.

  /*
   * BlockAck data structures.
   */
  typedef std::list<Ptr<WifiMacQueueItem>>::iterator
      BufferedPacketI; //!< buffered packet iterator typedef

  typedef std::pair<Mac48Address, uint8_t> AgreementKey; //!< agreement key typedef
  typedef std::pair<BlockAckAgreement, std::list<Ptr<WifiMacQueueItem>>>

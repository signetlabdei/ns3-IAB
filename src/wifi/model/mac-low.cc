...
public:
  ...
  void
  NotifyTxStart (Time duration, double txPowerDbm)
  {
  }
  void
...
{
  static TypeId tid =
      TypeId ("ns3::MacLow").SetParent<Object> ().SetGroupName ("Wifi").AddConstructor<MacLow> ();
  return tid;
}
...
MacLow::SetPhy (const Ptr<WifiPhy> phy)
{
  ...
  m_phy->TraceConnectWithoutContext ("PhyRxPayloadBegin",
                                     MakeCallback (&MacLow::RxStartIndication, this));
...
MacLow::ResetPhy (void)
{
  ...
  m_phy->SetReceiveOkCallback (
      MakeNullCallback<void, Ptr<WifiPsdu>, double, WifiTxVector, std::vector<bool>> ());
...
void
MacLow::StartTransmission (Ptr<WifiMacQueueItem> mpdu, MacLowTransmissionParameters params,
                           Ptr<Txop> txop)
{
   ...
   */
  if (hdr.IsQosData () && !hdr.GetAddr1 ().IsGroup () && !hdr.IsMoreFragments () &&
      hdr.GetFragmentNumber () == 0)
    {
      ...
      if (m_currentTxop->GetTxopLimit ().IsStrictlyPositive ())
        {
          txopLimit =
              m_currentTxop->GetTxopRemaining () - CalculateOverheadTxTime (mpdu, m_txParams);
  ...
  NS_LOG_DEBUG ("startTx size=" << m_currentPacket->GetSize () << ", to="
                                << m_currentPacket->GetAddr1 () << ", txop=" << m_currentTxop);
                                   ...
                                   uint32_t ampduSize, Time ppduDurationLimit)
{
  ...
  return IsWithinSizeAndTimeLimits (mpdu->GetSize (), mpdu->GetHeader ().GetAddr1 (),
                                    mpdu->GetHeader ().GetQosTid (), txVector, ampduSize,
                                    ppduDurationLimit);
}
  ...
  if (m_normalAckTimeoutEvent.IsRunning ())
    {
      ...
      m_normalAckTimeoutEvent = Simulator::Schedule (
          psduDuration + NanoSeconds (PSDU_DURATION_SAFEGUARD), &MacLow::NormalAckTimeout, this);
    }
  else if (m_blockAckTimeoutEvent.IsRunning ())
    {
      ...
      m_blockAckTimeoutEvent.Cancel ();
      NotifyAckTimeoutResetNow ();
      m_blockAckTimeoutEvent = Simulator::Schedule (
          psduDuration + NanoSeconds (PSDU_DURATION_SAFEGUARD), &MacLow::BlockAckTimeout, this);
    }
...
void
MacLow::ReceiveOk (Ptr<WifiMacQueueItem> mpdu, double rxSnr, WifiTxVector txVector,
                   bool ampduSubframe)
{
  ...
  if (hdr.IsRts ())
          ...
          if (isPrevNavZero && hdr.GetAddr1 () == m_self)
            {
              ...
              m_sendCtsEvent =
                  Simulator::Schedule (GetSifs (), &MacLow::SendCtsAfterRts, this, hdr.GetAddr2 (),
  ...
  else if (hdr.IsCts () && hdr.GetAddr1 () == m_self && m_ctsTimeoutEvent.IsRunning () &&
           m_currentPacket != 0)
    {
      if (ampduSubframe)
        {
      ...
      m_stationManager->ReportRxOk (m_currentPacket->GetAddr1 (), rxSnr, txVector.GetMode ());
      ...
      m_sendDataEvent =
          Simulator::Schedule (GetSifs (), &MacLow::SendDataAfterCts, this, hdr.GetDuration ());
    }
  else if (hdr.IsAck () && hdr.GetAddr1 () == m_self && m_normalAckTimeoutEvent.IsRunning () &&
           m_txParams.MustWaitNormalAck ())
    {
      ...
      if (!m_txParams.HasNextPacket ())
        {
          ...
          m_stationManager->ReportDataOk (
              ...
              txVector.GetMode (), tag.Get (), m_currentTxVector, m_currentPacket->GetSize ());
        }
      ...
      if (m_txParams.HasNextPacket ())
        {
          m_waitIfsEvent =
              Simulator::Schedule (GetSifs (), &MacLow::WaitIfsAfterEndTxFragment, this);
        }
  ...
  else if (hdr.IsBlockAck () && hdr.GetAddr1 () == m_self && m_txParams.MustWaitBlockAck () &&
              ...
              if ((*it).second.first.IsImmediateBlockAck ())
                {
                  ...
                  m_sendAckEvent = Simulator::Schedule (
                      GetSifs (), &MacLow::SendBlockAckAfterBlockAckRequest, this, blockAckReq,
                      hdr.GetAddr2 (), hdr.GetDuration (), txVector.GetMode (), rxSnr);
                }
              else
                {
          ...
          if (hdr.IsQosAck () && !ampduSubframe)
            {
              ...
              NS_ASSERT (m_sendAckEvent.IsExpired ());
              m_sendAckEvent =
                  Simulator::Schedule (GetSifs (), &MacLow::SendAckAfterData, this, hdr.GetAddr2 (),
          ...
          else
            {
              if (IsCfPeriod ())
              ...
              else
                {
                  ...
                  NS_ASSERT (m_sendAckEvent.IsExpired ());
                  m_sendAckEvent = Simulator::Schedule (GetSifs (), &MacLow::SendAckAfterData, this,
...
{
  NS_ASSERT (
      ackTxVector.GetMode ().GetModulationClass () !=
      WIFI_MOD_CLASS_HT); //Ack should always use non-HT PPDU (HT PPDU cases not supported yet)
...
{
  return m_phy->CalculateTxDuration (GetBlockAckRequestSize (type), blockAckReqTxVector,
                                     m_phy->GetPhyBand ());
}
...
{
  NS_ASSERT (
      ctsTxVector.GetMode ().GetModulationClass () !=
      WIFI_MOD_CLASS_HT); //CTS should always use non-HT PPDU (HT PPDU cases not supported yet)
                             ...
                             Mac48Address receiver) const
{
  ...
  if (params.MustWaitNormalAck ())
  ...
  else if (params.MustSendBlockAckRequest ())
    {
      ...
      WifiTxVector blockAckReqTxVector = GetBlockAckTxVector (m_self, dataTxVector.GetMode ());
      duration +=
          GetBlockAckRequestDuration (blockAckReqTxVector, params.GetBlockAckRequestType ());
...
MacLow::GetCtsTxVector (Mac48Address to, WifiMode rtsTxMode) const
{
  ...
  WifiTxVector v;
  v.SetMode (ctsMode);
  v.SetPreambleType (GetPreambleForTransmission (
      ctsMode.GetModulationClass (), m_stationManager->GetShortPreambleEnabled (),
      m_stationManager->UseGreenfieldForDestination (to)));
  ...
  uint16_t ctsTxGuardInterval =
      ConvertGuardIntervalToNanoSeconds (ctsMode, DynamicCast<WifiNetDevice> (m_phy->GetDevice ()));
...
MacLow::GetAckTxVector (Mac48Address to, WifiMode dataTxMode) const
{
  ...
  v.SetMode (ackMode);
  v.SetPreambleType (GetPreambleForTransmission (
      ackMode.GetModulationClass (), m_stationManager->GetShortPreambleEnabled (),
      m_stationManager->UseGreenfieldForDestination (to)));
  v.SetTxPowerLevel (m_stationManager->GetDefaultTxPowerLevel ());
  v.SetChannelWidth (GetChannelWidthForTransmission (ackMode, m_phy->GetChannelWidth ()));
  uint16_t ackTxGuardInterval =
      ConvertGuardIntervalToNanoSeconds (ackMode, DynamicCast<WifiNetDevice> (m_phy->GetDevice ()));
...
MacLow::GetBlockAckTxVector (Mac48Address to, WifiMode dataTxMode) const
{
  ...
  v.SetPreambleType (GetPreambleForTransmission (
      blockAckMode.GetModulationClass (), m_stationManager->GetShortPreambleEnabled (),
      m_stationManager->UseGreenfieldForDestination (to)));
  v.SetTxPowerLevel (m_stationManager->GetDefaultTxPowerLevel ());
  v.SetChannelWidth (GetChannelWidthForTransmission (blockAckMode, m_phy->GetChannelWidth ()));
  uint16_t blockAckTxGuardInterval = ConvertGuardIntervalToNanoSeconds (
      blockAckMode, DynamicCast<WifiNetDevice> (m_phy->GetDevice ()));
...
Time
MacLow::CalculateOverallTxTime (Ptr<const Packet> packet, const WifiMacHeader *hdr,
...
Time
MacLow::CalculateTransmissionTime (Ptr<const Packet> packet, const WifiMacHeader *hdr,
                                   const MacLowTransmissionParameters &params) const
{
  ...
  if (params.HasNextPacket ())
    {
      ...
      txTime += GetSifs ();
      txTime += m_phy->CalculateTxDuration (params.GetNextPacketSize (), dataTxVector,
                                            m_phy->GetPhyBand ());
    }
  return txTime;
}
      ...
      if (hdr.IsRts () && navUpdated)
        {
          ...
          m_navCounterResetCtsMissed = Simulator::Schedule (
              navCounterResetCtsMissedDelay, &MacLow::DoNavResetNow, this, Seconds (0));
        }
    }
}
...
MacLow::DoNavResetNow (Time duration)
{
  ...
  for (ChannelAccessManagersCI i = m_channelAccessManagers.begin ();
...
MacLow::ForwardDown (Ptr<const WifiPsdu> psdu, WifiTxVector txVector)
{
  ...
  NS_LOG_DEBUG ("send " << hdr.GetTypeString () << ", to=" << hdr.GetAddr1 ()
                        << ", size=" << psdu->GetSize () << ", mode=" << txVector.GetMode ()
                        << ", preamble=" << txVector.GetPreambleType ()
                        << ", duration=" << hdr.GetDuration () << ", seq=0x" << std::hex
                        << hdr.GetSequenceControl () << std::dec);

  if (hdr.IsCfPoll () && m_stationManager->GetPcfSupported ())
    {
      Simulator::Schedule (GetPifs () + m_phy->CalculateTxDuration (psdu->GetSize (), txVector,
                                                                    m_phy->GetPhyBand ()),
                           &MacLow::CfPollTimeout, this);
    }
...
MacLow::CtsTimeout (void)
{
  ...
  if (qosTxop != 0)
    {
      qosTxop->NotifyMissedCts (
          std::list<Ptr<WifiMacQueueItem>> (m_currentPacket->begin (), m_currentPacket->end ()));
    }
  else
    {
...
MacLow::SendRtsForPacket (void)
{
  ...
  duration += GetSifs ();
  duration += m_phy->CalculateTxDuration (m_currentPacket->GetSize (), m_currentTxVector,
                                          m_phy->GetPhyBand ());
  duration += GetResponseDuration (m_txParams, m_currentTxVector, m_currentPacket->GetAddr1 ());
  if (m_txParams.HasNextPacket ())
    {
      duration += m_phy->CalculateTxDuration (m_txParams.GetNextPacketSize (), m_currentTxVector,
                                              m_phy->GetPhyBand ());
      duration += GetResponseDuration (m_txParams, m_currentTxVector, m_currentPacket->GetAddr1 ());
    }
...
{
  Time txDuration =
      m_phy->CalculateTxDuration (m_currentPacket->GetSize (), dataTxVector, m_phy->GetPhyBand ());
  if (m_txParams.MustWaitNormalAck () && !IsCfPeriod ())
  ...
  else if (m_txParams.MustWaitBlockAck ())
    {
      ...
      WifiTxVector blockAckTxVector =
          GetBlockAckTxVector (m_currentPacket->GetAddr1 (), dataTxVector.GetMode ());
      Time timerDelay = txDuration + GetSifs () + GetSlotTime () +
...
MacLow::SendDataPacket (void)
{
  ...
  if (!IsCfPeriod ())
    {
      Time duration =
          GetResponseDuration (m_txParams, m_currentTxVector, m_currentPacket->GetAddr1 ());
      if (m_txParams.HasNextPacket ())
        {
          duration += GetSifs ();
      ...
      if (m_cfAckInfo.appendCfAck)
        {
          ...
          WifiTxVector tmpTxVector = GetDataTxVector (
              Create<const WifiMacQueueItem> (m_currentPacket->GetPayload (0), tmpHdr));
          if (tmpTxVector.GetMode ().GetDataRate (tmpTxVector) <
              m_currentTxVector.GetMode ().GetDataRate (m_currentTxVector))
            {
  ...
  if (m_txParams.MustSendBlockAckRequest ())
    {
      Ptr<QosTxop> qosTxop = DynamicCast<QosTxop> (m_currentTxop);
      NS_ASSERT (qosTxop != 0);
      auto bar = qosTxop->PrepareBlockAckRequest (m_currentPacket->GetAddr1 (),
                                                  *m_currentPacket->GetTids ().begin ());
...
void
MacLow::SendCtsAfterRts (Mac48Address source, Time duration, WifiTxVector rtsTxVector,
                         double rtsSnr)
{
...
MacLow::SendDataAfterCts (Time duration)
{
  ...
  if (m_txParams.HasNextPacket ())
    {
      newDuration += GetSifs ();
      newDuration += m_phy->CalculateTxDuration (m_txParams.GetNextPacketSize (), m_currentTxVector,
                                                 m_phy->GetPhyBand ());
      newDuration +=
          GetResponseDuration (m_txParams, m_currentTxVector, m_currentPacket->GetAddr1 ());
    }

  Time txDuration = m_phy->CalculateTxDuration (m_currentPacket->GetSize (), m_currentTxVector,
                                                m_phy->GetPhyBand ());
  duration -= txDuration;
  duration -= GetSifs ();
...
MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  ...
  duration -= GetSifs ();
  NS_ASSERT_MSG (duration.IsPositive (),
...
MacLow::ReceiveMpdu (Ptr<WifiMacQueueItem> mpdu)
{
  ...
  if (m_stationManager->GetHtSupported () || m_stationManager->GetVhtSupported () ||
          ...
          if (!QosUtilsIsOldPacket ((*it).second.first.GetStartingSequence (), seqNumber))
            {
              ...
              if (!IsInWindow (hdr.GetSequenceNumber (), (*it).second.first.GetStartingSequence (),
              ...
              RxCompleteBufferedPacketsUntilFirstLost (
                  originator,
                  tid); //forwards up packets starting from winstart and set winstart to last +1
            }
          return true;
        }
      return false;
  ...
  if (it != m_bAckAgreements.end ())
    {
      ...
      uint32_t mappedSeqControl =
          QosUtilsMapSeqControlToUniqueInteger (hdr.GetSequenceControl (), endSequence);

      BufferedPacketI i = (*it).second.second.begin ();
      for (; i != (*it).second.second.end () &&
             QosUtilsMapSeqControlToUniqueInteger ((*i)->GetHeader ().GetSequenceControl (),
                                                   endSequence) < mappedSeqControl;
           i++)
        {
        }
  ...
  if (respHdr->GetTimeout () != 0)
    {
      ...
      it->second.first.m_inactivityEvent = Simulator::Schedule (timeout, &QosTxop::SendDelbaFrame,
...
void
MacLow::RxCompleteBufferedPacketsWithSmallerSequence (uint16_t seq, Mac48Address originator,
                                                      uint8_t tid)
{
  AgreementsI it = m_bAckAgreements.find (std::make_pair (originator, tid));
  if (it != m_bAckAgreements.end ())
  ...
  if (it != m_bAckAgreements.end ())
    {
      ...
      BufferedPacketI i = (*it).second.second.begin ();
      for (; i != (*it).second.second.end () && guard == (*i)->GetHeader ().GetSequenceControl ();
           i++)
        {
          if (!(*i)->GetHeader ().IsMoreFragments ())
            {
...
void
MacLow::SendBlockAckResponse (const CtrlBAckResponseHeader *blockAck, Mac48Address originator,
                              bool immediate, Time duration, WifiMode blockAckReqTxMode,
                              double rxSnr)
{
  NS_LOG_FUNCTION (this);
  Ptr<Packet> packet = Create<Packet> ();
...
void
MacLow::SendBlockAckAfterAmpdu (uint8_t tid, Mac48Address originator, Time duration,
                                WifiTxVector blockAckReqTxVector, double rxSnr)
{
  NS_LOG_FUNCTION (this);
  if (!m_phy->IsStateTx () && !m_phy->IsStateRx ())
    {
      NS_LOG_FUNCTION (this << +tid << originator << duration.As (Time::S) << blockAckReqTxVector
                            << rxSnr);
      CtrlBAckResponseHeader blockAck;
      ...
      SendBlockAckResponse (&blockAck, originator, immediate, duration, blockAckTxVector.GetMode (),
                            rxSnr);
    }
  else
    {
...
void
MacLow::SendBlockAckAfterBlockAckRequest (const CtrlBAckRequestHeader reqHdr,
      ...
      if (it != m_bAckAgreements.end ())
        {
          ...
          if (!m_stationManager->GetHtSupported () && !m_stationManager->GetVhtSupported () &&
              !m_stationManager->GetHeSupported ())
            {
               ...
               */
              RxCompleteBufferedPacketsWithSmallerSequence (reqHdr.GetStartingSequenceControl (),
                                                            originator, tid);
              RxCompleteBufferedPacketsUntilFirstLost (originator, tid);
            }
          else
  ...
  if (agreement.GetTimeout () != 0)
    {
      ...
      agreement.m_inactivityEvent =
          Simulator::Schedule (timeout, &QosTxop::SendDelbaFrame, m_edca[ac], agreement.GetPeer (),
...
void
MacLow::DeaggregateAmpduAndReceive (Ptr<WifiPsdu> psdu, double rxSnr, WifiTxVector txVector,
                                    std::vector<bool> statusPerMpdu)
{
  ...
  bool ampduSubframe =
      txVector
          .IsAggregation (); //flag indicating the packet belongs to an A-MPDU and is not a VHT/HE single MPDU
          ...
          for (; n != psdu->end (); ++n, ++status)
            {
              ...
              NS_ABORT_MSG_IF (firsthdr.GetAddr1 () != m_self,
              ...
              if (*status) //PER and thus CRC check succeeded
                {
                  if (psdu->IsSingle ())
                    ...
                    {
                      m_sendAckEvent = Simulator::Schedule (
                          GetSifs (), &MacLow::SendBlockAckAfterAmpdu, this, firsthdr.GetQosTid (),
                          firsthdr.GetAddr2 (), firsthdr.GetDuration (), txVector, rxSnr);
                    }
                      ...
                      if (normalAck)
                        {
                          ...
                          AgreementsI it =
                              m_bAckAgreements.find (std::make_pair (firsthdr.GetAddr2 (), tid));
                          if (it != m_bAckAgreements.end ())
                            {
...
{
  NS_LOG_FUNCTION (this);
  Time remainingCfpDuration = std::min (m_cfpStart, m_cfpStart + m_cfpMaxDuration -
                                                        Simulator::Now () - m_cfpForeshortening);
...
MacLow::CanTransmitNextCfFrame (void) const
{
  ...
  Time nextTransmission =
      2 * m_phy->CalculateTxDuration (maxMacFrameSize, m_currentTxVector, m_phy->GetPhyBand ()) +
      3 * GetSifs () +
      m_phy->CalculateTxDuration (GetCfEndSize (), m_currentTxVector, m_phy->GetPhyBand ());

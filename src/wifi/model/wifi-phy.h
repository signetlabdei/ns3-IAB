...
public:
   ...
   */
  void StartReceivePreamble (Ptr<const WifiPpdu> ppdu, RxPowerWattPerChannelBand &rxPowersW,
                             Time rxDuration);
   ...
   */
  static Time CalculateTxDuration (Ptr<const WifiPsdu> psdu, const WifiTxVector &txVector,
                                   WifiPhyBand band);
  /**
   ...
   * \return the total amount of time this PHY will stay busy for the transmission of the PPDU
   */
  static Time CalculateTxDuration (WifiConstPsduMap psduMap, const WifiTxVector &txVector,
                                   WifiPhyBand band);
   ...
   */
  static Time GetPayloadDuration (uint32_t size, const WifiTxVector &txVector, WifiPhyBand band,
                                  MpduType mpdutype = NORMAL_MPDU, uint16_t staId = SU_STA_ID);
  /**
   * \param size the number of bytes in the packet to send
   * \param txVector the TXVECTOR used for the transmission of this packet
   * \param band the frequency band
  ...
  */
  ...
  void NotifyMonitorSniffRx (Ptr<const WifiPsdu> psdu, uint16_t channelFreqMhz,
                             ...
                             std::vector<bool> statusPerMpdu, uint16_t staId = SU_STA_ID);
   ...
   * \param txPowerW the transmit power in Watts
   */
  typedef void (*PsduTxBeginCallback) (WifiConstPsduMap psduMap, WifiTxVector txVector,
                                       double txPowerW);
  ...
  using ChannelTuple =
      std::tuple<uint8_t /* channel number */, uint16_t /* channel width */, int /* WifiPhyBand */,
...
protected:
  ...
  uint32_t
      m_txMpduReferenceNumber; //!< A-MPDU reference number to identify all transmitted subframes belonging to the same received A-MPDU
  uint32_t
      m_rxMpduReferenceNumber; //!< A-MPDU reference number to identify all received subframes belonging to the same received A-MPDU
  ...
  std::map<std::pair<uint64_t /* UID*/, WifiPreamble>, Ptr<Event>>
      m_currentPreambleEvents; //!< store event associated to a PPDU (that has a unique ID and preamble combination) whose preamble is being received

  uint64_t
      m_previouslyRxPpduUid; //!< UID of the previously received PPDU, reset to UINT64_MAX upon transmission
...
private:
   ...
   */
  TracedCallback<Ptr<const Packet>, uint16_t /* frequency (MHz) */, WifiTxVector, MpduInfo,
  ...
  double
      m_ccaEdThresholdW; //!< Clear channel assessment (CCA) energy detection (ED) threshold in watts
  double
      m_ccaSensitivityThresholdW; //!< Clear channel assessment (CCA) modulation and coding rate sensitivity threshold in watts
  ...
  bool
      m_channelAccessRequested; //!< Flag if channels access has been requested (used for OBSS_PD SR)

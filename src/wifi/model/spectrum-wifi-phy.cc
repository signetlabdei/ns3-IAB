  ...
  static TypeId tid =
      TypeId ("ns3::SpectrumWifiPhy")
          ...
          .AddAttribute ("TxMaskOuterBandMaximumRejection",
                         ...
                         MakeDoubleChecker<double> ())
          .AddTraceSource ("SignalArrival", "Signal arrival",
      ...
      if (GetFrequency () == 0)
      ...
      else
        {
          ...
          m_rxSpectrumModel = WifiSpectrumValueHelper::GetSpectrumModel (
              GetFrequency (), channelWidth, GetBandBandwidth (), GetGuardBandwidth (channelWidth));
                      ...
                      for (std::size_t phyIndex = 1; phyIndex <= nRus; phyIndex++)
                        {
                          ...
                          WifiSpectrumBand band = ConvertHeRuSubcarriers (
                              bw, GetGuardBandwidth (channelWidth), range, i);
                          std::size_t index =
...
SpectrumWifiPhy::StartRx (Ptr<SpectrumSignalParameters> rxParams)
{
  ...
  for (uint16_t bw = 160; bw > 20;
       bw = bw / 2) //20 MHz is handled apart since the totalRxPowerW is computed through it
    {
      for (uint8_t i = 0; i < (channelWidth / bw); i++)
        {
...
{
  return std::make_tuple (m_txMaskInnerBandMinimumRejection, m_txMaskOuterBandMinimumRejection,

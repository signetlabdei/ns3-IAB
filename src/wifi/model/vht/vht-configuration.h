...
public:
  ...
  using SecondaryCcaSensitivityThresholds =
      std::tuple<double, double,
                 double>; //!< Tuple identifying CCA sensitivity thresholds for secondary channels
...
private:
  ...
  std::map<uint16_t, double>
      m_secondaryCcaSensitivityThresholds; ///< CCA sensitivity thresholds for signals that do not occupy the primary channel, indexed by signal bandwidth (MHz)
};

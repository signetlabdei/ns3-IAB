 ...
 */
const std::map<uint16_t, WifiChannelListType> secondaryChannels{{20, WIFI_CHANLIST_SECONDARY},
                                                                {40, WIFI_CHANLIST_SECONDARY40},
                                                                {80, WIFI_CHANLIST_SECONDARY80}};

/* *NS_CHECK_STYLE_ON* */

VhtPhy::VhtPhy (bool buildModeList /* = true */) : HtPhy (1, false) //don't add HT modes to list
{
...
Time
VhtPhy::GetTrainingDuration (const WifiTxVector &txVector, uint8_t nDataLtf,
...
VhtPhy::GetNumberBccEncoders (const WifiTxVector &txVector) const
{
  ...
  auto iter = m_exceptionsMap.find (std::make_tuple (
      txVector.GetChannelWidth (), txVector.GetNss (), payloadMode.GetMcsValue ()));
...
VhtPhy::CreateVhtMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      "VhtMcs" + std::to_string (index), index, WIFI_MOD_CLASS_VHT, false,
...
{
  return IsCombinationAllowed (txVector.GetMode ().GetMcsValue (), txVector.GetChannelWidth (),
                               txVector.GetNss ());
}

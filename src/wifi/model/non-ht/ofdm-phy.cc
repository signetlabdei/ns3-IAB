...
/* *NS_CHECK_STYLE_OFF* */
const PhyEntity::PpduFormats OfdmPhy::m_ofdmPpduFormats{{WIFI_PREAMBLE_LONG,
                                                         {WIFI_PPDU_FIELD_PREAMBLE, //STF + LTF
...
const std::map<uint16_t, std::array<uint64_t, 8>> s_ofdmRatesBpsList = {
    ...
    {10, // MHz
     {3000000, 4500000, 6000000, 9000000, 12000000, 18000000, 24000000, 27000000}},
    {5, // MHz
     {1500000, 2250000, 3000000, 4500000, 6000000, 9000000, 12000000, 13500000}}};
...
Time
OfdmPhy::GetPayloadDuration (uint32_t size, const WifiTxVector &txVector, WifiPhyBand band,
                             MpduType /* mpdutype */, bool /* incFlag */,
                             uint32_t & /* totalAmpduSize */, double & /* totalAmpduNumSymbols */,
...
Ptr<WifiPpdu>
OfdmPhy::BuildPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector,
                    Time /* ppduDuration */)
{
...
Ptr<SpectrumValue>
OfdmPhy::GetTxPowerSpectralDensity (double txPowerW, Ptr<const WifiPpdu> /* ppdu */,
...
OfdmPhy::CreateOfdmMode (std::string uniqueName, bool isMandatory)
{
  ...
  return WifiModeFactory::CreateWifiMode (
      uniqueName, WIFI_MOD_CLASS_OFDM, isMandatory, MakeBoundCallback (&GetCodeRate, uniqueName),
      MakeBoundCallback (&GetConstellationSize, uniqueName), MakeCallback (&GetPhyRateFromTxVector),
...
uint64_t
OfdmPhy::CalculateDataRate (WifiCodeRate codeRate, uint16_t constellationSize,
                            uint16_t channelWidth)
{
  return CalculateDataRate (GetSymbolDuration (channelWidth), GetUsableSubcarriers (),

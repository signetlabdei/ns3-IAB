  ...
  if (!hdr.IsBlockAckReq () && (txParams.GetSize (receiver) == 0 || m_useExplicitBar))
    {
      ...
      acknowledgment->blockAckReqTxVector =
          m_mac->GetWifiRemoteStationManager ()->GetBlockAckTxVector (receiver,
                                                                      txParams.m_txVector);
  ...
  if (!psduInfo)
    {
      ...
      acknowledgment->stationsReplyingWithBlockAck.emplace (
          ...
          WifiDlMuTfMuBar::BlockAckInfo{edca->GetBaManager ()->GetBlockAckReqHeader (receiver, tid),
                                        blockAckTxVector, edca->GetBlockAckType (receiver, tid)});
  ...
  if (!psduInfo)
    {
      ...
      Ptr<QosTxop> edca = m_mac->GetQosTxop (QosUtilsMapTidToAc (tid));
      acknowledgment->stationsReplyingWithBlockAck.emplace (
          receiver, WifiDlMuAggregateTf::BlockAckInfo{
  ...
  if (trigger.IsBasic ())
    {
      ...
      acknowledgment->multiStaBaTxVector =
          m_mac->GetWifiRemoteStationManager ()->GetBlockAckTxVector (
              apMac->GetStaList ().find (staId)->second, acknowledgment->tbPpduTxVector);

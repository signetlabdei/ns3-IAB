...
Bar::Bar (Ptr<const WifiMacQueueItem> bar, uint8_t tid, bool skipIfNoDataQueued)
    : bar (bar), tid (tid), skipIfNoDataQueued (skipIfNoDataQueued)
{
...
BlockAckManager::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::BlockAckManager")
          ...
          .AddTraceSource ("AgreementState", "The state of the ADDBA handshake",
      ...
      if (agreement.GetTimeout () != 0)
        {
          ...
          agreement.m_inactivityEvent = Simulator::Schedule (
              timeout, &BlockAckManager::InactivityTimeout, this, recipient, tid);
        }
    }
...
BlockAckManager::HandleInFlightMpdu (PacketQueueI mpduIt, MpduStatus status, const AgreementsI &it,
                                     const Time &now)
{
...
void
BlockAckManager::NotifyAgreementEstablished (Mac48Address recipient, uint8_t tid,
                                             uint16_t startingSeq)
{
...
bool
BlockAckManager::SwitchToBlockAckIfNeeded (Mac48Address recipient, uint8_t tid,
                                           uint16_t startingSeq)
{
  NS_LOG_FUNCTION (this << recipient << +tid << startingSeq);

...
MgtProbeResponseHeader::GetSerializedSize (void) const
{
  ...
  size += m_ehtCapability.GetSerializedSize ();
  if (m_reducedNeighborReport != nullptr)
    size += m_reducedNeighborReport->GetSerializedSize ();
  if (m_multiLinkElement != nullptr)
...
MgtProbeResponseHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (i.GetDistanceFrom (tmp) == 0)
    m_reducedNeighborReport = nullptr;
  i = (m_multiLinkElement = Create<MultiLinkElement> (WIFI_MAC_MGT_BEACON))
          ->DeserializeIfPresent (tmp = i);
  if (i.GetDistanceFrom (tmp) == 0)
    m_multiLinkElement = nullptr;
...
MgtAssocRequestHeader::MgtAssocRequestHeader () : m_listenInterval (0)
{
}
...
MgtReassocRequestHeader::MgtReassocRequestHeader () : m_currentApAddr (Mac48Address ())
{
}
...
MgtAssocResponseHeader::MgtAssocResponseHeader () : m_aid (0)
{
}

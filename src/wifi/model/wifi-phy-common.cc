  ...
  if (mode.GetModulationClass () >= WIFI_MOD_CLASS_HE)
    {
      ...
      NS_ASSERT (
          heConfiguration); //If HE/EHT modulation is used, we should have a HE configuration attached
           ...
           mode.GetModulationClass () == WIFI_MOD_CLASS_VHT)
    {
      ...
      NS_ASSERT (
          htConfiguration); //If HT/VHT modulation is used, we should have a HT configuration attached
...
GetChannelWidthForTransmission (WifiMode mode, uint16_t maxAllowedChannelWidth)
{
  ...
  if (maxAllowedChannelWidth > 20 &&
      (modulationClass ==
           WifiModulationClass::WIFI_MOD_CLASS_OFDM // all non-HT OFDM control and management frames
       || modulationClass ==
...
GetPreambleForTransmission (WifiModulationClass modulation, bool useShortPreamble)
{
  if (modulation == WIFI_MOD_CLASS_EHT)
  ...
  else if (modulation == WIFI_MOD_CLASS_HT)
    {
      ...
      useShortPreamble) //ERP_DSSS is modeled through HR_DSSS (since same preamble and modulation)
    {

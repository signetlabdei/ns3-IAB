...
void
WifiPhyStateHelper::SwitchToTx (Time txDuration, WifiConstPsduMap psdus, double txPowerDbm,
...
void
WifiPhyStateHelper::NotifyRxMpdu (Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo,
                                  const WifiTxVector &txVector)
{
                                           ...
                                           const std::vector<bool> &statusPerMpdu)
{
  NS_LOG_FUNCTION (this << *psdu << rxSignalInfo << txVector << staId << statusPerMpdu.size ()
                        << std::all_of (statusPerMpdu.begin (), statusPerMpdu.end (),
                                        [] (bool v) { return v; })); //returns true if all true
...
WifiPhyStateHelper::SwitchFromRxAbort (uint16_t operatingWidth)
{
  ...
  NS_ASSERT (
      IsStateCcaBusy ()); //abort is called (with OBSS_PD_CCA_RESET reason) before RX is set by payload start
  NotifyRxEndOk ();
  DoSwitchFromRx ();

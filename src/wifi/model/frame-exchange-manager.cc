  ...
  if (m_phy)
    {
      m_phy->TraceDisconnectWithoutContext (
          "PhyRxPayloadBegin", MakeCallback (&FrameExchangeManager::RxStartIndication, this));
    }
  m_phy = 0;
  Object::DoDispose ();
}
  ...
  if (m_phy)
    {
      ...
      m_phy->SetReceiveOkCallback (
          MakeNullCallback<void, Ptr<WifiPsdu>, RxSignalInfo, WifiTxVector, std::vector<bool>> ());
      m_phy = nullptr;
    }
}
  ...
  if (mpdu->IsFragment ())
  ...
  else if (m_mac->GetWifiRemoteStationManager ()->NeedFragmentation (mpdu))
    {
      ...
      Ptr<Packet> fragment = m_fragmentedPacket->CreateFragment (
          0, m_mac->GetWifiRemoteStationManager ()->GetFragmentSize (mpdu, 0));
      // enqueue the first fragment
      Ptr<WifiMacQueueItem> item =
          Create<WifiMacQueueItem> (fragment, mpdu->GetHeader (), mpdu->GetTimeStamp ());
...
void
FrameExchangeManager::SendMpduWithProtection (Ptr<WifiMacQueueItem> mpdu,
                                              WifiTxParameters &txParams)
{
  ...
  if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NONE)
  ...
  else if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NORMAL_ACK)
    {
      m_mpdu->GetHeader ().SetDuration (
          GetFrameDurationId (m_mpdu->GetHeader (), GetPsduSize (m_mpdu, m_txParams.m_txVector),
...
uint32_t
FrameExchangeManager::GetPsduSize (Ptr<const WifiMacQueueItem> mpdu,
  ...
  if (protection->method == WifiProtection::NONE)
  ...
  else if (protection->method == WifiProtection::CTS_TO_SELF)
    {
      ...
      ctsToSelfProtection->protectionTime =
          m_phy->CalculateTxDuration (GetCtsSize (), ctsToSelfProtection->ctsTxVector,
                                      m_phy->GetPhyBand ()) +
          m_phy->GetSifs ();
    }
}
  ...
  if (header.IsMoreFragments ())
    {
      ...
      WifiTxVector ackTxVector = m_mac->GetWifiRemoteStationManager ()->GetAckTxVector (
          header.GetAddr1 (), txParams.m_txVector);
...
Time
FrameExchangeManager::GetRtsDurationId (const WifiTxVector &rtsTxVector, Time txDuration,
                                        Time response) const
{
  ...
  ctsTxVector =
      m_mac->GetWifiRemoteStationManager ()->GetCtsTxVector (m_self, rtsTxVector.GetMode ());
...
FrameExchangeManager::SendRts (const WifiTxParameters &txParams)
{
  ...
  // aRxPHYStartDelay equals the time to transmit the PHY header.
  Time timeout = m_phy->CalculateTxDuration (GetRtsSize (), rtsCtsProtection->rtsTxVector,
                                             m_phy->GetPhyBand ()) +
                 m_phy->GetSifs () + m_phy->GetSlot () +
                 m_phy->CalculatePhyPreambleAndHeaderDuration (rtsCtsProtection->ctsTxVector);
  NS_ASSERT (!m_txTimer.IsRunning ());
  m_txTimer.Set (WifiTxTimer::WAIT_CTS, timeout, &FrameExchangeManager::CtsTimeout, this, mpdu,
...
void
FrameExchangeManager::SendCtsAfterRts (const WifiMacHeader &rtsHdr, WifiMode rtsTxMode,
                                       double rtsSnr)
{
  NS_LOG_FUNCTION (this << rtsHdr << rtsTxMode << rtsSnr);

  WifiTxVector ctsTxVector =
      m_mac->GetWifiRemoteStationManager ()->GetCtsTxVector (rtsHdr.GetAddr2 (), rtsTxMode);
  DoSendCtsAfterRts (rtsHdr, ctsTxVector, rtsSnr);
}

Time
FrameExchangeManager::GetCtsToSelfDurationId (const WifiTxVector &ctsTxVector, Time txDuration,
                                              Time response) const
{
...
FrameExchangeManager::SendCtsToSelf (const WifiTxParameters &txParams)
{
  ...
  ForwardMpduDown (Create<WifiMacQueueItem> (Create<Packet> (), cts),
                                     ...
                                     double dataSnr)
{
  ...
  WifiTxVector ackTxVector =
      m_mac->GetWifiRemoteStationManager ()->GetAckTxVector (hdr.GetAddr2 (), dataTxVector);
  WifiMacHeader ack;
...
FrameExchangeManager::NotifyInternalCollision (Ptr<Txop> txop)
{
  ...
  Ptr<const WifiMacQueueItem> mpdu =
      (qosTxop ? qosTxop->PeekNextMpdu () : txop->GetWifiMacQueue ()->Peek ());
...
void
FrameExchangeManager::Receive (Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
                               std::vector<bool> perMpduStatus)
{
  NS_LOG_FUNCTION (this << psdu << rxSignalInfo << txVector << perMpduStatus.size ()
                        << std::all_of (perMpduStatus.begin (), perMpduStatus.end (),
                                        [] (bool v) { return v; }));
  ...
  if (hdr.IsCtl ())
    {
      if (hdr.IsRts ())
          ...
          if (m_navEnd <= Simulator::Now ())
            {
              ...
              Simulator::Schedule (m_phy->GetSifs (), &FrameExchangeManager::SendCtsAfterRts, this,
                                   hdr, txVector.GetMode (), rxSnr);
            }
          else
            {
               ...
               m_txTimer.GetReason () == WifiTxTimer::WAIT_CTS && m_mpdu)
        {
          ...
          m_mac->GetWifiRemoteStationManager ()->ReportRtsOk (m_mpdu->GetHeader (), rxSnr,
      ...
      if (hdr.GetAddr1 () == m_self)
        {
          NS_LOG_DEBUG ("Received " << hdr.GetTypeString () << " from=" << hdr.GetAddr2 ()
                                    << ", schedule ACK");
          Simulator::Schedule (m_phy->GetSifs (), &FrameExchangeManager::SendNormalAck, this, hdr,
                               txVector, rxSnr);
        }

...
{
  NS_FATAL_ERROR (
      "PPDU field is not a SIG field (no sense in retrieving the signaled mode) or is unsupported: "
      << field);
...
Ptr<WifiPpdu>
PhyEntity::BuildPpdu (const WifiConstPsduMap &psdus, const WifiTxVector &txVector,
                      Time /* ppduDuration */)
{
...
PhyEntity::GetPhyHeaderSnrPer (WifiPpduField field, Ptr<Event> event) const
{
  ...
  return m_wifiPhy->m_interference->CalculatePhyHeaderSnrPer (
      event, measurementChannelWidth, GetPrimaryBand (measurementChannelWidth), field);
}
      ...
      switch (status.actionIfFailure)
          ...
          if (status.reason == FILTERED)
            {
              ...
              m_wifiPhy->m_phyRxPayloadBeginTrace (
                  txVector,
                  NanoSeconds (
                      0)); //this callback (equivalent to PHY-RXSTART primitive) is also triggered for filtered PPDUs
            }
        ...
        case IGNORE:
          ...
          m_endRxPayloadEvents.push_back (Simulator::Schedule (
              GetRemainingDurationAfterField (ppdu, field), &PhyEntity::ResetReceive, this, event));
          break;
        default:
...
PhyEntity::DoStartReceivePayload (Ptr<Event> event)
{
  ...
  m_wifiPhy->m_phyRxPayloadBeginTrace (
      txVector,
      payloadDuration); //this callback (equivalent to PHY-RXSTART primitive) is triggered only if headers have been correctly decoded and that the mode within is supported
  ...
  for (size_t i = 0; i < nMpdus && mpdu != psdu->end (); ++mpdu)
    {
      ...
      Time mpduDuration =
          m_wifiPhy->GetPayloadDuration (size, txVector, m_wifiPhy->GetPhyBand (), mpduType, true,
      ...
      if (i == (nMpdus - 1) && !remainingAmpduDuration.IsZero ()) //no more MPDUs coming
        {
          if (remainingAmpduDuration <
            ...
            {
              mpduDuration +=
                  remainingAmpduDuration; //apply a correction just in case rounding had induced slight shift
            }
        }
      ...
      m_endOfMpduEvents.push_back (
          Simulator::Schedule (endOfMpduDuration, &PhyEntity::EndOfMpdu, this, event,
...
void
PhyEntity::EndOfMpdu (Ptr<Event> event, Ptr<const WifiPsdu> psdu, size_t mpduIndex,
...
PhyEntity::EndReceivePayload (Ptr<Event> event)
{
  ...
  double snr = m_wifiPhy->m_interference->CalculateSnr (
      event, channelWidthAndBand.first, txVector.GetNss (staId), channelWidthAndBand.second);
  ...
  if (std::count (statusPerMpduIt->second.begin (), statusPerMpduIt->second.end (), true))
    {
      ...
      m_wifiPhy->NotifyMonitorSniffRx (psdu, m_wifiPhy->GetFrequency (), txVector,
                                       signalNoiseIt->second, statusPerMpduIt->second, staId);
      RxSignalInfo rxSignalInfo;
      ...
      m_wifiPhy->m_previouslyRxPpduUid =
          ppdu->GetUid (); //store UID only if reception is successful (because otherwise trigger won't be read by MAC layer)
    }
  else
    {
                               ...
                               Time relativeMpduStart, Time mpduDuration)
{
  ...
  SnrPer snrPer = m_wifiPhy->m_interference->CalculatePayloadSnrPer (
      event, channelWidthAndBand.first, channelWidthAndBand.second, staId,
...
Ptr<Event>
PhyEntity::CreateInterferenceEvent (Ptr<const WifiPpdu> ppdu, const WifiTxVector &txVector,
                                    Time duration, RxPowerWattPerChannelBand &rxPower,
...
PhyEntity::DoEndReceivePreamble (Ptr<Event> event)
{
  ...
  return PhyFieldRxStatus (
      true); //always consider that preamble has been correctly received if preamble detection was OK
}
...
PhyEntity::EndPreambleDetectionPeriod (Ptr<Event> event)
{
  ...
  NS_ASSERT (
      m_wifiPhy->m_endPhyRxEvent
          .IsExpired ()); //since end of preamble reception is scheduled by this method upon success

  //calculate PER on the measurement channel for PHY headers
  uint16_t measurementChannelWidth = GetMeasurementChannelWidth (event->GetPpdu ());
  auto measurementBand = GetPrimaryBand (measurementChannelWidth);
  double maxRxPowerW =
      -1; //in case current event may not be sent on measurement channel (rxPowerW would be equal to 0)
  Ptr<Event> maxEvent;
  ...
  double snr = m_wifiPhy->m_interference->CalculateSnr (
      m_wifiPhy->m_currentEvent, measurementChannelWidth, 1, measurementBand);
  ...
  if ((!m_wifiPhy->m_preambleDetectionModel && maxRxPowerW > 0.0) ||
      (m_wifiPhy->m_preambleDetectionModel &&
       m_wifiPhy->m_preambleDetectionModel->IsPreambleDetected (
           m_wifiPhy->m_currentEvent->GetRxPowerW (measurementBand), snr, measurementChannelWidth)))
    {
      ...
      m_wifiPhy->m_endPhyRxEvent = Simulator::Schedule (
          durationTillEnd, &PhyEntity::EndReceiveField, this, WIFI_PPDU_FIELD_PREAMBLE, event);
    }
  else
    {
      ...
      DropPreambleEvent (m_wifiPhy->m_currentEvent->GetPpdu (), PREAMBLE_DETECT_FAILURE,
...
void
PhyEntity::NotifyCcaBusy (const Ptr<const WifiPpdu> /*ppdu*/, Time duration,
...
void
PhyEntity::Transmit (Time txDuration, Ptr<const WifiPpdu> ppdu, const WifiTxVector &txVector,
                     std::string type)
{
...
Time
PhyEntity::CalculateTxDuration (WifiConstPsduMap psduMap, const WifiTxVector &txVector,

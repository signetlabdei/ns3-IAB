...
public:
   ...
   */
  void
  SetWifiRemoteStationManagers (const std::vector<Ptr<WifiRemoteStationManager>> &stationManagers);
  /**
...
protected:
  ...
  struct LinkEntity
  {
    ...
    Ptr<WifiRemoteStationManager>
        stationManager; /**< Remote station manager (rate control, RTS/CTS/
...
private:
   ...
   */
  void ConfigureDcf (Ptr<Txop> dcf, uint32_t cwmin, uint32_t cwmax, std::list<bool> isDsss,
                     AcIndex ac);
  ...
  typedef TracedCallback<uint8_t, Ptr<const WifiMacQueueItem>, const WifiTxVector &>

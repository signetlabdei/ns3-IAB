...
static const uint8_t MAX_SUPPORTED_STREAMS =
    4; //!< Maximal number of streams supported by the PHY layer.
static const uint8_t MAX_HT_STREAM_GROUPS =
    4; //!< Maximal number of groups per stream in HT (2 possible channel widths and 2 possible SGI configurations).
static const uint8_t MAX_VHT_STREAM_GROUPS =
    8; //!< Maximal number of groups per stream in VHT (4 possible channel widths and 2 possible SGI configurations).
...
private:
   ...
   */
  Time CalculateMpduTxDuration (Ptr<WifiPhy> phy, uint8_t streams, uint8_t sgi, uint16_t chWidth,
   ...
   */
  void UpdatePacketCounters (MinstrelHtWifiRemoteStation *station, uint8_t nSuccessfulMpdus,
  ...
  Ptr<MinstrelWifiManager>
      m_legacyManager; //!< Pointer to an instance of MinstrelWifiManager. Used when 802.11n/ac not supported.

...
CtrlBAckRequestHeader::CtrlBAckRequestHeader ()
    : m_barAckPolicy (false), m_barType (BlockAckReqType::BASIC)
{
}
...
CtrlBAckResponseHeader::CtrlBAckResponseHeader () : m_baAckPolicy (false), m_tidInfo (0)
{
...
CtrlBAckResponseHeader::Print (std::ostream &os) const
{
  if (m_baType.m_variant != BlockAckType::MULTI_STA)
    {
      os << "TID_INFO=" << m_tidInfo << ", StartingSeq=0x" << std::hex << m_baInfo[0].m_startingSeq
         << std::dec;
    }
  else
    {
...
void
CtrlTriggerUserInfoField::SetBasicTriggerDepUserInfo (uint8_t spacingFactor, uint8_t tidLimit,
                                                      AcIndex prefAc)
{
...
CtrlTriggerHeader::GetHeTbTxVector (uint16_t staId) const
{
  ...
  v.SetHeMuUserInfo (staId, {userInfoIt->GetRuAllocation (),
                             HePhy::GetHeMcs (userInfoIt->GetUlMcs ()), userInfoIt->GetNss ()});
  return v;
}

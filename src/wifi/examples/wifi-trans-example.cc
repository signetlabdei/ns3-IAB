...
main (int argc, char **argv)
{
  ...
  cmd.AddValue ("verbose", "Display log messages for WifiSpectrumValueHelper and SpectrumWifiPhy",
                verbose);
  cmd.Parse (argc, argv);
  ...
  spectrumAnalyzerHelper.SetPhyAttribute (
      ...
      TimeValue (MicroSeconds (
          4))); //enough resolution to distinguish OFDM symbols (default 1ms too long even for PPDUs)

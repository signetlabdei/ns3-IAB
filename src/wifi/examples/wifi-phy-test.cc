...
private:
   ...
   */
  void Receive (Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
...
void
PsrExperiment::Receive (Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
                        std::vector<bool> statusPerMpdu)
{
  m_output.received++;
}
...
PsrExperiment::Input::Input ()
    : distance (5.0), txMode ("OfdmRate6Mbps"), txPowerLevel (0), packetSize (2304), nPackets (400)
{
}
...
void
CollisionExperiment::Receive (Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
                              std::vector<bool> statusPerMpdu)
{
  FlowIdTag tag;
...
PrintPsr (int argc, char *argv[])
{
  ...
  cmd.AddValue ("TxPowerLevel", "The power level index to use to send each packet",
                input.txPowerLevel);
  cmd.Parse (argc, argv);

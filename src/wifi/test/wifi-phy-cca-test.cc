...
const std::map<uint16_t, Time> PpduDurations = {
    {20, NanoSeconds (1009600)}, {40, NanoSeconds (533600)}, {80, NanoSeconds (275200)}};
...
private:
   ...
   */
  void VerifyCcaThreshold (const Ptr<PhyEntity> phy, const Ptr<const WifiPpdu> ppdu,
  ...
  double
      m_CcaSensitivityDbm; ///< The current CCA sensitivity threshold for signals that occupy the primary 20 MHz channel (in dBm)

  VhtConfiguration::SecondaryCcaSensitivityThresholds
      m_secondaryCcaSensitivityThresholds; ///< The current CCA sensitivity thresholds for signals that do not occupy the primary 20 MHz channel (in dBm)
...
WifiPhyCcaThresholdsTest::CreateDummyHtPpdu (uint16_t bandwidth)
{
  ...
  Ptr<WifiPsdu> psdu = CreateDummyPsdu ();
  return Create<HtPpdu> (psdu, txVector, P20_CENTER_FREQUENCY, MicroSeconds (100),
...
WifiPhyCcaThresholdsTest::CreateDummyVhtPpdu (uint16_t bandwidth)
{
  ...
  Ptr<WifiPsdu> psdu = CreateDummyPsdu ();
  return Create<VhtPpdu> (psdu, txVector, P20_CENTER_FREQUENCY, MicroSeconds (100),
                          WIFI_PHY_BAND_5GHZ, 0);
}
...
WifiPhyCcaThresholdsTest::CreateDummyHePpdu (uint16_t bandwidth)
{
  ...
  Ptr<WifiPsdu> psdu = CreateDummyPsdu ();
  return Create<HePpdu> (psdu, txVector, P20_CENTER_FREQUENCY, MicroSeconds (100),
                         WIFI_PHY_BAND_5GHZ, 0);
}
...
WifiPhyCcaThresholdsTest::DoSetup (void)
{
  ...
  auto channelNum = std::get<0> (*WifiPhyOperatingChannel::FindFirst (
      0, P20_CENTER_FREQUENCY, 20, WIFI_STANDARD_80211ax, WIFI_PHY_BAND_5GHZ));
...
WifiPhyCcaThresholdsTest::RunOne (void)
{
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_OFDM), nullptr, WIFI_CHANLIST_PRIMARY,
                      m_CcaEdThresholdDbm);

  //-----------------------------------------------------------------------------------------------------------------------------------

  //OFDM PHY: 20 MHz non-HT PPDU in primary channel (20 MHz) if power above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_OFDM), CreateDummyNonHtPpdu (),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //-----------------------------------------------------------------------------------------------------------------------------------

  //HT PHY: any signal in primary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HT), nullptr, WIFI_CHANLIST_PRIMARY,
                      m_CcaEdThresholdDbm);

  //HT PHY: any signal in primary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HT), nullptr, WIFI_CHANLIST_SECONDARY,
                      m_CcaEdThresholdDbm);

  //-----------------------------------------------------------------------------------------------------------------------------------

  //HT PHY: 20 MHz HT PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HT), CreateDummyHtPpdu (20),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //HT PHY: 40 MHz HT PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HT), CreateDummyHtPpdu (40),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //-----------------------------------------------------------------------------------------------------------------------------------

  //VHT PHY: any signal in primary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), nullptr, WIFI_CHANLIST_PRIMARY,
                      m_CcaEdThresholdDbm);

  //VHT PHY: any signal in secondary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), nullptr, WIFI_CHANLIST_SECONDARY,
                      m_CcaEdThresholdDbm);

  //VHT PHY: any signal in secondary40 channel (40 MHz) if power above CCA-ED threshold + 3dB
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), nullptr, WIFI_CHANLIST_SECONDARY40,
                      m_CcaEdThresholdDbm + 3);

  //VHT PHY: any signal in secondary80 channel (80 MHz) if power above CCA-ED threshold + 6dB
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), nullptr, WIFI_CHANLIST_SECONDARY80,
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (20),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //VHT PHY: 40 MHz VHT PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (40),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //VHT PHY: 80 MHz VHT PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (80),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //VHT PHY: 160 MHz VHT PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (160),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (20),
                      WIFI_CHANLIST_SECONDARY, std::get<0> (m_secondaryCcaSensitivityThresholds));
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (20),
                      WIFI_CHANLIST_SECONDARY40, std::get<0> (m_secondaryCcaSensitivityThresholds));
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (40),
                      WIFI_CHANLIST_SECONDARY40, std::get<1> (m_secondaryCcaSensitivityThresholds));
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (20),
                      WIFI_CHANLIST_SECONDARY80, std::get<0> (m_secondaryCcaSensitivityThresholds));
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (40),
                      WIFI_CHANLIST_SECONDARY80, std::get<1> (m_secondaryCcaSensitivityThresholds));
  ...
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_VHT), CreateDummyVhtPpdu (80),
                      WIFI_CHANLIST_SECONDARY80, std::get<2> (m_secondaryCcaSensitivityThresholds));

  //-----------------------------------------------------------------------------------------------------------------------------------

  //HE PHY: any signal in primary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), nullptr, WIFI_CHANLIST_PRIMARY,
                      m_CcaEdThresholdDbm);

  //HE PHY: any signal in secondary channel (20 MHz) if power above CCA-ED threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), nullptr, WIFI_CHANLIST_SECONDARY,
                      m_CcaEdThresholdDbm);

  //HE PHY: any signal in secondary40 channel (40 MHz) if power above CCA-ED threshold + 3dB
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), nullptr, WIFI_CHANLIST_SECONDARY40,
                      m_CcaEdThresholdDbm + 3);

  //HE PHY: any signal in secondary80 channel (80 MHz) if power above CCA-ED threshold + 6dB
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), nullptr, WIFI_CHANLIST_SECONDARY80,
                      m_CcaEdThresholdDbm + 6);

  //-----------------------------------------------------------------------------------------------------------------------------------

  //HE PHY: 20 MHz HE PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (20),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //HE PHY: 40 MHz HE PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (40),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //HE PHY: 80 MHz HE PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (80),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);

  //HE PHY: 160 MHz HE PPDU in primary channel (20 MHz) if power in primary above CCA sensitivty threshold
  VerifyCcaThreshold (m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (160),
                      WIFI_CHANLIST_PRIMARY, m_CcaSensitivityDbm);
  ...
  VerifyCcaThreshold (
      m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (40), WIFI_CHANLIST_SECONDARY40,
  ...
  //corresponding to a 40 MHz PPDU that does not occupy the primary 20 MHz and the OBSS-PD level plus 3 dB
  VerifyCcaThreshold (
      m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (40), WIFI_CHANLIST_SECONDARY80,
      std::max (m_obssPdLevel + 3.0, std::get<1> (m_secondaryCcaSensitivityThresholds)));
  ...
  VerifyCcaThreshold (
      m_phy->GetPhyEntity (WIFI_MOD_CLASS_HE), CreateDummyHePpdu (80), WIFI_CHANLIST_SECONDARY80,
...
private:
   ...
   */
  void StartSignal (Ptr<WaveformGenerator> signalGenerator, double txPowerDbm, uint16_t frequency,
                    uint16_t bandwidth, Time duration);
  /**
   ...
   */
  void CheckLastCcaBusyNotification (Time expectedEndTime, WifiChannelListType expectedChannelType,
   ...
   */
  void ScheduleTest (Time delay, const std::vector<TxSignalInfo> &generatedSignals,
...
void
WifiPhyCcaIndicationTest::StartSignal (Ptr<WaveformGenerator> signalGenerator, double txPowerDbm,
                                        ...
                                        const std::vector<CcaCheckPoint> &ccaCheckpoints)
{
  for (const auto &generatedPpdu : generatedPpdus)
    {
      Simulator::Schedule (delay + generatedPpdu.startTime, &WifiPhyCcaIndicationTest::SendHeSuPpdu,
                           this, generatedPpdu.power, generatedPpdu.centerFreq,
  ...
  for (const auto &generatedSignal : generatedSignals)
    {
      Simulator::Schedule (
          delay + generatedSignal.startTime, &WifiPhyCcaIndicationTest::StartSignal, this,
          m_signalGenerators.at (index++), generatedSignal.power, generatedSignal.centerFreq,
          generatedSignal.bandwidth, generatedSignal.duration);
    }

  for (const auto &checkpoint : ccaCheckpoints)
    {
      Simulator::Schedule (
          delay + checkpoint.timePoint, &WifiPhyCcaIndicationTest::CheckLastCcaBusyNotification,
          this, Simulator::Now () + delay + checkpoint.expectedCcaEndTime,
          checkpoint.expectedChannelListType, checkpoint.expectedPer20MhzDurations);
    }

  for (const auto &checkpoint : stateCheckpoints)
    {
      Simulator::Schedule (delay + checkpoint.timePoint, &WifiPhyCcaIndicationTest::CheckPhyState,
                           this, checkpoint.expectedPhyState);
    }
...
WifiPhyCcaIndicationTest::RunOne (void)
{
  ...
  ScheduleTest (delay, {{-65.0, MicroSeconds (0), MicroSeconds (100), P20_CENTER_FREQUENCY, 20}},
                {},
                {
  ...
  ScheduleTest (
      delay, {{-60.0, MicroSeconds (0), MicroSeconds (100), P20_CENTER_FREQUENCY, 20}}, {},
      {
      ...
      {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
        ((m_channelWidth > 20)
             ? ((m_channelWidth > 40)
                    ? ((m_channelWidth > 80)
  ...
  if (m_channelWidth > 20)
    {
      ...
      ScheduleTest (
          delay, {{-60.0, MicroSeconds (0), MicroSeconds (100), S20_CENTER_FREQUENCY, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY,
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
      ...
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), P40_CENTER_FREQUENCY, 40}}, {},
          {
              {aCcaTime, WifiPhyState::CCA_BUSY}, //CCA-BUSY after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::CCA_BUSY}, //CCA-BUSY just before the transmission ends
      ...
      ScheduleTest (
          ...
          {},
          {
              ...
              {MicroSeconds (50) + aCcaTime,
               WifiPhyState::
                   CCA_BUSY}, //state of primary is CCA-BUSY after aCCATime that followed the second transmission
          ...
          {{aCcaTime, //notification upon reception of the first signal
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            ...
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
                        ? std::vector<Time>{MicroSeconds (100), MicroSeconds (50), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0)}
                        : std::vector<Time>{MicroSeconds (100), MicroSeconds (50), MicroSeconds (0),
                                            MicroSeconds (0)})
      ...
      ScheduleTest (
          ...
          {},
          {
              ...
              {MicroSeconds (50) + aCcaTime,
               WifiPhyState::
                   CCA_BUSY}, //state of primary is still CCA-BUSY after aCCATime that followed the second transmission
              {MicroSeconds (100) - smallDelta,
          ...
          {{aCcaTime, //notification upon reception of the first signal
            ...
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
                        ? std::vector<Time>{MicroSeconds (100), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0)}
                        : std::vector<Time>{MicroSeconds (100), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0)})
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            ...
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
                        ? std::vector<Time>{MicroSeconds (50), MicroSeconds (100), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0)}
                        : std::vector<Time>{MicroSeconds (50), MicroSeconds (100), MicroSeconds (0),
                                            MicroSeconds (0)})
      ...
      ScheduleTest (delay, {}, {{-75.0, MicroSeconds (0), S20_CENTER_FREQUENCY, 20}},
                    {
                        {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
                        {PpduDurations.at (20) - smallDelta,
                         WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          ...
          {{aCcaTime, PpduDurations.at (20), WIFI_CHANLIST_SECONDARY,
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
                        ? std::vector<Time>{NanoSeconds (0), PpduDurations.at (20), NanoSeconds (0),
                                            NanoSeconds (0), NanoSeconds (0), NanoSeconds (0),
      ...
      Simulator::Schedule (
          delay, &WifiPhyCcaIndicationTest::LogScenario, this,
          "Reception of a 20 MHz signal that occupies S20 above ED threshold followed by a 40 MHz "
          "HE PPDU that occupies P40 below CCA sensitivity threshold");
      ScheduleTest (
          delay,
          ...
          {{MicroSeconds (50) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY,
            ((m_channelWidth > 20)
                 ? ((m_channelWidth > 40)
                        ? ((m_channelWidth > 80)
                               ? std::vector<Time>{MicroSeconds (0), MicroSeconds (100),
           ...
           {MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY,
            ((m_channelWidth > 40)
                 ? ((m_channelWidth > 80)
                        ? std::vector<Time>{MicroSeconds (0), MicroSeconds (46), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                            MicroSeconds (0), MicroSeconds (0)}
                        : std::vector<Time>{MicroSeconds (0), MicroSeconds (46), MicroSeconds (0),
                                            MicroSeconds (0)})
  ...
  if (m_channelWidth > 40)
    {
      ...
      Simulator::Schedule (delay, &WifiPhyCcaIndicationTest::LogScenario, this,
                           "Reception of a 20 MHz signal that occupies the first subchannel of S40 "
                           "below ED threshold");
      ScheduleTest (
          delay, {{-65.0, MicroSeconds (0), MicroSeconds (100), S40_CENTER_FREQUENCY - 10, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      Simulator::Schedule (delay, &WifiPhyCcaIndicationTest::LogScenario, this,
                           ...
                           "above ED threshold");
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S40_CENTER_FREQUENCY - 10, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY40,
            ((m_channelWidth > 80)
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY40,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (100), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
                 : std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (100)})}});
      delay += Seconds (1.0);
      ...
      Simulator::Schedule (delay, &WifiPhyCcaIndicationTest::LogScenario, this,
                           ...
                           "S40 above ED threshold");
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S40_CENTER_FREQUENCY, 40}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), P80_CENTER_FREQUENCY, 80}}, {},
          {
              {aCcaTime, WifiPhyState::CCA_BUSY}, //CCA-BUSY after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::CCA_BUSY}, //CCA-BUSY just before the transmission ends
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (100), MicroSeconds (100), MicroSeconds (100),
                                     MicroSeconds (100), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
                 : std::vector<Time>{MicroSeconds (100), MicroSeconds (100), MicroSeconds (100),
                                     MicroSeconds (100)})}});
      ...
      ScheduleTest (
          ...
          {{aCcaTime, //notification upon reception of the first signal
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            MicroSeconds (50) + MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (100), MicroSeconds (0), MicroSeconds (50),
                                     MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
                 : std::vector<Time>{MicroSeconds (100), MicroSeconds (0), MicroSeconds (50),
                                     MicroSeconds (0)})}});
      ...
      ScheduleTest (
          ...
          {{aCcaTime, //notification upon reception of the first signal
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            ...
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (0), MicroSeconds (100), MicroSeconds (50),
                                     MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
                 : std::vector<Time>{MicroSeconds (0), MicroSeconds (100), MicroSeconds (50),
                                     MicroSeconds (0)})}});
      ...
      ScheduleTest (delay, {}, {{-75.0, MicroSeconds (0), S40_CENTER_FREQUENCY, 40}},
                    {
                        {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
                        {PpduDurations.at (20) - smallDelta,
                         WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          ...
          {{aCcaTime, PpduDurations.at (40), WIFI_CHANLIST_SECONDARY40,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{NanoSeconds (0), NanoSeconds (0), PpduDurations.at (40),
                                     PpduDurations.at (40), NanoSeconds (0), NanoSeconds (0),
                                     NanoSeconds (0), NanoSeconds (0)}
                 : std::vector<Time>{NanoSeconds (0), NanoSeconds (0), PpduDurations.at (40),
                                     PpduDurations.at (40)})}});
      delay += Seconds (1.0);
      ...
      Simulator::Schedule (
          delay, &WifiPhyCcaIndicationTest::LogScenario, this,
          "Reception of a 40 MHz signal that occupies S40 above ED threshold followed by a 80 MHz "
          "HE PPDU that occupies P80 below CCA sensitivity threshold");
      ScheduleTest (
          delay,
          ...
          {{MicroSeconds (50) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY40,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (100),
                                     MicroSeconds (100), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
           ...
           {MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY40,
            ((m_channelWidth > 80)
                 ? std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (46),
                                     MicroSeconds (46), MicroSeconds (0), MicroSeconds (0),
                                     MicroSeconds (0), MicroSeconds (0)}
                 : std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (46),
                                     MicroSeconds (46)})}});
      delay += Seconds (1.0);
  ...
  else // 20 or 40 MHz receiver
    {
      ...
      ScheduleTest (
          ...
          {{aCcaTime, MicroSeconds (16), WIFI_CHANLIST_PRIMARY,
            ((m_channelWidth > 20)
                 ? ((m_channelWidth > 40)
                        ? ((m_channelWidth > 80)
                               ? std::vector<Time>{Seconds (0), Seconds (0), Seconds (0),
                        ...
                        : std::vector<Time>{Seconds (0), Seconds (0)})
                 : std::vector<Time>{})},
           {PpduDurations.at (80) - smallDelta, PpduDurations.at (80), WIFI_CHANLIST_PRIMARY,
            ((m_channelWidth > 20)
                 ? ((m_channelWidth > 40)
                        ? ((m_channelWidth > 80)
                               ? std::vector<Time>{Seconds (0), Seconds (0), Seconds (0),
                 ...
                 ? ((m_channelWidth > 40)
                        ? ((m_channelWidth > 80)
                               ? std::vector<Time>{NanoSeconds (271200), NanoSeconds (271200),
                                                   NanoSeconds (271200), NanoSeconds (271200),
                                                   NanoSeconds (0), NanoSeconds (0),
                                                   NanoSeconds (0), NanoSeconds (0)}
                               : std::vector<Time>{NanoSeconds (271200), NanoSeconds (271200),
                                                   NanoSeconds (271200), NanoSeconds (271200)})
                 ...
                 ? ((m_channelWidth > 40)
                        ? ((m_channelWidth > 80)
                               ? std::vector<Time>{NanoSeconds (243200), NanoSeconds (243200),
                                                   NanoSeconds (243200), NanoSeconds (243200),
                                                   NanoSeconds (0), NanoSeconds (0),
                                                   NanoSeconds (0), NanoSeconds (0)}
                               : std::vector<Time>{NanoSeconds (243200), NanoSeconds (243200),
                                                   NanoSeconds (243200), NanoSeconds (243200)})
      ...
      ScheduleTest (delay, {}, {{-50.0, MicroSeconds (0), S40_CENTER_FREQUENCY, 40}},
                    {
                        {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
                        {PpduDurations.at (20) - smallDelta,
                         WifiPhyState::IDLE}, //IDLE just before the transmission ends
  ...
  if (m_channelWidth > 80)
    {
      ...
      Simulator::Schedule (delay, &WifiPhyCcaIndicationTest::LogScenario, this,
                           "Reception of a 20 MHz signal that occupies the first subchannel of S80 "
                           "below ED threshold");
      ScheduleTest (
          delay, {{-65.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 30, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      Simulator::Schedule (delay, &WifiPhyCcaIndicationTest::LogScenario, this,
                           ...
                           "above ED threshold");
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 30, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY80,
            std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
      ...
      ScheduleTest (
          delay, {{-65.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 10, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 10, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          delay, {{-65.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 20, 40}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY - 20, 40}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
          ...
          {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_SECONDARY80,
            std::vector<Time>{MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
                              MicroSeconds (0), MicroSeconds (100), MicroSeconds (100),
                              MicroSeconds (0), MicroSeconds (0)}}});
      delay += Seconds (1.0);
      ...
      ScheduleTest (
          delay, {{-55.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY, 80}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (delay,
                    ...
                    {{MicroSeconds (100) - smallDelta, MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
                      std::vector<Time>{MicroSeconds (100), MicroSeconds (100), MicroSeconds (100),
                                        MicroSeconds (100), MicroSeconds (100), MicroSeconds (100),
                                        MicroSeconds (100), MicroSeconds (100)}}});
      delay += Seconds (1.0);
      ...
      ScheduleTest (
          ...
          {{aCcaTime, //notification upon reception of the first signal
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            MicroSeconds (50) + MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
            std::vector<Time>{MicroSeconds (100), MicroSeconds (0), MicroSeconds (00),
                              MicroSeconds (0), MicroSeconds (0), MicroSeconds (0),
      ...
      Simulator::Schedule (
          delay, &WifiPhyCcaIndicationTest::LogScenario, this,
          "Reception of a 40 MHz HE PPDU that occupies S40 below CCA sensitivity threshold");
      ScheduleTest (delay, {}, {{-70.0, MicroSeconds (0), S80_CENTER_FREQUENCY, 80}},
                    {
                        {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
                        {PpduDurations.at (20) - smallDelta,
                         WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          ...
          {{aCcaTime, PpduDurations.at (80), WIFI_CHANLIST_SECONDARY80,
            std::vector<Time>{NanoSeconds (0), NanoSeconds (0), NanoSeconds (0), NanoSeconds (0),
                              PpduDurations.at (80), PpduDurations.at (80), PpduDurations.at (80),
                              PpduDurations.at (80)}}});
      delay += Seconds (1.0);
      ...
      ScheduleTest (
          delay, {{-60.0, MicroSeconds (0), MicroSeconds (100), S80_CENTER_FREQUENCY + 30, 20}}, {},
          {
              {aCcaTime, WifiPhyState::IDLE}, //IDLE after aCCATime
              {MicroSeconds (100) - smallDelta,
               WifiPhyState::IDLE}, //IDLE just before the transmission ends
      ...
      ScheduleTest (
          ...
          {{aCcaTime, MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
           ...
           {MicroSeconds (50) + aCcaTime, //notification upon reception of the second signal
            MicroSeconds (100), WIFI_CHANLIST_PRIMARY,
            std::vector<Time>{MicroSeconds (50), MicroSeconds (50), MicroSeconds (50),
                              MicroSeconds (50), MicroSeconds (0), MicroSeconds (0),
                              MicroSeconds (0), MicroSeconds (200)}}});
      delay += Seconds (1.0);
    }

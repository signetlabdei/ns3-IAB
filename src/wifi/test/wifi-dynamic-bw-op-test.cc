...
void
WifiUseAvailBwTest::Transmit (uint8_t bss, WifiConstPsduMap psduMap, WifiTxVector txVector,
                              double txPowerW)
{
  ...
  if (bss == 1 && psdu->GetNMpdus () == m_txPkts && now >= Seconds (1.5))
    {
      ...
      Simulator::Schedule (MicroSeconds (1), [&] () {
        ...
        NS_TEST_EXPECT_MSG_EQ (
            cam->GetLargestIdlePrimaryChannel (MicroSeconds (1), Simulator::Now ()), m_bss0Width,
...
WifiDynamicBwOpTestSuite::WifiDynamicBwOpTestSuite () : TestSuite ("wifi-dynamic-bw-op", UNIT)
{
  /**
   *                    primary20
   *          ┌────────┬────────┐

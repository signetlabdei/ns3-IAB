...
public:
   ...
   * \param perMpduStatus per MPDU reception status
   */
  void ReceiveUl (uint8_t bss, Ptr<WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
...
void
WifiPrimaryChannelsTest::Transmit (std::string context, WifiConstPsduMap psduMap,
...
WifiPrimaryChannelsTest::DoSetup (void)
{
  ...
  for (uint8_t bss = 0; bss < m_nBss; bss++)
    {
      ...
      mac.SetType ("ns3::ApWifiMac", "Ssid", SsidValue (Ssid ("wifi-ssid-" + std::to_string (bss))),
                   "BeaconInterval", TimeValue (MicroSeconds (102400)), "EnableBeaconJitter",
                   BooleanValue (false));
  ...
  m_triggerTxVector = WifiTxVector (OfdmPhy::GetOfdmRate6Mbps (), 0, WIFI_PREAMBLE_LONG, 800, 1, 1,
                                    0, 20, false, false, false);
  m_trigger = Create<WifiPsdu> (pkt, hdr);

  m_triggerTxDuration = WifiPhy::CalculateTxDuration (
      m_trigger->GetSize (), m_triggerTxVector, apDev->GetMac ()->GetWifiPhy ()->GetPhyBand ());
}
...
WifiPrimaryChannelsTest::DoRun (void)
{
      ...
      for (uint16_t i = 1; i < m_nStationsPerBss; i++)
        {
          ...
          Simulator::Schedule (i * MicroSeconds (102400), &WifiMac::SetSsid, dev->GetMac (),
  ...
  for (uint16_t txChannelWidth = 20, nRounds = 2, nApsPerRound = m_channelWidth / 20 / 2;
       txChannelWidth <= m_channelWidth; txChannelWidth *= 2, nRounds *= 2, nApsPerRound /= 2)
    {
          ...
          for (uint16_t i = 0; i < nApsPerRound; i++)
            {
              ...
              txBss.insert (ap);
              Simulator::Schedule (m_time, &WifiPrimaryChannelsTest::SendDlSuPpdu, this, ap,
                                   txChannelWidth);
            }
          // check that the SU frames were correctly received
          Simulator::Schedule (m_time + roundDuration,
                               &WifiPrimaryChannelsTest::CheckReceivedSuPpdus, this, txBss,
                               txChannelWidth);
                  ...
                  for (uint16_t i = 0; i < nApsPerRound; i++)
                    {
                      uint16_t ap = round + i * nRounds;
                      txBss.insert (ap);
                      Simulator::Schedule (m_time, &WifiPrimaryChannelsTest::SendDlMuPpdu, this, ap,
                                           txChannelWidth, ruType, nRus);
                    }
                  // check that the MU frame was correctly received
                  Simulator::Schedule (m_time + roundDuration,
                                       &WifiPrimaryChannelsTest::CheckReceivedMuPpdus, this, txBss,
                                       txChannelWidth, ruType, nRus, /* isDlMu */ true);
                  m_time += roundDuration;
                }
            }
        }
              ...
              if (nRus > 0)
                {
                  for (uint16_t i = 0; i < nApsPerRound; i++)
                    {
                      uint16_t ap = round + i * nRounds;
                      txBss.insert (ap);
                      Simulator::Schedule (m_time, &WifiPrimaryChannelsTest::SendHeTbPpdu, this, ap,
                                           txChannelWidth, ruType, nRus);
                    }
                  ...
                  Simulator::Schedule (m_time + m_triggerTxDuration +
                                       ...
                                       &WifiPrimaryChannelsTest::CheckReceivedTriggerFrames, this,
                                       txBss, txChannelWidth);
                  Simulator::Schedule (m_time + roundDuration,
                                       &WifiPrimaryChannelsTest::CheckReceivedMuPpdus, this, txBss,
                                       txChannelWidth, ruType, nRus, /* isDlMu */ false);
                  m_time += roundDuration;
                }
            }
        }
...
WifiPrimaryChannelsTest::SendDlSuPpdu (uint8_t bss, uint16_t txChannelWidth)
{
  ...
  WifiTxVector txVector = WifiTxVector (HePhy::GetHeMcs8 (), 0, WIFI_PREAMBLE_HE_SU, 800, 1, 1, 0,
...
void
WifiPrimaryChannelsTest::SendDlMuPpdu (uint8_t bss, uint16_t txChannelWidth, HeRu::RuType ruType,
                                       std::size_t nRus)
{
  ...
  WifiTxVector txVector = WifiTxVector (HePhy::GetHeMcs8 (), 0, WIFI_PREAMBLE_HE_MU, 800, 1, 1, 0,
                                        txChannelWidth, false, false, false, bssColor);
  WifiMacHeader hdr;
  hdr.SetType (WIFI_MAC_QOSDATA);
  hdr.SetQosTid (0);
...
void
WifiPrimaryChannelsTest::SendHeTbPpdu (uint8_t bss, uint16_t txChannelWidth, HeRu::RuType ruType,
                                       std::size_t nRus)
{
  ...
  Simulator::Schedule (m_triggerTxDuration + apDev->GetPhy ()->GetSifs (),
                       &WifiPrimaryChannelsTest::DoSendHeTbPpdu, this, bss, txChannelWidth, ruType,
                       nRus);
}

void
WifiPrimaryChannelsTest::DoSendHeTbPpdu (uint8_t bss, uint16_t txChannelWidth, HeRu::RuType ruType,
                                         std::size_t nRus)
{
  auto apDev = DynamicCast<WifiNetDevice> (m_apDevices.Get (bss));
  uint8_t bssColor = apDev->GetHeConfiguration ()->GetBssColor ();
  ...
  uint16_t length = 0;
  WifiTxVector trigVector (HePhy::GetHeMcs8 (), 0, WIFI_PREAMBLE_HE_TB, 3200, 1, 1, 0,
                           txChannelWidth, false, false, false, bssColor);
  ...
  for (std::size_t i = 1; i <= nRus; i++)
    {
      ...
      WifiTxVector txVector (HePhy::GetHeMcs8 (), 0, WIFI_PREAMBLE_HE_TB, 3200, 1, 1, 0,
                             txChannelWidth, false, false, false, bssColor);
      txVector.SetHeMuUserInfo (staId, {{ruType, index, primary80}, HePhy::GetHeMcs8 (), 1});
      ...
      if (duration.IsZero ())
        {
          ...
          duration = WifiPhy::CalculateTxDuration (
              psdu->GetSize (), txVector, staDev->GetMac ()->GetWifiPhy ()->GetPhyBand (), staId);
...
void
Wifi20MHzChannelIndicesTest::RunOne (uint8_t primary20, const std::set<uint8_t> &secondary20,

...
private:
  Time
  GetSifs (void) const override
  {
    return m_sifs;
  }

  Time
  GetSlot (void) const override
  {
    return m_slot;
  }

  Time
  GetEifsNoDifs (void) const override
  {
    return m_eifsNoDifs;
  }
...
public:
   ...
   * \param test the test channel access manager
   */
  FrameExchangeManagerStub (ChannelAccessManagerTest<TxopType> *test) : m_test (test)
  {
  }
  /**
   ...
   */
  bool
  StartTransmission (Ptr<Txop> dcf, uint16_t allowedWidth) override
  {
  ...
  void
  NotifyInternalCollision (Ptr<Txop> txop) override
  {
  ...
  void
  NotifySwitchingStartNow (Time duration) override
  {
...
template <typename TxopType>
ChannelAccessManagerTest<TxopType>::ChannelAccessManagerTest () : TestCase ("ChannelAccessManager")
{
}
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyTxStartNow,
...
template <typename TxopType>
void
ChannelAccessManagerTest<TxopType>::ExpectInternalCollision (uint64_t time, uint32_t nSlots,
                                                             uint32_t from)
{
...
{
  Simulator::Schedule (MicroSeconds (time) - Now (), &ChannelAccessManagerTest::DoCheckBusy, this,
                       busy);
}
...
template <typename TxopType>
void
ChannelAccessManagerTest<TxopType>::StartTest (uint64_t slotTime, uint64_t sifs,
                                               uint64_t eifsNoDifsNoSifs, uint32_t ackTimeoutValue,
                                               uint16_t chWidth)
{
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyRxStartNow,
                       m_ChannelAccessManager, MicroSeconds (duration));
                                                   ...
                                                   uint64_t timeUntilError)
{
  ...
  Simulator::Schedule (MicroSeconds (at + timeUntilError) - Now (),
                       ...
                       MicroSeconds (duration - timeUntilError), WIFI_CHANLIST_PRIMARY,
                       std::vector<Time>{});
}
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyNavResetNow,
                       m_ChannelAccessManager, MicroSeconds (duration));
}
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyNavStartNow,
                       m_ChannelAccessManager, MicroSeconds (duration));
}
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyAckTimeoutResetNow,
...
ChannelAccessManagerTest<TxopType>::AddAccessRequestWithAckTimeout (uint64_t at, uint64_t txTime,
                                                                    ...
                                                                    uint32_t from)
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManagerTest::DoAccessRequest, this,
                       txTime, expectedGrantTime, m_txop[from]);
}

template <typename TxopType>
void
ChannelAccessManagerTest<TxopType>::AddAccessRequestWithSuccessfullAck (
    uint64_t at, uint64_t txTime, uint64_t expectedGrantTime, uint32_t ackDelay, uint32_t from)
{
...
ChannelAccessManagerTest<TxopType>::AddCcaBusyEvt (uint64_t at, uint64_t duration,
                                                   ...
                                                   const std::vector<Time> &per20MhzDurations)
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifyCcaBusyStartNow,
                       m_ChannelAccessManager, MicroSeconds (duration), channelType,
                       per20MhzDurations);
}
...
{
  Simulator::Schedule (MicroSeconds (at) - Now (), &ChannelAccessManager::NotifySwitchingStartNow,
                       m_ChannelAccessManager, MicroSeconds (duration));
}
...
LargestIdlePrimaryChannelTest::RunOne (uint16_t chWidth, WifiChannelListType busyChannel)
{
  ...
  Simulator::Schedule (checkTime1 - start, [=] () {
    ...
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval1, checkTime1), idleWidth,
  ...
  Simulator::Schedule (checkTime2 - start, [=] () {
    ...
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval2, checkTime2), idleWidth,
                           "Incorrect width of the idle channel in an interval "
  ...
  Simulator::Schedule (checkTime3 - start, [=] () {
    ...
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval3, end3), idleWidth,
                           "Incorrect width of the idle channel in an interval "
  ...
  Simulator::Schedule (checkTime4 - start, [=] () {
    ...
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval4, end4), chWidth,
  ...
  Simulator::Schedule (checkTime5 - start, [=] () {
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval5, checkTime5), chWidth,
                           "Incorrect width of the idle channel in an interval "
  ...
  Simulator::Schedule (checkTime6 - start, [=] () {
    ...
    NS_TEST_EXPECT_MSG_EQ (m_cam->GetLargestIdlePrimaryChannel (interval6, checkTime6), 0,
                           "Incorrect width of the idle channel in an interval "
      ...
      for (const auto busyChannel : busyChannels)
        {
          Simulator::Schedule (Seconds (delay), [this, chWidth, busyChannel] () {
            ...
            m_phy = CreateObject<SpectrumWifiPhy> ();
            m_phy->SetOperatingChannel (WifiPhy::ChannelTuple{0, chWidth, WIFI_PHY_BAND_5GHZ, 0});

...
TestMultiUserScheduler::SelectTxFormat (void)
{
  ...
  if (m_txFormat == SU_TX || m_txFormat == DL_MU_TX ||
      (m_txFormat == UL_MU_TX && m_ulTriggerType == TriggerFrameType::BSRP_TRIGGER))
    {
      ...
      Time duration =
          WifiPhy::CalculateTxDuration (ampduSize, txVector, m_apMac->GetWifiPhy ()->GetPhyBand (),
      ...
      for (auto &sta : staList)
        {
          Ptr<const WifiMacQueueItem> peeked =
              m_apMac->GetQosTxop (AC_BE)->PeekNextMpdu (0, sta.second);
          ...
          Ptr<WifiMacQueueItem> mpdu = m_apMac->GetQosTxop (AC_BE)->GetNextMpdu (
              peeked, m_txParams, m_availableTime, m_initialFrame);
          if (!mpdu)
            {
...
public:
   ...
   */
  void Transmit (std::string context, WifiConstPsduMap psduMap, WifiTxVector txVector,
                 double txPowerW);
  /**
...
void
OfdmaAckSequenceTest::Transmit (std::string context, WifiConstPsduMap psduMap,
  ...
  if (!psduMap.begin ()->second->GetHeader (0).IsBeacon () && Simulator::Now () > Seconds (1.5))
    {
      ...
      for (const auto &psduPair : psduMap)
        {
          NS_LOG_INFO ("Sending " << psduPair.second->GetHeader (0).GetTypeString () << " #MPDUs "
                                  << psduPair.second->GetNMpdus () << " txDuration " << txDuration
                                  << " duration/ID "
                                  << psduPair.second->GetHeader (0).GetDuration ()
              ...
              if (m_muEdcaParameterSet.muTimer > 0 && m_muEdcaParameterSet.muAifsn > 0)
                {
                  ...
                  qosTxop->TraceConnectWithoutContext (
                      "CwTrace", MakeCallback (&OfdmaAckSequenceTest::TraceCw, this).Bind (i));
                }
              else
                {
...
OfdmaAckSequenceTest::CheckResults (Time sifs, Time slotTime, uint8_t aifsn)
{
  ...
  Time tEnd, // TX end for a frame
      ...
      ifs = (m_txopLimit > 0 ? sifs : sifs + aifsn * slotTime), navEnd;
  ...
  NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[1].endTx,
  ...
  NS_TEST_EXPECT_MSG_LT (tStart, tEnd + sifs + tolerance,
                         "QoS Null frame in HE TB PPDU sent too late");
  NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[2].endTx,
                               "Duration/ID in BSRP Trigger Frame is too short");
  ...
  NS_TEST_EXPECT_MSG_LT (tStart, tEnd + sifs + tolerance,
                         "QoS Null frame in HE TB PPDU sent too late");
  NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[3].endTx,
                               "Duration/ID in BSRP Trigger Frame is too short");
  ...
  NS_TEST_EXPECT_MSG_LT (tStart, tEnd + sifs + tolerance,
                         "QoS Null frame in HE TB PPDU sent too late");
  NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[4].endTx,
                               "Duration/ID in BSRP Trigger Frame is too short");
  ...
  NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[10].endTx,
  ...
  NS_TEST_EXPECT_MSG_EQ (m_txPsdus[11].psduMap.size (), 4,
  ...
  NS_TEST_EXPECT_MSG_GT (navEnd, m_txPsdus[11].endTx,
  ...
  if (m_dlMuAckType == WifiAcknowledgment::DL_MU_BAR_BA_SEQUENCE)
    {
      ...
      if (m_txopLimit > 0)
        {
          // DL MU PPDU's NAV
          NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[18].endTx,
  ...
  else if (m_dlMuAckType == WifiAcknowledgment::DL_MU_TF_MU_BAR)
    {
      ...
      if (m_txopLimit > 0)
        {
          // DL MU PPDU's NAV
          NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[13].endTx,
                                       "Duration/ID in the DL MU PPDU is too short");
          NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[14].endTx,
                                       "Duration/ID in the DL MU PPDU is too short");
          NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[15].endTx,
                                       "Duration/ID in the DL MU PPDU is too short");
          NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[16].endTx,
                                       "Duration/ID in the DL MU PPDU is too short");
        }
  ...
  else if (m_dlMuAckType == WifiAcknowledgment::DL_MU_AGGREGATE_TF)
    {
      ...
      NS_TEST_EXPECT_MSG_LT (tStart, tEnd + sifs + tolerance,
                             "Block Ack in HE TB PPDU sent too late");
      NS_TEST_EXPECT_MSG_GT_OR_EQ (navEnd + tolerance, m_txPsdus[12].endTx,
                                   "Duration/ID in the DL MU PPDU is too short");
  ...
  if (m_muEdcaParameterSet.muTimer > 0 && m_muEdcaParameterSet.muAifsn == 0)
  ...
  else if (m_muEdcaParameterSet.muTimer > 0 && m_muEdcaParameterSet.muAifsn > 0)
    {
      ...
      for (const auto &cwValue : m_cwValues)
        {
          NS_TEST_EXPECT_MSG_EQ ((cwValue == 2 || cwValue >= m_muEdcaParameterSet.muCwMin), true,
...
OfdmaAckSequenceTest::DoRun (void)
{
  ...
  mac.SetType ("ns3::StaWifiMac", "BE_MaxAmsduSize", UintegerValue (0), "BE_MaxAmpduSize",
               UintegerValue (0), "Ssid", SsidValue (ssid),
               /* setting blockack threshold for sta's BE queue */
               "BE_BlockAckThreshold", UintegerValue (2), "ActiveProbing", BooleanValue (false));

  m_staDevices = wifi.Install (phy, mac, wifiStaNodes);

  mac.SetType ("ns3::ApWifiMac", "BE_MaxAmsduSize", UintegerValue (0), "BE_MaxAmpduSize",
               UintegerValue (0), "Ssid", SsidValue (ssid), "BeaconGeneration",
               BooleanValue (true));
  ...
  CheckResults (dev->GetMac ()->GetWifiPhy ()->GetSifs (),
                dev->GetMac ()->GetWifiPhy ()->GetSlot (), apBeQosTxop->Txop::GetAifsn ());
...
WifiMacOfdmaTestSuite::WifiMacOfdmaTestSuite () : TestSuite ("wifi-mac-ofdma", UNIT)
{
  ...
  for (auto &muEdcaParameterSet : MuEdcaParams{{0, 0, 0, 0} /* no MU EDCA */,
    ...
    {
      AddTestCase (new OfdmaAckSequenceTest (20, WifiAcknowledgment::DL_MU_BAR_BA_SEQUENCE, 10000,
                                             5440, 15, muEdcaParameterSet),
                   TestCase::QUICK);
      AddTestCase (new OfdmaAckSequenceTest (20, WifiAcknowledgment::DL_MU_AGGREGATE_TF, 10000,
                                             5440, 15, muEdcaParameterSet),
                   TestCase::QUICK);
      AddTestCase (new OfdmaAckSequenceTest (20, WifiAcknowledgment::DL_MU_TF_MU_BAR, 10000, 5440,
                                             15, muEdcaParameterSet),
                   TestCase::QUICK);
      AddTestCase (new OfdmaAckSequenceTest (40, WifiAcknowledgment::DL_MU_BAR_BA_SEQUENCE, 10000,
                                             0, 15, muEdcaParameterSet),
                   TestCase::QUICK);
      AddTestCase (new OfdmaAckSequenceTest (40, WifiAcknowledgment::DL_MU_AGGREGATE_TF, 10000, 0,
                                             15, muEdcaParameterSet),
                   TestCase::QUICK);
      AddTestCase (new OfdmaAckSequenceTest (40, WifiAcknowledgment::DL_MU_TF_MU_BAR, 10000, 0, 15,
                                             muEdcaParameterSet),
                   TestCase::QUICK);
    }
}

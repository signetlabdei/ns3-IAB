...
void
WifiTxopTest::Transmit (std::string context, WifiConstPsduMap psduMap, WifiTxVector txVector,
                        double txPowerW)
{
...
WifiTxopTest::CheckResults (void)
{
  Time tEnd, // TX end for a frame
      ...
      slot = DynamicCast<WifiNetDevice> (m_apDevices.Get (0))->GetPhy ()->GetSlot (), navEnd;
  ...
  NS_TEST_ASSERT_MSG_EQ (
      m_txPsdus[10].header.GetDuration (), RoundDurationId (m_txopLimit - m_txPsdus[10].txDuration),

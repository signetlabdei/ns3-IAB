...
SpectrumWifiPhyBasicTest::MakeSignal (double txPowerWatts)
{
  ...
  Ptr<SpectrumValue> txPowerSpectrum = WifiSpectrumValueHelper::CreateOfdmTxPowerSpectralDensity (
      FREQUENCY, CHANNEL_WIDTH, txPowerWatts, GUARD_WIDTH);
...
SpectrumWifiPhyBasicTest::DoRun (void)
{
  ...
  Simulator::Schedule (MicroSeconds (4000000), &SpectrumWifiPhyBasicTest::SendSignal, this,
                       txPowerWatts);
  Simulator::Schedule (MicroSeconds (4000001), &SpectrumWifiPhyBasicTest::SendSignal, this,
                       txPowerWatts);
  Simulator::Run ();
  Simulator::Destroy ();
...
SpectrumWifiPhyListenerTest::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (m_listener->m_notifyMaybeCcaBusyStart, 2,
                         "Didn't receive NotifyCcaBusyStart (once preamble is detected + prolonged "
                         "by L-SIG reception, then switched to Rx by at the beginning of data)");
...
SpectrumWifiPhyFilterTest::SpectrumWifiPhyFilterTest ()
    : TestCase ("SpectrumWifiPhy test RX filters"), m_txChannelWidth (20), m_rxChannelWidth (20)
{
}

SpectrumWifiPhyFilterTest::SpectrumWifiPhyFilterTest (std::string name) : TestCase (name)
{
}
...
{
  WifiTxVector txVector = WifiTxVector (HePhy::GetHeMcs0 (), 0, WIFI_PREAMBLE_HE_SU, 800, 1, 1, 0,
...
SpectrumWifiPhyFilterTest::RxCallback (Ptr<const Packet> p, RxPowerWattPerChannelBand rxPowersW)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (numBands, expectedNumBands,
  ...
  if ((m_txChannelWidth <= m_rxChannelWidth) && (channelWidth >= 20))
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (rxPowerPrimaryChannel20, expectedRxPowerPrimaryChannel20,
...
SpectrumWifiPhyFilterTest::RunOne (void)
{
  ...
  auto txChannelNum = std::get<0> (*WifiPhyOperatingChannel::FindFirst (
      0, txFrequency, m_txChannelWidth, WIFI_STANDARD_80211ax, WIFI_PHY_BAND_5GHZ));
  ...
  auto rxChannelNum = std::get<0> (*WifiPhyOperatingChannel::FindFirst (
      0, rxFrequency, m_rxChannelWidth, WIFI_STANDARD_80211ax, WIFI_PHY_BAND_5GHZ));
              ...
              for (std::size_t index = 1; index <= HeRu::GetNRus (bw, ruType); index++)
                {
                  ...
                  WifiSpectrumBand band = m_rxPhy->ConvertHeRuSubcarriers (
                      bw, m_rxPhy->GetGuardBandwidth (m_rxChannelWidth), range, i);
...
SpectrumWifiPhyTestSuite::SpectrumWifiPhyTestSuite () : TestSuite ("wifi-spectrum-wifi-phy", UNIT)
{

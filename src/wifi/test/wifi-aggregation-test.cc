...
AmpduAggregationTest::AmpduAggregationTest ()
    : TestCase ("Check the correctness of MPDU aggregation operations"), m_discarded (false)
{
}
...
AmpduAggregationTest::DoRun (void)
{
  ...
  Ptr<WifiMacQueueItem> item =
      m_mac->GetBEQueue ()->GetNextMpdu (peeked, txParams, Time::Min (), true);
  ...
  NS_TEST_EXPECT_MSG_EQ (m_mac->GetBEQueue ()->GetWifiMacQueue ()->GetNPackets (), 0,
...
TwoLevelAggregationTest::DoRun (void)
{
  ...
  m_mac->GetBEQueue ()->GetWifiMacQueue ()->Enqueue (
      Create<WifiMacQueueItem> (Create<Packet> (1500), hdr));
  m_mac->GetBEQueue ()->GetWifiMacQueue ()->Enqueue (
      Create<WifiMacQueueItem> (Create<Packet> (1500), hdr));
  m_mac->GetBEQueue ()->GetWifiMacQueue ()->Enqueue (
      Create<WifiMacQueueItem> (Create<Packet> (1500), hdr));
  ...
  m_mac->GetVIQueue ()->SetAttribute (
      "TxopLimits", AttributeContainerValue<TimeValue> (std::vector<Time>{MicroSeconds (3008)}));
  ...
  for (uint8_t i = 0; i < 10; i++)
    {
      m_mac->GetVIQueue ()->GetWifiMacQueue ()->Enqueue (
          Create<WifiMacQueueItem> (Create<Packet> (1300), hdr));
    }
...
HeAggregationTest::DoRunSubTest (uint16_t bufferSize)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (
      m_mac->GetBEQueue ()->GetWifiMacQueue ()->GetNPackets (), expectedRemainingPacketsInQueue,
      "queue should contain 300 - " << bufferSize << " = " << expectedRemainingPacketsInQueue
                                    << " packets");
...
void
PreservePacketsInAmpdus::NotifyPsduForwardedDown (WifiConstPsduMap psduMap, WifiTxVector txVector,
                                                  double txPowerW)
{
  NS_TEST_EXPECT_MSG_EQ ((psduMap.size () == 1 && psduMap.begin ()->first == SU_STA_ID), true,
...
PreservePacketsInAmpdus::DoRun (void)
{
  ...
  mac.SetType ("ns3::StaWifiMac", "BE_MaxAmsduSize", UintegerValue (4500), "BE_MaxAmpduSize",
               UintegerValue (7500), "Ssid", SsidValue (ssid),
  ...
  Simulator::Schedule (Seconds (1.5), &PacketSocketClient::SetAttribute, client, "Interval",
  ...
  sta_device->GetPhy ()->TraceConnectWithoutContext (
      "PhyTxPsduBegin", MakeCallback (&PreservePacketsInAmpdus::NotifyPsduForwardedDown, this));
  ap_device->GetMac ()->TraceConnectWithoutContext (
      "MacRx", MakeCallback (&PreservePacketsInAmpdus::NotifyMacForwardUp, this));
...
WifiAggregationTestSuite::WifiAggregationTestSuite () : TestSuite ("wifi-aggregation", UNIT)
{

...
AnimationInterface &
AnimationInterface::EnableIpv4RouteTracking (std::string fileName, Time startTime, Time stopTime,
                                             Time pollInterval)
{
...
AnimationInterface &
AnimationInterface::EnableIpv4RouteTracking (std::string fileName, Time startTime, Time stopTime,
                                             NodeContainer nc, Time pollInterval)
{
  m_routingNc = nc;
...
void
AnimationInterface::SetBackgroundImage (std::string fileName, double x, double y, double scaleX,
...
void
AnimationInterface::WriteRoutePath (uint32_t nodeId, std::string destination,
...
void
AnimationInterface::WriteNonP2pLinkProperties (uint32_t id, std::string ipv4Address,
...
void
AnimationInterface::Ipv4TxTrace (std::string context, Ptr<const Packet> p, Ptr<Ipv4> ipv4,
...
void
AnimationInterface::Ipv4RxTrace (std::string context, Ptr<const Packet> p, Ptr<Ipv4> ipv4,
                                 uint32_t interfaceIndex)
{
  const Ptr<const Node> node = GetNodeFromContext (context);
...
void
AnimationInterface::Ipv4DropTrace (std::string context, const Ipv4Header &ipv4Header,
...
void
AnimationInterface::DevTxTrace (std::string context, Ptr<const Packet> p, Ptr<NetDevice> tx,
                                Ptr<NetDevice> rx, Time txTime, Time rxTime)
{
  ...
  WriteXmlP ("p", tx->GetNode ()->GetId (), fbTx, lbTx, rx->GetNode ()->GetId (), fbRx, lbRx,
             m_enablePacketMetadata ? GetPacketMetadata (p) : "");
}

void
AnimationInterface::GenericWirelessTxTrace (std::string context, Ptr<const Packet> p,
...
void
AnimationInterface::GenericWirelessRxTrace (std::string context, Ptr<const Packet> p,
                                            ProtocolType protocolType)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
...
void
AnimationInterface::WifiPhyTxBeginTrace (std::string context, WifiConstPsduMap psduMap,
                                         [[maybe_unused]] WifiTxVector txVector, double txPowerW)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr<NetDevice> ndev = GetNetDeviceFromContext (context);
      ...
      for (auto &mpdu : *PeekPointer (psdu.second))
        {
          ...
          OutputWirelessPacketTxInfo (mpdu->GetProtocolDataUnit (), pendingPackets->at (gAnimUid),
...
void
AnimationInterface::OutputWirelessPacketTxInfo (Ptr<const Packet> p, AnimPacketInfo &pktInfo,
                                                uint64_t animUid)
{
  CheckMaxPktsPerTraceFile ();
  uint32_t nodeId = 0;
...
void
AnimationInterface::OutputWirelessPacketRxInfo (Ptr<const Packet> p, AnimPacketInfo &pktInfo,
                                                uint64_t animUid)
{
  CheckMaxPktsPerTraceFile ();
...
AnimationInterface::OutputCsmaPacket (Ptr<const Packet> p, AnimPacketInfo &pktInfo)
{
  ...
  WriteXmlP ("p", nodeId, pktInfo.m_fbTx, pktInfo.m_lbTx, rxId, pktInfo.m_fbRx, pktInfo.m_lbRx,
             m_enablePacketMetadata ? GetPacketMetadata (p) : "");
}

void
AnimationInterface::AddPendingPacket (ProtocolType protocolType, uint64_t animUid,
...
bool
AnimationInterface::IsPacketPending (uint64_t animUid,
...
AnimationInterface::ConnectLteEnb (Ptr<Node> n, Ptr<LteEnbNetDevice> nd, uint32_t devIndex)
{
  ...
  if (dlPhy)
    {
      dlPhy->TraceConnect ("TxStart", oss.str (),
                           MakeCallback (&AnimationInterface::LteSpectrumPhyTxStart, this));
      dlPhy->TraceConnect ("RxStart", oss.str (),
                           MakeCallback (&AnimationInterface::LteSpectrumPhyRxStart, this));
    }
  if (ulPhy)
    {
      ulPhy->TraceConnect ("TxStart", oss.str (),
                           MakeCallback (&AnimationInterface::LteSpectrumPhyTxStart, this));
      ulPhy->TraceConnect ("RxStart", oss.str (),
                           MakeCallback (&AnimationInterface::LteSpectrumPhyRxStart, this));
    }
}
              ...
              if (!ipv4Addresses.empty ())
                {
                  ...
                  WriteNonP2pLinkProperties (
                      n->GetId (), GetIpv4Address (dev) + "~" + GetMacAddress (dev), channelType);
                }
              else if (!ipv6Addresses.empty ())
                {
                  NS_LOG_INFO ("Writing Ipv6 link");
                  WriteNonP2pLinkProperties (
                      n->GetId (), GetIpv6Address (dev) + "~" + GetMacAddress (dev), channelType);
                }
              continue;
            }
...
AnimationInterface::TrackWifiMacCounters ()
{
  ...
  Simulator::Schedule (m_wifiMacCountersPollInterval, &AnimationInterface::TrackWifiMacCounters,
                       this);
}
...
AnimationInterface::TrackWifiPhyCounters ()
{
  ...
  Simulator::Schedule (m_wifiPhyCountersPollInterval, &AnimationInterface::TrackWifiPhyCounters,
                       this);
}
...
void
AnimationInterface::RecursiveIpv4RoutePathSearch (std::string from, std::string to,
                                                  Ipv4RoutePathElements &rpElements)
{
...
void
AnimationInterface::WriteXmlRp (uint32_t nodeId, std::string destination,
                                Ipv4RoutePathElements rpElements)
{
...
void
AnimationInterface::WriteXmlP (uint64_t animUid, std::string pktType, uint32_t tId, double fbRx,
                               double lbRx)
{
...
void
AnimationInterface::WriteXmlAddNodeCounter (uint32_t nodeCounterId, std::string counterName,
...
void
AnimationInterface::WriteXmlUpdateNodeCounter (uint32_t nodeCounterId, uint32_t nodeId,
                                               double counterValue)
{
...
void
AnimationInterface::WriteXmlUpdateBackground (std::string fileName, double x, double y,
...
void
AnimationInterface::WriteXmlNonP2pLinkProperties (uint32_t id, std::string ipAddress,
                                                  std::string channelType)
{
...
AnimationInterface::AnimPacketInfo::AnimPacketInfo ()
    : m_txnd (0), m_txNodeId (0), m_fbTx (0), m_lbTx (0), m_lbRx (0)
{
}
...
AnimationInterface::AnimPacketInfo::AnimPacketInfo (Ptr<const NetDevice> txnd, const Time fbTx,
                                                    uint32_t txNodeId)
    : m_txnd (txnd), m_txNodeId (0), m_fbTx (fbTx.GetSeconds ()), m_lbTx (0), m_lbRx (0)
{
  if (!m_txnd)
    {
      m_txNodeId = txNodeId;

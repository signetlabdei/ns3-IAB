...
public:
   ...
   * Counter Types
   */
  typedef enum { UINT32_COUNTER, DOUBLE_COUNTER } CounterType;
   ...
   */
  AnimationInterface &EnableIpv4RouteTracking (std::string fileName, Time startTime, Time stopTime,
                                               NodeContainer nc, Time pollInterval = Seconds (5));
   ...
   *
   */
  void SetBackgroundImage (std::string fileName, double x, double y, double scaleX, double scaleY,
                           double opacity);
...
private:
  ...
  struct LinkPairCompare
  {
     ...
     */
    bool
    operator() (P2pLinkNodeIdPair first, P2pLinkNodeIdPair second) const
    {
  ...
  typedef enum { UAN, LTE, WIFI, WIMAX, CSMA, LRWPAN, WAVE } ProtocolType;
   ...
   * \param interfaceIndex the interface index
   */
  void Ipv4DropTrace (std::string context, const Ipv4Header &ipv4Header, Ptr<const Packet> p,
   ...
   */
  void DevTxTrace (std::string context, Ptr<const Packet> p, Ptr<NetDevice> tx, Ptr<NetDevice> rx,
   ...
   */
  void WifiPhyTxBeginTrace (std::string context, WifiConstPsduMap psduMap, WifiTxVector txVector,
                            double txPowerW);
  /**
   ...
   */
  void WriteXmlP (std::string pktType, uint32_t fId, double fbTx, double lbTx, uint32_t tId,
                  double fbRx, double lbRx, std::string metaInfo = "");
  /**
   * Write XMLP function
   * \param animUid the UID
   * \param pktType the packet type
   ...
   */
  void WriteXmlUpdateBackground (std::string fileName, double x, double y, double scaleX,

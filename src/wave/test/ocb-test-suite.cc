...
private:
   ...
   */
  void PhyRxOkTrace (std::string context, Ptr<const Packet> packet, double snr, WifiMode mode,
...
void
OcbWifiMacTestCase::PhyRxOkTrace (std::string context, Ptr<const Packet> packet, double snr,
...
void
OcbWifiMacTestCase::PhyTxTrace (std::string context, Ptr<const Packet> packet, WifiMode mode,
                                WifiPreamble preamble, uint8_t txPower)
{
  WifiMacHeader h;
  packet->PeekHeader (h);
...
OcbWifiMacTestCase::DoRun ()
{
  ...
  Simulator::Run ();
  Simulator::Destroy ();
  NS_TEST_ASSERT_MSG_EQ (macassoc_time.GetNanoSeconds (), 0,
                         "In Ocb mode, there is no associate state machine");
  NS_TEST_ASSERT_MSG_LT (
      phytx_time, phyrx_time,
      "before mobile node receives frames from far static node, it can send data packet directly");
...
OcbTestSuite::OcbTestSuite () : TestSuite ("wave-80211p-ocb", UNIT)
{

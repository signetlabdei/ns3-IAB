...
public:
   ...
   */
  CoordinationListener (DefaultChannelScheduler *scheduler) : m_scheduler (scheduler)
  {
  }
  virtual void
  NotifyCchSlotStart (Time duration)
  {
    m_scheduler->NotifyCchSlotStart (duration);
  }
  virtual void
  NotifySchSlotStart (Time duration)
  {
    m_scheduler->NotifySchSlotStart (duration);
  }
  virtual void
  NotifyGuardSlotStart (Time duration, bool cchi)
  {
...
DefaultChannelScheduler::AssignContinuousAccess (uint32_t channelNumber, bool immediate)
{
  ...
  if (immediate || m_coordinator->IsSchInterval ())
  ...
  else
    {
      ...
      m_waitEvent = Simulator::Schedule (wait, &DefaultChannelScheduler::AssignContinuousAccess,
                                         this, sch, false);
...
bool
DefaultChannelScheduler::AssignExtendedAccess (uint32_t channelNumber, uint32_t extends,
                                               bool immediate)
{
  ...
  if (immediate || m_coordinator->IsSchInterval ())
    {
      ...
      Time extendedDuration = m_coordinator->NeedTimeToCchInterval () +
                              MilliSeconds (extends * sync.GetMilliSeconds ());
      ...
      m_extendEvent = Simulator::Schedule (extendedDuration,
                                           &DefaultChannelScheduler::ReleaseAccess, this, sch);
    }
  else
    {
      Time wait = m_coordinator->NeedTimeToSchInterval ();
      m_waitEvent = Simulator::Schedule (wait, &DefaultChannelScheduler::AssignExtendedAccess, this,
                                         sch, extends, false);
      m_waitChannelNumber = sch;
  ...
  if (m_channelNumber != 0)
    {
      ...
      NS_LOG_DEBUG ("channel access is already assigned for other SCHs, thus cannot assign default "
                    "CCH access.");
      return false;
    }

...
void
WaveMacLow::StartTransmission (Ptr<WifiMacQueueItem> mpdu, MacLowTransmissionParameters params,
                               Ptr<Txop> dca)
{
  ...
  Time transmissionTime =
      MacLow::CalculateTransmissionTime (mpdu->GetPacket (), &mpdu->GetHeader (), params);
  ...
  if (transmissionTime > remainingTime)
    {
      ...
      NS_LOG_DEBUG ("Because the required transmission time = "
                    << transmissionTime.As (Time::MS)

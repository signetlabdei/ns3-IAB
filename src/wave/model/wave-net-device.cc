...
WaveNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::WaveNetDevice")
          ...
          .AddAttribute ("MacEntities", "The MAC layer attached to this device.", ObjectMapValue (),
          ...
          .AddAttribute (
              "VsaManager", "The VSA manager attached to this device.", PointerValue (),
              MakePointerAccessor (&WaveNetDevice::SetVsaManager, &WaveNetDevice::GetVsaManager),
              MakePointerChecker<VsaManager> ());
  return tid;
}

WaveNetDevice::WaveNetDevice (void) : m_txProfile (0)
{
...
bool
WaveNetDevice::SendX (Ptr<Packet> packet, const Address &dest, uint32_t protocol,
                      const TxInfo &txInfo)
{
...
bool
WaveNetDevice::SendFrom (Ptr<Packet> packet, const Address &source, const Address &dest,
                         uint16_t protocol)
{

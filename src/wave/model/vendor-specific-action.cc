...
OrganizationIdentifier::OrganizationIdentifier (void) : m_type (Unknown)
{
...
OrganizationIdentifier::Deserialize (Buffer::Iterator start)
{
  ...
  for (std::vector<OrganizationIdentifier>::iterator i = OrganizationIdentifiers.begin ();
       i != OrganizationIdentifiers.end (); ++i)
    {
...
VendorSpecificActionHeader::VendorSpecificActionHeader (void)
    : m_oi (), m_category (CATEGORY_OF_VSA)
{
  NS_LOG_FUNCTION (this);
}
...
static VscCallback null_callback =
    MakeNullCallback<bool, Ptr<WifiMac>, const OrganizationIdentifier &, Ptr<const Packet>,
                     const Address &> ();

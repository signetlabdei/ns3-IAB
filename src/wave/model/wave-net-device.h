...
struct TxInfo
{
  ...
  /// Initializer
  TxInfo () : channelNumber (CCH), priority (7), txPowerLevel (8)
  {
  }
  /**
   * Initializer
   ...
   */
  TxInfo (uint32_t channel, uint32_t prio = 7, WifiMode rate = WifiMode (),
          WifiPreamble preamble = WIFI_PREAMBLE_LONG, uint32_t powerLevel = 8)
...
struct TxProfile
{
  ...
  /// Initializer
  TxProfile (void)
      : channelNumber (SCH1), adaptable (false), txPowerLevel (4), preamble (WIFI_PREAMBLE_LONG)
  {
  ...
  TxProfile (uint32_t channel, bool adapt = true, uint32_t powerLevel = 4)
      : channelNumber (channel), adaptable (adapt), txPowerLevel (powerLevel)
  {
    dataRate = WifiMode ("OfdmRate6MbpsBW10MHz");

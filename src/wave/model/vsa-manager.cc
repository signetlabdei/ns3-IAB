...
VsaManager::VsaManager (void) : m_device (0)
{
...
void
VsaManager::DoSendVsa (enum VsaTransmitInterval interval, uint32_t channel, Ptr<Packet> vsc,
  ...
  if (interval == VSA_TRANSMIT_IN_SCHI)
    {
      ...
      if (wait != Seconds (0))
        {
          Simulator::Schedule (wait, &VsaManager::DoSendVsa, this, interval, channel, vsc, oi,
                               peer);
          return;
        }
    }
...
bool
VsaManager::ReceiveVsc (Ptr<WifiMac> mac, const OrganizationIdentifier &oi, Ptr<const Packet> vsc,
                        const Address &src)
{

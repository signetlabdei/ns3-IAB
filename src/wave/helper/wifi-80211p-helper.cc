...
NetDeviceContainer
Wifi80211pHelper::Install (const WifiPhyHelper &phyHelper, const WifiMacHelper &macHelper,
                           NodeContainer c) const
{
  [[maybe_unused]] QosWaveMacHelper const *qosMac =
      dynamic_cast<QosWaveMacHelper const *> (&macHelper);
  if (qosMac == 0)
    {
      [[maybe_unused]] NqosWaveMacHelper const *nqosMac =
          dynamic_cast<NqosWaveMacHelper const *> (&macHelper);
      if (nqosMac == 0)
        {

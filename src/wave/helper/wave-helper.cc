...
void
YansWavePhyHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous,
  ...
  for (i = phys.begin (); i != phys.end (); ++i)
    {
      ...
      phy->TraceConnectWithoutContext (
          "MonitorSnifferTx", MakeBoundCallback (&YansWavePhyHelper::PcapSniffTxEvent, file));
      phy->TraceConnectWithoutContext (
          "MonitorSnifferRx", MakeBoundCallback (&YansWavePhyHelper::PcapSniffRxEvent, file));
    }
}

void
YansWavePhyHelper::EnableAsciiInternal (Ptr<OutputStreamWrapper> stream, std::string prefix,
...
WaveHelper::Default (void)
{
  ...
  helper.SetRemoteStationManager ("ns3::ConstantRateWifiManager", "DataMode",
...
NetDeviceContainer
WaveHelper::Install (const WifiPhyHelper &phyHelper, const WifiMacHelper &macHelper,

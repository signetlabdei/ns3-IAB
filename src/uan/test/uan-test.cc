...
private:
   ...
   */
  uint32_t DoOnePhyTest (Time t1, Time t2, uint32_t r1, uint32_t r2, Ptr<UanPropModel> prop,
                         uint16_t mode1 = 0, uint16_t mode2 = 0);
  /**
...
bool
UanTest::RxPacket (Ptr<NetDevice> dev, Ptr<const Packet> pkt, [[maybe_unused]] uint16_t mode,
...
uint32_t
UanTest::DoOnePhyTest (Time txTime1, Time txTime2, uint32_t r1, uint32_t r2, Ptr<UanPropModel> prop,
...
UanTest::DoPhyTests ()
{
  ...
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (DoOnePhyTest (Seconds (1.0), Seconds (2.99), 50, 50, prop), 0,
  ...
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (
      DoOnePhyTest (Seconds (1.0), Seconds (1.0 + 7.01 * (13.0 / 80.0)), 50, 50, prop), 0,
  ...
  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (
      DoOnePhyTest (Seconds (1.0), Seconds (2.99), 50, 50, prop, 0, 0), 0,
      "Expected collision with both packets lost");

  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (
      DoOnePhyTest (Seconds (1.0), Seconds (2.99), 50, 50, prop, 0, 2), 17,
      "Expected collision with only one packets lost");

  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (
      DoOnePhyTest (Seconds (1.0), Seconds (2.99), 50, 50, prop, 0, 5), 34,
      "Expected no collision");

  NS_TEST_ASSERT_MSG_EQ_RETURNS_BOOL (
      DoOnePhyTest (Seconds (1.0), Seconds (2.99), 50, 50, prop, 2, 3), 34,
      "Expected no collision");
...
UanTest::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (resp.GetPathLossDb (), -44.1753, 0.001,
                             "Got BH Pathloss outside of tolerance");

  NS_TEST_ASSERT_MSG_EQ_TOL (resp.GetPdp ().GetTap (4).GetAmp (), 0.14159, 0.001,
...
UanTestSuite::UanTestSuite () : TestSuite ("devices-uan", UNIT)
{

...
Experiment::Run (uint32_t param)
{
  ...
  uan.SetPhy ("ns3::UanPhyDual", "SupportedModesPhy1", UanModesListValue (m_dataModes),
              "SupportedModesPhy2", UanModesListValue (m_controlModes));

  uan.SetMac ("ns3::UanMacRcGw", "NumberOfRates", UintegerValue (m_numRates), "NumberOfNodes",
              UintegerValue (nNodes), "MaxReservations", UintegerValue (a), "SIFS",
              TimeValue (m_sifs), "MaxPropDelay", TimeValue (pDelay), "FrameSize",
  ...
  uan.SetMac ("ns3::UanMacRc", "NumberOfRates", UintegerValue (m_numRates), "MaxPropDelay",
              TimeValue (pDelay));
  NodeContainer nodes;
  nodes.Create (nNodes);
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("NumberRates", "Number of divided rates ( (NumberRates+1)%TotalRate should be 0)",
                exp.m_numRates);
  cmd.AddValue ("MaxRange", "Maximum range between gateway and acoustic node", exp.m_maxRange);
  cmd.AddValue ("SimMin", "Minimum parameter to test (nodes if DoNode=1, \"a\" param otherwise)",
                exp.m_simMin);
  cmd.AddValue ("SimMax", "Maximum parameter to test (nodes if DoNode=1, \"a\" param otherwise)",
                exp.m_simMax);
  ...
  cmd.AddValue ("DoNode", "1 for do max nodes simulation (invalidates AMin and AMax values)",
                exp.m_doNode);

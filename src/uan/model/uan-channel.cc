...
UanChannel::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::UanChannel")
          ...
          .AddAttribute ("PropagationModel", "A pointer to the propagation model.",
                         StringValue ("ns3::UanPropModelIdeal"),
                         MakePointerAccessor (&UanChannel::m_prop),
                         MakePointerChecker<UanPropModel> ())
          .AddAttribute ("NoiseModel", "A pointer to the model of the channel ambient noise.",
...
UanChannel::UanChannel () : Channel (), m_prop (0), m_cleared (false)
{
}
...
void
UanChannel::TxPacket (Ptr<UanTransducer> src, Ptr<Packet> packet, double txPowerDb,
                      UanTxMode txMode)
{
      ...
      if (src != i->second)
        {
          ...
          Simulator::ScheduleWithContext (dstNodeId, delay, &UanChannel::SendUp, this, j, copy,
                                          rxPowerDb, txMode, pdp);
        }
      j++;
    }
}

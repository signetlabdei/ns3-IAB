...
UanMacCw::UanMacCw ()
    : UanMac (), m_phy (0), m_pktTx (0), m_txOngoing (false), m_state (IDLE), m_cleared (false)
...
UanMacCw::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::UanMacCw")
          ...
          .AddAttribute ("CW", "The MAC parameter CW.", UintegerValue (10),
                         MakeUintegerAccessor (&UanMacCw::m_cw), MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("SlotTime", "Time slot duration for MAC backoff.",
                         TimeValue (MilliSeconds (20)), MakeTimeAccessor (&UanMacCw::m_slotTime),
                         MakeTimeChecker ())
          .AddTraceSource ("Enqueue", "A packet arrived at the MAC for transmission.",
                           MakeTraceSourceAccessor (&UanMacCw::m_enqueueLogger),
                           "ns3::UanMacCw::QueueTracedCallback")
          .AddTraceSource ("Dequeue", "A was passed down to the PHY from the MAC.",
                           MakeTraceSourceAccessor (&UanMacCw::m_dequeueLogger),
                           "ns3::UanMacCw::QueueTracedCallback")
          .AddTraceSource ("RX", "A packet was destined for this MAC and was received.",
        ...
        if (m_phy->IsStateBusy ())
          {
            ...
            NS_ASSERT (m_phy->GetTransducer ()->GetArrivalList ().size () >= 1 ||
                       m_phy->IsStateTx ());
          }
        else
          {
            ...
            NS_ASSERT (m_phy->GetTransducer ()->GetArrivalList ().size () == 0 &&
                       !m_phy->IsStateTx ());
            m_state = TX;

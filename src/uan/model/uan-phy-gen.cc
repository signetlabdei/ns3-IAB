...
double
UanPhyCalcSinrDefault::CalcSinrDb (Ptr<Packet> pkt, Time arrTime, double rxPowerDb,
                                   double ambNoiseDb, UanTxMode mode, UanPdp pdp,
...
{
  static TypeId tid =
      TypeId ("ns3::UanPhyCalcSinrFhFsk")
          .SetParent<UanPhyCalcSinr> ()
          .SetGroupName ("Uan")
          .AddConstructor<UanPhyCalcSinrFhFsk> ()
          .AddAttribute ("NumberOfHops", "Number of frequencies in hopping pattern.",
                         UintegerValue (13), MakeUintegerAccessor (&UanPhyCalcSinrFhFsk::m_hops),
                         MakeUintegerChecker<uint32_t> ());
  return tid;
}
double
UanPhyCalcSinrFhFsk::CalcSinrDb (Ptr<Packet> pkt, Time arrTime, double rxPowerDb, double ambNoiseDb,
  ...
  for (; it != arrivalList.end (); it++)
    {
      ...
      double intPower = 0.0;
      if (tDelta <
          ts) // Case where there is overlap of a symbol due to interferer arriving just after desired signal
        {
          //Appears to be just the first two elements of the sum in Parrish paper, eq. 14
          intPower += intPdp.SumTapsNc (Time (), ts - tDelta);
          intPower += intPdp.SumTapsNc (ts - tDelta + clearingTime, 2 * ts - tDelta + clearingTime);
        }
...
{
  static TypeId tid =
      TypeId ("ns3::UanPhyPerGenDefault")
          .SetParent<UanPhyPer> ()
          .SetGroupName ("Uan")
          .AddConstructor<UanPhyPerGenDefault> ()
          .AddAttribute ("Threshold", "SINR cutoff for good packet reception.", DoubleValue (8),
...
TypeId
UanPhyPerUmodem::GetTypeId (void)
{
...
{
  UanModesList l;
  l.AppendMode (UanTxModeFactory::CreateMode (UanTxMode::FSK, 80, 80, 22000, 4000, 13,
                                              "FH-FSK")); // micromodem only
  l.AppendMode (UanTxModeFactory::CreateMode (UanTxMode::PSK, 200, 200, 22000, 4000, 4, "QPSK"));
  l.AppendMode (UanTxModeFactory::CreateMode (UanTxMode::PSK, 5000, 5000, 25000, 5000, 4,
                                              "QPSK")); // micromodem2
  ...
  static TypeId tid =
      TypeId ("ns3::UanPhyGen")
          ...
          .AddAttribute ("CcaThreshold",
                         ...
                         DoubleValue (10), MakeDoubleAccessor (&UanPhyGen::m_ccaThreshDb),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("RxThreshold", "Required SNR for signal acquisition in dB.",
                         DoubleValue (10), MakeDoubleAccessor (&UanPhyGen::m_rxThreshDb),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("TxPower", "Transmission output power in dB.", DoubleValue (190),
                         MakeDoubleAccessor (&UanPhyGen::m_txPwrDb), MakeDoubleChecker<double> ())
          .AddAttribute ("SupportedModes", "List of modes supported by this PHY.",
                         UanModesListValue (UanPhyGen::GetDefaultModes ()),
                         MakeUanModesListAccessor (&UanPhyGen::m_modes), MakeUanModesListChecker ())
          .AddAttribute ("PerModel", "Functor to calculate PER based on SINR and TxMode.",
                         StringValue ("ns3::UanPhyPerGenDefault"),
                         MakePointerAccessor (&UanPhyGen::m_per), MakePointerChecker<UanPhyPer> ())
          .AddAttribute ("SinrModel", "Functor to calculate SINR based on pkt arrivals and modes.",
        ...
        if (newsinr > m_rxThreshDb)
          {
            ...
            m_pktRxPdp = pdp;
            double txdelay = pkt->GetSize () * 8.0 / txMode.GetDataRateBps ();
            m_rxEndEvent = Simulator::Schedule (Seconds (txdelay), &UanPhyGen::RxEndEvent, this,
                                                pkt, rxPowerDb, txMode);
...
void
UanPhyGen::NotifyTransStartTx (Ptr<Packet> packet, [[maybe_unused]] double txPowerDb,
                               UanTxMode txMode)
{
  if (m_pktRx)
    {
...
double
UanPhyGen::CalculateSinrDb (Ptr<Packet> pkt, Time arrTime, double rxPowerDb, UanTxMode mode,
                            UanPdp pdp)
{

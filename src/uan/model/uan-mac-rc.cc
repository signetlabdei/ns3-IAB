...
Reservation::Reservation () : m_length (0), m_frameNo (0), m_retryNo (0), m_transmitted (false)
{
}

Reservation::Reservation (std::list<std::pair<Ptr<Packet>, Mac8Address>> &list, uint8_t frameNo,
                          uint32_t maxPkts)
    : m_frameNo (frameNo), m_retryNo (0), m_transmitted (false)
{
...
UanMacRc::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::UanMacRc")
          ...
          .AddAttribute ("RetryRate", "Number of retry attempts per second (of RTS/GWPING).",
                         DoubleValue (1 / 5.0), MakeDoubleAccessor (&UanMacRc::m_retryRate),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("MaxFrames", "Maximum number of frames to include in a single RTS.",
                         UintegerValue (1), MakeUintegerAccessor (&UanMacRc::m_maxFrames),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("QueueLimit", "Maximum packets to queue at MAC.", UintegerValue (10),
                         MakeUintegerAccessor (&UanMacRc::m_queueLimit),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("SIFS", "Spacing to give between frames (this should match gateway).",
                         TimeValue (Seconds (0.2)), MakeTimeAccessor (&UanMacRc::m_sifs),
                         MakeTimeChecker ())
          .AddAttribute ("NumberOfRates", "Number of rate divisions supported by each PHY.",
                         UintegerValue (0), MakeUintegerAccessor (&UanMacRc::m_numRates),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("MinRetryRate", "Smallest allowed RTS retry rate.", DoubleValue (0.01),
                         MakeDoubleAccessor (&UanMacRc::m_minRetryRate),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("RetryStep", "Retry rate increment.", DoubleValue (0.01),
                         MakeDoubleAccessor (&UanMacRc::m_retryStep), MakeDoubleChecker<double> ())
          .AddAttribute ("MaxPropDelay", "Maximum possible propagation delay to gateway.",
                         TimeValue (Seconds (2)), MakeTimeAccessor (&UanMacRc::m_learnedProp),
                         MakeTimeChecker ())
          .AddTraceSource ("Enqueue", "A  (data) packet arrived at MAC for transmission.",
                           MakeTraceSourceAccessor (&UanMacRc::m_enqueueLogger),
                           "ns3::UanMacRc::QueueTracedCallback")
          .AddTraceSource ("Dequeue", "A  (data) packet was passed down to PHY from MAC.",
                           MakeTraceSourceAccessor (&UanMacRc::m_dequeueLogger),
                           "ns3::UanMacRc::QueueTracedCallback")
          .AddTraceSource ("RX", "A packet was destined for and received at this MAC layer.",
...
void
UanMacRc::ScheduleData (const UanHeaderRcCts &ctsh, const UanHeaderRcCtsGlobal &ctsg,
                        uint32_t ctsBytes)
{
  ...
  m_learnedProp =
      Simulator::Now () - ctsg.GetTxTimeStamp () - Seconds (ctsBytes * 8.0 / currentBps);
  ...
  if (phy1ok && !phyDual->IsPhy2Tx () & !m_rtsBlocked)
    {
      ...
      pkt->AddHeader (UanHeaderCommon (Mac8Address::ConvertFrom (GetAddress ()),
                                       ...
                                       static_cast<uint8_t> (TYPE_GWPING), 0));
  ...
  if (phy1ok && !phyDual->IsPhy2Tx () && !m_rtsBlocked)
    {
      ...
      pkt->AddHeader (UanHeaderCommon (Mac8Address::ConvertFrom (GetAddress ()),
                                       Mac8Address::GetBroadcast (),
                                       static_cast<uint8_t> (TYPE_RTS), 0));
      SendPacket (pkt, m_currentRate + m_numRates);
    }
  m_state = RTSSENT;
  NS_ASSERT (!m_rtsEvent.IsRunning ());

...
UanMacRcGw::UanMacRcGw () : UanMac (), m_state (IDLE), m_currentRateNum (0), m_cleared (false)
{
  UanHeaderCommon ch;
  UanHeaderRcRts rts;
  UanHeaderRcCts cts;
...
UanMacRcGw::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::UanMacRcGw")
          ...
          .AddAttribute ("MaxReservations", "Maximum number of reservations to accept per cycle.",
                         UintegerValue (10), MakeUintegerAccessor (&UanMacRcGw::m_maxRes),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("NumberOfRates", "Number of rates per Phy layer.", UintegerValue (1023),
          ...
          .AddAttribute ("MaxPropDelay",
                         ...
                         TimeValue (Seconds (2)), MakeTimeAccessor (&UanMacRcGw::m_maxDelta),
                         MakeTimeChecker ())
          .AddAttribute (
              "SIFS", "Spacing between frames to account for timing error and processing delay.",
              TimeValue (Seconds (0.2)), MakeTimeAccessor (&UanMacRcGw::m_sifs), MakeTimeChecker ())
          .AddAttribute ("NumberOfNodes",
                         "Number of non-gateway nodes in this gateway's neighborhood.",
                         UintegerValue (10), MakeUintegerAccessor (&UanMacRcGw::m_numNodes),
                         MakeUintegerChecker<uint32_t> ())
          ...
          .AddAttribute ("TotalRate",
                         ...
                         UintegerValue (4096), MakeUintegerAccessor (&UanMacRcGw::m_totalRate),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("RateStep", "Increments available for rate assignment in bps.",
                         UintegerValue (4), MakeUintegerAccessor (&UanMacRcGw::m_rateStep),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("FrameSize", "Size of data frames in bytes.", UintegerValue (1000),
          ...
          .AddTraceSource ("Cycle", "Trace cycle statistics.",
...
bool
UanMacRcGw::Enqueue (Ptr<Packet> packet, [[maybe_unused]] uint16_t protocolNumber,
...
UanMacRcGw::StartCycle (void)
{
  ...
  if (numRts == 0)
    {
      ...
      m_state = INCYCLE;
      m_cycleLogger (Simulator::Now (), Seconds (0), numRts, totalBytes, effWinSize.GetSeconds (),
                     ctlRate, actualX);
      return;
    }
  ...
  m_cycleLogger (Simulator::Now (), minPdelay, numRts, totalBytes, cycleSeconds.GetSeconds (),
                 ctlRate, actualX);
}
...
double
UanMacRcGw::ComputeAlpha (uint32_t totalFrames, uint32_t totalBytes, [[maybe_unused]] uint32_t n,

...
uint32_t numIab = 2;
unsigned int expectedPackets = std::ceil ((simTimeSeconds * 1e3 - appStartMs) / ipi) * numIab;
...
main (int argc, char *argv[])
{
  ...
  // Layer 1
  iabPositionAlloc->Add (Vector (-80.0, 5.0, 5.0));
  iabPositionAlloc->Add (Vector (-50.0, 8.0, 5.0));

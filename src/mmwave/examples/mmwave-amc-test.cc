...
void
updateSnr (double snrInit, Ptr<MmWaveEnbNetDevice> enbDev, Ptr<SpectrumModel> model,
           Ptr<MmWaveAmc> amc)
{
  ...
  Simulator::Schedule (MilliSeconds (snrUpdateIntervalMs), &updateSnr, snrInit + 0.1, enbDev, model,
                       amc);
}
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("snrUpdateIntervalMs", "Period after which distance is updated",
                snrUpdateIntervalMs);
  cmd.Parse (argc, argv);
  ...
  Simulator::Schedule (MilliSeconds (0), &updateSnr, snrMinDb, enbMmwDev,

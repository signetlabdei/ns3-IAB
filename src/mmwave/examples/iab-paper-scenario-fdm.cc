...
std::string dataSensorRate = "10Kbps";
double powerTx = 30;
double bw = 400e6;
double fc0 = 26e9;
double fc1 = 28e9;
...
NS_LOG_COMPONENT_DEFINE ("IabInmarsatScenarioPaperFdm");
...
{
  auto packetSize = PacketByteSizeFromBitrateAndIpi (params.rate, params.IPI);
...
main (int argc, char *argv[])
{
  ...
  CommandLine cmd;
  cmd.AddValue("fc0", "CC 0 central frequency", fc0);
  cmd.AddValue("fc1", "CC 1 central frequency", fc1);
  cmd.AddValue("bandwidth", "System bandwidth in Hz", bw);
  ...
  // Do not use TDM between DUs at different layers
  Config::SetDefault ("ns3::MmWaveFlexTtiMacScheduler::UseTdmBetweenDuAndMtInterfaces", BooleanValue(false));
  ...
  // CC 0
	...
	phyMacConfig0->SetBandwidth(bw / 2);
	phyMacConfig0->SetCentreFrequency(fc0);
	...
	// CC 1
	phyMacConfig1->SetBandwidth(bw / 2);
	phyMacConfig1->SetCentreFrequency(fc1);
  ...
  iabPositionAlloc->Add (Vector (375*scenScale, 225*scenScale, 5.0));
  iabPositionAlloc->Add (Vector (500*scenScale, 575*scenScale, 5.0));
  // Layer 2
  iabPositionAlloc->Add (Vector (-975*scenScale, 675*scenScale, 5.0));
  iabPositionAlloc->Add (Vector (-525*scenScale, 875*scenScale, 5.0));
  iabPositionAlloc->Add (Vector (600*scenScale, 1000*scenScale, 5.0));
  // Layer 3
  iabPositionAlloc->Add (Vector (400*scenScale, 1300*scenScale, 5.0));
  iabPositionAlloc->Add (Vector (700*scenScale, 1400*scenScale, 5.0));
  ...
  uint8_t ccId = 0;
  Ptr<MmWaveEnbNetDevice> enbDev = DynamicCast<MmWaveEnbNetDevice> (enbMmWaveDev.Get (0));
  enbDev->GetAntenna (ccId)->SetAttribute ("BearingAngle", DoubleValue (M_PI / 2));
  ...
  *dlNodeAppStream->GetStream () << "rx_tx time packet_size port_to port_from delay\n";
  ...
  ulAppParams.IPI = MilliSeconds (ipi);

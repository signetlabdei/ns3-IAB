...
static ns3::GlobalValue g_dist ("dist", "Distance from eNB", ns3::UintegerValue (190),
                                ns3::MakeUintegerChecker<uint32_t> ());
static ns3::GlobalValue g_outPath ("outPath", "The path of output log files",
                                   ns3::StringValue ("./"), ns3::MakeStringChecker ());
static ns3::GlobalValue g_congestionControl ("congestionControl", "CC algorithm",
...
static ns3::GlobalValue g_harqEnabled ("harqEnabled", "If true, enable harq",
                                       ns3::BooleanValue (true), ns3::MakeBooleanChecker ());
...
main (int argc, char *argv[])
{
  ...
  Ptr<RateErrorModel> em1 = CreateObjectWithAttributes<RateErrorModel> (
      "RanVar", StringValue ("ns3::UniformRandomVariable[Min=0.0,Max=1.0]"), "ErrorRate",
      DoubleValue (0), "ErrorUnit", EnumValue (RateErrorModel::ERROR_UNIT_PACKET));
  ...
  Simulator::Schedule (Seconds (2), &ChangeSpeed, nodes.Get (0),
  ...
  for (uint32_t i = 0; i < numberOfNodes; i++)
    {
      ...
      LinuxStackHelper::RunIp (serverNodes.Get (i), Seconds (0.01), cmd_oss.str ().c_str ());
      cmd_oss.str ("");
      cmd_oss << "route add 8.0.0.0/8 via " << if3.GetAddress (1, 0) << " dev sim" << 1 << " table "
              << (2);
  ...
  for (uint32_t i = 0; i < numberOfNodes; i++)
    {
      LinuxStackHelper::RunIp (nodes.Get (i), Seconds (0.1),
      ...
      LinuxStackHelper::RunIp (serverNodes.Get (i), Seconds (0.1),

...
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << (uint32_t) ccId << '\t'
                        << size << std::endl;
}
...
main (int argc, char *argv[])
{
  bool useCa = true;
  uint32_t bandDiv =
      2; // if useCa=true, CC0 get (1-1/bandDiv)*totalBandwidth, CC1 get (1/bandDiv)*totalBandwidth
  bool useRR = false;
  bool useEpc = false;
  bool blockage = false;
  ...
  if (useEpc)
    {
      ...
      remoteHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.255.0.0"),
                                                  1);
    }
  ...
  uemobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel", "Bounds",

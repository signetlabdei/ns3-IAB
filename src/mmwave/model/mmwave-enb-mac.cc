...
MmWaveMacEnbMemberPhySapUser::MmWaveMacEnbMemberPhySapUser (MmWaveEnbMac *mac) : m_mac (mac)
{
}
...
MmWaveMacMemberMacSchedSapUser::MmWaveMacMemberMacSchedSapUser (MmWaveEnbMac *mac) : m_mac (mac)
{
  //     Some blank spaces
}
...
public:
  ...
  virtual void
  CschedCellConfigCnf (const struct MmWaveMacCschedSapUser::CschedCellConfigCnfParameters &params);
  virtual void
  CschedUeConfigCnf (const struct MmWaveMacCschedSapUser::CschedUeConfigCnfParameters &params);
  virtual void
  CschedLcConfigCnf (const struct MmWaveMacCschedSapUser::CschedLcConfigCnfParameters &params);
  virtual void
  CschedLcReleaseCnf (const struct MmWaveMacCschedSapUser::CschedLcReleaseCnfParameters &params);
  virtual void
  CschedUeReleaseCnf (const struct MmWaveMacCschedSapUser::CschedUeReleaseCnfParameters &params);
...
MmWaveMacMemberMacCschedSapUser::MmWaveMacMemberMacCschedSapUser (MmWaveEnbMac *mac) : m_mac (mac)
{
}
...
void
MmWaveMacMemberMacCschedSapUser::CschedUeConfigCnf (
...
void
MmWaveMacMemberMacCschedSapUser::CschedLcConfigCnf (
...
template <class C>
EnbMacMemberMmWaveMacSapProvider<C>::EnbMacMemberMmWaveMacSapProvider (C *mac) : m_mac (mac)
{
}
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveEnbMac")
          ...
          .AddAttribute (
              ...
              UintegerValue (50), MakeUintegerAccessor (&MmWaveEnbMac::m_preambleTransMax),
          ...
          .AddAttribute (
              ...
              UintegerValue (0), MakeUintegerAccessor (&MmWaveEnbMac::m_componentCarrierId),
              MakeUintegerChecker<uint8_t> (0, 4))
          .AddTraceSource ("DlMacTxCallback", "MAC transmission with tb size and number of retx.",
                           MakeTraceSourceAccessor (&MmWaveEnbMac::m_macDlTxSizeRetx),
                           "ns3::LteRlc::RetransmissionCountCallback")
          .AddTraceSource ("TxMacPacketTraceEnb", "Packets transmitted by EnbMac",
                           MakeTraceSourceAccessor (&MmWaveEnbMac::m_txMacPacketTraceEnb),
                           "ns3::EnbTxRxPacketCount::TracedCallback")
          .AddTraceSource ("SchedulingTraceEnb", "Information regarding scheduling allocation.",
...
MmWaveEnbMac::MmWaveEnbMac (void)
    : m_ccmMacSapUser (0), m_frameNum (0), m_sfNum (0), m_slotNum (0), m_tbUid (0)
{
              ...
              if (dciElem.m_ndi == 1)
                {
                  ...
                  MacPduInfo macPduInfo (pduSfn, ttiAllocInfo.m_dci.m_tbSize, rlcPduInfo.size (),
                                         dciElem);
                  ...
                  m_txMacPacketTraceEnb (rnti, m_componentCarrierId,
                  ...
                  if (dciElem.m_tbSize > 0)
                    {
                      ...
                      for (std::list<Ptr<Packet>>::const_iterator j = pb->Begin (); j != pb->End ();
                           ++j)
                        {
                          ...
                          tag.SetSfn (SfnSf (ind.m_sfnSf.m_frameNum, ind.m_sfnSf.m_sfNum,
                                             ind.m_sfnSf.m_slotNum, dciElem.m_symStart));
...
MmWaveEnbMac::TraceSchedInfo (MmWaveMacSchedSapUser::SchedConfigIndParameters ind)
{
  ...
  mmWaveSchedInfo.m_cellId = m_cellId;

  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveFlexTtiPfMacScheduler")
          ...
          .AddAttribute (
              ...
              MakeBooleanAccessor (&MmWaveFlexTtiPfMacScheduler::m_dlOnly), MakeBooleanChecker ())
          .AddAttribute (
              "UlSchedOnly", "Only schedule uplink traffic (for testing)", BooleanValue (false),
              MakeBooleanAccessor (&MmWaveFlexTtiPfMacScheduler::m_ulOnly), MakeBooleanChecker ())
    ...
    const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters &params)
{
  ...
  for (std::map<uint16_t, UeSchedInfo>::iterator ueIt = m_ueSchedInfoMap.begin ();
       ueIt != m_ueSchedInfoMap.end (); ueIt++)
    {
          ...
          if ((ueInfo->m_allocUlLast || ueInfo->m_dlAllocDone) && !ueInfo->m_ulAllocDone)
            {
              ...
              ueInfo->m_avgTputUl =
                  ...
                  ((1.0 / m_timeWindow) * ((double) ueInfo->m_ulTbSize /
                                           (m_phyMacConfig->GetSlotPeriod ().GetSeconds ())));
              ueAlloc = true;
            }
          else if (!ueInfo->m_dlAllocDone)
            {
              ...
              ueInfo->m_avgTputDl =
                  ...
                  ((1.0 / m_timeWindow) * ((double) ueInfo->m_dlTbSize /
                                           (m_phyMacConfig->GetSlotPeriod ().GetSeconds ())));
              ueAlloc = true;
            }

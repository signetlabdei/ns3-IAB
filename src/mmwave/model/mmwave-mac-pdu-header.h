...
struct MacSubheader
{
  MacSubheader () : m_lcid (0), m_size (0)
  {
  }

  MacSubheader (uint8_t lcid, uint32_t size) : m_lcid (lcid), m_size (size)
  {
  }
...
public:
  ...
  void
  SetSubheaders (std::vector<MacSubheader> macSubheaderList)
  {
    m_subheaderList = macSubheaderList;
  }

  std::vector<MacSubheader>
  GetSubheaders (void)
  {

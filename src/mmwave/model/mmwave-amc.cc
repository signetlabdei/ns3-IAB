...
MmWaveAmc::MmWaveAmc (const Ptr<MmWavePhyMacCommon> phyMacParams) : m_phyMacConfig{phyMacParams}
{
}
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveAmc")
          ...
          .AddAttribute ("Ber",
                         ...
                         DoubleValue (0.00001), MakeDoubleAccessor (&MmWaveAmc::SetBer),
...
MmWaveAmc::CalculateTbSize (uint8_t mcs, uint8_t nSym) const
{
  ...
  uint32_t cbSize = m_errorModel->GetMaxCbSize (
      payloadSize, mcs); // max size of a code block (including m_crcLen)
  if (tbSize > cbSize) // segmentation of the transport block occurs
    {
      ...
      tbSize =
          ...
          static_cast<uint32_t> (
              C *
              m_crcLen); //subtract bits of m_crcLen used in code blocks, in case of code block segmentation
    }
...
MmWaveAmc::CreateCqiFeedbackWbTdma (const SpectrumValue &sinr, uint8_t &mcs) const
{
  ...
  if (m_amcModel == ShannonModel)
  ...
  else if (m_amcModel == ErrorModel)
    {
      ...
      while (mcs <= m_errorModel->GetMaxMcs ())
        {
          output = m_errorModel->GetTbDecodificationStats (
              sinr, rbMap, CalculateTbSize (mcs, m_numSymForCqi), mcs,

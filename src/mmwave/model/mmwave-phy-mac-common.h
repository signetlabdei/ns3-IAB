...
struct SfnSf
{
  ...
  Encode () const
  {
    return ((uint64_t (m_frameNum) << 24) | ((m_sfNum & 0xFF) << 16) | ((m_slotNum & 0xFF) << 8) |
            (m_symStart & 0xFF));
  }
...
struct DlCqiInfo
{
  uint16_t m_rnti;
  uint8_t m_ri;
  enum DlCqiType { WB, SB } m_cqiType;
...
struct UlCqiInfo
{
  ...
  enum UlCqiType { SRS, PUSCH, PUCCH_1, PUCCH_2, PRACH } m_type;
};
...
{
  uint16_t m_rnti;
  enum MacCeType { BSR, PHR, CRNTI } m_macCeType;
...
struct UlHarqInfo
{
  uint16_t m_rnti; //!< the RNTI
  uint8_t m_harqProcessId; //!< the HARQ process ID
  enum ReceptionStatus { Ok, NotOk, NotValid } m_receptionStatus; //!< the status of the process
  uint8_t m_numRetx; //!< the number of retransmissions
};
...
private:
  ...
  uint32_t
      m_numRefSymbols; // TODO this parameter is used by MmWaveAmc::GetTbSizeFromMcs, which is never used
  ...
  uint32_t
      m_ulSchedDelay; //!<  Delay between the UL-DCI transmission and when such scheduled UL will take place, defined as # of NR slots.

...
MmWaveUeRrcProtocolIdeal::MmWaveUeRrcProtocolIdeal ()
    : m_ueRrcSapProvider (0), m_enbRrcSapProvider (0)
{
...
MmWaveUeRrcProtocolIdeal::DoSendRrcConnectionRequest (LteRrcSap::RrcConnectionRequest msg)
{
  ...
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteEnbRrcSapProvider::RecvRrcConnectionRequest,
                       m_enbRrcSapProvider, m_rnti, msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteEnbRrcSapProvider::RecvRrcConnectionSetupCompleted,
                       m_enbRrcSapProvider, m_rnti, msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteEnbRrcSapProvider::RecvMeasurementReport,
                       m_enbRrcSapProvider, m_rnti, msg);
}
                                                              ...
                                                              uint16_t mmWaveCellId)
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                       ...
                       m_enbRrcSapProvider, m_rnti, mmWaveRnti, mmWaveCellId);
}
...
MmWaveUeRrcProtocolIdeal::SetEnbRrcSapProvider ()
{
  ...
  Ptr<MmWaveIabNetDevice> mmWaveIabDev;
      ...
      for (int j = 0; (j < nDevs) && (!found); j++)
        {
          ...
          mmWaveIabDev = node->GetDevice (j)->GetObject<MmWaveIabNetDevice> ();
          ...
          else if (mmWaveIabDev)
            {
              if (mmWaveIabDev->HasCellId (cellId))
                {
                  found = true;
                  break;
                }
  ...
  if (enbDev)
    ...
    {
      m_enbRrcSapProvider = mmWaveIabDev->GetDuRrc ()->GetLteEnbRrcSapProvider ();
      Ptr<MmWaveEnbRrcProtocolIdeal> duRrcProtocolIdeal =
          mmWaveIabDev->GetDuRrc ()->GetObject<MmWaveEnbRrcProtocolIdeal> ();
      duRrcProtocolIdeal->SetUeRrcSapProvider (m_rnti, m_ueRrcSapProvider);
    }
  else
    {
...
MmWaveEnbRrcProtocolIdeal::MmWaveEnbRrcProtocolIdeal () : m_enbRrcSapProvider (0)
{
  NS_LOG_FUNCTION (this);
      ...
      for (int j = 0; j < nDevs; ++j)
        {
          ...
          else if (mcUeDev)
            {
              ...
              if (ueRrc->GetCellId () == cellId) // actually is using 2 connections
              ...
              else if (mcUeDev->GetMmWaveRrc () && mcUeDev->GetMmWaveRrc ()->GetCellId () == cellId)
                {
                  ...
                  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                                       &LteUeRrcSapProvider::RecvSystemInformation,
                                       mcUeDev->GetMmWaveRrc ()->GetLteUeRrcSapProvider (), msg);
                }
            }
          else if (lteUeDev)
            {
              ueRrc = lteUeDev->GetRrc ();
              NS_LOG_LOGIC ("considering UE IMSI " << lteUeDev->GetImsi () << " that has cellId "
                                                   << ueRrc->GetCellId ());
              if (ueRrc->GetCellId () == cellId)
                {
          ...
          if (iabDev)
            {
              ueRrc = iabDev->GetMtRrc ();
              NS_LOG_LOGIC ("considering UE IMSI " << mmWaveUeDev->GetImsi () << " that has cellId "
                                                   << ueRrc->GetCellId ());
              if (ueRrc->GetCellId () == cellId)
                {
                  NS_LOG_LOGIC ("sending SI to IMSI " << iabDev->GetImsi ());
                  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                                       &LteUeRrcSapProvider::RecvSystemInformation,
                                       ueRrc->GetLteUeRrcSapProvider (), msg);
                }
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionSetup,
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionReconfiguration,
                       GetUeRrcSapProvider (rnti), msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionReestablishment,
                       GetUeRrcSapProvider (rnti), msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionRelease,
                       GetUeRrcSapProvider (rnti), msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionReject,
                       GetUeRrcSapProvider (rnti), msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectionSwitch,
                       GetUeRrcSapProvider (rnti), msg);
}
...
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY, &LteUeRrcSapProvider::RecvRrcConnectToMmWave,
                       GetUeRrcSapProvider (rnti), mmWaveCellId);
}

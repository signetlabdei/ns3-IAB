  ...
  if (stateIt == m_ueState.end ())
    {
      ...
      std::pair<std::map<uint16_t, std::map<uint8_t, LteEnbCmacSapProvider::LcInfo>>::iterator,
          ...
          retA = m_rlcLcInstantiated.insert (
              std::pair<uint16_t, std::map<uint8_t, LteEnbCmacSapProvider::LcInfo>> (rnti, emptyA));
...
void
MmWaveNoOpComponentCarrierManager::DoAddLc (LteEnbCmacSapProvider::LcInfo lcInfo,
                                            LteMacSapUser *msu)
{
  NS_LOG_FUNCTION (this);
...
MmWaveNoOpComponentCarrierManager::DoSetupDataRadioBearer (EpsBearer bearer, uint32_t bearerId,

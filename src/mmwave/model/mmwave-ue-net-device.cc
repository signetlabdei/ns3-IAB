...
MmWaveUeNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::MmWaveUeNetDevice")
          ...
          .AddAttribute ("mmWaveUeRrc", "The RRC associated to this UeNetDevice", PointerValue (),
          ...
          .AddAttribute ("Imsi", "International Mobile Subscriber Identity assigned to this UE",
                         UintegerValue (0), MakeUintegerAccessor (&MmWaveUeNetDevice::m_imsi),
                         MakeUintegerChecker<uint64_t> ())
          .AddAttribute ("LteUeRrc", "The RRC layer associated with the ENB", PointerValue (),
                         MakePointerAccessor (&MmWaveUeNetDevice::m_rrc),
                         MakePointerChecker<LteUeRrc> ());
  return tid;
}
...
void
MmWaveUeNetDevice::SetTargetIab (Ptr<MmWaveIabNetDevice> iab)
{
  NS_LOG_FUNCTION (this << iab);
  m_targetIab = iab;
}
...
MmWaveUeNetDevice::GetTargetIab (void)
{
  NS_LOG_FUNCTION (this);
  return m_targetIab;
}

...
MmWaveEesmErrorModel::MappingSinrBler (double sinr, uint8_t mcs, uint32_t cbSizeBit)
{
  ...
  if (sinr_db < GetSinrDbVectorFromSimulatedValues (bg_type, mcs, cbIt->first).front ())
  ...
  else
    {
      ...
      auto sinrIt = std::upper_bound (
          GetSinrDbVectorFromSimulatedValues (bg_type, mcs, cbIt->first).begin (),
          GetSinrDbVectorFromSimulatedValues (bg_type, mcs, cbIt->first).end (), sinr_db);
      ...
      auto sinr_index = std::distance (
          GetSinrDbVectorFromSimulatedValues (bg_type, mcs, cbIt->first).begin (), sinrIt);
...
MmWaveEesmErrorModel::CodeBlockSegmentation (uint32_t B, GraphType bg_type) const
{
  ...
  auto ZcIt = std::upper_bound (LiftingSizeTableBG.begin (), --LiftingSizeTableBG.end (),
...
MmWaveEesmErrorModel::GetTbDecodificationStats (const SpectrumValue &sinr,
                                                const std::vector<int> &map, uint32_t size,
                                                uint8_t mcs,
...
MmWaveEesmErrorModel::GetTbBitDecodificationStats (const SpectrumValue &sinr,
                                                   const std::vector<int> &map, uint32_t sizeBit,
                                                   uint8_t mcs,
                                                   const MmWaveErrorModelHistory &sinrHistory)
{
  NS_LOG_FUNCTION (this);
...
uint32_t
MmWaveEesmErrorModel::GetPayloadSize (uint32_t usefulSc, uint8_t mcs, uint32_t rbNum,
                                      Mode mode) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (GetMcsEcrTable () != nullptr);

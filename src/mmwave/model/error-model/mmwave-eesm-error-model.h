...
public:
   ...
   */
  virtual Ptr<MmWaveErrorModelOutput>
  GetTbDecodificationStats (const SpectrumValue &sinr, const std::vector<int> &map, uint32_t size,
                            uint8_t mcs, const MmWaveErrorModelHistory &sinrHistory) override;
   ...
   *
   */
  virtual uint32_t GetPayloadSize (uint32_t usefulSc, uint8_t mcs, uint32_t rbNum,
...
protected:
   ...
   */
  virtual double
  ComputeSINR (const SpectrumValue &sinr, const std::vector<int> &map, uint8_t mcs,
               uint32_t sizeBit,
...
private:
  ...
  GetTbBitDecodificationStats (const SpectrumValue &sinr, const std::vector<int> &map,
   ...
   * \return
   */
  const std::vector<double> &GetSinrDbVectorFromSimulatedValues (GraphType graphType, uint8_t mcs,
   ...
   * \return
   */
  const std::vector<double> &GetBLERVectorFromSimulatedValues (GraphType graphType, uint8_t mcs,
                                                               uint32_t cbSizeIndex) const;
};

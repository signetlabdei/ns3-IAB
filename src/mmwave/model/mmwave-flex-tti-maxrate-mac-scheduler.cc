            ...
            if (itCqi == m_ueUlCqi.end ())
            ...
            else
              {
                ...
                NS_LOG_INFO ("UL CQI report for RNTI "
                             ...
                             << " subframe " << subframeNum << " startSym " << startSymIdx);
              }
          }
...
unsigned
MmWaveFlexTtiMaxRateMacScheduler::CalcMinTbSizeNumSym (unsigned mcs, unsigned bufSize,
                                                       unsigned &tbSize)
{
              ...
              if (symAvail >= dciInfoReTx.m_numSym)
                {
                  ...
                  NS_LOG_DEBUG (
                      "UE" << dciInfoReTx.m_rnti << " gets UL slots " << +dciInfoReTx.m_symStart
                           << "-" << +(dciInfoReTx.m_symStart + dciInfoReTx.m_numSym - 1) << " tbs "
                           << dciInfoReTx.m_tbSize << " harqId " << +dciInfoReTx.m_harqProcess
                           << " rv " << +dciInfoReTx.m_rv << " in frame " << ret.m_sfnSf.m_frameNum
                           << " subframe " << +ret.m_sfnSf.m_sfNum << " RETX");
                  ret.m_slotAllocInfo.m_ttiAllocInfo.push_back (ttiInfo);
                  ret.m_slotAllocInfo.m_numSymAlloc += dciInfoReTx.m_numSym;
                  ...
                  if (ret.m_slotAllocInfo.m_ttiAllocInfo[islot].m_tddMode ==
                      TtiAllocInfo::UL_slotAllocInfo)
                    {
                      ...
                      for (unsigned jslot = islot + 1;
                           jslot < ret.m_slotAllocInfo.m_ttiAllocInfo.size (); jslot++)
                        {

...
MmWaveEnbPhy::MmWaveEnbPhy (Ptr<MmWaveSpectrumPhy> dlPhy, Ptr<MmWaveSpectrumPhy> ulPhy)
    : MmWavePhy (dlPhy, ulPhy), m_prevSlot (0), m_prevTtiDir (TtiAllocInfo::NA), m_currSymStart (0)
{
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveEnbPhy")
          ...
          .AddAttribute (
              ...
              BooleanValue (false), MakeBooleanAccessor (&MmWaveEnbPhy::m_noiseAndFilter),
              MakeBooleanChecker ())
          ...
          .AddAttribute ("Transient",
                         ...
                         IntegerValue (320000), MakeIntegerAccessor (&MmWaveEnbPhy::m_transient),
                         MakeIntegerChecker<int> ())
          .AddAttribute (
              "NoiseFigure",
          ...
          .AddAttribute ("UlSpectrumPhy",
                         ...
                         MakePointerChecker<MmWaveSpectrumPhy> ())
          .AddTraceSource ("UlSinrTrace", "UL SINR statistics.",
      ...
      if (Simulator::Now () > Seconds (2.1) && Simulator::Now () < Seconds (2.3))
        {
          ...
          NS_LOG_DEBUG ("(std::all_of(prov.begin(),prov.end(), [](double j){nan;})) "
                        << (std::all_of (prov.begin (), prov.end (),
                                         [] (double k) { return !std::isnan (k); })));
          NS_LOG_DEBUG (
              "(std::all_of(provNoisy.begin(),provNoisy.end(), [](double p){return p > 10;})) "
              << (std::all_of (provNoisy.begin (), provNoisy.end (),
                               [] (double p) { return p > 10; })));
        }
...
std::vector<double>
MmWaveEnbPhy::MakeFilter (std::vector<double> noisySinr, std::vector<double> realSinr,
...
{
  Ptr<SpectrumValue> psd = MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity (
      m_phyMacConfig, m_txPower, m_listOfSubchannels);
  return psd;
}
...
MmWaveEnbPhy::UpdateUeSinrEstimate ()
{
  ...
  for (std::map<uint64_t, Ptr<NetDevice>>::iterator ue = m_ueAttachedImsiMap.begin ();
       ue != m_ueAttachedImsiMap.end (); ++ue)
    {
      // distinguish between MC UE, MmW UE and IAB MT
      ...
      Ptr<MmWaveIabNetDevice> iabDev = DynamicCast<MmWaveIabNetDevice> (ue->second);
      ...
      // create tx psd
      Ptr<SpectrumValue>
          txPsd = // it is the eNB that dictates the conf, m_listOfSubchannels contains all the subch
          MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity (m_phyMacConfig, ueTxPower,
                                                                   m_listOfSubchannels);
      ...
      // adjust beamforming of antenna model wrt user
      // make sure this uses the MT antenna when considering IAB nodes
      m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
          ue->second, DynamicCast<PhasedArrayModel> (uePhy->GetDlSpectrumPhy ()->GetAntenna ()));

      Ptr<MmWaveIabNetDevice> iabNetDevice = DynamicCast<MmWaveIabNetDevice> (m_netDevice);
      if (!iabNetDevice)
        {
        ...
        {
          uePhy->GetDlSpectrumPhy ()->ConfigureBeamformingForGivenAntenna (
              m_netDevice, iabNetDevice->GetDuAntenna (m_componentCarrierId));
        }
      ...
      *totalReceivedPsd += *rxPsd;
      ...
      if (ueNetDevice)
        ...
        { // target not set yet
          // TODO: Towards donor --> towards parent IAB node
          if ((iabDev->GetTargetDonor () != m_netDevice) && (iabDev->GetTargetDonor ()))
            {
              uePhy->GetDlSpectrumPhy ()->ConfigureBeamforming (iabDev->GetTargetDonor ());
            }
        }
      else
        {
      ...
      if (m_noiseAndFilter)
        {
          ...
          if (iteratorSinr !=
              m_sinrVector
                  .end ()) // this map has already been initialized, so I can add a new element for the SINR collection
            {
            ...
            {
              m_sinrVector.insert (std::pair<pairDevices_t, std::vector<double>> (
                  pairDevices, std::vector<double> ()));
          ...
          if (iteratorFinalTrace == m_finalSinrVector.end ())
            {
              m_samplesFilter.insert (std::pair<pairDevices_t, std::pair<uint64_t, uint64_t>> (
                  pairDevices, std::pair<uint64_t, uint64_t> ()));
              m_finalSinrVector.insert (std::pair<pairDevices_t, std::vector<double>> (
                  pairDevices, std::vector<double> ()));
            }
          ...
          if (iteratorSinrToFilter == m_sinrVectorToFilter.end ())
            {
              m_sinrVectorToFilter.insert (std::pair<pairDevices_t, std::vector<double>> (
                  pairDevices, std::vector<double> ()));
              m_sinrVectorNoisy.insert (std::pair<pairDevices_t, std::vector<double>> (
                  pairDevices, std::vector<double> ()));
            }
          ...
          if (Now ().GetMicroSeconds () <= m_transient)
            ...
            {
              m_sinrVectorNoisy.at (pairDevices)
                  .erase (m_sinrVectorNoisy.at (pairDevices).begin ());
              m_sinrVectorNoisy.at (pairDevices).push_back (sinrNoisy);
          ...
          if (Now ().GetMicroSeconds () >
              m_transient) // apply filter only when I have a sufficiently large set of SINR samples
            {
              ...
              * referred to the uplink reference signal that is used to build the RT in the LteEnbRrc class */
              ...
              NS_LOG_DEBUG (" mmWave eNB " << m_cellId << " reports the SINR "
                                           ...
                                           << ue->first);
              m_sinrMap[ue->first] =
                  sampleToForward; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< in order to FORWARD to LteEnbRrc the value of SINR for the RT
  ...
  Simulator::Schedule (MicroSeconds (m_updateSinrPeriod), &MmWaveEnbPhy::UpdateUeSinrEstimate,
                       this); // call again after m_updateSinrPeriod microseconds
}
...
MmWaveEnbPhy::StartTti (void)
{
  ...
  if (m_ttiIndex == 0) // First TTI: reserved DL control
    {
      ...
      SendCtrlChannels (
          ctrlMsgs, ttiPeriod - NanoSeconds (1.0)); // -1 ns ensures control ends before data period
    }
  ...
  else if (currTti.m_tddMode == TtiAllocInfo::DL_slotAllocInfo) // Scheduled DL data Tti
    {
      ...
      Simulator::Schedule (NanoSeconds (1.0), &MmWaveEnbPhy::SendDataChannels, this, pktBurst,
                           ttiPeriod - NanoSeconds (2.0), currTti);
    }
  else if (currTti.m_tddMode == TtiAllocInfo::UL_slotAllocInfo) // Scheduled UL data Tti
    {
      ...
      m_downlinkSpectrumPhy->AddExpectedTb (
          currTti.m_dci.m_rnti, currTti.m_dci.m_ndi, currTti.m_dci.m_tbSize, currTti.m_dci.m_mcs,
          m_channelChunks, currTti.m_dci.m_harqProcess, currTti.m_dci.m_rv, false,
      ...
      for (uint8_t i = 0; i < m_deviceMap.size (); i++)
        {
          ...
          Ptr<MmWaveIabNetDevice> iabDev = DynamicCast<MmWaveIabNetDevice> (m_deviceMap.at (i));
          ...
          if (ueDev)
            ...
            {
              ueRnti = mcUeDev->GetMmWavePhy ()->GetRnti ();
              associatedEnb = mcUeDev->GetMmWaveTargetEnb ();
            }
          else if (iabDev)
            {
              ueRnti = iabDev->GetMtPhy ()->GetRnti ();
              associatedEnb = iabDev->GetTargetDonor ();
            }
          else
            {
              NS_ABORT_MSG ("Unrecognized device");
            }
          ...
          if (currTti.m_rnti == ueRnti && m_netDevice == associatedEnb)
            {
              if (ueDev || mcUeDev)
                ...
                {
                  m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
                      m_deviceMap.at (i), iabDev->GetMtAntenna (m_componentCarrierId));
                }
  ...
  // TODO: fix assert check, get the enbApp of the Donor.
  // uint32_t iabDepth = enbApp->GetIabNodeDepth (m_cellId);

  // if (iabDepth%2 == 0) // even layer
  // {
      ...
      for (uint8_t i = 0; i < m_deviceMap.size (); i++)
        {
          uint64_t ueRnti = 0;
          Ptr<mmwave::MmWaveUeNetDevice> ueDev =
              DynamicCast<MmWaveUeNetDevice> (m_deviceMap.at (i));
          Ptr<McUeNetDevice> ueMcDev = DynamicCast<McUeNetDevice> (m_deviceMap.at (i));
          Ptr<MmWaveIabNetDevice> iabDev = DynamicCast<MmWaveIabNetDevice> (m_deviceMap.at (i));
          ...
          if (ueDev)
            {
              ueRnti = ueDev->GetPhy ()->GetRnti ();
              Ptr<MmWaveIabNetDevice> iabDev = DynamicCast<MmWaveIabNetDevice> (m_netDevice);
              if (iabDev)
                {
                  associatedEnb = ueDev->GetTargetIab ();
                }
              else
                {
                  associatedEnb = ueDev->GetTargetEnb ();
          ...
          // TODO: Enable data TX also towards MT of IAB nodes
          if (slotInfo.m_dci.m_rnti == ueRnti && m_netDevice == associatedEnb && (ueDev || ueMcDev))
            {
              NS_LOG_DEBUG ("Change Beamforming Vector");
              m_downlinkSpectrumPhy->ConfigureBeamforming (m_deviceMap.at (i));
              break;
            }
          else if (slotInfo.m_dci.m_rnti == ueRnti && m_netDevice == associatedEnb && iabDev)
            {
              NS_LOG_DEBUG ("Change Beamforming Vector");
              m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
                  m_deviceMap.at (i), iabDev->GetMtAntenna (m_componentCarrierId));
...
MmWaveEnbPhy::PhyDataPacketReceived (Ptr<Packet> p)
{
  Simulator::ScheduleWithContext (m_netDevice->GetNode ()->GetId (),
                                  ...
                                  &MmWaveEnbPhySapUser::ReceivePhyPdu, m_phySapUser, p);

...
MmWaveUePhy::MmWaveUePhy (Ptr<MmWaveSpectrumPhy> dlPhy, Ptr<MmWaveSpectrumPhy> ulPhy)
    : MmWavePhy (dlPhy, ulPhy), m_prevSlot (0), m_rnti (0)
{
  NS_LOG_FUNCTION (this);
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveUePhy")
          ...
          .AddAttribute ("OutageThreshold", "SNR threshold for outage events [dB]",
                         DoubleValue (-5.0), MakeDoubleAccessor (&MmWaveUePhy::m_outageThreshold),
          ...
          .AddAttribute ("CqiReportPeriod",
                         ...
                         UintegerValue (10), MakeUintegerAccessor (&MmWaveUePhy::SetWbCqiPeriod),
...
MmWaveUePhy::SetWbCqiPeriod (uint16_t period)
{
  const std::set<uint16_t> supportedValues = {
      4,  5,  8,  10,  16,
      20, 40, 80, 160, 320}; // CSI-ReportPeriodicityAndOffset, see TS 38.331 V16.0.0 Sec 6.3.2
...
MmWaveUePhy::CreateTxPowerSpectralDensity ()
{
  Ptr<SpectrumValue> psd = MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity (
      m_phyMacConfig, m_txPower, m_subChannelsForTx);
  return psd;
}
...
void
MmWaveUePhy::RegisterToIab (uint16_t cellId, Ptr<MmWavePhyMacCommon> config)
{
  NS_LOG_FUNCTION (this);
  m_cellId = cellId;
  m_phyReset = false;
  ...
  Ptr<MmWaveIabNetDevice> iabNetDevice = m_registeredIab.find (cellId)->second.second;
  if (DynamicCast<mmwave::MmWaveUeNetDevice> (m_netDevice))
    {
      DynamicCast<mmwave::MmWaveUeNetDevice> (m_netDevice)->SetTargetIab (iabNetDevice);
    }
  else if (DynamicCast<McUeNetDevice> (m_netDevice))
    {
      DynamicCast<McUeNetDevice> (m_netDevice)->SetMmWaveTargetIab (iabNetDevice);
    }
  NS_LOG_LOGIC ("UE register to iab " << m_cellId);

  // point the beam towards the serving BS
  m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
      iabNetDevice, iabNetDevice->GetDuAntenna (m_componentCarrierId));
  ...
  m_downlinkSpectrumPhy->GetSpectrumChannel ()->AddRx (m_downlinkSpectrumPhy);
  m_downlinkSpectrumPhy->SetCellId (m_cellId);
  NS_LOG_INFO ("Registered to IAB with CellId " << m_cellId);
}

void
MmWaveUePhy::RegisterOtherIab (uint16_t cellId, Ptr<MmWavePhyMacCommon> config,
                               Ptr<MmWaveIabNetDevice> iabNetDevice)
{
  NS_ASSERT_MSG (m_registeredIab.find (cellId) == m_registeredIab.end (), "Iab already registered");
  std::pair<Ptr<MmWavePhyMacCommon>, Ptr<MmWaveIabNetDevice>> pair (config, iabNetDevice);
  m_registeredIab[cellId] = pair;
  // point the beam towards the serving BS
  m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
      iabNetDevice, iabNetDevice->GetDuAntenna (m_componentCarrierId));
}
...
MmWaveUePhy::StartTti ()
{
  ...
  if (m_cellId > 0)
    {
      bool isConnectedToIab = (m_registeredIab.find (m_cellId) != m_registeredIab.end ());
      bool isConnectedToEnb = (m_registeredEnb.find (m_cellId) != m_registeredEnb.end ());

      NS_ASSERT (isConnectedToEnb != isConnectedToIab);

      if (isConnectedToIab)
        {
          // point the beam towards the serving IAB
          NS_LOG_LOGIC ("UE " << m_rnti << " cid " << m_cellId << " IAB");
          auto iabNetDevice = m_registeredIab.find (m_cellId)->second.second;

          m_downlinkSpectrumPhy->ConfigureBeamformingForGivenAntenna (
              iabNetDevice, iabNetDevice->GetDuAntenna (m_componentCarrierId));
        }
      else if (isConnectedToEnb)
        {
          // point the beam towards the serving BS
          NS_LOG_LOGIC ("UE " << m_rnti << " cid " << m_cellId << " BF");
          m_downlinkSpectrumPhy->ConfigureBeamforming (
              m_registeredEnb.find (m_cellId)->second.second);
        }
      else
        {
          NS_ABORT_MSG ("Cell ID not recognized");
        }
    }
  ...
  if (m_ttiIndex == 0) // First TTI: reserved DL control
  ...
  else if (currTti.m_dci.m_format == DciInfoElementTdma::UL_dci) // Scheduled UL data Tti
    {
      ...
      if (pktBurst)
        {
          std::list<Ptr<MmWaveControlMessage>> ctrlMsg = GetControlMessages ();
          m_sendDataChannelEvent = Simulator::Schedule (
              NanoSeconds (1.0), &MmWaveUePhy::SendDataChannels, this, pktBurst, ctrlMsg,
              currTtiDuration - NanoSeconds (2.0), m_slotNum);
        }
    }
  else
    {
...
MmWaveUePhy::EndTti ()
{
  ...
  if (m_ttiIndex == m_currSlotAllocInfo.m_ttiAllocInfo.size () -
                        1) // End of this slot, as last TTI always happens at the last OFDM symbol
    {
      ...
      Simulator::Schedule (nextSlotDelay, &MmWaveUePhy::SlotIndication, this, frameNum, sfNum,
                           slotNum);
    }
  else
    {
  ...
  if (!m_phyReset)
    {
      Simulator::Schedule (MicroSeconds (m_phyMacConfig->GetTbDecodeLatency ()),
                           &MmWaveUePhy::DelayPhyDataPacketReceived, this, p);
    }
...
void
MmWaveUePhy::SendDataChannels (Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage>> ctrlMsg,
...
MmWaveUePhy::ReceiveLteDlHarqFeedback (DlHarqInfo m)
{
  ...
  m_sendDlHarqFeedbackEvent =
      Simulator::Schedule (MicroSeconds (m_phyMacConfig->GetTbDecodeLatency ()),
                           &MmWaveUePhy::DoSendControlMessage, this, msg);
...
MmWaveUePhy::DoSynchronizeWithEnb (uint16_t cellId)
{
  ...
  if (cellId == 0)
  ...
  else
    {
      ...
      else if (m_registeredIab.find (cellId) != m_registeredIab.end ())
        {
          RegisterToIab (m_registeredIab.find (cellId)->first,
                         m_registeredIab.find (cellId)->second.first);
        }
      else
        {
          NS_FATAL_ERROR ("Unknown eNB or IAB");
        }
    }
}

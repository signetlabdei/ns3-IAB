...
MmWaveBeamformingModel::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::MmWaveBeamformingModel")
          .SetParent<Object> ()
          .AddAttribute ("Device", "The current NetDevice", PointerValue (0),
                         MakePointerAccessor (&MmWaveBeamformingModel::SetDevice,
                                              &MmWaveBeamformingModel::GetDevice),
                         MakePointerChecker<NetDevice> ())
          .AddAttribute ("Antenna", "The antenna of the Device to configure", PointerValue (0),
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveSvdBeamforming")
          ...
          .AddAttribute (
              ...
              PointerValue (), MakePointerAccessor (&MmWaveSvdBeamforming::m_channel),
              MakePointerChecker<MatrixBasedChannelModel> ())
          .AddAttribute (
              "MaxIterations",
              "Maximum number of iterations to numerically approximate the SVD decomposition",
              UintegerValue (30), MakeUintegerAccessor (&MmWaveSvdBeamforming::m_maxIterations),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Tolerance", "Tolerance to numerically approximate the SVD decomposition",
                         DoubleValue (1e-8),
                         MakeDoubleAccessor (&MmWaveSvdBeamforming::m_tolerance),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("UseCache",
                         "Use the cache for the BF vectors. Do not set it to false unless you have "
                         "very good reasons",
                         BooleanValue (true),
...
MmWaveSvdBeamforming::MmWaveSvdBeamforming () : m_useCache{false}
{
  NS_LOG_FUNCTION (this);
}
...
MmWaveCodebookBeamforming::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::MmWaveCodebookBeamforming")
          ...
          .AddAttribute ("SpectrumPropagationLossModel", "Pointer to SpectrumPropagationLossModel",
                         PointerValue (), MakePointerAccessor (&MmWaveCodebookBeamforming::m_splm),
                         MakePointerChecker<SpectrumPropagationLossModel> ())
          .AddAttribute ("PhasedArraySpectrumPropagationLossModel",
                         "Pointer to SpectrumPropagationLossModel", PointerValue (),
                         MakePointerAccessor (&MmWaveCodebookBeamforming::m_pSplm),
                         MakePointerChecker<PhasedArraySpectrumPropagationLossModel> ())
          .AddAttribute ("MmWavePhyMacCommon", "Pointer to MmWavePhyMacCommon", PointerValue (),
                         MakePointerAccessor (&MmWaveCodebookBeamforming::SetMmWavePhyMacCommon),
                         MakePointerChecker<MmWavePhyMacCommon> ())
          .AddAttribute (
              "UpdatePeriod", "Specify the channel coherence time", TimeValue (MilliSeconds (0.0)),
              MakeTimeAccessor (&MmWaveCodebookBeamforming::m_updatePeriod), MakeTimeChecker ());
  return tid;
}
...
MmWaveCodebookBeamforming::SetMmWavePhyMacCommon (Ptr<MmWavePhyMacCommon> mwpmc)
{
  ...
  m_txPsd = MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity (
      mwpmc, 0.0, activeRbs); // TODO should i copy this?
}
...
MmWaveCodebookBeamforming::ComputeBeamformingCodebookMatrix (
    Ptr<NetDevice> otherDevice, Ptr<PhasedArrayModel> otherAntenna) const
{
  NS_LOG_FUNCTION (this << otherDevice << otherAntenna);
      ...
      for (uint32_t otherIdx = 0; otherIdx < otherCodebook->GetCodebookSize (); otherIdx++)
        {
          ...
          if (m_splm)
            ...
            {
              rxPsd = m_pSplm->CalcRxPowerSpectralDensity (rxParams, thisMob, otherMob, m_antenna,
                                                           otherAntenna);
            }

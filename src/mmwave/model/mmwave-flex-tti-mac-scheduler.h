...
private:
  struct UeSchedInfo
  {
    ...
    uint32_t
        m_maxDlBufSize; // DL TB size needed to encode the DL buffer (this parameter is also used to temporarily store the DL buffer size)
    uint32_t
        m_maxUlBufSize; // UL TB size needed to encode the UL buffer (this parameter is also used to temporarily store the UL buffer size)
  ...
  void
  DoSchedUlCqiInfoReq (const struct MmWaveMacSchedSapProvider::SchedUlCqiInfoReqParameters &params);

  void DoSchedUlMacCtrlInfoReq (
      const struct MmWaveMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters &params);

  void
  DoSchedTriggerReq (const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters &params);

  bool
  DoSchedDlTriggerReq (const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters &params,
                       MmWaveMacSchedSapUser::SchedConfigIndParameters &ret, uint32_t frameNum,
                       unsigned int sfNum, unsigned int islot);

  bool
  DoSchedUlTriggerReq (const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters &params,
                       MmWaveMacSchedSapUser::SchedConfigIndParameters &ret, uint32_t frameNum,
                       unsigned int sfNum, unsigned int islot);

...
MmWaveSpectrumPhy::ConfigureBeamforming (Ptr<NetDevice> device)
{
  ...
  Ptr<MmWaveIabNetDevice> mmIabNetDevice = DynamicCast<MmWaveIabNetDevice> (device);

  if (mmIabNetDevice)
    {
      NS_ABORT_MSG (
          "Configure Beamforming with IAB device, use ConfigureBeamformingForGivenAntenna! ");
    }

  // test if device is a MmWaveNetDevice
  Ptr<MmWaveEnbNetDevice> mmEnbNetDevice = DynamicCast<MmWaveEnbNetDevice> (device);

  if (mmEnbNetDevice)
    {
      antenna = mmEnbNetDevice->GetAntenna (m_componentCarrierId);
    }

  Ptr<MmWaveUeNetDevice> mmUeNetDevice = DynamicCast<MmWaveUeNetDevice> (device);

  if (mmUeNetDevice)
    {
      antenna = mmUeNetDevice->GetAntenna (m_componentCarrierId);
    }
...
void
MmWaveSpectrumPhy::ConfigureBeamformingForGivenAntenna (Ptr<NetDevice> device,
                                                        Ptr<PhasedArrayModel> antenna)
{
  NS_LOG_FUNCTION (this << device);
...
MmWaveSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{
  ...
  if ((enbTx && enbRx) | (((!iabTx && !enbTx)) && (!iabRx && !enbRx)))
    {
  ...
  if (mmwaveDataRxParams)
    {
      ...
      // TODO in case our device is a UE or IAB node's MT, the code below checks if the UePhy is
      ...
      if ((ueRx) && (ueRx->GetPhy (m_componentCarrierId)->IsReceptionEnabled () == false))
        { // if the first cast is 0 (the device is MC) then this if will not be executed
          NS_LOG_WARN ("RX is not allocated, dropping packet");
          isAllocated = false;
        }
      else if ((rxMcUe) &&
      ...
      else if (iabRx && // if IAB node
               (iabRx->GetMtPhy ()->GetDlSpectrumPhy () == this ||
                iabRx->GetMtPhy ()->GetUlSpectrumPhy () == this) && // and the MT is receiving
               iabRx->GetMtPhy (m_componentCarrierId)->IsReceptionEnabled () == false)
        {
          NS_LOG_WARN ("RX is not allocated, dropping packet");
          isAllocated = false;
        }
              ...
              if (!itTb->second.m_harqFeedbackSent)
                {
                  ...
                  if (!itTb->second.m_expected.m_isDownlink) // UPLINK TB
                      ...
                      if (itTb->second.m_isCorrupted)
                        {
                          ...
                          NS_LOG_DEBUG ("UE" << rnti << " send UL-HARQ-NACK"
                                             << " harqId "
                      ...
                      else
                        {
                          ...
                          NS_LOG_DEBUG ("UE" << rnti << " send UL-HARQ-ACK"
                                             << " harqId "
                                             << +itTb->second.m_expected.m_harqProcessId << " size "
                                             << itTb->second.m_expected.m_tbSize << " mcs "
                                             << +itTb->second.m_expected.m_mcs << " tbler "
                    ...
                    { // DOWNLIK TB
                      ...
                      if (itTb->second.m_isCorrupted)
                        {
                          NS_LOG_DEBUG ("UE" << rnti << " send DL-HARQ-NACK"
                                             << " harqId "
                                             << +itTb->second.m_expected.m_harqProcessId << " size "
                                             << itTb->second.m_expected.m_tbSize << " mcs "
                                             << +itTb->second.m_expected.m_mcs << " tbler "

...
FileBeamformingCodebook::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::FileBeamformingCodebook")
          ...
          .AddAttribute ("CodebookFilename", "The filename for the codebook file", StringValue (""),
...
FileBeamformingCodebook::ParseCodeword (const std::string &line, uint32_t cwSize)
{
  ...
  NS_ABORT_MSG_IF (cw.size () != cwSize, "Codeword of unexpected size: cw.size ()="
                                             << cw.size () << ", cwSize=" << cwSize);
  return cw;
}

  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveFlexTtiMaxWeightMacScheduler")
          ...
          .AddAttribute ("HarqEnabled", "Activate/Deactivate the HARQ [by default is active].",
                         ...
                         MakeBooleanChecker ())
          .AddAttribute ("FixedMcsDl", "Fix MCS to value set in McsDlDefault", BooleanValue (false),
                         MakeBooleanAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_fixedMcsDl),
                         MakeBooleanChecker ())
          .AddAttribute ("McsDefaultDl", "Fixed DL MCS", UintegerValue (1),
                         MakeUintegerAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_mcsDefaultDl),
                         MakeUintegerChecker<uint8_t> ())
          .AddAttribute ("FixedMcsUl", "Fix MCS to value set in McsUlDefault (for testing)",
                         BooleanValue (false),
                         MakeBooleanAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_fixedMcsUl),
                         MakeBooleanChecker ())
          .AddAttribute ("McsDefaultUl", "Fixed UL MCS (for testing)", UintegerValue (1),
                         MakeUintegerAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_mcsDefaultUl),
                         MakeUintegerChecker<uint8_t> ())
          .AddAttribute ("DlSchedOnly", "Only schedule downlink traffic (for testing)",
                         BooleanValue (false),
                         MakeBooleanAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_dlOnly),
                         MakeBooleanChecker ())
          .AddAttribute ("UlSchedOnly", "Only schedule uplink traffic (for testing)",
                         BooleanValue (false),
          ...
          .AddAttribute (
              ...
              MakeEnumAccessor<MmWaveFlexTtiMaxWeightMacScheduler::AlgType> (&MmWaveFlexTtiMaxWeightMacScheduler::m_algorithm),
              MakeEnumChecker (MmWaveFlexTtiMaxWeightMacScheduler::DELIVERY_DEBT, "DeliveryDebt",
                               MmWaveFlexTtiMaxWeightMacScheduler::EDF, "EDF"))
          .AddAttribute ("FixedTti", "Fix slot size", BooleanValue (false),
                         MakeBooleanAccessor (&MmWaveFlexTtiMaxWeightMacScheduler::m_fixedTti),
                         MakeBooleanChecker ())
          .AddAttribute ("SymPerSlot", "Number of symbols per slot in Fixed TTI mode",
                         UintegerValue (6),
...
unsigned
MmWaveFlexTtiMaxWeightMacScheduler::CalcMinTbSizeNumSym (unsigned mcs, unsigned bufSize,
                                                         unsigned &tbSize)
{
              ...
              if (symAvail >= dciInfoReTx.m_numSym)
                {
                  ...
                  NS_LOG_DEBUG (
                      "UE" << dciInfoReTx.m_rnti << " gets DL slots " << +dciInfoReTx.m_symStart
                           << "-" << +(dciInfoReTx.m_symStart + dciInfoReTx.m_numSym - 1) << " tbs "
                           << dciInfoReTx.m_tbSize << " harqId " << +dciInfoReTx.m_harqProcess
                           << " harqId " << +dciInfoReTx.m_harqProcess << " rv "
                           << +dciInfoReTx.m_rv << " in frame " << ret.m_sfnSf.m_frameNum
                           << " subframe " << +ret.m_sfnSf.m_sfNum << " RETX");
              ...
              if (symAvail >= dciInfoReTx.m_numSym)
                {
                  ...
                  ttiInfo.m_dci = dciInfoReTx;
                  NS_LOG_DEBUG (
                      "UE" << dciInfoReTx.m_rnti << " gets UL slots " << +dciInfoReTx.m_symStart
                           << "-" << +(dciInfoReTx.m_symStart + dciInfoReTx.m_numSym - 1) << " tbs "
                           << dciInfoReTx.m_tbSize << " harqId " << +dciInfoReTx.m_harqProcess
                           << " rv " << +dciInfoReTx.m_rv << " in frame " << ulSfn.m_frameNum
                           << " subframe " << +ulSfn.m_sfNum << " RETX");
                  ret.m_slotAllocInfo.m_ttiAllocInfo.push_back (ttiInfo);
                  ret.m_slotAllocInfo.m_numSymAlloc += dciInfoReTx.m_numSym;
      ...
      while (symAvail > 0)
        {
          std::sort (m_flowHeap.begin (), m_flowHeap.end (),
                     MmWaveFlexTtiMaxWeightMacScheduler::
                         CompareFlowWeightsEdf); // Sort flow heap by relative deadline
                  ...
                  if (cqi != 0)
                    {
                      ...
                      uint32_t numSymReq =
                          ...
                          ueInfo->m_dlSymbols;
                      if (numSymReq <=
                          (unsigned) symAvail) // sufficient symbols to TX whole RLC PDU at this MCS
                        {
                              ...
                              if (numSymFixed > numSymReq)
                                {
                                  ...
                                  pduSize = m_amc->CalculateTbSize (
                                                ueInfo->m_dlMcs, ueInfo->m_dlSymbols + numSymReq) -
                                            ueInfo->m_dlTbSize;
                                }
                            }
                      ...
                      else // insufficient symbols, allocate remaining symbols (must segment RLC PDU)
                        {
                          ...
                          uint32_t tbSize = m_amc->CalculateTbSize (ueInfo->m_dlMcs,
                  ...
                  if (cqi != 0)
                    {
                      ...
                      if (numSymReq <=
                              ...
                              if (numSymFixed > numSymReq)
                                {
                                  ...
                                  // recalculate TB size in case numSymReq increased
                                  pduSize = m_amc->CalculateTbSize (
                                                ueInfo->m_ulMcs, ueInfo->m_ulSymbols + numSymReq) -
                                            ueInfo->m_ulTbSize;
                                }
                            }
                        ...
                        {
                          // get maximum TB size from MCS and available symbols
                          uint32_t tbSize = m_amc->CalculateTbSize (ueInfo->m_ulMcs,
                  ...
                  if (ret.m_slotAllocInfo.m_ttiAllocInfo[iTti].m_tddMode ==
                      TtiAllocInfo::UL_slotAllocInfo)
                    {
                      ...
                      for (unsigned jTti = iTti + 1;
                           jTti < ret.m_slotAllocInfo.m_ttiAllocInfo.size (); jTti++)
                        {
...
void
MmWaveFlexTtiMaxWeightMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid,
                                                           uint16_t size)
{
  NS_LOG_FUNCTION (this);

...
MmWaveLteUeRrcProtocolReal::DoSendRrcConnectionRequest (LteRrcSap::RrcConnectionRequest msg)
{
  ...
  Simulator::Schedule (RRC_REAL_MSG_DELAY, &LteEnbRrcSapProvider::RecvRrcConnectionRequest,
...
MmWaveLteUeRrcProtocolReal::SetEnbRrcSapProvider ()
{
  ...
  if (enbDev)
  ...
  else if (mmWaveIabDev)
    {
      ...
      Ptr<MmWaveLteEnbRrcProtocolReal> enbRrcProtocolReal =
          mmWaveIabDev->GetDuRrc ()->GetObject<MmWaveLteEnbRrcProtocolReal> ();
      enbRrcProtocolReal->SetUeRrcSapProvider (m_rnti, m_ueRrcSapProvider);
    }
  else
    {
...
MmWaveLteEnbRrcProtocolReal::MmWaveLteEnbRrcProtocolReal () : m_enbRrcSapProvider (0)
{
  NS_LOG_FUNCTION (this);
      ...
      for (int j = 0; j < nDevs; ++j)
        {
          ...
          if (ueDev)
          ...
          else if (iabDev)
            {
              ...
              NS_LOG_LOGIC ("considering MT IMSI " << iabDev->GetImsi () << " that has cellId "
                                                   << ueRrc->GetCellId ());
              if (ueRrc->GetCellId () == cellId)
                {
  ...
  switch (rrcUlDcchMessage.GetMessageType ())
    ...
    case 5:
      ...
      m_enbRrcSapProvider->RecvRrcSecondaryCellInitialAccessSuccessful (
          params.rnti, rrcNotifyPair.second, rrcNotifyPair.first);
      break;
    }
}

...
public:
  ...
  void RegisterToIab (uint16_t cellId, Ptr<MmWavePhyMacCommon> config);
  void RegisterOtherIab (uint16_t cellId, Ptr<MmWavePhyMacCommon> config,
  ...
  void SendDataChannels (Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage>> ctrlMsg,
...
private:
  ...
  */
  ...
  uint16_t
      m_wbCqiPeriod; //!< The period of the WB CQI, in number of slots. See TS 38.331 V16.0.0 Sec 6.3.2
  Time m_wbCqiLast;
  ...
  bool
      m_ulGrant; // true if no uplink grant in subframe, need to transmit UL control in PUCCH instead
  ...
  std::map<uint16_t, std::pair<Ptr<MmWavePhyMacCommon>, Ptr<MmWaveIabNetDevice>>> m_registeredIab;

...
public:
  ...
  std::vector<double> MakeFilter (std::vector<double>, std::vector<double>,
...
private:
  ...
  */
  ...
  std::map<pairDevices_t, std::vector<double>>
      m_finalSinrVector; // array containing all  SINR values after the filtering for a specific pair (UE-eNB)
  ...
  bool
      m_noiseAndFilter; // If true, use noisy SINR samples, filtered. If false, just use the SINR measure

  Ptr<MmWaveHarqPhy> m_harqPhyModule;
  std::vector<int> m_channelChunks;

  uint8_t
      m_currSymStart; //!< Beginning of the current TTI, expressed as OFDM symbol # within the NR slot

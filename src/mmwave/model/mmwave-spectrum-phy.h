...
struct ExpectedTb
{
  ExpectedTb (uint8_t ndi, uint32_t tbSize, uint8_t mcs, const std::vector<int> &rbBitmap,
              uint8_t harqProcessId, uint8_t rv, bool isDownlink, uint8_t symStart, uint8_t numSym)
      : m_ndi (ndi),
        m_tbSize (tbSize),
        m_mcs (mcs),
        m_rbBitmap (rbBitmap),
...
{
  TransportBlockInfo (const ExpectedTb &expected) : m_expected (expected)
  {
  }
...
typedef std::unordered_map<uint16_t, TransportBlockInfo>
    TbInfoMap_t; //!< Transport block map per RNTI of TBs which are expected to be received by reading DL or UL DCIs
...
public:
  ...
  enum State { IDLE = 0, TX = 1, RX_DATA = 2, RX_CTRL = 3 };
  ...
  *
  * \param device target device
  * \param antenna target antenna
  */
  void ConfigureBeamformingForGivenAntenna (Ptr<NetDevice> device, Ptr<PhasedArrayModel> antenna);
  ...
  bool StartTxDataFrames (Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage>> ctrlMsgList,
   ...
   */
  void AddExpectedTb (uint16_t rnti, uint8_t ndi, uint32_t tbSize, uint8_t mcs,
                      std::vector<int> map, uint8_t harqId, uint8_t rv, bool downlink,
...
private:
  ...
  bool
      m_ctrlErrorModelEnabled; // when true (default) the phy error model is enabled for DL ctrl frame

...
private:
  ...
  GetFirstEigenvector (MatrixBasedChannelModel::Complex2DVector A) const;

  Ptr<MatrixBasedChannelModel>
      m_channel; //!< pointer to the MatrixChannel, to retrieve the matrix on which the SVD should be computed
  ...
  uint32_t
      m_maxIterations; //!< Maximum number of iterations to numerically approximate the SVD decomposition
  double m_tolerance; //!< Tolerance to numerically approximate the SVD decomposition
  bool
      m_useCache; //!< Cache the channel matrix whenever possible. NOTE: the SVD decomposition can be extremely computationally expensive, caching is suggested.
};

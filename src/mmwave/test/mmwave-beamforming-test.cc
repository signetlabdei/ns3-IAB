...
MmWaveDftBeamformingTestCase::DoRun (void)
{
  ...
  // create the antenna
  Ptr<PhasedArrayModel> thisAntenna = CreateObjectWithAttributes<UniformPlanarArray> (
      "NumRows", UintegerValue (4), "NumColumns", UintegerValue (4));
  ...
  // create the antenna
  Ptr<PhasedArrayModel> otherAntenna = CreateObjectWithAttributes<UniformPlanarArray> (
      "NumRows", UintegerValue (4), "NumColumns", UintegerValue (4));

  // create the beamforming module
  Ptr<MmWaveDftBeamforming> bfModule = CreateObjectWithAttributes<MmWaveDftBeamforming> (
      "Device", PointerValue (thisDevice), "Antenna", PointerValue (thisAntenna));
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (maxAngle.GetAzimuth (), losAngle.GetAzimuth (), angleStep / 2,
                             "|AF| should be max in the LOS direction");
  NS_TEST_ASSERT_MSG_EQ_TOL (maxAngle.GetInclination (), losAngle.GetInclination (), angleStep / 4,
                             "|AF| should be max in the LOS direction");
  NS_TEST_ASSERT_MSG_EQ_TOL (maxGain, std::sqrt (thisAntenna->GetNumberOfElements ()), 0.05,
...
MmWaveSvdBeamformingTestCase::DoRun (void)
{
  ...
  Ptr<PhasedArrayModel> txAntenna = CreateObjectWithAttributes<UniformPlanarArray> (
      "NumRows", UintegerValue (4), "NumColumns", UintegerValue (4), "AntennaElement",
  ...
  Ptr<PhasedArrayModel> rxAntenna = CreateObjectWithAttributes<UniformPlanarArray> (
      "NumRows", UintegerValue (2), "NumColumns", UintegerValue (2), "AntennaElement",
      PointerValue (CreateObject<IsotropicAntennaModel> ()));
  ...
  Ptr<MmWaveSvdBeamforming> bfModule = CreateObjectWithAttributes<MmWaveSvdBeamforming> (
      "Device", PointerValue (txDevice), "Antenna", PointerValue (txAntenna), "ChannelModel",
      PointerValue (channelModel), "MaxIterations", UintegerValue (100), "Tolerance",
      DoubleValue (1e-50));
  ...
  PhasedArrayModel::ComplexVector manualTxBfVector = GetManualBfVector (
      txAntenna, Angles (DegreesToRadians (aodAz[0]), DegreesToRadians (aodEl[0])));
  PhasedArrayModel::ComplexVector manualRxBfVector = GetManualBfVector (
      rxAntenna, Angles (DegreesToRadians (aoaAz[0]), DegreesToRadians (aoaEl[0])));
...
MmWaveBeamformingTest::MmWaveBeamformingTest () : TestSuite ("mmwave-beamforming-test", UNIT)
{

  ...
  for (auto ccIndex : mmWaveEnbDev->GetCcMap ())
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (phyFreq, splmFreq.Get (),
      ...
      NS_TEST_ASSERT_MSG_EQ (
          ccm, threeGppSplmCcm,
          "The channel condition model associated to the propagation loss model is not the same as "
          "the one associated to the spectrum propagation loss model");
    }
}
...
MmWaveChannelModelInitializationTestSuite::MmWaveChannelModelInitializationTestSuite ()
    : TestSuite ("mmwave-channel-model-initialization-test", TestSuite::Type::UNIT)
{
  // TestDuration for TestCase can be QUICK, EXTENSIVE or TAKES_FOREVER
  AddTestCase (new MmwaveThreeGppChannelInitializationTestCase, TestCase::Duration::QUICK);
}

...
CoreNetworkStatsCalculator::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::CoreNetworkStatsCalculator")
          ...
          .SetGroupName ("Lte")
          .AddAttribute ("X2FileName", "Name of the file where the packet rx on X2 will be logged.",
...
void
CoreNetworkStatsCalculator::LogX2Packet (std::string path, uint16_t sourceCellId,
                                         uint16_t targetCellId, uint32_t size, uint64_t delay,
                                         bool data)
{
...
void
CoreNetworkStatsCalculator::LogMmePacket (std::string path, uint16_t sourceCellId,
                                          uint16_t targetCellId, uint32_t size, uint64_t delay)
{
  NS_LOG_FUNCTION (this << "LogX2Packet" << sourceCellId << targetCellId << size << delay);

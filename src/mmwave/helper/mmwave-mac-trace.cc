  ...
  if (!m_schedAllocTraceFile.is_open ())
    {
      ...
      m_schedAllocTraceFile
          << "frame\tsubF\tslot\tcellId\trnti\tfirstSym\tnumSym\ttype\ttddMode\tretxNum\tccId"
          << std::endl;
    }
  ...
  for (auto iTti : allocInfo.m_ttiAllocInfo)
    {
      ...
      m_schedAllocTraceFile << +dlSfn.m_frameNum << "\t" << +dlSfn.m_sfNum << "\t"
                            << +dlSfn.m_slotNum << "\t" << schedParams.m_cellId << "\t"
                            << +iTti.m_dci.m_rnti << "\t" << +iTti.m_dci.m_symStart << "\t"
                            << +iTti.m_dci.m_numSym << "\t" << iTti.m_ttiType << "\t"
                            << iTti.m_tddMode << "\t" << +iTti.m_dci.m_rv << "\t"
                            << +schedParams.m_ccId << std::endl;
    }
}

      ...
      m_s1apUdpPort (36412)
{
  ...
  retval = mmeS1apSocket->Bind (InetSocketAddress (
      Ipv4Address::GetAny (), m_s1apUdpPort)); // it listens on any IP, port m_s1apUdpPort
  NS_ASSERT (retval == 0);
                                    ...
                                    bool isIabNode)
{
  ...
  // create a point to point link between the new eNB/IAB donor and the SGW with
  // the corresponding new NetDevices on each side
  Ipv4Address sgwAddress, enbAddress;
  Ptr<Socket> enbS1uSocket{nullptr};
  ...
  if (!isIabNode)
    {
      ...
      enbAddress = enbSgwIpIfaces.GetAddress (0);
      sgwAddress = enbSgwIpIfaces.GetAddress (1);

      // create S1-U socket for the ENB
      enbS1uSocket = Socket::CreateSocket (enb, TypeId::LookupByName ("ns3::UdpSocketFactory"));
  ...
  // the corresponding new NetDevices on each side
  // this concerns both eNBs, IAB donor and IAB nodes
  ...
  int retval = enbS1apSocket->Bind (InetSocketAddress (mme_enbAddress, m_s1apUdpPort));
  NS_ASSERT (retval == 0);

  Ptr<Socket> enbLteSocket{nullptr}, enbLteSocket6{nullptr};
  if (!isIabNode)
    {
      // create IPv4 LTE socket for the ENB
      enbLteSocket = Socket::CreateSocket (enb, TypeId::LookupByName ("ns3::PacketSocketFactory"));
      ...
      // create IPv6 LTE socket for the ENB
      enbLteSocket6 = Socket::CreateSocket (enb, TypeId::LookupByName ("ns3::PacketSocketFactory"));
  ...
  NS_LOG_INFO ("create EpcEnbApplication/EpcIabApplication");
  Ptr<Application> duApp;
  if (isIabNode)
    {
      duApp = CreateObject<EpcIabApplication> (cellId);
    }
  else
    {
      duApp = CreateObject<EpcEnbApplication> (enbLteSocket, enbLteSocket6, enbS1uSocket,
                                               enbAddress, sgwAddress, cellId);
    }

  enb->AddApplication (duApp);
  NS_ASSERT (enb->GetNApplications () == 1);
  NS_ASSERT_MSG ((enb->GetApplication (0)->GetObject<EpcEnbApplication> () && (!isIabNode)) ||
                     (enb->GetApplication (0)->GetObject<EpcIabApplication> () && isIabNode),
                 "cannot retrieve EpcEnbApplication/EpcIabApplication");
  ...
  /**
   * For IAB nodes, the following quantities represent:
   * enbAddress: the UE-like IP address assigned to the IAB node
   * sgwAddress: a blank IP address (there is no S1-U interface)
   * while for IAB donors/gNBs the enbAddress represents the IP address
   * of the S1-U interface
   */

  //TODO: retrieve and pass the BAP address
  auto mmwDonorDev = DynamicCast<MmWaveEnbNetDevice> (lteEnbNetDevice);
  auto mmwIabDev = DynamicCast<MmWaveIabNetDevice> (lteEnbNetDevice);
  auto lteEnbDev = DynamicCast<LteEnbNetDevice> (lteEnbNetDevice);
  NS_ASSERT ((!isIabNode && mmwDonorDev) || (isIabNode && mmwIabDev) ||
                                                ...
                                                Ptr<EpcTft> tft, EpsBearer bearer)
{
  ...
  uint32_t bearerId = m_mmeApp->AddBearer (imsi, tft, bearer);
...
Ptr<const EpcSgwPgwApplication>
MmWavePointToPointEpcHelper::GetEpcSgwPgwApplication ()
{
  NS_LOG_FUNCTION (this);
  return m_sgwPgwApp;
}

Ptr<EpcSgwPgwApplication>
MmWavePointToPointEpcHelper::GetNonConstEpcSgwPgwApplication ()
{
  NS_LOG_FUNCTION (this);
  return m_sgwPgwApp;
}

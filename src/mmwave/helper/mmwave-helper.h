...
public:
   ...
   * \param value value to set
   */
  void SetIabPhasedArrayModelAttribute (std::string name, const AttributeValue &value);
   ...
   * \param value value to set
   */
  void SetIabBeamformingCodebookAttribute (std::string name, const AttributeValue &value);
  ...
  /**
   * Attach IAB nodes to its parent IAB node
   */
  void AttachIabTotIabWithIndex (NetDeviceContainer iabDevices, uint32_t indexIabNode,
                                 uint32_t indexIabParent, NetDeviceContainer enbDevices,
                                 uint32_t indexDonor);

  /**
   * Attach IAB nodes to the spcecified IAB donor
   */
  void AttachIabTotDonorWithIndex (NetDeviceContainer iabDevices, NetDeviceContainer enbDevices,
                                   uint32_t index);

  /**
   * Attach IAB nodes to the spcecified IAB donor
   */
  void AttachIabTotDonorWithIndex (Ptr<NetDevice> iabDevice, NetDeviceContainer enbDevices,
                                   uint32_t index);

  /**
   * Attach MC ueDevices to the closest LTE enbDevice, register all MmWave eNBs to the MmWaveUePhy
   */
  void AttachToClosestEnb (NetDeviceContainer ueDevices, NetDeviceContainer mmWaveEnbDevices,
                           NetDeviceContainer lteEnbDevices);

  /**
   * Attach mmWave-only ueDevices to the closest iabDevice
   */
  void AttachToClosestIab (NetDeviceContainer ueDevices, NetDeviceContainer iabDevices);
   ...
   */
  void AttachToEnbWithIndex (Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices,
                             uint32_t index);

  /**
   * Attach to an IAB selecting which one with an index
   * \param ueDevice the ueNetDevice
   * \param iabDevices all the IABs
   * \param index an index to select the IAB (cellId - 1)
   */
  void AttachToIabWithIndex (Ptr<NetDevice> ueDevice, NetDeviceContainer iabDevices,
                             uint32_t index);
   ...
   */
  void AttachIrToClosestEnb (NetDeviceContainer ueDevices, NetDeviceContainer mmWaveEnbDevices,
                             NetDeviceContainer lteEnbDevices);
...
private:
  ...
  void AttachToClosestIab (Ptr<NetDevice> ueDevice, NetDeviceContainer iabDevices);
  void AttachMcToClosestEnb (Ptr<NetDevice> ueDevice, NetDeviceContainer mmWaveEnbDevices,
                             NetDeviceContainer lteEnbDevices);
  ...
  std::string
      m_spectrumPropagationLossModelType; //!< the type of the SpectrumPropagationLossModel to use (if needed)

  ObjectFactory m_enbNetDeviceFactory;
  ObjectFactory m_iabNetDeviceFactory;
  ...
  ObjectFactory
      m_pathlossModelFactory; // Each channel (mmWave, LteUl & LteDl) may have a different pathloss with diff attributes
  ObjectFactory
      m_spectrumPropagationLossModelFactory; //!< the factory for the SpectrumPropagationLossModel objects
  ...
  uint16_t m_bapAddressCounter;
  uint16_t m_bapPathIdCounter;

  Ptr<MmWavePhyTrace> m_phyStats;
  Ptr<MmWaveMacTrace> m_enbStats;
  Ptr<MmWaveBapTrace> m_bapStats;
  ...
  ObjectFactory m_iabPhasedArrayModelFactory; ///Factory of PhasedArrayModel objects for IABs
  ...
  ObjectFactory m_iabBeamformingCodebookFactory; /// Factory of beamforming codebooks for IABs

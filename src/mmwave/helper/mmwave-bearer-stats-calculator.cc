...
MmWaveBearerStatsCalculator::MmWaveBearerStatsCalculator (std::string protocolType)
    : m_firstWrite (true), m_pendingOutput (false), m_aggregatedStats (false)
{
  NS_LOG_FUNCTION (this);
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveBearerStatsCalculator")
          ...
          .AddAttribute ("AggregatedStats",
                         ...
                         MakeBooleanChecker ())
          .AddAttribute ("StartTime", "Start time of the on going epoch.", TimeValue (Seconds (0.)),
                         MakeTimeAccessor (&MmWaveBearerStatsCalculator::SetStartTime,
                                           &MmWaveBearerStatsCalculator::GetStartTime),
                         MakeTimeChecker ())
          .AddAttribute ("EpochDuration", "Epoch duration.", TimeValue (Seconds (0.25)),
          ...
          .AddAttribute (
              ...
              MakeStringAccessor (&LteStatsCalculator::SetDlOutputFilename), MakeStringChecker ())
          .AddAttribute (
              "UlRlcOutputFilename", "Name of the file where the uplink results will be saved.",
              StringValue ("UlRlcStats.txt"),
              MakeStringAccessor (&LteStatsCalculator::SetUlOutputFilename), MakeStringChecker ())
...
void
MmWaveBearerStatsCalculator::UlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid,
                                      uint32_t packetSize)
{
...
void
MmWaveBearerStatsCalculator::DlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid,
                                      uint32_t packetSize)
{
...
void
MmWaveBearerStatsCalculator::UlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid,
...
void
MmWaveBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid,
                                      uint32_t packetSize, uint64_t delay)
{
...
MmWaveBearerStatsCalculator::RescheduleEndEpoch (void)
{
  ...
  m_endEpochEvent = Simulator::Schedule (m_startTime + m_epochDuration,
                                         &MmWaveBearerStatsCalculator::EndEpoch, this);
}

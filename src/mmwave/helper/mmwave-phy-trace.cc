...
MmWavePhyTrace::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::MmWavePhyTrace")
          ...
          .AddAttribute (
              "OutputFilename", "Name of the file where the uplink results will be saved.",
              StringValue ("RxPacketTrace.txt"),
              MakeStringAccessor (&MmWavePhyTrace::SetPhyRxOutputFilename), MakeStringChecker ())
...
MmWavePhyTrace::ReportCurrentCellRsrpSinrCallback (Ptr<MmWavePhyTrace> phyStats, std::string path,
                                                   uint64_t imsi, SpectrumValue &sinr,
                                                   SpectrumValue &power)
{
...
void
MmWavePhyTrace::ReportDownLinkTBSize (Ptr<MmWavePhyTrace> phyStats, std::string path, uint64_t imsi,
                                      uint64_t tbSize)
{
  ...
  if (!m_ulPhyTraceFile.is_open ())
    {
      ...
      m_ulPhyTraceFile << "frame\tsubF\tslot\trnti\tfirstSym\tnumSym\ttype\ttddMode\tretxNum\tccId"
                       << std::endl;
    }

  // Trace the UL PHY transmission info
  m_ulPhyTraceFile << +param.m_frameNum << "\t" << +param.m_sfNum << "\t" << +param.m_slotNum
                   << "\t" << +param.m_rnti << "\t" << +param.m_symStart << "\t" << +param.m_numSym
                   << "\t" << +param.m_ttiType << "\t" << +param.m_tddMode << "\t" << +param.m_rv
                   << "\t" << +param.m_ccId << std::endl;
}
  ...
  if (!m_dlPhyTraceFile.is_open ())
    {
      ...
      m_dlPhyTraceFile << "frame\tsubF\tslot\trnti\tfirstSym\tnumSym\ttype\ttddMode\tretxNum\tccId"
                       << std::endl;
    }
...
void
MmWavePhyTrace::RxPacketTraceUeCallback (Ptr<MmWavePhyTrace> phyStats, std::string path,
...
void
MmWavePhyTrace::RxPacketTraceEnbCallback (Ptr<MmWavePhyTrace> phyStats, std::string path,
                                          RxPacketTraceParams params)
{
  if (!m_rxPacketTraceFile.is_open ())
    {

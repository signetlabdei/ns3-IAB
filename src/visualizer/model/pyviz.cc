...
PyVizPacketTag::PyVizPacketTag () : Tag ()
{
}
...
void
PyViz::DoPause (std::string const &message)
{
...
void
PyViz::Pause (std::string const &message)
{
...
PyViz::SimulatorRunUntil (Time time)
{
  ...
  m_runUntil = time;
  m_stop = false;
  Simulator::ScheduleWithContext (Simulator::NO_CONTEXT, time - Simulator::Now (),
                                  &PyViz::CallbackStopSimulation, this);
...
bool
PyViz::TransmissionSampleKey::operator<(PyViz::TransmissionSampleKey const &other) const
{
...
PyViz::TraceIpv4Drop (std::string context, ns3::Ipv4Header const &hdr, Ptr<const Packet> packet,
                      ns3::Ipv4L3Protocol::DropReason reason, Ptr<Ipv4> dummy_ipv4,
                      uint32_t interface)
{
...
void
PyViz::TraceNetDevRxCommon (std::string const &context, Ptr<const Packet> packet,
                            Mac48Address const &from)
{
  uint32_t uid;
  PyVizPacketTag tag;
  ...
  if (g_log.IsEnabled (ns3::LOG_DEBUG))
    {
      for (std::map<TransmissionSampleKey, TransmissionSampleValue>::const_iterator iter =
           ...
           iter != m_transmissionSamples.end (); iter++)
        {
...
void
PyViz::TraceNetDevTxWimax (std::string context, Ptr<const Packet> packet,
...
void
PyViz::TraceNetDevRxWimax (std::string context, Ptr<const Packet> packet,
...
void
PyViz::TraceNetDevTxLte (std::string context, Ptr<const Packet> packet,
                         Mac48Address const &destination)
{
  NS_LOG_FUNCTION (context);
  TraceNetDevTxCommon (context, packet, destination);
...
public:
   ...
   */
  FastClipping (Vector2 clipMin, Vector2 clipMax) : m_clipMin (clipMin), m_clipMax (clipMax)
  {
  }
                     ...
                     double &lineX1, double &lineY1, double &lineX2, double &lineY2)
{
  ...
  FastClipping::Line line = {
      {lineX1, lineY1}, {lineX2, lineY2}, (lineX2 - lineX1), (lineY2 - lineY1)};

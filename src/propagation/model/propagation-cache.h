...
public:
   ...
   */
  Ptr<T>
  GetPathData (Ptr<const MobilityModel> a, Ptr<const MobilityModel> b, uint32_t modelUid)
  {
   ...
   */
  void
  AddPathData (Ptr<T> data, Ptr<const MobilityModel> a, Ptr<const MobilityModel> b,
               uint32_t modelUid)
  {
    PropagationPathIdentifier key = PropagationPathIdentifier (a, b, modelUid);
...
private:
  ...
  struct PropagationPathIdentifier
  {
     ...
     * @param modelUid model UID
     */
    PropagationPathIdentifier (Ptr<const MobilityModel> a, Ptr<const MobilityModel> b,
                               uint32_t modelUid)
     ...
     */
    bool
    operator<(const PropagationPathIdentifier &other) const
    {

...
V2vUrbanProbChCondModelTestCase::DoRun (void)
{
  ...
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0396 * 10.0) *
                              exp (-(log (10.0) - 5.2718) * (log (10.0) - 5.2718) / 3.4827)));
  ...
  testVector.m_typeId = ProbabilisticV2vUrbanChannelConditionModel::GetTypeId ();
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0396 * 100.0) *
  ...
  testVector.m_typeId = ProbabilisticV2vUrbanChannelConditionModel::GetTypeId ();
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0312 * 10.0) *
                              exp (-(log (10.0) - 5.0063) * (log (10.0) - 5.0063) / 2.4544)));
  ...
  testVector.m_typeId = ProbabilisticV2vUrbanChannelConditionModel::GetTypeId ();
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0312 * 100.0) *
  ...
  testVector.m_typeId = ProbabilisticV2vUrbanChannelConditionModel::GetTypeId ();
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0242 * 10.0) *
                              exp (-(log (10.0) - 5.0115) * (log (10.0) - 5.0115) / 2.2092)));
  ...
  testVector.m_typeId = ProbabilisticV2vUrbanChannelConditionModel::GetTypeId ();
  testVector.m_pNlosv = std::min (
      1.0, std::max (0.0, 1 / (0.0242 * 100.0) *
  ...
  for (uint32_t i = 0; i < m_testVectors.GetN (); ++i)
    {
      ...
      for (uint32_t j = 0; j < numberOfReps; j++)
        {
          Simulator::Schedule (MilliSeconds (10 * j),
                               &V2vHighwayProbChCondModelTestCase::EvaluateChannelCondition, this,
                               a, b);
        }
...
{
  AddTestCase (
      new V2vUrbanProbChCondModelTestCase,
      TestCase::
          QUICK); // test for a fully probabilistic model (NLOS vs LOS vs NLOSv), in V2V urban scenario
  AddTestCase (
      new V2vHighwayProbChCondModelTestCase,
      TestCase::
          QUICK); // test for a fully probabilistic model (NLOS vs LOS vs NLOSv), in V2V highway scenario*/
}

  ...
  for (uint32_t i = 0; i < m_testVectors.GetN (); i++)
    {
      ...
      NS_TEST_EXPECT_MSG_EQ_TOL (lossModel->CalcRxPower (testVector.m_pt, a, b), testVector.m_pr,
                                 m_tolerance, "Got unexpected rcv power");
    }
...
private:
   ...
   */
  void RunTest (uint16_t testNum, std::string propagationLossModelType, double hBs, double hUt,
                double distance, bool shadowingEnabled);
  ...
  typedef struct
  {
    ...
    double
        m_shadowingStdLos; //!< the standard deviation of the shadowing component in the LOS case in dB
    double
        m_shadowingStdNlos; //!< the standard deviation of the shadowing component in the NLOS case in dB
  } TestVector;
...
void
ThreeGppShadowingTestCase::EvaluateLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b,
                                         uint8_t testNum)
{
...
void
ThreeGppShadowingTestCase::RunTest (uint16_t testNum, std::string propagationLossModelType,
                                    double hBs, double hUt, double distance, bool shadowingEnabled)
{
  ...
  Ptr<ChannelConditionModel> nlosCondModel = CreateObject<NeverLosChannelConditionModel> ();
  Simulator::Schedule (Seconds (99.5), &ThreeGppShadowingTestCase::ChangeChannelCondition, this,
                       nlosCondModel);
  ...
  for (int i = 0; i < 200; i++)
    {
      if (i % 2 == 0)
        Simulator::Schedule (MilliSeconds (1000 * i), &ThreeGppShadowingTestCase::EvaluateLoss,
                             this, a, b, testNum);
      else
        Simulator::Schedule (MilliSeconds (1000 * i), &ThreeGppShadowingTestCase::EvaluateLoss,
                             this, b, a, testNum);
  ...
  for (uint16_t tvIndex = 0; tvIndex < m_testVectors.GetN (); tvIndex++)
    {
      ...
      for (uint16_t sampleIndex = 0; sampleIndex < numSamples; sampleIndex++)
        {
          RunTest (sampleIndex, tv.m_propagationLossModelType, tv.m_hBs, tv.m_hUt, tv.m_distance,
                   true);
        }

      // analyze the results
      std::vector<double>
          mean_vector; // the vector containing the mean propagation loss for each position (sample mean)
      ...
      for (uint16_t i = 0; i < mean_vector.size () / 2; i++)
        {
          ...
          NS_TEST_EXPECT_MSG_EQ_TOL (
              z, 0.0, 1.96, "Null hypothesis test (LOS case) for the shadowing component rejected");
        }

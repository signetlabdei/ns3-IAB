      ...
      if (fragmentOffset == 0)
          ...
          if (protocol == UdpL4Protocol::PROT_NUMBER && payloadSize >= 8)
            {
              ...
              if (!isLastFragment)
                {
                  std::tuple<uint32_t, uint32_t, uint8_t, uint16_t> fragmentKey = std::make_tuple (
                      ipv4Header.GetSource ().Get (), ipv4Header.GetDestination ().Get (), protocol,
      ...
      else
        {
          ...
          std::tuple<uint32_t, uint32_t, uint8_t, uint16_t> fragmentKey =
              std::make_tuple (ipv4Header.GetSource ().Get (), ipv4Header.GetDestination ().Get (),
                               protocol, ipv4Header.GetIdentification ());
      ...
      for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++it)
        {
          ...
          if (tft->Matches (direction, remoteAddressIpv4, localAddressIpv4, remotePort, localPort,
                            tos))
            {
      ...
      for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++it)
        {
          ...
          NS_LOG_LOGIC (" Ptr<EpcTft>: " << it->second);
          Ptr<EpcTft> tft = it->second;
          if (tft->Matches (direction, remoteAddressIpv6, localAddressIpv6, remotePort, localPort,
                            tos))
            {
              NS_LOG_LOGIC ("matches with TFT ID = " << it->first);
              return it->first; // the id of the matching TFT

...
LteFfrDistributedAlgorithm::LteFfrDistributedAlgorithm () : m_ffrSapUser (0), m_ffrRrcSapUser (0)
{
  ...
  static TypeId tid =
      TypeId ("ns3::LteFfrDistributedAlgorithm")
          ...
          .AddAttribute ("EdgePowerOffset",
                         ...
                         MakeUintegerChecker<uint8_t> ())
          .AddAttribute ("EdgeRbNum", "Number of RB that can be used in edge Sub-band",
                         UintegerValue (8),
  ...
  if (it != m_rntp.end ())
  ...
  else
    {
      m_rntp.insert (std::pair<uint16_t, std::vector<bool>> (
          neighborCellId, params.cellInformationList[0].relativeNarrowbandTxBand.rntpPerPrbList));
    }
}

void
LteFfrDistributedAlgorithm::UpdateNeighbourMeasurements (uint16_t rnti, uint16_t cellId,
                                                         uint8_t rsrp, uint8_t rsrq)
{

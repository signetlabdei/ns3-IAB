...
UeMemberLteUeCmacSapUser::UeMemberLteUeCmacSapUser (LteUeRrc *rrc) : m_rrc (rrc)
{
}
  ...
  static TypeId tid =
      TypeId ("ns3::LteUeRrc")
          ...
          .AddAttribute ("Srb1", "SignalingRadioBearerInfo for SRB1", PointerValue (),
                         ...
                         MakePointerChecker<LteSignalingRadioBearerInfo> ())
          .AddAttribute ("CellId", "Serving cell identifier",
          ...
          .AddAttribute ("T300",
                         ...
                         TimeValue (MilliSeconds (100)), MakeTimeAccessor (&LteUeRrc::m_t300),
                         MakeTimeChecker ())
          .AddTraceSource ("MibReceived", "trace fired upon reception of Master Information Block",
          ...
          .AddTraceSource ("StateTransition", "trace fired upon every UE RRC state transition",
          ...
          .AddTraceSource ("HandoverEndOk",
                           ...
                           "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddTraceSource ("HandoverEndError", "trace fired upon failure of a handover procedure",
                           MakeTraceSourceAccessor (&LteUeRrc::m_handoverEndErrorTrace),
                           "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddTraceSource ("SCarrierConfigured", "trace fired after configuring secondary carriers",
                           MakeTraceSourceAccessor (&LteUeRrc::m_sCarrierConfiguredTrace),
                           "ns3::LteUeRrc::SCellConfiguredTracedCallback")
          .AddTraceSource ("SwitchToLte", "trace fired upon switching to LTE RAT",
                           MakeTraceSourceAccessor (&LteUeRrc::m_switchToLteTrace),
                           "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddTraceSource ("SwitchToMmWave", "trace fired upon switching to MmWave RAT",
                           MakeTraceSourceAccessor (&LteUeRrc::m_switchToMmWaveTrace),
                           "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
          .AddAttribute ("SecondaryRRC",
                         "True if this is the RRC in charge of the secondary cell (MmWaveCell) for "
                         "a MC device",
                         BooleanValue (false), MakeBooleanAccessor (&LteUeRrc::m_isSecondaryRRC),
                         MakeBooleanChecker ())
          .AddAttribute ("InterRatHoCapable",
                         "True if this RRC supports hard handover between LTE and MmWave",
                         BooleanValue (false), MakeBooleanAccessor (&LteUeRrc::m_interRatHoCapable),
                         MakeBooleanChecker ());
  return tid;
}
...
void
LteUeRrc::DoSendData (Ptr<Packet> packet, uint32_t bid)
{
  NS_LOG_FUNCTION (this << packet);

  uint32_t drbid = Bid2Drbid (bid);

  if (drbid != 0)
    {
      std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator it = m_drbMap.find (drbid);
      ...
      if (params.m_componentCarrierId != 0)
        {
          ...
          SaveScellUeMeasurements (newMeasIt->m_cellId, newMeasIt->m_rsrp, newMeasIt->m_rsrq,
                                   useLayer3Filtering, params.m_componentCarrierId);
        }
      else
        {
          SaveUeMeasurements (newMeasIt->m_cellId, newMeasIt->m_rsrp, newMeasIt->m_rsrq,
                              useLayer3Filtering);
        }
    }
      ...
      switch (m_state)
        ...
        case CONNECTED_REESTABLISHING:
          ...
          rc.numberOfRaPreambles =
              msg.sib2.radioResourceConfigCommon.rachConfigCommon.preambleInfo.numberOfRaPreambles;
          ...
          if (m_state == CONNECTED_HANDOVER && m_interRatHoCapable && !m_ncRaStarted)
            {
              ...
              m_cmacSapProvider.at (0)->StartNonContentionBasedRandomAccessProcedure (
                  m_rnti, m_rachConfigDedicated.raPreambleIndex,
      ...
      if (msg.haveMobilityControlInfo)
        {
          ...
          if (m_interRatHoCapable)
            ...
            {
              m_cmacSapProvider.at (0)->StartNonContentionBasedRandomAccessProcedure (
                  m_rnti, mci.rachConfigDedicated.raPreambleIndex,
          ...
          if (!m_isSecondaryRRC)
          ...
          else
            {
              ...
              m_asSapUser->NotifySecondaryCellHandoverStarted (oldRnti, m_rnti, m_cellId,
...
LteUeRrc::DoRecvRrcConnectionSwitch (LteRrcSap::RrcConnectionSwitch msg)
{
  ...
  std::vector<uint32_t> drbidList = msg.drbidList;
  for (auto iter = drbidList.begin (); iter != drbidList.end (); ++iter)
    {
                  ...
                  if (rlcTypeId != LteRlcSm::GetTypeId ())
                    {
                      ...
                      if (m_bap)
                        {
                          rlc->SetLteRlcSapUser (m_bap->GetLteRlcSapUser ());
                        }
                      else
                        {
                  ...
                  if (m_rlcMap.find (*iter) != m_rlcMap.end ())
                    {
                      ...
                      CopyRlcBuffers (m_rlcMap.find (*iter)->second->m_rlc, pdcp,
                                      m_rlcMap.find (*iter)->second->logicalChannelIdentity);
                    }
                  ...
                  for (uint32_t i = 0; i < m_numberOfComponentCarriers; i++)
                    {
                      m_mmWaveCmacSapProvider.at (i)->AddLc (
                          m_rlcMap.find (*iter)->second->logicalChannelIdentity, lcConfig,
                          rlc->GetLteMacSapUser ());
                    }
  ...
  if (rlc->GetObject<LteRlcAm> ())
    {
      ...
      if (rlcAm->GetTransmittingRlcSduBufferSize () > 0)
        { //something inside the RLC AM's transmitting buffer
          ...
          m_rlcBufferToBeForwarded.insert (m_rlcBufferToBeForwarded.end (), txonBuffer.begin (),
                                           txonBuffer.end ());
...
void
LteUeRrc::DoNotifySecondaryCellHandover (uint16_t oldRnti, uint16_t newRnti, uint16_t mmWaveCellId,
  ...
  for (dtamIt = rrcd.drbToAddModList.begin (); dtamIt != rrcd.drbToAddModList.end (); ++dtamIt)
    {
      ...
      std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator drbMapIt =
          ...
          if (pdcp)
            {
              ...
              for (uint32_t i = 0; i < m_numberOfMmWaveComponentCarriers; i++)
                {
                  m_mmWaveCmacSapProvider.at (i)->AddLc (dtamIt->logicalChannelIdentity, lcConfig,
                                                         rlc->GetLteMacSapUser ());
                }
              if (rlcTypeId != LteRlcSm::GetTypeId ())
                {
    ...
    LteRrcSap::NonCriticalExtensionConfiguration nonCec)
{
  ...
  for (std::list<LteRrcSap::SCellToAddMod>::iterator it = nonCec.sCellsToAddModList.begin ();
       it != nonCec.sCellsToAddModList.end (); it++)
    {
      ...
      LteRrcSap::PdschConfigDedicated pdschConfigDedicated =
          scell.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell.pdschConfigDedicated;
  ...
  for (dtamIt = rrcd.drbToAddModList.begin (); dtamIt != rrcd.drbToAddModList.end (); ++dtamIt)
    {
      NS_LOG_INFO (this << " IMSI " << m_imsi << " adding/modifying DRBID "
                        << +(dtamIt->drbIdentity) << " EPS BID " << +(dtamIt->epsBearerIdentity)
                        << " LC " << +(dtamIt->logicalChannelIdentity));
      NS_ASSERT_MSG (dtamIt->logicalChannelIdentity > 2,
                     "LCID value " << dtamIt->logicalChannelIdentity << " is reserved for SRBs");

      std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator drbMapIt = m_drbMap.begin ();
      ...
      if (drbMapIt == m_drbMap.end ())
        {
          ...
          drbInfo->m_rlc = rlc;
          drbInfo->m_epsBearerIdentity = dtamIt->drbIdentity;
          ...
          if (!m_bapAddressFromCellIdCallback.IsNull ())
            {
              // retrieve the BAP address of the other endpoint of the bearer, i.e.,
              // the DU providing connectivity to this MT
              uint16_t enbBapAddress = m_bapAddressFromCellIdCallback (m_cellId);

              // store the corresponding RLC SAP provider in the BAP
              m_bap->AddPairRlcMap (enbBapAddress, rlc->GetLteRlcSapProvider ());
              NS_LOG_DEBUG ("Stored MT endpoint of DRB: " << dtamIt->drbIdentity 
                            << " at device of IMSI: " << m_imsi 
                            << " and attached to cell ID: " << m_cellId 
                            << " as " << rlc->GetLteRlcSapProvider ());
            }

          m_bid2DrbidMap[dtamIt->drbIdentity] = dtamIt->drbIdentity;
          ...
          m_drbMap.insert (
              std::pair<uint32_t, Ptr<LteDataRadioBearerInfo>> (dtamIt->drbIdentity, drbInfo));
          ...
          std::vector<LteUeCcmRrcSapProvider::LcsConfig> lcOnCcMapping =
              m_ccmRrcSapProvider->AddLc (dtamIt->logicalChannelIdentity, lcConfig,
                                          rlc->GetLteMacSapUser ());
          ...
          for (itLcOnCcMapping = lcOnCcMapping.begin (); itLcOnCcMapping != lcOnCcMapping.end ();
               ++itLcOnCcMapping)
            {
              ...
              m_cmacSapProvider.at (index)->AddLc (dtamIt->logicalChannelIdentity, lcConfigFromCcm,
                                                   msu);
            }
        ...
        {
          NS_LOG_DEBUG ("request to modify existing DRBID");
          Ptr<LteDataRadioBearerInfo> drbInfo = drbMapIt->second;
          NS_LOG_INFO ("Is MC " << dtamIt->is_mc);
  ...
  for (dtdmIt = rrcd.drbToReleaseList.begin (); dtdmIt != rrcd.drbToReleaseList.end (); ++dtdmIt)
    {
      ...
      for (uint32_t i = 0; i < m_numberOfComponentCarriers; i++)
        {
          m_cmacSapProvider.at (i)->RemoveLc (drbid % 11 + 2);
        }
    }
}
...
LteUeRrc::ApplyMeasConfig (LteRrcSap::MeasConfig mc)
{
  ...
  for (std::list<uint8_t>::iterator it = mc.measObjectToRemoveList.begin ();
       it != mc.measObjectToRemoveList.end (); ++it)
    {
  ...
  for (std::list<LteRrcSap::MeasObjectToAddMod>::iterator it = mc.measObjectToAddModList.begin ();
       it != mc.measObjectToAddModList.end (); ++it)
    {
  ...
  for (std::list<uint8_t>::iterator it = mc.reportConfigToRemoveList.begin ();
       it != mc.reportConfigToRemoveList.end (); ++it)
    {
  ...
  for (std::list<LteRrcSap::ReportConfigToAddMod>::iterator it =
       ...
       it != mc.reportConfigToAddModList.end (); ++it)
    {
      // simplifying assumptions
  ...
  for (std::list<uint8_t>::iterator it = mc.measIdToRemoveList.begin ();
       it != mc.measIdToRemoveList.end (); ++it)
    {
      uint8_t measId = *it;
      NS_LOG_LOGIC (this << " deleting measId " << (uint32_t) measId);
  ...
  for (std::list<LteRrcSap::MeasIdToAddMod>::iterator it = mc.measIdToAddModList.begin ();
       it != mc.measIdToAddModList.end (); ++it)
    {
...
LteUeRrc::SaveUeMeasurements (uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering)
{
  ...
  NS_LOG_DEBUG (this << " IMSI " << m_imsi << " state " << ToString (m_state) << ", measured cell "
                     << m_cellId << ", new RSRP " << rsrp << " stored " << storedMeasIt->second.rsrp
      ...
      if (reportConfigEutra.timeToTrigger == 0)
      ...
      else
        {
          ...
          t.timer = Simulator::Schedule (MilliSeconds (reportConfigEutra.timeToTrigger),
                                         &LteUeRrc::VarMeasReportListAdd, this, measId,
                                         concernedCellsEntry);
  ...
  if (eventLeavingCondApplicable)
    {
      ...
      if (reportConfigEutra.timeToTrigger == 0)
      ...
      else
        {
          ...
          t.timer = Simulator::Schedule (MilliSeconds (reportConfigEutra.timeToTrigger),
                                         &LteUeRrc::VarMeasReportListErase, this, measId,
                                         concernedCellsLeaving, reportOnLeave);
...
LteUeRrc::VarMeasReportListAdd (uint8_t measId, ConcernedCells_t enteringCells)
{
  ...
  measReportIt->second.periodicReportTimer = Simulator::Schedule (
      UE_MEASUREMENT_REPORT_DELAY, &LteUeRrc::SendMeasurementReport, this, measId);
...
LteUeRrc::LeaveConnectedMode ()
{
  ...
  std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator it;
...
uint32_t
LteUeRrc::Bid2Drbid (uint32_t bid)
{
  std::map<uint32_t, uint32_t>::iterator it = m_bid2DrbidMap.find (bid);
...
void
LteUeRrc::SetBapAddressFromCellIdCallback (Callback<uint16_t, uint16_t> cb)
{
  NS_LOG_FUNCTION (this);

  NS_ASSERT (!cb.IsNull ());

  m_bapAddressFromCellIdCallback = cb;
}

void
LteUeRrc::SetBapEnb (Ptr<Bap> bap)
{
  NS_LOG_FUNCTION (this);

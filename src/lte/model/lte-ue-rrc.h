...
public:
  ...
  void SetBapAddressFromCellIdCallback (Callback<uint16_t, uint16_t> cb);

  void SetBapEnb (Ptr<Bap> bap);
   ...
   * \param [in] otherCid
   */
  typedef void (*MibSibHandoverTracedCallback) (uint64_t imsi, uint16_t cellId, uint16_t rnti,
                                                uint16_t otherCid);
...
private:
   ...
   * \param bid the BID
   */
  void DoSendData (Ptr<Packet> packet, uint32_t bid);
  /// Disconnect function
  void DoDisconnect ();
  void DoNotifySecondaryCellConnected (uint16_t mmWaveRnti, uint16_t mmWaveCellId);
  void DoNotifySecondaryCellHandover (uint16_t oldRnti, uint16_t newRnti, uint16_t mmWaveCellId,
  ...
  void SaveScellUeMeasurements (uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering,
  ...
  std::map<uint32_t, uint32_t> m_bid2DrbidMap; ///< bid to DR bid map
...
public:
  ...
  Callback<uint16_t, uint16_t> m_bapAddressFromCellIdCallback{
      MakeNullCallback<uint16_t, uint16_t> ()};

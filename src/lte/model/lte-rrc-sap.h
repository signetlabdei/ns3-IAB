...
public:
  ...
  {
    /// the config action
    enum action { SETUP, RESET } type; ///< action type
   ...
   */
  static double
  ConvertPdschConfigDedicated2Double (PdschConfigDedicated pdschConfigDedicated)
  {
    double pa = 0;
  ...
  struct DrbToAddMod
  {
    uint32_t epsBearerIdentity; ///< EPS bearer identity
    uint32_t drbIdentity; ///< DRB identity
  ...
  struct RadioResourceConfigDedicated
  {
    ...
    std::list<uint32_t> drbToReleaseList; ///< DRB to release list
  ...
  struct ReportConfigEutra
  {
    ...
    /// the report purpose
    enum report { REPORT_STRONGEST_CELLS, REPORT_CGI } purpose; ///< purpose
  ...
  struct MeasGapConfig
  {
    ...
    enum action { SETUP, RESET } type; ///< action type
    /// the gap offest
    enum gap { GP0, GP1 } gapOffsetChoice; ///< gap offset
  ...
  struct PhysicalConfigDedicatedSCell
  {
    ...
    bool
        crossCarrierSchedulingConfig; ///< currently implemented as boolean variable --> implementing crossCarrierScheduling is out of the scope of this GSoC proposal
    bool havePdschConfigDedicated; ///< have PDSCH config dedicated?
    PdschConfigDedicated pdschConfigDedicated; ///< PDSCH config dedicated
...
public:
   ...
   */
  virtual void
  SendRrcConnectionReconfigurationCompleted (RrcConnectionReconfigurationCompleted msg) = 0;
   ...
   */
  virtual void
  SendRrcConnectionReestablishmentRequest (RrcConnectionReestablishmentRequest msg) = 0;
   ...
   */
  virtual void
  SendRrcConnectionReestablishmentComplete (RrcConnectionReestablishmentComplete msg) = 0;
...
public:
   ...
   */
  virtual void
  RecvRrcConnectionReconfigurationCompleted (uint16_t rnti,
   ...
   */
  virtual void
  RecvRrcConnectionReestablishmentRequest (uint16_t rnti,
   ...
   */
  virtual void
  RecvRrcConnectionReestablishmentComplete (uint16_t rnti,
  ...
  virtual void RecvRrcSecondaryCellInitialAccessSuccessful (uint16_t rnti, uint16_t mmWaveRnti,
...
public:
  ...
  virtual void
  SendRrcConnectionReconfigurationCompleted (RrcConnectionReconfigurationCompleted msg);
...
template <class C>
MemberLteUeRrcSapProvider<C>::MemberLteUeRrcSapProvider (C *owner) : m_owner (owner)
{
}
...
template <class C>
MemberLteEnbRrcSapProvider<C>::MemberLteEnbRrcSapProvider (C *owner) : m_owner (owner)
{
}
...
MemberLteEnbRrcSapProvider<C>::RecvRrcSecondaryCellInitialAccessSuccessful (uint16_t rnti,
                                                                            ...
                                                                            uint16_t mmWaveCellId)
{
  Simulator::ScheduleNow (&C::DoRecvRrcSecondaryCellInitialAccessSuccessful, m_owner, rnti,
                          mmWaveRnti, mmWaveCellId);
}

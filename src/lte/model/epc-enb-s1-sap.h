...
public:
   ...
   */
  virtual void DoSendReleaseIndication (uint64_t imsi, uint16_t rnti, uint32_t bearerId) = 0;

  struct BearerToBeSwitched
  {
    uint32_t epsBearerId;
    uint32_t teid;
  };
...
public:
  ...
  struct DataRadioBearerSetupRequestParameters
  {
    ...
    EpsBearer bearer; /**< the characteristics of the bearer to be set
                         up */
    uint32_t bearerId; /**< the EPS Bearer Identifier */
...
public:
  ...
  virtual void DoSendReleaseIndication (uint64_t imsi, uint16_t rnti, uint32_t bearerId);
...
template <class C>
MemberEpcEnbS1SapProvider<C>::MemberEpcEnbS1SapProvider (C *owner) : m_owner (owner)
{
}
...
template <class C>
void
MemberEpcEnbS1SapProvider<C>::DoSendReleaseIndication (uint64_t imsi, uint16_t rnti,
                                                       uint32_t bearerId)
{
...
template <class C>
MemberEpcEnbS1SapUser<C>::MemberEpcEnbS1SapUser (C *owner) : m_owner (owner)
{
}

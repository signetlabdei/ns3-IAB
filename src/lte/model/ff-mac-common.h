...
struct DlDciListElement_s
{
  ...
  /// Ngap enum
  enum Ngap_e { GAP1, GAP2, NotValid_Ngap } m_nGap{NotValid_Ngap}; ///< the gap
...
struct LogicalChannelConfigListElement_s
{
  ...
  /// Direction enum
  enum Direction_e { DIR_UL, DIR_DL, DIR_BOTH, NotValid } m_direction{NotValid}; ///< the direction
...
struct PhichListElement_s
{
  uint16_t m_rnti{UINT16_MAX}; ///< RNTI
  /// Phich enum
  enum Phich_e { ACK, NACK, NotValid } m_phich{NotValid}; ///< the phich
};
...
{
  /// Type enum
  enum Type_e { BCCH, PCCH, NotValid } m_type{NotValid}; ///< the type
...
struct UlInfoListElement_s
{
  ...
  enum ReceptionStatus_e { Ok, NotOk, NotValid } m_receptionStatus{NotValid}; ///< the status
  uint8_t m_tpc{UINT8_MAX}; ///< Tx power control command
};
...
struct MacCeListElement_s
{
  uint16_t m_rnti{UINT16_MAX}; ///< RNTI
  /// MAC CE type enum
  enum MacCeType_e { BSR, PHR, CRNTI, NotValid } m_macCeType{NotValid}; ///< MAC CE type
...
struct UlCqi_s
{
  ...
  /// Type_e enumeration
  enum Type_e { SRS, PUSCH, PUCCH_1, PUCCH_2, PRACH, NotValid } m_type{NotValid}; ///< type
};

  ...
  static TypeId tid =
      TypeId ("ns3::LteRlcAm")
          ...
          .AddAttribute ("ReorderingTimer",
                         ...
                         MakeTimeAccessor (&LteRlcAm::m_reorderingTimerValue), MakeTimeChecker ())
          ...
          .AddAttribute ("ReportBufferStatusTimer",
                         ...
                         TimeValue (MilliSeconds (20)),
                         MakeTimeAccessor (&LteRlcAm::m_rbsTimerValue), MakeTimeChecker ())
          ...
          .AddAttribute ("EnableAQM", "Enable active queue management (CoDel)",
                         BooleanValue (false), MakeBooleanAccessor (&LteRlcAm::m_enableAqm),
                         MakeBooleanChecker ())
  ...
  if (txOpParams.bytes < 4)
    {
      ...
      NS_ASSERT_MSG (false, "TxOpportunity (size = "
      ...
      for (sn = m_vtA; sn < m_vtS; sn++)
        {
          ...
          if (found == true)
            {
              if ((packet->GetSize () <= txOpParams.bytes) || m_txOpportunityForRetxAlwaysBigEnough)
                {
              ...
              if (m_sdusAssembleBuffer.size () == 1)
                {
                  ...
                  m_reassembleExpectedSeqNumber =
                      currSeqNumber -
                      1; //With loss + SI_SF: Jump here until sdusBuffer is larger than 1, meaning a new SDU begins.
                }
              else
                {
                  m_assemblingState = WAITING_SI_SF;
...
LteRlcAm::CreateRlcSduBuffer ()
{
  ...
  for (std::vector<Ptr<Packet>>::iterator it = m_transmittingRlcSdus.begin ();
       it != m_transmittingRlcSdus.end (); ++it)
    {
...
LteRlcAm::DoReceivePdu (LteMacSapUser::ReceivePduParameters rxPduParams)
{
  ...
  if (rlcAmHeader.IsDataPdu ())
    {
      NS_LOG_INFO (this << " RLC DoReceivePdu from RNTI " << m_rnti << " lcid " << (uint32_t) m_lcid
                        << " packet size " << rxPduParams.p->GetSize () << " time "
                        << Simulator::Now ().GetSeconds ());
              ...
              if (m_txedBuffer.at (seqNumberValue).m_pdu)
                {
                  ...
                  m_txCompletedCallback (m_rnti, m_lcid,
          ...
          switch (framingInfo)
            ...
            case (LteRlcAmHeader::FIRST_BYTE | LteRlcAmHeader::LAST_BYTE):
              ...
              for (it = m_sdusBuffer.begin (); it != m_sdusBuffer.end (); it++)
                {
                  NS_LOG_LOGIC ("Triggering PDCP for PDU");
            ...
            case (LteRlcAmHeader::FIRST_BYTE | LteRlcAmHeader::NO_LAST_BYTE):
              ...
              while (m_sdusBuffer.size () > 1)
                {
                  NS_LOG_LOGIC ("Triggering PDCP for full PDU");

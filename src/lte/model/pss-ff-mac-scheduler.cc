  ...
  static TypeId tid =
      TypeId ("ns3::PssFfMacScheduler")
          ...
          .AddAttribute (
              ...
              UintegerValue (1000), MakeUintegerAccessor (&PssFfMacScheduler::m_cqiTimersThreshold),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("PssFdSchedulerType", "FD scheduler in PSS (default value is PFsch)",
                         StringValue ("PFsch"),
                         MakeStringAccessor (&PssFfMacScheduler::m_fdSchedulerType),
                         MakeStringChecker ())
          .AddAttribute ("nMux", "The number of UE selected by TD scheduler (default value is 0)",
                         UintegerValue (0), MakeUintegerAccessor (&PssFfMacScheduler::m_nMux),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("HarqEnabled", "Activate/Deactivate the HARQ [by default is active].",
                         BooleanValue (true), MakeBooleanAccessor (&PssFfMacScheduler::m_harqOn),
                         MakeBooleanChecker ())
          .AddAttribute ("UlGrantMcs", "The MCS of the UL grant, must be [0..15] (default 0)",
                         UintegerValue (0), MakeUintegerAccessor (&PssFfMacScheduler::m_ulGrantMcs),
          ...
          if (m_nMux > 0)
          ...
          else
            {
              ...
              if (ueSet1.size () + ueSet2.size () <= 2)
                ...
                nMux =
                    (int) ((ueSet1.size () + ueSet2.size ()) /
                           2); // TD scheduler only transfers half selected UE per RTT to TD scheduler
            }
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it--)
            {

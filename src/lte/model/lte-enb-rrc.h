...
public:
   ...
   *
   */
  void SetupDataRadioBearer (EpsBearer bearer, uint32_t bearerId, uint32_t gtpTeid,
   ...
   */
  void ReleaseDataRadioBearer (uint32_t drbid);
   ...
   * \param p the packet
   */
  void SendData (uint32_t bid, Ptr<Packet> p);
   ...
   */
  void RecvRlcSetupCompleted (uint32_t drbid);
   ...
   */
  void
  RecvRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg);
  /**
   ...
   */
  void
  RecvRrcConnectionReestablishmentComplete (LteRrcSap::RrcConnectionReestablishmentComplete msg);
  /**
   ...
   * \param [in] newState
   */
  typedef void (*StateTracedCallback) (const uint64_t imsi, const uint16_t cellId,
  ...
  /**
     ...
     */
  void RecvConnectionSwitchToMmWave (bool useMmWaveConnection, uint32_t drbid);
...
private:
   ...
   */
  void ForwardRlcBuffers (Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp, uint32_t gtpTeid,
                          bool mcLteToMmWaveForwarding, bool mcMmToMmWaveForwarding, uint32_t bid);
   ...
   */
  uint32_t AddDataRadioBearerInfo (Ptr<LteDataRadioBearerInfo> radioBearerInfo);
   ...
   */
  Ptr<LteDataRadioBearerInfo> GetDataRadioBearerInfo (uint32_t drbid);
   ...
   * \param drbid the Data Radio Bearer id
   */
  void RemoveDataRadioBearerInfo (uint32_t drbid);
   ...
   */
  uint32_t Lcid2Drbid (uint8_t lcid);
   ...
   */
  uint8_t Drbid2Lcid (uint32_t drbid);
   ...
   */
  uint32_t Lcid2Bid (uint8_t lcid);
   ...
   * \return the corresponding EPS Bearer Identifier
   */
  uint32_t Drbid2Bid (uint32_t drbid);
   ...
   * \return the corresponding Data Radio Bearer Id
   */
  uint32_t Bid2Drbid (uint32_t bid);
  ...
  uint32_t m_lastAllocatedDrbid; ///< last allocated Data Radio Bearer ID

  /**
   * The `DataRadioBearerMap` attribute. List of UE DataRadioBearerInfo by
   * DRBID.
   */
  std::map<uint32_t, Ptr<LteDataRadioBearerInfo>> m_drbMap;

  /**
   * Map the drb into a RLC (used for remote independent RLC in an MC setup)
   */
  std::map<uint32_t, Ptr<RlcBearerInfo>> m_rlcMap;
  /**
  ...
  bool
      m_pendingConnectToMmWave; ///< this attribute is set to true when RecvRrcConnectionSetupCompleted
  ...
  std::list<uint32_t> m_drbsToBeStarted;
...
public:
   ...
   */
  void DoSendReleaseDataRadioBearer (uint64_t imsi, uint16_t rnti, uint32_t bearerId);
   ...
   * \param [in] rnti
   */
  typedef void (*ConnectionHandoverTracedCallback) (const uint64_t imsi, const uint16_t cellId,
                                                    const uint16_t rnti);
   ...
   * \param [in] targetCid
   */
  typedef void (*HandoverStartTracedCallback) (const uint64_t imsi, const uint16_t cellId,
                                               const uint16_t rnti, const uint16_t targetCid);
   ...
   */
  typedef void (*ReceiveReportTracedCallback) (const uint64_t imsi, const uint16_t cellId,
                                               const uint16_t rnti,
                                               const LteRrcSap::MeasurementReport report);

  typedef void (*NotifyMmWaveSinrTracedCallback) (uint64_t imsi, uint16_t cellId, long double sinr);

  /**
    * Different secondary cell handover modes
    */
  enum HandoverMode { FIXED_TTT = 1, DYNAMIC_TTT = 2, THRESHOLD = 3 };
...
private:
   ...
   */
  void
  DoRecvRrcConnectionReestablishmentRequest (uint16_t rnti,
   ...
   */
  void
  DoRecvRrcConnectionReestablishmentComplete (uint16_t rnti,
  ...
  void DoRecvRrcSecondaryCellInitialAccessSuccessful (uint16_t rnti, uint16_t mmWaveRnti,
   ...
   */
  void
  DoDataRadioBearerSetupRequest (EpcEnbS1SapUser::DataRadioBearerSetupRequestParameters params);
  /**
   ...
   */
  void
  DoPathSwitchRequestAcknowledge (EpcEnbS1SapUser::PathSwitchRequestAcknowledgeParameters params);
...
private:
   ...
   * @params the value of the SINR for this cell
   */
  void TttBasedHandover (std::map<uint64_t, CellSinrMap>::iterator imsiIter, double sinrDifference,
                         uint16_t maxSinrCellId, double maxSinrDb);
  ...
  bool
      m_reportAllUeMeas; // if true, the MmWave eNB reports to the coordinator all the received UE measures, i.e. one per CC
  ...
  Callback<std::optional<uint16_t>, uint64_t> m_bapAddressFromImsiCallback{
      MakeNullCallback<std::optional<uint16_t>, uint64_t> ()};

  Callback<Ipv4Address, uint64_t> m_donorIpAddressCallback{
      MakeNullCallback<Ipv4Address, uint64_t> ()};

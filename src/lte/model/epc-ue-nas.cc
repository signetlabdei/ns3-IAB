...
EpcUeNas::EpcUeNas () : m_state (OFF), m_csgId (0), m_asSapProvider (0), m_bidCounter (0)
{
...
EpcUeNas::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::EpcUeNas")
          ...
          .AddTraceSource ("StateTransition", "fired upon every UE NAS state transition",
...
EpcUeNas::Send (Ptr<Packet> packet, uint16_t protocolNumber)
{
  ...
  switch (m_state)
    {
      case ACTIVE: {
        uint32_t bid = m_tftClassifier.Classify (packet, EpcTft::UPLINK, protocolNumber);
        if (bid == 0)
          {
            return false;
          }
  ...
  if (m_mmWaveAsSapProvider != 0)
    {
      ...
      m_mmWaveAsSapProvider->ForceCampedOnEnb (
          mmWaveCellId, m_dlEarfcn); // TODO probably the second argument is useless
...
void
EpcUeNas::DoNotifySecondaryCellHandoverStarted (uint16_t oldRnti, uint16_t newRnti,
...
EpcUeNas::DoActivateEpsBearer (EpsBearer bearer, Ptr<EpcTft> tft)
{
  ...
  uint32_t bid = m_imsi * 11 + (++m_bidCounter);
  ...
  switch (m_state)
    ...
    case ACTIVE:
      for (std::list<BearerToBeActivated>::iterator it = m_bearersToBeActivatedList.begin ();
           it != m_bearersToBeActivatedList.end (); m_bearersToBeActivatedList.erase (it++))
        {

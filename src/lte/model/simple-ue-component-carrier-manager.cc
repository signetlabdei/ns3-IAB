...
SimpleUeCcmMacSapUser::SimpleUeCcmMacSapUser (SimpleUeComponentCarrierManager *mac) : m_mac (mac)
{
}
    ...
    LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  for (std::map<uint8_t, LteMacSapProvider *>::iterator it = m_macSapProvidersMap.begin ();
       it != m_macSapProvidersMap.end (); it++)
    {
      if (it->first == 0)
        {
  ...
  for (uint8_t ncc = 0; ncc < m_noOfComponentCarriers; ncc++)
    {
      ...
      if (ccLcMapIt != m_componentCarrierLcMap.end ())
      ...
      else
        {
          ...
          std::pair<std::map<uint8_t, std::map<uint8_t, LteMacSapProvider *>>::iterator, bool> ret =
              m_componentCarrierLcMap.insert (

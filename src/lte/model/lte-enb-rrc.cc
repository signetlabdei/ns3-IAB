...
UeManager::DoDispose ()
{
  ...
  for (std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator it = m_drbMap.begin ();
       it != m_drbMap.end (); ++it)
    {
...
void
UeManager::SetupDataRadioBearer (EpsBearer bearer, uint32_t bearerId, uint32_t gtpTeid,
                                 Ipv4Address transportLayerAddress)
{
  NS_LOG_FUNCTION (this << (uint32_t) m_rnti);

  Ptr<LteDataRadioBearerInfo> drbInfo = CreateObject<LteDataRadioBearerInfo> ();
  uint32_t drbid = AddDataRadioBearerInfo (drbInfo);
  uint8_t lcid = Drbid2Lcid (drbid);
  uint32_t bid = Drbid2Bid (drbid);
  NS_ASSERT_MSG (bearerId == 0 || bid == bearerId,
                 "bearer ID mismatch ("
                     << +bid << " != " << +bearerId
                     << ", the assumption that ID are"
                        "allocated in the same way by MME and RRC is not valid any more for imsi "
                     << m_imsi);
  ...
  if (rlcTypeId != LteRlcSm::GetTypeId ())
    {
      ...
      Ptr<InnerPdcp> innerPdcp = CreateObject<InnerPdcp> ();
      ...
      if (m_rrc->m_bap != nullptr // Skip this for LTE devices
          ...
          && !m_rrc->m_bapAddressFromImsiCallback.IsNull ()) // Skip for standalone root IAB DU
        {
          ...
          uint64_t imsiUeOrMt = m_rrc->m_rntiImsiMap.find (m_rnti)->second;
          auto bapAddressMt = m_rrc->m_bapAddressFromImsiCallback (imsiUeOrMt);

          if (m_rrc->m_bap->IsDonor ())
            {
              NS_LOG_FUNCTION ("A device is connecting to the donor");
              if (bapAddressMt) // an IAB node is connecting to the donor
                {
                  NS_LOG_FUNCTION ("An IAB node is connecting to this donor");
                  rlc->SetLteRlcSapUser (m_rrc->m_bap->GetLteRlcSapUser ());
                  m_rrc->m_bap->SetInnerPdcp (innerPdcp);
                  innerPdcp->SetPdcpSapProvider (
                      pdcp->GetLteRlcSapUser ()); // interface to connect the InnerPDCP to the PDCP
                  drbInfo->m_innerPdcp = innerPdcp;
                  drbInfo->m_pdcp = pdcp;
                }
              else // a UE is connecting to the donor
                {
                  NS_LOG_FUNCTION ("A UE is connecting to the donor");
                  rlc->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ());
                  drbInfo->m_pdcp = pdcp;
                }
            }
            ...
            {
              NS_LOG_FUNCTION ("A device is connecting to the IAB node");
              if (bapAddressMt) // an IAB node is connecting to this IAB node
                {
                  NS_LOG_FUNCTION ("An IAB node is connecting to this IAB node");

                  //TODO: check if this is enough
                  rlc->SetLteRlcSapUser (m_rrc->m_bap->GetLteRlcSapUser ());
                }
              else // a UE is connecting to this IAB node
                {
                  NS_LOG_FUNCTION ("A UE is connecting to this IAB node");
                  Ipv4Address ipDonor = m_rrc->m_donorIpAddressCallback (imsiUeOrMt);
                  innerPdcp->SetDonorIpAddress (ipDonor);
                  innerPdcp->SetBap (m_rrc->m_bap);
                  rlc->SetLteRlcSapUser (innerPdcp->GetLteRlcSapUser ());
                  drbInfo->m_innerPdcp = innerPdcp;
                }
            }
        }
  ...
  if (m_rrc->m_bap != nullptr // Skip this for LTE devices
      ...
      && !m_rrc->m_bapAddressFromImsiCallback.IsNull ()) // Skip for standalone root IAB DU
    {
      ...
      auto bapAddressMt = m_rrc->m_bapAddressFromImsiCallback (imsiUe);

      if (bapAddressMt.has_value ()) // Neglect if IMSI not found. Should happen only for actual UEs
        {
          NS_LOG_DEBUG ("Stored DU endpoint of DRB: " << bearerId 
                         << " towards BAP address: " << *bapAddressMt
                         << " as " << rlc->GetLteRlcSapProvider ());
          m_rrc->m_bap->AddPairRlcMap (*bapAddressMt, rlc->GetLteRlcSapProvider ());
        }
    }
  ...
  for (itLcOnCcMapping = lcOnCcMapping.begin (); itLcOnCcMapping != lcOnCcMapping.end ();
       ++itLcOnCcMapping)
    {
      NS_LOG_DEBUG (this << " RNTI " << itLcOnCcMapping->lc.rnti << "Lcid "
                         << +itLcOnCcMapping->lc.lcId << " lcGroup " << +itLcOnCcMapping->lc.lcGroup
...
{
  NS_LOG_FUNCTION (this << (uint32_t) m_rnti);
  for (std::list<uint32_t>::iterator drbIdIt = m_drbsToBeStarted.begin ();
       drbIdIt != m_drbsToBeStarted.end (); ++drbIdIt)
    {
      std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator drbIt = m_drbMap.find (*drbIdIt);
...
void
UeManager::ReleaseDataRadioBearer (uint32_t drbid)
{
...
void
LteEnbRrc::DoSendReleaseDataRadioBearer (uint64_t imsi, uint16_t rnti, uint32_t bearerId)
{
...
UeManager::RecvHandoverRequestAck (EpcX2SapUser::HandoverRequestAckParams params)
{
  ...
  for (std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator drbIt = m_drbMap.begin ();
  ...
  if (m_rrc->m_ismmWave && m_isMc)
    {
      ...
      for (std::map<uint32_t, Ptr<RlcBearerInfo>>::iterator rlcIt = m_rlcMap.begin ();
...
void
UeManager::SendData (uint32_t bid, Ptr<Packet> p)
{
  ...
  if (m_isMc)
    {
      ...
      uint32_t bid = Drbid2Bid (params.drbid);
...
void
UeManager::RecvRlcSetupCompleted (uint32_t drbid)
{
...
UeManager::SendRrcConnectionSwitch (bool useMmWaveConnection)
{
  ...
  std::vector<uint32_t> drbidVector;
  for (std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator it = m_drbMap.begin ();
       it != m_drbMap.end (); ++it)
    {
      if (it->second->m_isMc)
...
void
UeManager::RecvConnectionSwitchToMmWave (bool useMmWaveConnection, uint32_t drbid)
{
...
{
  NS_LOG_FUNCTION (this);
  uint32_t bearerId = m_imsi * 11 + ++m_lastAllocatedDrbid;

  NS_ASSERT (m_drbMap.find (bearerId) == m_drbMap.end ());

  m_drbMap.insert (std::pair<uint32_t, Ptr<LteDataRadioBearerInfo>> (bearerId, drbInfo));
  drbInfo->m_drbIdentity = bearerId;
  return bearerId;
}

Ptr<LteDataRadioBearerInfo>
UeManager::GetDataRadioBearerInfo (uint32_t drbid)
{
...
void
UeManager::RemoveDataRadioBearerInfo (uint32_t drbid)
{
  NS_LOG_FUNCTION (this << (uint32_t) drbid);
  std::map<uint32_t, Ptr<LteDataRadioBearerInfo>>::iterator it = m_drbMap.find (drbid);
  NS_ASSERT_MSG (it != m_drbMap.end (),
...
UeManager::BuildRadioResourceConfigDedicated ()
{
  ...
  if (m_drbMap.size () == 0 && m_isMc &&
    ...
    {
      for (std::map<uint32_t, Ptr<RlcBearerInfo>>::iterator it = m_rlcMap.begin ();
           it != m_rlcMap.end (); ++it)
        {
          LteRrcSap::DrbToAddMod dtam;
...
{
  NS_ASSERT (lcid > 2);
  return lcid - 2 + m_imsi * 11;
}

uint8_t
UeManager::Drbid2Lcid (uint32_t drbid)
{
  return drbid % 11 + 2;
}
uint32_t
...
{
  return Lcid2Drbid (lcid);
}

uint8_t
UeManager::Bid2Lcid (uint32_t bid)
{
  return Drbid2Lcid (bid);
}

uint32_t
UeManager::Drbid2Bid (uint32_t drbid)
{
  return drbid;
}

uint32_t
UeManager::Bid2Drbid (uint32_t bid)
{
  return bid;
}
...
LteEnbRrc::SetBapAddressFromImsiCallback (Callback<std::optional<uint16_t>, uint64_t> cb)
{
  ...
  m_bapAddressFromImsiCallback = cb;
}

void
LteEnbRrc::SetDonorIpAddressCallback (Callback<Ipv4Address, uint64_t> cb)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (!cb.IsNull ());

  m_donorIpAddressCallback = cb;
}

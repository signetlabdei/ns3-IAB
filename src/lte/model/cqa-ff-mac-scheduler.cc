  ...
  static TypeId tid =
      TypeId ("ns3::CqaFfMacScheduler")
          ...
          .AddAttribute (
              ...
              UintegerValue (1000), MakeUintegerAccessor (&CqaFfMacScheduler::m_cqiTimersThreshold),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("CqaMetric", "CqaFfMacScheduler metric type that can be: CqaFf, CqaPf",
                         StringValue ("CqaFf"),
                         MakeStringAccessor (&CqaFfMacScheduler::m_CqaMetric), MakeStringChecker ())
          .AddAttribute ("HarqEnabled", "Activate/Deactivate the HARQ [by default is active].",
                         BooleanValue (true), MakeBooleanAccessor (&CqaFfMacScheduler::m_harqOn),
                         MakeBooleanChecker ())
          .AddAttribute ("UlGrantMcs", "The MCS of the UL grant, must be [0..15] (default 0)",
                         UintegerValue (0), MakeUintegerAccessor (&CqaFfMacScheduler::m_ulGrantMcs),
    ...
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters &params)
{
  ...
  for (itLogicalChannels = m_ueLogicalChannelsConfigList.begin ();
       itLogicalChannels != m_ueLogicalChannelsConfigList.end (); itLogicalChannels++)
    {
  ...
  for (std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator itrbr =
           m_rlcBufferReq.begin ();
      ...
      if (itGBRgroups != map_GBRHOLgroupToUE.end ())
        {
          ...
          map_nonGBRHOLgroupToUE
              .end ()) // if there are no more flows with retransmission queue start to scheduler flows with transmission queue
        {
          ...
          for (std::set<LteFlowId_t>::iterator it = itCurrentGroup->second.begin ();
               it != itCurrentGroup->second.end (); it++)
            {
              ...
              int tbSize =
                  ...
                  8; // similar to calculation of TB size (size of TB in bytes according to table 7.1.7.2.1-1 of 36.213)
    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters &params)
{
  ...
  if (rbPerFlow < 3)
    {
      rbPerFlow =
          3; // at least 3 rbg per flow (till available resource) to ensure TxOpportunity >= 7 bytes
    }
  int rbAllocated = 0;

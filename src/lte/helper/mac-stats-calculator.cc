...
MacStatsCalculator::MacStatsCalculator () : m_dlFirstWrite (true), m_ulFirstWrite (true)
{
  ...
  static TypeId tid =
      TypeId ("ns3::MacStatsCalculator")
          ...
          .AddAttribute (
              ...
              MakeStringAccessor (&MacStatsCalculator::SetDlOutputFilename), MakeStringChecker ())
          .AddAttribute (
              "UlOutputFilename", "Name of the file where the uplink results will be saved.",
              StringValue ("UlMacStats.txt"),
              MakeStringAccessor (&MacStatsCalculator::SetUlOutputFilename), MakeStringChecker ());
  return tid;
}
...
void
MacStatsCalculator::DlScheduling (uint16_t cellId, uint64_t imsi,
...
void
MacStatsCalculator::DlSchedulingCallback (Ptr<MacStatsCalculator> macStats, std::string path,
                                          DlSchedulingCallbackInfo dlSchedulingCallbackInfo)
{

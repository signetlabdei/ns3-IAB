  ...
  static TypeId tid =
      TypeId ("ns3::MacTxStatsCalculator")
          ...
          .AddAttribute ("DlMacTxFilename",
                         ...
                         MakeStringChecker ())
          .AddAttribute (
              "UlMacTxFilename", "Name of the file where the MAC uplink tx results will be saved.",
              StringValue ("UlMacTx.txt"),
              MakeStringAccessor (&MacTxStatsCalculator::m_retxUlFilename), MakeStringChecker ());
  return tid;
}
...
void
MacTxStatsCalculator::RegisterMacTxDl (uint16_t rnti, uint16_t cellId, uint32_t packetSize,
                                       uint8_t numRetx)
{
...
void
MacTxStatsCalculator::RegisterMacTxUl (uint16_t rnti, uint16_t cellId, uint32_t packetSize,
                                       uint8_t numRetx)
{

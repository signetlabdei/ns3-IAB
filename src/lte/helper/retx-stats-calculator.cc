  ...
  static TypeId tid =
      TypeId ("ns3::RetxStatsCalculator")
          ...
          .AddAttribute ("DlRlcRetxFilename",
                         ...
                         MakeStringChecker ())
          .AddAttribute (
              "UlRlcRetxFilename", "Name of the file where the uplink retx results will be saved.",
              StringValue ("UlRlcRetx.txt"),
              MakeStringAccessor (&RetxStatsCalculator::m_retxUlFilename), MakeStringChecker ());
  return tid;
}
...
void
RetxStatsCalculator::RegisterRetxDl (uint64_t imsi, uint16_t cellId, uint16_t rnti, uint8_t lcid,
...
void
RetxStatsCalculator::RegisterRetxUl (uint64_t imsi, uint16_t cellId, uint16_t rnti, uint8_t lcid,
                                     uint32_t packetSize, uint32_t numRetx)
{

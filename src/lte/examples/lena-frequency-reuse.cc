          ...
          if (enbdev)
            {
              ...
              outFile << "set label \"" << enbdev->GetCellId () << "\" at " << pos.x << "," << pos.y
                      << " left font \"Helvetica,4\" textcolor rgb \"white\" front  point pt 2 ps "
                         "0.3 lc rgb \"white\" offset 0,0"
                      << std::endl;
            }
        }
    }
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("generateSpectrumTrace", "if true, will generate a Spectrum Analyzer trace",
  ...
  if (frAlgorithmType == "ns3::LteFrHardAlgorithm")
  ...
  else if (frAlgorithmType == "ns3::LteFfrSoftAlgorithm")
    {
      ...
      lteHelper->SetFfrAlgorithmAttribute (
          "MediumAreaPowerOffset", UintegerValue (LteRrcSap::PdschConfigDedicated::dB_1dot77));

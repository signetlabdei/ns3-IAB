...
public:
  static bool
      s_writeResults; ///< write results flag, determines whether to write results to outoput files
   ...
   */
  CarrierAggregationTestCase (uint16_t nUser, uint16_t dist, uint32_t dlbandwidth,
                              uint32_t ulBandwidth, uint32_t numberOfComponentCarriers);
   ...
   */
  void UlScheduling (uint32_t frameNo, uint32_t subframeNo, uint16_t rnti, uint8_t mcs,
...
private:
   ...
   */
  static std::string BuildNameString (uint16_t nUser, uint16_t dist, uint32_t dlBandwidth,
                                      uint32_t ulBandwidth, uint32_t numberOfComponentCarriers);

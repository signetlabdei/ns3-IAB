...
LteEpcE2eDataTestCase::DoRun ()
{
  ...
  enbMobility.SetPositionAllocator ("ns3::GridPositionAllocator", "MinX", DoubleValue (0.0), "MinY",
                                    DoubleValue (0.0), "DeltaX", DoubleValue (10000.0), "DeltaY",
                                    DoubleValue (10000.0), "GridWidth", UintegerValue (3),
  ...
  for (std::vector<EnbTestData>::iterator enbit = m_enbTestData.begin ();
       enbit < m_enbTestData.end (); ++enbit, ++enbLteDevIt)
    {
      ...
      ueMobility.SetPositionAllocator ("ns3::UniformDiscPositionAllocator", "X",
                                       DoubleValue (enbPosition.x), "Y",
                                       DoubleValue (enbPosition.y), "rho", DoubleValue (100.0));
          ...
          for (uint32_t b = 0; b < ueit->bearers.size (); ++b)
            {
              ...
              NS_TEST_ASSERT_MSG_EQ (txPktsPdcpDl, expectedPkts,
                                     "wrong TX PDCP packets in downlink for IMSI="
                                         << imsi << " LCID=" << (uint16_t) lcid);

              NS_TEST_ASSERT_MSG_EQ (rxPktsPdcpDl, expectedPkts,
                                     "wrong RX PDCP packets in downlink for IMSI="
                                         << imsi << " LCID=" << (uint16_t) lcid);
              NS_TEST_ASSERT_MSG_EQ (txPktsPdcpUl, expectedPkts,
                                     "wrong TX PDCP packets in uplink for IMSI="
                                         << imsi << " LCID=" << (uint16_t) lcid);
              NS_TEST_ASSERT_MSG_EQ (rxPktsPdcpUl, expectedPkts,
                                     "wrong RX PDCP packets in uplink for IMSI="
                                         << imsi << " LCID=" << (uint16_t) lcid);

              NS_TEST_ASSERT_MSG_EQ (rxBytesDl, expectedBytes,
                                     "wrong total received bytes in downlink");
              NS_TEST_ASSERT_MSG_EQ (rxBytesUl, expectedBytes,
...
LteEpcE2eDataTestSuite::LteEpcE2eDataTestSuite () : TestSuite ("lte-epc-e2e-data", TestSuite::Type::SYSTEM)
{
  ...
  v7.push_back (e7);
  AddTestCase (new LteEpcE2eDataTestCase ("1 eNB, 1UE with 2 bearers", v7), TestCase::Duration::EXTENSIVE);
  ...
  v9.push_back (e9);
  AddTestCase (new LteEpcE2eDataTestCase ("1 eNB, 1UE with aggregation", v9), TestCase::Duration::EXTENSIVE);
}

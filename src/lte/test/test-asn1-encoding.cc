...
public:
   ...
   */
  static std::string
  sprintPacketContentsHex (Ptr<Packet> pkt)
  {
   ...
   */
  static std::string
  sprintPacketContentsBin (Ptr<Packet> pkt)
  {
    uint32_t psize = pkt->GetSize ();
    uint8_t buffer[psize];
   ...
   */
  static void
  LogPacketContents (Ptr<Packet> pkt)
  {
  ...
  template <class T>
  static void
  LogPacketInfo (T source, std::string s)
  {
...
RrcHeaderTestCase::CreateRadioResourceConfigDedicated ()
{
  ...
  rrd.drbToReleaseList = std::list<uint32_t> (4, 2);
...
RrcHeaderTestCase::AssertEqualRadioResourceConfigDedicated (
    LteRrcSap::RadioResourceConfigDedicated rrcd1, LteRrcSap::RadioResourceConfigDedicated rrcd2)
{
  NS_TEST_ASSERT_MSG_EQ (rrcd1.srbToAddModList.size (), rrcd2.srbToAddModList.size (),
  ...
  for (; it1 != srcSrbToAddModList.end (); it1++, it2++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (it1->logicalChannelConfig.priority, it2->logicalChannelConfig.priority,
  ...
  NS_TEST_ASSERT_MSG_EQ (rrcd1.drbToAddModList.size (), rrcd2.drbToAddModList.size (),
  ...
  for (; it3 != srcDrbToAddModList.end (); it3++, it4++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (it3->logicalChannelIdentity, it4->logicalChannelIdentity,
                             "logicalChannelIdentity");
      NS_TEST_ASSERT_MSG_EQ (it3->epsBearerIdentity, it4->epsBearerIdentity, "epsBearerIdentity");

      NS_TEST_ASSERT_MSG_EQ (it3->logicalChannelConfig.priority, it4->logicalChannelConfig.priority,
                             "logicalChannelConfig.priority");
  ...
  NS_TEST_ASSERT_MSG_EQ (rrcd1.drbToReleaseList.size (), rrcd2.drbToReleaseList.size (),
                         "DrbToReleaseList different sizes");

  std::list<uint32_t> srcDrbToReleaseList = rrcd1.drbToReleaseList;
  std::list<uint32_t> dstDrbToReleaseList = rrcd2.drbToReleaseList;
  std::list<uint32_t>::iterator it5 = srcDrbToReleaseList.begin ();
  std::list<uint32_t>::iterator it6 = dstDrbToReleaseList.begin ();
  ...
  NS_TEST_ASSERT_MSG_EQ (rrcd1.havePhysicalConfigDedicated, rrcd2.havePhysicalConfigDedicated,
...
RrcConnectionSetupTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (source.GetRrcTransactionIdentifier (),
                         destination.GetRrcTransactionIdentifier (), "RrcTransactionIdentifier");
...
RrcConnectionReconfigurationTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (source.GetRrcTransactionIdentifier (),
                         destination.GetRrcTransactionIdentifier (), "RrcTransactionIdentifier");
  NS_TEST_ASSERT_MSG_EQ (source.GetHaveMeasConfig (), destination.GetHaveMeasConfig (),
                         "GetHaveMeasConfig");
  NS_TEST_ASSERT_MSG_EQ (source.GetHaveMobilityControlInfo (),
                         destination.GetHaveMobilityControlInfo (), "GetHaveMobilityControlInfo");
...
HandoverPreparationInfoTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (source.GetAsConfig ().sourceUeIdentity,
                         destination.GetAsConfig ().sourceUeIdentity, "sourceUeIdentity");
  ...
  NS_TEST_ASSERT_MSG_EQ (source.GetAsConfig ().sourceDlCarrierFreq,
                         destination.GetAsConfig ().sourceDlCarrierFreq, "sourceDlCarrierFreq");
...
RrcConnectionReestablishmentRequestTestCase::DoRun (void)
{
  ...
  // Check that the destination and source headers contain the same values
  NS_TEST_ASSERT_MSG_EQ (source.GetUeIdentity ().cRnti, destination.GetUeIdentity ().cRnti,
                         "cRnti");
  NS_TEST_ASSERT_MSG_EQ (source.GetUeIdentity ().physCellId,
                         destination.GetUeIdentity ().physCellId, "physCellId");
  NS_TEST_ASSERT_MSG_EQ (source.GetReestablishmentCause (), destination.GetReestablishmentCause (),
...
RrcConnectionRejectTestCase::DoRun (void)
{
  ...
  // Check that the destination and source headers contain the same values
  NS_TEST_ASSERT_MSG_EQ (source.GetMessage ().waitTime, destination.GetMessage ().waitTime,
                         "Different waitTime!");
...
MeasurementReportTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (srcMeas.haveMeasResultNeighCells, dstMeas.haveMeasResultNeighCells,
      ...
      for (; itsrc != srcMeas.measResultListEutra.end (); itsrc++, itdst++)
        {
          ...
          if (itsrc->haveCgiInfo)
            {
              NS_TEST_ASSERT_MSG_EQ (itsrc->cgiInfo.plmnIdentity, itdst->cgiInfo.plmnIdentity,
                                     "Different cgiInfo.plmnIdentity!");
              NS_TEST_ASSERT_MSG_EQ (itsrc->cgiInfo.cellIdentity, itdst->cgiInfo.cellIdentity,
          ...
          NS_TEST_ASSERT_MSG_EQ (itsrc->haveRsrpResult, itdst->haveRsrpResult,
          ...
          NS_TEST_ASSERT_MSG_EQ (itsrc->haveRsrqResult, itdst->haveRsrqResult,
...
Asn1EncodingSuite::Asn1EncodingSuite () : TestSuite ("test-asn1-encoding", TestSuite::Type::UNIT)
{

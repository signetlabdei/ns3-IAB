 ...
 * TestCase Data
 */
LteInterferenceHardFrTestCase::LteInterferenceHardFrTestCase (std::string name, double d1,
...
LteInterferenceStrictFrTestCase::LteInterferenceStrictFrTestCase (
    std::string name, double d1, double d2, double commonDlSinr, double commonUlSinr,
    double edgeDlSinr, double edgeUlSinr, uint32_t rspqThreshold)
    : TestCase ("Test: " + name),
      m_d1 (d1),
      m_d2 (d2),
  ...
  for (uint32_t i = 0; i < 12; i++)
    {
      double dlSinr1 = dlSinr1Catcher.GetValue ()->operator[] (i);
      double dlSinr1Db = 10.0 * std::log10 (dlSinr1);
      NS_TEST_ASSERT_MSG_EQ_TOL (dlSinr1Db, m_commonDlSinrDb, 0.01,
                                 "Wrong SINR in DL! (eNB1 --> UE1)");

      double dlSinr2 = dlSinr2Catcher.GetValue ()->operator[] (i);
      double dlSinr2Db = 10.0 * std::log10 (dlSinr2);
      NS_TEST_ASSERT_MSG_EQ_TOL (dlSinr2Db, m_commonDlSinrDb, 0.01,
                                 "Wrong SINR in DL! (eNB2 --> UE2)");
    }
  ...
  for (uint32_t i = 12; i < 18; i++)
    {
      double dlSinr1 = dlSinr1Catcher.GetValue ()->operator[] (i);
      double dlSinr1Db = 10.0 * std::log10 (dlSinr1);
      NS_TEST_ASSERT_MSG_EQ_TOL (dlSinr1Db, m_edgeDlSinrDb, 0.01,
                                 "Wrong SINR in DL! (eNB1 --> UE1)");

...
LteSpectrumModelTestCase::LteSpectrumModelTestCase (const char *str, uint16_t earfcn, uint8_t bw,
...
LteSpectrumModelTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_SPECTRUM_MODEL_EQ_TOL ((*m_actual), (*m_expected), 0.0000001,
...
public:
   ...
   */
  LteNoisePsdTestCase (const char *str, uint16_t earfcn, uint8_t bw, double noiseFigureDb,
...
LteNoisePsdTestCase::LteNoisePsdTestCase (const char *str, uint16_t earfcn, uint8_t bw,
                                          double noiseFigureDb, SpectrumValue &expected)
    : TestCase (str),
      m_actual (
          LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (earfcn, bw, noiseFigureDb)),
...
{
  NS_TEST_ASSERT_MSG_EQ (m_actual->GetSpectrumModelUid (), m_expected->GetSpectrumModelUid (),
                         "SpectrumModel UID mismatch");
  NS_TEST_ASSERT_MSG_SPECTRUM_VALUE_EQ_TOL ((*m_actual), (*m_expected), 0.0000001,
...
public:
   ...
   * \param expected executed spectrum value
   */
  LteTxPsdTestCase (const char *str, uint16_t earfcn, uint8_t bw, double txPowerDbm,
                    std::vector<int> activeRbs, SpectrumValue &expected);
...
LteTxPsdTestCase::LteTxPsdTestCase (const char *str, uint16_t earfcn, uint8_t bw, double txPowerDbm,
                                    std::vector<int> activeRbs, SpectrumValue &expected)
    : TestCase (str),
      m_actual (

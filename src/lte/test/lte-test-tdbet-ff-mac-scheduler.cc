...
LenaTestTdBetFfMacSchedulerSuite::LenaTestTdBetFfMacSchedulerSuite ()
    : TestSuite ("lte-tdbet-ff-mac-scheduler", TestSuite::Type::SYSTEM)
{
  ...
  AddTestCase (new LenaTdBetFfMacSchedulerTestCase1 (1, 100000, 0, 0, errorModel), TestCase::Duration::QUICK);
...
LenaTdBetFfMacSchedulerTestCase1::LenaTdBetFfMacSchedulerTestCase1 (uint16_t nUser, double dist,
                                                                    double thrRefDl,
                                                                    double thrRefUl,
...
LenaTdBetFfMacSchedulerTestCase1::DoRun (void)
{
  ...
  */
  for (int i = 0; i < m_nUser; i++)
    {
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) dlDataRxed.at (i) / statsDuration, m_thrRefDl,
                                 m_thrRefDl * tolerance, " Unfair Throughput!");
    }
  ...
  */
  for (int i = 0; i < m_nUser; i++)
    {
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) ulDataRxed.at (i) / statsDuration, m_thrRefUl,
                                 m_thrRefUl * tolerance, " Unfair Throughput!");
    }
  Simulator::Destroy ();
}
...
LenaTdBetFfMacSchedulerTestCase2::LenaTdBetFfMacSchedulerTestCase2 (
    std::vector<double> dist, std::vector<uint32_t> estAchievableRateDl,
    std::vector<uint32_t> estThrTdBetUl, bool errorModelEnabled)
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) dlDataRxed.at (i) / statsDuration, estUeThr,
                                 estUeThr * tolerance, " Unfair Throughput!");
    }
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL (
          (double) ulDataRxed.at (i) / statsDuration, (double) m_estThrTdBetUl.at (i),
          (double) m_estThrTdBetUl.at (i) * tolerance, " Unfair Throughput!");
    }
  Simulator::Destroy ();
}

...
LenaTestPhyErrorModelSuite::LenaTestPhyErrorModelSuite ()
    : TestSuite ("lte-phy-error-model", TestSuite::Type::SYSTEM)
{
  ...
  for (uint32_t rngRun = 1; rngRun <= 3; ++rngRun)
    {
      ...
      AddTestCase (new LenaDlCtrlPhyErrorModelTestCase (3, 1040, 0.045, 21, Seconds (0.04), rngRun),
                   (rngRun == 1) ? TestCase::Duration::EXTENSIVE : TestCase::Duration::TAKES_FOREVER);
...
LenaDataPhyErrorModelTestCase::LenaDataPhyErrorModelTestCase (uint16_t nUser, uint16_t dist,
                                                              double blerRef,
...
LenaDlCtrlPhyErrorModelTestCase::LenaDlCtrlPhyErrorModelTestCase (uint16_t nEnb, uint16_t dist,
                                                                  double blerRef,
                                                                  uint16_t toleranceRxPackets,
                                                                  Time statsStartTime,
                                                                  uint32_t rngRun)

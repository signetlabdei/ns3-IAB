...
LenaTestHarqSuite::LenaTestHarqSuite () : TestSuite ("lte-harq", TestSuite::Type::SYSTEM)
{
...
LenaHarqTestCase::LenaHarqTestCase (uint16_t nUser, uint16_t dist, uint16_t tbSize, double amcBer,
                                    double thrRef)
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL (txed / statsDuration, m_throughputRef, m_throughputRef * tolerance,
                                 " Unexpected Throughput!");
      NS_TEST_ASSERT_MSG_EQ_TOL (rxed / statsDuration, m_throughputRef, m_throughputRef * tolerance,
                                 " Unexpected Throughput!");
    }

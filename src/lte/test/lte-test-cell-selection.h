...
public:
  ...
  {
    Vector
        position; ///< The position, relative to the inter site distance, where the UE will be spawned in the simulation.
  ...
  LteCellSelectionTestCase (std::string name, bool isEpcMode, bool isIdealRrc,
                            double interSiteDistance, std::vector<UeSetup_t> ueSetupList);
...
private:
   ...
   */
  void StateTransitionCallback (std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti,

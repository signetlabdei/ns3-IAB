...
LteRlcAmE2eTestSuite::LteRlcAmE2eTestSuite () : TestSuite ("lte-rlc-am-e2e", TestSuite::Type::SYSTEM)
{
          ...
          for (uint32_t sduArrivalType = 0; sduArrivalType <= 1; ++sduArrivalType)
            {
              ...
              AddTestCase (
                  new LteRlcAmE2eTestCase (name.str (), runs[s], losses[l], bulkSduArrival),
                  testDuration);
            }
        }
    }

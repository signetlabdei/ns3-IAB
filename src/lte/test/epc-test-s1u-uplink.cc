...
public:
   ...
   * \param bid the BID
   */
  EpsBearerTagUdpClient (uint16_t rnti, uint32_t bid);
...
private:
  ...
  uint32_t m_bid; ///< the bearer identificator
};
  ...
  static TypeId tid =
      TypeId ("ns3::EpsBearerTagUdpClient")
          ...
          .AddAttribute ("Interval", "The time to wait between packets", TimeValue (Seconds (1.0)),
                         MakeTimeAccessor (&EpsBearerTagUdpClient::m_interval), MakeTimeChecker ())
...
EpsBearerTagUdpClient::EpsBearerTagUdpClient (uint16_t rnti, uint32_t bid)
...
EpcS1uUlTestCase::DoRun ()
{
  ...
  for (std::vector<EnbUlTestData>::iterator enbit = m_enbUlTestData.begin ();
       enbit < m_enbUlTestData.end (); ++enbit)
    {
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          PacketSinkHelper packetSinkHelper (
              "ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), udpSinkPort));
...
EpcS1uUlTestSuite::EpcS1uUlTestSuite () : TestSuite ("epc-s1u-uplink", TestSuite::Type::SYSTEM)
{
  std::vector<EnbUlTestData> v1;
  EnbUlTestData e1;
  UeUlTestData f1 (1, 100, 1, 12);
  e1.ues.push_back (f1);
  v1.push_back (e1);
  ...
  EnbUlTestData e2;
  UeUlTestData f2_1 (1, 100, 1, 12);
  e2.ues.push_back (f2_1);
  UeUlTestData f2_2 (2, 200, 2, 23);
  ...
  EnbUlTestData e3;
  UeUlTestData f3_1 (3, 50, 1, 12);
  e3.ues.push_back (f3_1);
  UeUlTestData f3_2 (5, 1472, 2, 23);
  e3.ues.push_back (f3_2);
  UeUlTestData f3_3 (1, 1, 3, 34);
  e3.ues.push_back (f3_2);
  ...
  EnbUlTestData e5;
  UeUlTestData f5 (10, 3000, 1, 12);
  e5.ues.push_back (f5);
  v5.push_back (e5);
  AddTestCase (new EpcS1uUlTestCase ("1 eNB, 10 pkts 3000 bytes each", v5), TestCase::Duration::QUICK);

  std::vector<EnbUlTestData> v6;
  EnbUlTestData e6;
  UeUlTestData f6 (50, 3000, 1, 12);
  e6.ues.push_back (f6);
  v6.push_back (e6);
  AddTestCase (new EpcS1uUlTestCase ("1 eNB, 50 pkts 3000 bytes each", v6), TestCase::Duration::QUICK);

  std::vector<EnbUlTestData> v7;
  EnbUlTestData e7;
  UeUlTestData f7 (10, 15000, 1, 12);
  e7.ues.push_back (f7);
  v7.push_back (e7);
  AddTestCase (new EpcS1uUlTestCase ("1 eNB, 10 pkts 15000 bytes each", v7), TestCase::Duration::QUICK);

  std::vector<EnbUlTestData> v8;
  EnbUlTestData e8;
  UeUlTestData f8 (100, 15000, 1, 12);
  e8.ues.push_back (f8);
  v8.push_back (e8);
  AddTestCase (new EpcS1uUlTestCase ("1 eNB, 100 pkts 15000 bytes each", v8), TestCase::Duration::QUICK);
}

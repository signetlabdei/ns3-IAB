  ...
  if (haveContext)
    {
      Simulator::ScheduleWithContext (node->GetId (), Seconds (0),
                                      &LtePdcpSapProvider::TransmitPdcpSdu, m_pdcpSapProvider, p);
    }
  else
    {
...
LteTestMac::SendTxOpportunity (Time time, uint32_t bytes)
{
  ...
  if (haveContext)
    {
      Simulator::ScheduleWithContext (node->GetId (), time, &LteMacSapUser::NotifyTxOpportunity,
  ...
  if (m_txOpportunityMode == RANDOM_MODE)
    {
      if (m_txOppTime != Seconds (0))
        {
          Simulator::Schedule (m_txOppTime, &LteTestMac::SendTxOpportunity, this, m_txOppTime,
                               m_txOppSize);
        }
    }
}
...
LteTestMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  if (m_device)
    ...
    {
      Simulator::Schedule (Seconds (0.1), &LteMacSapUser::ReceivePdu, m_macLoopback->m_macSapUser,
                           rxPduParams);
    }
  else
    {
  ...
  if (m_txOpportunityMode == AUTOMATIC_MODE)
    {
      ...
      while (size > 0)
        {
          EventId e = Simulator::Schedule (time, &LteMacSapUser::NotifyTxOpportunity, m_macSapUser,
                                           txOpParmas);
...
EpcTestRrc::EpcTestRrc () : m_s1SapProvider (0)
{
  NS_LOG_FUNCTION (this);

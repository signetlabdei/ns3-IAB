...
LteUplinkDataSinrTestCase::DoRun (void)
{
  ...
  */
  ...
  NS_TEST_ASSERT_MSG_SPECTRUM_VALUE_EQ_TOL (*(actualSinrCatcher.GetValue ()), *m_expectedSinr,
                                            0.0000001, "Data Frame - Wrong SINR !");
  ulPhy->Dispose ();
  Simulator::Destroy ();
}
...
LteUplinkSrsSinrTestCase::LteUplinkSrsSinrTestCase (Ptr<SpectrumValue> sv1, Ptr<SpectrumValue> sv2,
                                                    Ptr<SpectrumValue> sinr, std::string name)
...
LteUplinkSrsSinrTestCase::DoRun (void)
{
  ...
  */
  ...
  NS_TEST_ASSERT_MSG_SPECTRUM_VALUE_EQ_TOL (*m_actualSinr, *m_expectedSinr, 0.0000001,

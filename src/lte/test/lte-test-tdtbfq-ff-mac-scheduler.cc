...
LenaTestTdTbfqFfMacSchedulerSuite::LenaTestTdTbfqFfMacSchedulerSuite ()
    : TestSuite ("lte-tdtbfq-ff-mac-scheduler", TestSuite::Type::SYSTEM)
{
...
LenaTdTbfqFfMacSchedulerTestCase1::LenaTdTbfqFfMacSchedulerTestCase1 (
    uint16_t nUser, double dist, double thrRefDl, double thrRefUl, uint16_t packetSize,
...
LenaTdTbfqFfMacSchedulerTestCase2::LenaTdTbfqFfMacSchedulerTestCase2 (
    std::vector<double> dist, std::vector<uint32_t> estThrTdTbfqDl,
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) dlDataRxed.at (i) / statsDuration,
                                 m_estThrTdTbfqDl.at (i), m_estThrTdTbfqDl.at (i) * tolerance,
                                 " Unfair Throughput!");
    }

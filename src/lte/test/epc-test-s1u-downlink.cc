...
EpcS1uDlTestCase::EpcS1uDlTestCase (std::string name, std::vector<EnbDlTestData> v)
    : TestCase (name), m_enbDlTestData (v)
{
}
...
EpcS1uDlTestCase::DoRun ()
{
  ...
  for (std::vector<EnbDlTestData>::iterator enbit = m_enbDlTestData.begin ();
       enbit < m_enbDlTestData.end (); ++enbit)
    {
      ...
      epcHelper->AddDu (enb, enbDevice, cellId);
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          epcHelper->ActivateEpsBearer (ueLteDevice, imsi, EpcTft::Default (),
  ...
  for (std::vector<EnbDlTestData>::iterator enbit = m_enbDlTestData.begin ();
       enbit < m_enbDlTestData.end (); ++enbit)
    {
      for (std::vector<UeDlTestData>::iterator ueit = enbit->ues.begin (); ueit < enbit->ues.end ();
           ++ueit)
        {
          NS_TEST_ASSERT_MSG_EQ (ueit->serverApp->GetTotalRx (), (ueit->numPkts) * (ueit->pktSize),
...
EpcS1uDlTestSuite::EpcS1uDlTestSuite () : TestSuite ("epc-s1u-downlink", TestSuite::Type::SYSTEM)
{
  ...
  e5.ues.push_back (f5);
  v5.push_back (e5);
  AddTestCase (new EpcS1uDlTestCase ("1 eNB, 10 pkts 3000 bytes each", v5), TestCase::Duration::QUICK);
  ...
  e6.ues.push_back (f6);
  v6.push_back (e6);
  AddTestCase (new EpcS1uDlTestCase ("1 eNB, 50 pkts 3000 bytes each", v6), TestCase::Duration::QUICK);
  ...
  e7.ues.push_back (f7);
  v7.push_back (e7);
  AddTestCase (new EpcS1uDlTestCase ("1 eNB, 10 pkts 15000 bytes each", v7), TestCase::Duration::QUICK);
  ...
  e8.ues.push_back (f8);
  v8.push_back (e8);
  AddTestCase (new EpcS1uDlTestCase ("1 eNB, 100 pkts 15000 bytes each", v8), TestCase::Duration::QUICK);
}

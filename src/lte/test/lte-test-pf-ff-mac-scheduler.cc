...
LenaTestPfFfMacSchedulerSuite::LenaTestPfFfMacSchedulerSuite ()
    : TestSuite ("lte-pf-ff-mac-scheduler", TestSuite::Type::SYSTEM)
{
  ...
  AddTestCase (new LenaPfFfMacSchedulerTestCase1 (1, 100000, 0, 0, errorModel), TestCase::Duration::QUICK);
...
LenaPfFfMacSchedulerTestCase1::LenaPfFfMacSchedulerTestCase1 (uint16_t nUser, double dist,
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) ulDataRxed.at (i) / statsDuration,
                                 (double) m_estThrPfUl.at (i),
                                 (double) m_estThrPfUl.at (i) * tolerance, " Unfair Throughput!");
    }
  Simulator::Destroy ();
}

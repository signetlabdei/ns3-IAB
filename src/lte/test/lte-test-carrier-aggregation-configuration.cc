...
public:
  ...
  CarrierAggregationConfigTestCase (uint32_t numberOfNodes, uint16_t numberOfComponentCarriers,
      ...
      : TestCase (BuildNameString (numberOfNodes, numberOfComponentCarriers, configToCheck,
                                   simulationDuration)),
...
private:
  ...
  std::string BuildNameString (uint32_t numberOfNodes, uint16_t numberOfComponentCarriers,
...
void
CarrierAggregationConfigTestCase::Evaluate (std::string context, Ptr<LteUeRrc> ueRrc,
                                            std::list<LteRrcSap::SCellToAddMod> sCellToAddModList)
{
...
CarrierAggregationConfigTestSuite::CarrierAggregationConfigTestSuite ()
    : TestSuite ("lte-carrier-aggregation-configuration", TestSuite::Type::SYSTEM)
{
  std::vector<ConfigToCheck> configToCheck;
  ...
  AddTestCase (new CarrierAggregationConfigTestCase (numberOfNodes, numberOfComponentCarriers,

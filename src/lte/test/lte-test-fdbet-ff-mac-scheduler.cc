...
LenaTestFdBetFfMacSchedulerSuite::LenaTestFdBetFfMacSchedulerSuite ()
    : TestSuite ("lte-fdbet-ff-mac-scheduler", TestSuite::Type::SYSTEM)
{
  ...
  AddTestCase (new LenaFdBetFfMacSchedulerTestCase1 (1, 100000, 0, 0, errorModel), TestCase::Duration::QUICK);
...
LenaFdBetFfMacSchedulerTestCase1::LenaFdBetFfMacSchedulerTestCase1 (uint16_t nUser, double dist,
                                                                    double thrRefDl,
                                                                    double thrRefUl,
...
LenaFdBetFfMacSchedulerTestCase2::LenaFdBetFfMacSchedulerTestCase2 (
    std::vector<double> dist, std::vector<uint32_t> achievableRateDl,
    std::vector<uint32_t> estThrFdBetUl, bool errorModelEnabled)
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL (
          (double) ulDataRxed.at (i) / statsDuration, (double) m_estThrFdBetUl.at (i),
          (double) m_estThrFdBetUl.at (i) * tolerance, " Unfair Throughput!");
    }
  Simulator::Destroy ();
}

...
public:
   ...
   * \param hex string
   */
  RlcAmStatusPduTestCase (SequenceNumber10 ackSn, std::list<SequenceNumber10> nackSnList,
                          std::string hex);
...
RlcAmStatusPduTestCase::DoRun ()
{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      m_hex, hex, "serialized packet content " << hex << " differs from test vector " << m_hex);
...
LteRlcHeaderTestSuite::LteRlcHeaderTestSuite () : TestSuite ("lte-rlc-header", TestSuite::Type::UNIT)
{
  NS_LOG_FUNCTION (this);

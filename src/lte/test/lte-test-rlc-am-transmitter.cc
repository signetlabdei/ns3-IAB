...
LteRlcAmTransmitterTestSuite::LteRlcAmTransmitterTestSuite ()
    : TestSuite ("lte-rlc-am-transmitter", TestSuite::Type::SYSTEM)
{
  // LogLevel logLevel = (LogLevel)(LOG_PREFIX_FUNC | LOG_PREFIX_TIME | LOG_LEVEL_ALL);
  // LogComponentEnable ("LteRlcAmTransmitterTest", logLevel);

  AddTestCase (new LteRlcAmTransmitterOneSduTestCase ("One SDU, one PDU"), TestCase::Duration::QUICK);
  AddTestCase (new LteRlcAmTransmitterSegmentationTestCase ("Segmentation"), TestCase::Duration::QUICK);
  AddTestCase (new LteRlcAmTransmitterConcatenationTestCase ("Concatenation"), TestCase::Duration::QUICK);
...
LteRlcAmTransmitterTestCase::LteRlcAmTransmitterTestCase (std::string name) : TestCase (name)
{
}
...
void
LteRlcAmTransmitterTestCase::CheckDataReceived (Time time, std::string shouldReceived,
                                                std::string assertMsg)
{
  Simulator::Schedule (time, &LteRlcAmTransmitterTestCase::DoCheckDataReceived, this,
                       shouldReceived, assertMsg);
}
...
LteRlcAmTransmitterReportBufferStatusTestCase::DoRun (void)
{
  ...
  CheckDataReceived (Seconds (1.600), "UVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVW",
                     "SDU #8 is not OK");

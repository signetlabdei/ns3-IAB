...
public:
  ...
  LteX2HandoverMeasuresTestCase (uint32_t nEnbs, uint32_t nUes, uint32_t nDedicatedBearers,
                                 ...
                                 std::string checkPointEventListName, bool useUdp,
                                 std::string schedulerType, std::string handoverAlgorithmType,
...
std::string
LteX2HandoverMeasuresTestCase::BuildNameString (uint32_t nEnbs, uint32_t nUes,
...
LteX2HandoverMeasuresTestCase::LteX2HandoverMeasuresTestCase (
    uint32_t nEnbs, uint32_t nUes, uint32_t nDedicatedBearers,
    std::list<CheckPointEvent> checkPointEventList, std::string checkPointEventListName,
    bool useUdp, std::string schedulerType, std::string handoverAlgorithmType, bool admitHo,
    bool useIdealRrc)
    : TestCase (BuildNameString (nEnbs, nUes, nDedicatedBearers, checkPointEventListName, useUdp,
                                 schedulerType, handoverAlgorithmType, admitHo, useIdealRrc)),
      m_nEnbs (nEnbs),
      m_nUes (nUes),
...
LteX2HandoverMeasuresTestCase::DoRun ()
{
  NS_LOG_FUNCTION (this << BuildNameString (m_nEnbs, m_nUes, m_nDedicatedBearers,
                                            m_checkPointEventListName, m_useUdp, m_schedulerType,
                                            m_handoverAlgorithmType, m_admitHo, m_useIdealRrc));
  ...
  for (NetDeviceContainer::Iterator it = ueDevices.Begin (); it != ueDevices.End (); ++it)
    {
      ...
      Simulator::Schedule (maxRrcConnectionEstablishmentDuration,
                           &LteX2HandoverMeasuresTestCase::CheckConnected, this, *it,
                           enbDevices.Get (0));
    }
  ...
  for (std::list<CheckPointEvent>::iterator checkPointEventIt = m_checkPointEventList.begin ();
       checkPointEventIt != m_checkPointEventList.end (); ++checkPointEventIt)
    {
      for (Time checkPointTime = checkPointEventIt->checkStartTime;
        ...
        {
          Simulator::Schedule (checkPointTime, &LteX2HandoverMeasuresTestCase::CheckConnected, this,
          ...
          Simulator::Schedule (saveStatsTime, &LteX2HandoverMeasuresTestCase::SaveStats, this,
                               checkPointEventIt->ueDeviceIndex);

          Time checkStats = saveStatsTime + m_statsDuration;
          Simulator::Schedule (checkStats, &LteX2HandoverMeasuresTestCase::CheckStats, this,
                               checkPointEventIt->ueDeviceIndex);
...
LteX2HandoverMeasuresTestCase::CheckStats (uint32_t ueIndex)
{
  ...
  for (std::list<BearerData>::iterator it = m_ueDataVector.at (ueIndex).bearerDataList.begin ();
       it != m_ueDataVector.at (ueIndex).bearerDataList.end (); ++it)
    {
      ...
      if (it->ulSink)
        {
          NS_TEST_ASSERT_MSG_GT (ulRx, 0.500 * expectedBytes,

...
LenaTestCqaFfMacSchedulerSuite::LenaTestCqaFfMacSchedulerSuite ()
    : TestSuite ("lte-cqa-ff-mac-scheduler", TestSuite::Type::SYSTEM)
{
...
LenaCqaFfMacSchedulerTestCase1::LenaCqaFfMacSchedulerTestCase1 (uint16_t nUser, double dist,
...
LenaCqaFfMacSchedulerTestCase2::DoRun (void)
{
  ...
  for (int i = 0; i < m_nUser; i++)
    {
      NS_TEST_ASSERT_MSG_EQ_TOL ((double) dlDataRxed.at (i) / statsDuration, m_estThrCqaDl.at (i),
                                 m_estThrCqaDl.at (i) * tolerance, " Unfair Throughput!");
    }

...
LteDownlinkPowerControlSpectrumValueTestCase::LteDownlinkPowerControlSpectrumValueTestCase (
    std::string name, uint16_t earfcn, uint8_t bw, double powerTx, std::map<int, double> powerTxMap,
    std::vector<int> activeRbs, SpectrumValue &expected)
    : TestCase ("Downlink Power Control: " + name),
      m_actual (LteSpectrumValueHelper::CreateTxPowerSpectralDensity (earfcn, bw, powerTx,
                                                                      powerTxMap, activeRbs)),
...
LteDownlinkPowerControlTestCase::LteDownlinkPowerControlTestCase (bool changePower, uint8_t pa,
                                                                  std::string name)
    : TestCase ("Downlink Power Control: " + name)
{
...
LteDownlinkPowerControlTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (
      powerDiff, m_expectedPowerDiff, 0.001,
...
NotifyConnectionReconfigurationUe (
    LteDownlinkPowerControlRrcConnectionReconfigurationTestCase *testcase, std::string context,

...
public:
   ...
   */
  LteRrcConnectionEstablishmentTestCase (uint32_t nUes, uint32_t nBearers, uint32_t tConnBase,
                                         uint32_t tConnIncrPerUe, uint32_t delayDiscStart,
...
protected:
   ...
   */
  static std::string BuildNameString (uint32_t nUes, uint32_t nBearers, uint32_t tConnBase,
                                      uint32_t tConnIncrPerUe, uint32_t delayDiscStart,
   ...
   * \param rnti the RNTI
   */
  void ConnectionEstablishedCallback (std::string context, uint64_t imsi, uint16_t cellId,
                                      uint16_t rnti);
  /**
   ...
   * \param rnti the RNTI
   */
  void ConnectionTimeoutCallback (std::string context, uint64_t imsi, uint16_t cellId,
                                  uint16_t rnti);
  ...
  uint32_t
      m_tConnIncrPerUe; ///< additional connection time increment for each UE index (0...nUes-1) in ms
...
std::string
LteRrcConnectionEstablishmentTestCase::BuildNameString (uint32_t nUes, uint32_t nBearers,
...
LteRrcConnectionEstablishmentTestCase::LteRrcConnectionEstablishmentTestCase (
    uint32_t nUes, uint32_t nBearers, uint32_t tConnBase, uint32_t tConnIncrPerUe,
    uint32_t delayDiscStart, bool errorExpected, bool useIdealRrc, bool admitRrcConnectionRequest,
    std::string description)
    : TestCase (BuildNameString (nUes, nBearers, tConnBase, tConnIncrPerUe, delayDiscStart,
                                 useIdealRrc, admitRrcConnectionRequest, description)),
      m_nUes (nUes),
      m_nBearers (nBearers),
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      Simulator::Schedule (MilliSeconds (tc), &LteRrcConnectionEstablishmentTestCase::Connect, this,
                           ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc),
                           &LteRrcConnectionEstablishmentTestCase::CheckConnected, this, *it,
                           enbDevice);
          ...
          while (enbBearerIt != enbDataRadioBearerMapValue.End () &&
                 ueBearerIt != ueDataRadioBearerMapValue.End ())
            {
              ...
              NS_TEST_ASSERT_MSG_EQ ((uint32_t) enbDrbInfo->m_drbIdentity,
                                     (uint32_t) ueDrbInfo->m_drbIdentity, "drbIdentity differs");
                                                          ...
                                                          Ptr<NetDevice> enbDevice)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      ...
      true, "it should not happen that connection is completed both at the UE and at the eNB side");
}
...
LteRrcConnectionEstablishmentErrorTestCase::DoRun ()
{
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      //void (LteHelper::* overloadedAttachFunctionPointer) (Ptr<NetDevice>, Ptr<NetDevice>) = &LteHelper::Attach;
      //Simulator::Schedule (MilliSeconds (tc), overloadedAttachFunctionPointer, lteHelper, *it, enbDevice);
      Simulator::Schedule (MilliSeconds (tc), &LteRrcConnectionEstablishmentErrorTestCase::Connect,
                           this, ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc),
                           &LteRrcConnectionEstablishmentErrorTestCase::CheckConnected, this, *it,
                           enbDevice);
  ...
  Simulator::Schedule (m_jumpAwayTime, &LteRrcConnectionEstablishmentErrorTestCase::JumpAway, this);
  ...
  Simulator::Schedule (m_jumpAwayTime + MilliSeconds (100),
                       &LteRrcConnectionEstablishmentErrorTestCase::JumpBack, this);
...
LteRrcTestSuite::LteRrcTestSuite () : TestSuite ("lte-rrc", TestSuite::Type::SYSTEM)
{

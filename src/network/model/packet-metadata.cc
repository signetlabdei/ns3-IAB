...
PacketMetadata::Enable ()
{
  ...
  NS_ASSERT_MSG (!m_metadataSkipped, "Error: attempting to enable the packet metadata "
...
uint16_t
PacketMetadata::AddBig (uint32_t next, uint32_t prev, const PacketMetadata::SmallItem *item,
...
void
PacketMetadata::ReplaceTail (PacketMetadata::SmallItem *item, PacketMetadata::ExtraItem *extraItem,
                             uint32_t available)
{
...
uint32_t
PacketMetadata::ReadItems (uint16_t current, struct PacketMetadata::SmallItem *item,
      ...
      if (uid != 0)
        {
          ...
          buffer = AddToRaw (reinterpret_cast<const uint8_t *> (uidString.c_str ()), uidStringSize,
...
uint8_t *
PacketMetadata::AddToRawU16 (const uint16_t &data, uint8_t *start, uint8_t *current,
                             uint32_t maxSize)
{
...
uint8_t *
PacketMetadata::AddToRawU32 (const uint32_t &data, uint8_t *start, uint8_t *current,
                             uint32_t maxSize)
{
  NS_LOG_FUNCTION (data << &start << &current << maxSize);
  // First check buffer overflow
...
uint8_t *
PacketMetadata::AddToRawU64 (const uint64_t &data, uint8_t *start, uint8_t *current,
                             uint32_t maxSize)
{
  NS_LOG_FUNCTION (data << &start << &current << maxSize);
  // First check buffer overflow
...
uint8_t *
PacketMetadata::AddToRaw (const uint8_t *data, uint32_t dataSize, uint8_t *start, uint8_t *current,
                          uint32_t maxSize)
{
...
uint8_t *
PacketMetadata::ReadFromRawU8 (uint8_t &data, const uint8_t *start, const uint8_t *current,
                               uint32_t maxSize)
{
  NS_LOG_FUNCTION (static_cast<uint32_t> (data) << &start << &current << maxSize);
...
uint8_t *
PacketMetadata::ReadFromRawU16 (uint16_t &data, const uint8_t *start, const uint8_t *current,
                                uint32_t maxSize)
{
  NS_LOG_FUNCTION (data << &start << &current << maxSize);
  // First check buffer underflow
...
uint8_t *
PacketMetadata::ReadFromRawU32 (uint32_t &data, const uint8_t *start, const uint8_t *current,
                                uint32_t maxSize)
{
  NS_LOG_FUNCTION (data << &start << &current << maxSize);
  // First check buffer underflow
...
uint8_t *
PacketMetadata::ReadFromRawU64 (uint64_t &data, const uint8_t *start, const uint8_t *current,
                                uint32_t maxSize)
{
  NS_LOG_FUNCTION (data << &start << &current << maxSize);
  // First check buffer underflow

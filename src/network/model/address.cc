...
Address::Address (uint8_t type, const uint8_t *buffer, uint8_t len) : m_type (type), m_len (len)
{
...
Address::Address (const Address &address) : m_type (address.m_type), m_len (address.m_len)
{
  NS_ASSERT (m_len <= MAX_SIZE);
...
operator<< (std::ostream &os, const Address &address)
{
  ...
  os.fill ('0');
  os << std::setw (2) << (uint32_t) address.m_type << "-" << std::setw (2)

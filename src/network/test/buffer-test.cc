...
BufferTest::BufferTest () : TestCase ("Buffer")
{
}
...
BufferTest::DoRun ()
{
  ...
  i = buff64.Begin ();
  NS_TEST_ASSERT_MSG_EQ (i.ReadLsbtohU64 (), 0x0123456789abcdefLLU,
...
#pragma clang diagnostic pop
  ...
  /// See \bugid{54}
  {
    ...
    NS_TEST_EXPECT_MSG_EQ (memcmp (inputBuffer.PeekData (), outputBuffer.PeekData (), chunkSize), 0,
                           "memcp works");
  }
  ...
  for (uint32_t i = 0; i < sizeBuffer; i++)
    {
      NS_TEST_ASSERT_MSG_EQ (cBuf[i], *(((const uint8_t *) ct.c_str ()) + i),
...
BufferTestSuite::BufferTestSuite () : TestSuite ("buffer", UNIT)
{

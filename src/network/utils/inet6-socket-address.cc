...
Inet6SocketAddress::Inet6SocketAddress (Ipv6Address ipv6, uint16_t port)
    : m_ipv6 (ipv6), m_port (port)
{
  NS_LOG_FUNCTION (this << ipv6 << port);
}

Inet6SocketAddress::Inet6SocketAddress (Ipv6Address ipv6) : m_ipv6 (ipv6), m_port (0)
{
  NS_LOG_FUNCTION (this << ipv6);
}

Inet6SocketAddress::Inet6SocketAddress (const char *ipv6, uint16_t port)
    : m_ipv6 (Ipv6Address (ipv6)), m_port (port)
{
  NS_LOG_FUNCTION (this << ipv6 << port);
}

Inet6SocketAddress::Inet6SocketAddress (const char *ipv6) : m_ipv6 (Ipv6Address (ipv6)), m_port (0)
{
  NS_LOG_FUNCTION (this << ipv6);
}

Inet6SocketAddress::Inet6SocketAddress (uint16_t port)
    : m_ipv6 (Ipv6Address::GetAny ()), m_port (port)
{

...
private:
  ...
  struct MakeGetItem
  {
    /**
         ...
         */
    static Ptr<Item>
    GetItem (const Container &, const ConstIterator it)
    {
      return *it;
    }
  };
  ...
  template <class T>
  struct MakeGetItem<
      T, std::void_t<decltype (std::declval<T> ().GetItem (std::declval<ConstIterator> ()))>>
  {
    /**
         ...
         */
    static Ptr<Item>
    GetItem (const Container &container, const ConstIterator it)
    {
  ...
  static TypeId tid =
      TypeId (name)
          ...
          .AddTraceSource ("Drop", "Drop a packet (for whatever reason).",
                           MakeTraceSourceAccessor (&Queue<Item, Container>::m_traceDrop), tcbName)
          .AddTraceSource (
              "DropBeforeEnqueue", "Drop a packet before enqueue.",
              MakeTraceSourceAccessor (&Queue<Item, Container>::m_traceDropBeforeEnqueue), tcbName)
          .AddTraceSource (
              "DropAfterDequeue", "Drop a packet after dequeue.",
              MakeTraceSourceAccessor (&Queue<Item, Container>::m_traceDropAfterDequeue), tcbName);
  return tid;
}

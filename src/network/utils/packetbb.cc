...
PbbPacket::operator== (const PbbPacket &other) const
{
  ...
  for (tmi = MessageBegin (), omi = other.MessageBegin ();
       tmi != MessageEnd () && omi != other.MessageEnd (); tmi++, omi++)
    {
      if (**tmi != **omi)
        {
          return false;
...
PbbMessage::operator== (const PbbMessage &other) const
{
  ...
  for (tai = AddressBlockBegin (), oai = other.AddressBlockBegin ();
       tai != AddressBlockEnd () && oai != other.AddressBlockEnd (); tai++, oai++)
    {
      if (**tai != **oai)
        {
          return false;
...
PbbAddressBlock::operator== (const PbbAddressBlock &other) const
{
  ...
  for (tai = AddressBegin (), oai = other.AddressBegin ();
       tai != AddressEnd () && oai != other.AddressEnd (); tai++, oai++)
    {
      if (*tai != *oai)
        {
          return false;
  ...
  for (tpi = PrefixBegin (), opi = other.PrefixBegin ();
       tpi != PrefixEnd () && opi != other.PrefixEnd (); tpi++, opi++)
    {
      if (*tpi != *opi)
        {
          return false;
...
void
PbbAddressBlock::GetHeadTail (uint8_t *head, uint8_t &headlen, uint8_t *tail,

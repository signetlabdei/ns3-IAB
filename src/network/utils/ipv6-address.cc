...
/**
     ...
     */
static uint32_t
lookuphash (unsigned char *k, uint32_t length, uint32_t level)
{
...
{
  NS_LOG_FUNCTION (this);
  static uint8_t v4MappedPrefix[12] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
Ipv6Prefix::Ipv6Prefix (const char *prefix, uint8_t prefixLength)
{
  ...
  NS_ASSERT_MSG (autoLength <= prefixLength, "Ipv6Prefix: address and prefix are not compatible: "
                                                 << Ipv6Address (prefix) << "/" << +prefixLength);

...
void
ErrorChannel::Send (Ptr<Packet> p, uint16_t protocol, Mac48Address to, Mac48Address from,
  ...
  for (std::vector<Ptr<SimpleNetDevice>>::const_iterator i = m_devices.begin ();
       i != m_devices.end (); ++i)
    {
      ...
      if (m_jumping)
          ...
          if (m_jumpingState % 2)
            {
              Simulator::ScheduleWithContext (tmp->GetNode ()->GetId (), Seconds (0),
                                              &SimpleNetDevice::Receive, tmp, p->Copy (), protocol,
                                              to, from);
            }
          else
            {
        ...
        {
          Simulator::ScheduleWithContext (tmp->GetNode ()->GetId (), Seconds (0),
                                          &SimpleNetDevice::Receive, tmp, p->Copy (), protocol, to,
                                          from);
        }
    }
}

...
{
  static TypeId tid =
      TypeId ("ns3::PacketProbe")
          .SetParent<Probe> ()
          .SetGroupName ("Network")
          .AddConstructor<PacketProbe> ()
          .AddTraceSource ("Output", "The packet that serve as the output for this probe",
                           MakeTraceSourceAccessor (&PacketProbe::m_output),
                           "ns3::Packet::TracedCallback")
          .AddTraceSource ("OutputBytes", "The number of bytes in the packet",
...
PacketProbe::ConnectByObject (std::string traceSource, Ptr<Object> obj)
{
  ...
  bool connected = obj->TraceConnectWithoutContext (
      traceSource, MakeCallback (&ns3::PacketProbe::TraceSink, this));
  return connected;
}

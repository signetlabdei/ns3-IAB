...
private:
  /**
     ...
     */
  void ReceivePacket (Ptr<const Packet> pkt, const Address &from, const Address &dst,
...
void
NeighborCacheExample::ReceivePacket (Ptr<const Packet> pkt, const Address &from, const Address &dst,
                                     const SeqTsSizeHeader &header)
{
...
NeighborCacheExample::CommandSetup (int argc, char **argv)
{
  ...
  cmd.AddValue ("useNetDeviceContainer", "Generate neighbor cache for specific netDeviceContainer",
                m_useNetDeviceContainer);
  cmd.AddValue ("useInterfaceContainer", "Generate neighbor cache for specific interfaceContainer",
      ...
      if (m_sendTraffic)
        {
          ...
          apps.Get (1)->TraceConnectWithoutContext (
              "RxWithSeqTsSize", MakeCallback (&NeighborCacheExample::ReceivePacket, this));

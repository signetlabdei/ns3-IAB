...
public:
  ...
  TcpLinuxRenoSSTest (uint32_t segmentSize, uint32_t packetSize, uint32_t packets,
                      uint32_t initialCwnd, uint32_t delayedAck, uint32_t expectedCwnd,
...
TcpLinuxRenoSSTest::TcpLinuxRenoSSTest (uint32_t segmentSize, uint32_t packetSize, uint32_t packets,
...
TcpLinuxRenoSSTest::CWndTrace (uint32_t oldValue, uint32_t newValue)
{
  ...
  if (m_initial)
    {
      m_initial = false;
      NS_TEST_ASSERT_MSG_EQ (newValue, m_initialCwnd * m_segmentSize,
...
public:
  /**
     ...
     * \param desc The test description.
     */
  TcpLinuxRenoCongAvoidTest (uint32_t segmentSize, uint32_t packetSize, uint32_t packets,
                             uint32_t initialCwnd, uint32_t initialSSThresh, uint32_t delayedAck,
                             uint32_t expectedCwnd, TypeId &congControl, const std::string &desc);
...
TcpLinuxRenoCongAvoidTest::TcpLinuxRenoCongAvoidTest (uint32_t segmentSize, uint32_t packetSize,

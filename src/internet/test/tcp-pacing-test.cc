...
public:
  ...
  TcpPacingTest (uint32_t segmentSize, uint32_t packetSize, uint32_t packets,
                 uint16_t pacingSsRatio, uint16_t pacingCaRatio, uint32_t ssThresh,
                 bool paceInitialWindow, uint32_t delAckMaxCount, const TypeId &congControl,
...
TcpPacingTest::TcpPacingTest (uint32_t segmentSize, uint32_t packetSize, uint32_t packets,
                              uint16_t pacingSsRatio, uint16_t pacingCaRatio, uint32_t ssThresh,
                              bool paceInitialWindow, uint32_t delAckMaxCount, const TypeId &typeId,
                              const std::string &desc)
    : TcpGeneralTest (desc),
      ...
      if (!m_isFullCwndSent && !m_isConnAboutToEnd && beyondInitialDataSegment)
        {
          ...
          NS_TEST_ASSERT_MSG_LT_OR_EQ (
              ...
              errorMargin.GetSeconds (), "Packet delivery in slow start didn't match pacing rate");
...
public:
  TcpPacingTestSuite () : TestSuite ("tcp-pacing-test", UNIT)
  {
    ...
    AddTestCase (new TcpPacingTest (segmentSize, packetSize, numPackets, pacingSsRatio,
                                    pacingCaRatio, ssThresh, paceInitialWindow, delAckMaxCount, tid,
                                    description),
                 TestCase::QUICK);

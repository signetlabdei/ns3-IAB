...
public:
  ...
  void
  Serialize (TagBuffer buffer) const override
  {
    buffer.WriteU64 (token);
  }

  void
  Deserialize (TagBuffer buffer) override
  {
...
void
Ipv6FragmentationTest::HandleReadIcmpClient (Ipv6Address icmpSource, uint8_t icmpTtl,
...
{
  NS_TEST_EXPECT_MSG_LT_OR_EQ (packet->GetSize (), ipv6->GetMtu (interface),
...
Ipv6FragmentationTest::DoRun ()
{
  ...
  Ptr<BinaryErrorModel> serverDevErrorModel = CreateObject<BinaryErrorModel> ();
  {
    ...
    ipv6->TraceConnectWithoutContext ("Rx",
  ...
  Ptr<BinaryErrorModel> clientDevErrorModel = CreateObject<BinaryErrorModel> ();
  {
    ...
    ipv6->AddAddress (netdev_idx, ipv6Addr);
    ipv6->SetUp (netdev_idx);
    ipv6->TraceConnectWithoutContext ("Tx",
  ...
  for (int i = 0; i < 5; i++)
    {
      ...
      NS_TEST_EXPECT_MSG_EQ (recvSize, packetSizes[i],
                             "Packet size not correct: recvSize: " << recvSize << " packetSizes["
                                                                   << i << "]: " << packetSizes[i]);

      m_receivedPacketServer->CopyData (recvBuffer, 65000);
      NS_TEST_EXPECT_MSG_EQ (memcmp (m_data, recvBuffer, m_receivedPacketServer->GetSize ()), 0,

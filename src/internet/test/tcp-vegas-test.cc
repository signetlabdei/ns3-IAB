...
public:
  ...
  TcpVegasTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh, Time rtt,
                uint32_t segmentsAcked, SequenceNumber32 nextTxSeq, SequenceNumber32 lastAckedSeq,
...
TcpVegasTest::TcpVegasTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh, Time rtt,
...
public:
  ...
  {
    AddTestCase (
        new TcpVegasTest (38 * 1446, 1446, 40 * 1446, MilliSeconds (106), 1,
                          SequenceNumber32 (2893), SequenceNumber32 (5785),
                          "Vegas test on cWnd and ssThresh when in slow start and diff > gamma"),
        TestCase::QUICK);
    AddTestCase (
        new TcpVegasTest (5 * 536, 536, 10 * 536, MilliSeconds (118), 1, SequenceNumber32 (3216),
                          SequenceNumber32 (3753),
                          "Vegas test on cWnd and ssThresh when in slow start and diff < gamma"),
        TestCase::QUICK);
    AddTestCase (new TcpVegasTest (60 * 346, 346, 40 * 346, MilliSeconds (206), 1,
                                   SequenceNumber32 (20761), SequenceNumber32 (21107),
                                   "Vegas test on cWnd and ssThresh when diff > beta"),
                 TestCase::QUICK);
    AddTestCase (new TcpVegasTest (15 * 1446, 1446, 10 * 1446, MilliSeconds (106), 1,
                                   SequenceNumber32 (21691), SequenceNumber32 (24583),
                                   "Vegas test on cWnd and ssThresh when diff < alpha"),
                 TestCase::QUICK);
    AddTestCase (new TcpVegasTest (20 * 746, 746, 10 * 746, MilliSeconds (109), 1,
                                   SequenceNumber32 (14921), SequenceNumber32 (15667),

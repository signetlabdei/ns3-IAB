...
protected:
  ...
  void UpdatedRttHistory (const SequenceNumber32 &seq, uint32_t sz, bool isRetransmission,
                                         ...
                                         bool isRetransmission, SocketWho who)
{
  ...
  if (seq < m_highestTxSeq)
    ...
    {
      NS_TEST_ASSERT_MSG_EQ (isRetransmission, false, "Incorrectly flagging seq as retransmission");
      m_dataCount++;
    }
  else if (seq == m_highestTxSeq && m_dataCount > 0)
    {
...
public:
  /**
     ...
     */
  TcpRttEstimationWithLossTest (const std::string &desc, bool enableTs, uint32_t pktCount,
...
TcpRttEstimationWithLossTest::TcpRttEstimationWithLossTest (const std::string &desc, bool enableTs,
                                                            uint32_t pktCount,
                                                            std::vector<uint32_t> toDrop)
    : TcpRttEstimationTest (desc, enableTs, pktCount), m_toDrop (toDrop)
{
}

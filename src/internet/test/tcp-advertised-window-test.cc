...
public:
  ...
  /**
     ...
     */
  void
  SetExpectedSegmentSize (uint16_t seg)
  {
    m_segmentSize = seg;
  };
...
public:
  ...
  /**
     ...
     */
  TcpDropRatioErrorModel (double dropRatio) : TcpGeneralErrorModel (), m_dropRatio (dropRatio)
  {
...
public:
  /**
     ...
     */
  TcpAdvertisedWindowTest (const std::string &desc, uint32_t size, uint32_t packets,
                           double lossRatio);
...
TcpAdvertisedWindowTest::TcpAdvertisedWindowTest (const std::string &desc, uint32_t size,
                                                  uint32_t packets, double lossRatio)
    : TcpGeneralTest (desc), m_pktSize (size), m_pktCount (packets), m_lossRatio (lossRatio)
{
}
...
TcpAdvWindowOnLossTest::TcpAdvWindowOnLossTest (const std::string &desc, uint32_t size,
                                                uint32_t packets, std::vector<uint32_t> &toDrop)
    : TcpGeneralTest (desc), m_pktSize (size), m_pktCount (packets), m_toDrop (toDrop)
{
}
...
public:
  TcpAdvertisedWindowTestSuite () : TestSuite ("tcp-advertised-window-test", UNIT)
  {
    AddTestCase (new TcpAdvertisedWindowTest ("TCP advertised window size, small seg + no loss",
                 ...
                 TestCase::QUICK);
    AddTestCase (
        new TcpAdvertisedWindowTest ("TCP advertised window size, small seg + loss", 500, 100, 0.1),
        TestCase::QUICK);
    AddTestCase (new TcpAdvertisedWindowTest ("TCP advertised window size, large seg + no loss",
                                              1000, 100, 0.0),
                 TestCase::QUICK);
    AddTestCase (new TcpAdvertisedWindowTest ("TCP advertised window size, large seg + small loss",
                                              1000, 100, 0.1),
                 TestCase::QUICK);
    AddTestCase (new TcpAdvertisedWindowTest ("TCP advertised window size, large seg + big loss",
                 ...
                 TestCase::QUICK);
    AddTestCase (
        new TcpAdvertisedWindowTest ("TCP advertised window size, complete loss", 1000, 100, 1.0),
        TestCase::QUICK);

    std::vector<uint32_t> toDrop;
    toDrop.push_back (8001);
    toDrop.push_back (9001);
    AddTestCase (new TcpAdvWindowOnLossTest ("TCP advertised window size, after FIN loss", 1000, 10,
                                             toDrop));
  }
};

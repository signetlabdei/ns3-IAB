...
public:
  /**
     ...
     */
  TcpScalableIncrementTest (uint32_t cWnd, uint32_t segmentSize, uint32_t segmentsAcked,
...
TcpScalableIncrementTest::TcpScalableIncrementTest (uint32_t cWnd, uint32_t segmentSize,
                                                    uint32_t segmentsAcked, const std::string &name)
    : TestCase (name), m_cWnd (cWnd), m_segmentSize (segmentSize), m_segmentsAcked (segmentsAcked)
{
}
...
TcpScalableDecrementTest::TcpScalableDecrementTest (uint32_t cWnd, uint32_t segmentSize,
                                                    const std::string &name)
  ...
  TcpScalableTestSuite () : TestSuite ("tcp-scalable-test", UNIT)
  {
    ...
    AddTestCase (
        new TcpScalableIncrementTest (
            38, 1, 100, "Scalable increment test on cWnd = 38 segments and segmentSize = 1 byte"),
        TestCase::QUICK);
    AddTestCase (new TcpScalableIncrementTest (
                     53 * 1446, 1446, 50,
    ...
    AddTestCase (
        new TcpScalableDecrementTest (
            38, 1, "Scalable decrement test on cWnd = 38 segments and segmentSize = 1 byte"),
        TestCase::QUICK);

...
public:
  /**
     ...
     */
  ClassicRecoveryTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh, uint32_t dupAckCount,
...
ClassicRecoveryTest::ClassicRecoveryTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh,
...
ClassicRecoveryTest::DoRun ()
{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      m_state->m_cWndInfl, (m_state->m_ssThresh + (m_dupAckCount * m_state->m_segmentSize)),
...
public:
  ClassicRecoveryTestSuite () : TestSuite ("tcp-classic-recovery-test", UNIT)
  {
    AddTestCase (new ClassicRecoveryTest (3000, 500, 2500, 3,
                 ...
                 TestCase::QUICK);
    AddTestCase (new ClassicRecoveryTest (3000, 1000, 2500, 3,
                                          "Classic recovery test with 1000 bytes segmentSize"),
                 TestCase::QUICK);
    AddTestCase (new ClassicRecoveryTest (3000, 500, 2500, 4,
                                          "Classic recovery test with 4 DupAck threshold"),
                 TestCase::QUICK);
    AddTestCase (new ClassicRecoveryTest (3000, 500, 1000, 3,
                                          "Classic recovery test with 1000 bytes ssThresh"),
                 TestCase::QUICK);
    AddTestCase (new ClassicRecoveryTest (2500, 500, 2500, 3,
                                          "Classic recovery test with same cWnd and ssThresh"),
                 TestCase::QUICK);
    AddTestCase (new ClassicRecoveryTest (1000, 500, 2500, 3,

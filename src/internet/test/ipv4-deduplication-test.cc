...
class Ipv4DeduplicationTest : public TestCase
{
  ...
  /**
     * Enum of test types
     */
  enum MODE { ENABLED = 0, DISABLED, DEGENERATE }; // enabled, but expiration time too low
...
Ipv4DeduplicationTest::ReceivePkt (Ptr<Socket> socket)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (availableData, packet->GetSize (),
...
void
Ipv4DeduplicationTest::DropPkt (const Ipv4Header &ipHeader, Ptr<const Packet> packet,
  ...
  for (auto end = nodes.End (), iter = nodes.Begin (); iter != end; ++iter)
    {
      ...
      staticRouting.AddMulticastRoute (*iter, Ipv4Address::GetAny (), targetAddr.c_str (), *diter,
...
Ipv4DeduplicationTest::CheckPackets (const std::string &name)
{
  ...
  std::map<std::string, uint32_t> packets = {{"A", 14}, {"B", 16}, {"C", 16}, {"D", 16}, {"E", 4}};

  // a priori determined packet receptions based on
  std::map<std::string, uint32_t> packetsDuped = {{"A", 0}, {"B", 1}, {"C", 1}, {"D", 1}, {"E", 1}};
  ...
  std::map<std::string, uint32_t> degenerates = {{"A", 3}, {"B", 4}, {"C", 4}, {"D", 3}, {"E", 2}};
...
Ipv4DeduplicationTest::CheckDrops (const std::string &name)
{
  ...
  if (drops[name])
    {
      NS_TEST_ASSERT_MSG_NE ((m_dropCountMap.find (name) == m_dropCountMap.end ()), true,
    ...
    {
      NS_TEST_EXPECT_MSG_EQ ((m_dropCountMap.find (name) == m_dropCountMap.end ()), true,
...
Ipv4DeduplicationTestSuite::Ipv4DeduplicationTestSuite () : TestSuite ("ipv4-deduplication", UNIT)
{

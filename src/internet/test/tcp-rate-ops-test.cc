  ...
  for (uint8_t i = 0; i < 100; ++i)
    {
      ...
      Simulator::Schedule (Time (Seconds (i * 0.01)), &TcpRateLinuxBasicTest::SendSkb, this,
                           m_skbs[i]);
    }

  for (uint8_t i = 0; i < 100; ++i)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * 0.1)), &TcpRateLinuxBasicTest::SkbDelivered,
                           this, m_skbs[i]);
    }
...
TcpRateLinuxBasicTest::SendSkb (TcpTxItem *skb)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (skb->GetRateInformation ().m_delivered, m_delivered,
                         "SKB should have delivered equal to current value of total delivered");

  if (isStartOfTransmission)
    {
      NS_TEST_ASSERT_MSG_EQ (skb->GetRateInformation ().m_deliveredTime, Simulator::Now (),
                             "SKB should have updated the delivered time to current value");
    }
  else
    {
      NS_TEST_ASSERT_MSG_EQ (skb->GetRateInformation ().m_deliveredTime, m_deliveredTime,
                             "SKB should have updated the delivered time to current value");
    }
}
...
TcpRateLinuxWithSocketsTest::RateUpdatedTrace (const TcpRateLinux::TcpRateConnection &rate)
{
  ...
  if (m_bytesInFlight == 0)
    {
      NS_TEST_ASSERT_MSG_EQ (rate.m_firstSentTime, Simulator::Now (),
                             "FirstSentTime should be current time when bytes inflight is zero");
      NS_TEST_ASSERT_MSG_EQ (rate.m_deliveredTime, Simulator::Now (),
                             "Delivered time should be current time when bytes inflight is zero");
    }
  NS_TEST_ASSERT_MSG_GT_OR_EQ (rate.m_delivered, m_prevRate.m_delivered,
    ...
    : TestCase (testString), m_segmentSize (segmentSize)
{
  ...
  m_rateOps->TraceConnectWithoutContext (
      "TcpRateUpdated", MakeCallback (&TcpRateLinuxWithBufferTest::RateUpdatedTrace, this));
  m_rateOps->TraceConnectWithoutContext (
...
{
  NS_LOG_DEBUG ("Rate sample updated " << sample);
  NS_TEST_ASSERT_MSG_EQ (sample.m_ackedSacked, m_expectedAckedSacked,
...
public:
  ...
  {
    AddTestCase (new TcpRateLinuxBasicTest (1000, SequenceNumber32 (20), SequenceNumber32 (11), 1,
                                            "Testing SkbDelivered and SkbSent"),
                 TestCase::QUICK);
    AddTestCase (
        new TcpRateLinuxBasicTest (1000, SequenceNumber32 (11), SequenceNumber32 (11), 2,
                                   "Testing SkbDelivered and SkbSent with app limited data"),
        TestCase::QUICK);

    std::vector<uint32_t> toDrop;
    toDrop.push_back (4001);
    AddTestCase (new TcpRateLinuxWithSocketsTest (
                     "Checking Rate sample value without SACK, one drop", false, toDrop),
                 TestCase::QUICK);

    AddTestCase (new TcpRateLinuxWithSocketsTest ("Checking Rate sample value with SACK, one drop",
                                                  true, toDrop),
                 TestCase::QUICK);
    toDrop.push_back (4001);
    AddTestCase (new TcpRateLinuxWithSocketsTest (
    ...
    AddTestCase (new TcpRateLinuxWithSocketsTest ("Checking Rate sample value with SACK, two drop",
                                                  true, toDrop),
                 TestCase::QUICK);

    AddTestCase (new TcpRateLinuxWithBufferTest (
                     1000, "Checking rate sample values with arbitary SACK Block"),
                 TestCase::QUICK);

    AddTestCase (new TcpRateLinuxWithBufferTest (
                     500, "Checking rate sample values with arbitary SACK Block"),
                 TestCase::QUICK);
  }
};

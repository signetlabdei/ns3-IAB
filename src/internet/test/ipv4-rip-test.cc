...
Ipv4RipTest::Ipv4RipTest () : TestCase ("RIP")
{
}
...
Ipv4RipTest::DoRun ()
{
  ...
  NS_TEST_EXPECT_MSG_EQ (rxSocket->Bind (InetSocketAddress (Ipv4Address ("10.0.2.2"), 1234)), 0,
                         "trivial");
...
Ipv4RipSplitHorizonStrategyTest::ReceivePktProbe (Ptr<Socket> socket)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (availableData, receivedPacketProbe->GetSize (),
                         "Received Packet size is not equal to the Rx buffer size");
          ...
          if (iter->GetPrefix () == "10.0.1.0")
            {
              ...
              NS_TEST_EXPECT_MSG_EQ (correct, true,
...
Ipv4RipSplitHorizonStrategyTest::DoRun ()
{
  ...
  NS_TEST_EXPECT_MSG_EQ (rxSocket->Bind (InetSocketAddress (Ipv4Address ("224.0.0.9"), 520)), 0,
                         "trivial");

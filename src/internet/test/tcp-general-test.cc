...
void
TcpGeneralTest::SendPacket (Ptr<Socket> socket, uint32_t pktSize, uint32_t pktCount,
                            Time pktInterval)
{
  ...
  if (pktCount > 0)
    {
      ...
      Simulator::Schedule (pktInterval, &TcpGeneralTest::SendPacket, this, socket, pktSize,
...
TcpGeneralTest::DoRun ()
{
  ...
  m_senderSocket->TraceConnectWithoutContext (
      "BytesInFlight", MakeCallback (&TcpGeneralTest::BytesInFlightTrace, this));
  ...
  m_senderSocket->TraceConnectWithoutContext (
      "HighestSequence", MakeCallback (&TcpGeneralTest::HighestTxSeqTrace, this));
  m_senderSocket->m_rateOps->TraceConnectWithoutContext (
      "TcpRateUpdated", MakeCallback (&TcpGeneralTest::RateUpdatedTrace, this));
  m_senderSocket->m_rateOps->TraceConnectWithoutContext (
      "TcpRateSampleUpdated", MakeCallback (&TcpGeneralTest::RateSampleUpdatedTrace, this));
  ...
  Simulator::ScheduleWithContext (nodes.Get (0)->GetId (), m_startTime, &TcpGeneralTest::SendPacket,
                                  this, m_senderSocket, m_pktSize, m_pktCount,
...
void
TcpGeneralTest::UpdateRttHistoryCb (Ptr<const TcpSocketBase> tcp, const SequenceNumber32 &seq,
...
void
TcpGeneralTest::TxPacketCb (const Ptr<const Packet> p, const TcpHeader &h,
                            const Ptr<const TcpSocketBase> tcp)
{
  if (tcp->GetNode () == m_receiverSocket->GetNode ())
    {

...
public:
  ...
  TcpBicIncrementTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh,
                       uint32_t segmentsAcked, uint32_t lastMaxCwnd, const std::string &name);
...
TcpBicIncrementTest::TcpBicIncrementTest (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh,
...
public:
  /**
     ...
     * \param name Test description.
     */
  TcpBicDecrementTest (uint32_t cWnd, uint32_t segmentSize, BooleanValue fastConvergence,
                       uint32_t lastMaxCwnd, const std::string &name);
...
public:
  ...
  {
    AddTestCase (
        new TcpBicIncrementTest (10 * 536, 536, 9 * 536, 11, 0,
                                 "Bic increment test: under lowCwnd & enough ACKs received"),
        TestCase::QUICK);
    AddTestCase (
        new TcpBicIncrementTest (10 * 536, 536, 9 * 536, 8, 0,
                                 "Bic increment test: under lowCwnd but not enough ACKs received"),
        TestCase::QUICK);
    AddTestCase (
        new TcpBicIncrementTest (18 * 1446, 1446, 15 * 1446, 5, 90,
                                 "Bic increment test: linear increase when distance exceeds S_max"),
        TestCase::QUICK);
    AddTestCase (
        new TcpBicIncrementTest (18 * 1446, 1446, 15 * 1446, 24, 20,

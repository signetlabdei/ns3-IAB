...
public:
  /**
     ...
     */
  TcpIllinoisTest (uint32_t cWnd, uint32_t ssThresh, uint32_t segmentSize, uint32_t cntRtt,
                   Time maxRtt, uint32_t segmentsAcked, SequenceNumber32 nextTxSeq,
...
TcpIllinoisTest::TcpIllinoisTest (uint32_t cWnd, uint32_t ssThresh, uint32_t segmentSize,
...
TcpIllinoisTest::CalculateBeta (Ptr<TcpIllinois> cong, double da, double dm)
{
  ...
  if (da <= d2)
    ...
    {
      m_beta =
          (betaMin.Get () * d3 - betaMax.Get () * d2 + (betaMax.Get () - betaMin.Get ()) * da) /
          (d3 - d2);
    }
...
public:
  ...
  {
    AddTestCase (new TcpIllinoisTest (38 * 1446, 40 * 1446, 1446, 2, MilliSeconds (105), 2,
    ...
    AddTestCase (new TcpIllinoisTest (
                     60 * 346, 40 * 346, 346, 2, MilliSeconds (100), 2, SequenceNumber32 (2893),
                     SequenceNumber32 (5785),
                     "Illinois test on cWnd and ssThresh when avg queueing delay is at minimum"),
                 TestCase::QUICK);
    AddTestCase (new TcpIllinoisTest (
                     38 * 1446, 40 * 1446, 1446, 5, MilliSeconds (110), 2, SequenceNumber32 (2893),
                     SequenceNumber32 (5785),
                     "Illinois test on cWnd and ssThresh when avg queueing delay is at maximum"),
                 TestCase::QUICK);
    AddTestCase (new TcpIllinoisTest (40 * 1446, 38 * 1446, 1446, 2, MilliSeconds (105), 55,
                                      SequenceNumber32 (2893), SequenceNumber32 (5785),

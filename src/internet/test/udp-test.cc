...
UdpSocketLoopbackTest::UdpSocketLoopbackTest () : TestCase ("UDP loopback test")
{
}
...
UdpSocketLoopbackTest::DoRun ()
{
  ...
  Simulator::Run ();
  Simulator::Destroy ();
  NS_TEST_EXPECT_MSG_EQ (m_receivedPacket->GetSize (), 246,
...
Udp6SocketLoopbackTest::Udp6SocketLoopbackTest () : TestCase ("UDP6 loopback test")
{
}
...
UdpSocketImplTest::UdpSocketImplTest () : TestCase ("UDP socket implementation")
{
}
...
UdpSocketImplTest::ReceivePkt2 (Ptr<Socket> socket)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (availableData, m_receivedPacket2->GetSize (),
                         "ReceivedPacket size is not equal to the Rx buffer size");
}
...
UdpSocketImplTest::DoRun ()
{
  ...
  Ptr<Socket> rxSocket = rxSocketFactory->CreateSocket ();
  NS_TEST_EXPECT_MSG_EQ (rxSocket->Bind (InetSocketAddress (Ipv4Address ("10.0.0.1"), 1234)), 0,
                         "trivial");
  rxSocket->SetRecvCallback (MakeCallback (&UdpSocketImplTest::ReceivePkt, this));

  Ptr<Socket> rxSocket2 = rxSocketFactory->CreateSocket ();
  NS_TEST_EXPECT_MSG_EQ (rxSocket2->Bind (InetSocketAddress (Ipv4Address ("10.0.1.1"), 1234)), 0,
                         "trivial");
  ...
  rxSocket2->SetRecvCallback (MakeCallback (&UdpSocketImplTest::ReceivePkt2, this));
  NS_TEST_EXPECT_MSG_EQ (rxSocket2->Bind (InetSocketAddress (Ipv4Address ("0.0.0.0"), 1234)), 0,
                         "trivial");
  ...
  NS_TEST_EXPECT_MSG_EQ (txSocket->GetErrno (), Socket::ERROR_NOTCONN,
  ...
  NS_TEST_EXPECT_MSG_EQ (peerAddress, peer,
...
Udp6SocketImplTest::Udp6SocketImplTest () : TestCase ("UDP6 socket implementation")
{
}
...
Udp6SocketImplTest::DoRun ()
{
  ...
  NS_TEST_EXPECT_MSG_EQ (
      rxSocket2->Bind (Inet6SocketAddress (Ipv6Address ("2001:0100:1::1"), 1234)), 0, "trivial");
  ...
  rxSocket2 = rxSocketFactory->CreateSocket ();
  rxSocket2->SetRecvCallback (MakeCallback (&Udp6SocketImplTest::ReceivePkt2, this));
  NS_TEST_EXPECT_MSG_EQ (rxSocket2->Bind (Inet6SocketAddress (Ipv6Address ("::"), 1234)), 0,
                         "trivial");

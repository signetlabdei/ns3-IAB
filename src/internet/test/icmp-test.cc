...
IcmpEchoReplyTestCase::IcmpEchoReplyTestCase () : TestCase ("ICMP:EchoReply test case")
{
}
...
IcmpEchoReplyTestCase::SendData (Ptr<Socket> socket, Ipv4Address dst)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (socket->SendTo (p, 0, realTo), (int) p->GetSize (),
...
IcmpEchoReplyTestCase::DoRun ()
{
  ...
  NS_TEST_EXPECT_MSG_EQ (m_receivedPacket->GetSize (), 28,
...
IcmpTimeExceedTestCase::IcmpTimeExceedTestCase () : TestCase ("ICMP:TimeExceedReply test case")
{
}
...
IcmpTimeExceedTestCase::DoRun ()
{
  ...
  socket->SetIpTtl (1);
  Simulator::ScheduleWithContext (socket->GetNode ()->GetId (), Seconds (0),
                                  &IcmpTimeExceedTestCase::SendData, this, socket,
                                  i2.GetAddress (1, 0));
  Simulator::Run ();

  NS_TEST_EXPECT_MSG_EQ (m_receivedPacket->GetSize (), 56,
...
IcmpV6EchoReplyTestCase::IcmpV6EchoReplyTestCase () : TestCase ("ICMPV6:EchoReply test case")
{
}
  ...
  if (Inet6SocketAddress::IsMatchingType (from))
    {
      ...
      NS_TEST_EXPECT_MSG_EQ (ipv6.GetNextHeader (), Ipv6Header::IPV6_ICMPV6,
...
IcmpV6EchoReplyTestCase::DoRun ()
{
  ...
  Simulator::ScheduleWithContext (socket->GetNode ()->GetId (), Seconds (0),
                                  &IcmpV6EchoReplyTestCase::SendData, this, socket,
                                  i.GetAddress (1, 1));
  Simulator::Run ();

  NS_TEST_EXPECT_MSG_EQ (m_receivedPacket->GetSize (), 52,
...
IcmpV6TimeExceedTestCase::IcmpV6TimeExceedTestCase () : TestCase ("ICMPV6:TimeExceed test case")
{
}
...
IcmpV6TimeExceedTestCase::DoRun ()
{
  ...
  Simulator::ScheduleWithContext (socket->GetNode ()->GetId (), Seconds (0),
                                  &IcmpV6TimeExceedTestCase::SendData, this, socket,
                                  interfaces2.GetAddress (1, 1));
  Simulator::Run ();

  NS_TEST_EXPECT_MSG_EQ (m_receivedPacket->GetSize (), 100,
                         " Unexpected ICMPV6_ECHO_REPLY packet size");
...
IcmpTestSuite::IcmpTestSuite () : TestSuite ("icmp", UNIT)
{

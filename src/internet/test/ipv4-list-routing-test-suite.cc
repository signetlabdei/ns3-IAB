...
public:
  Ptr<Ipv4Route>
  RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif,
  ...
  void
  NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address) override
  {
  }

  void
  NotifyRemoveAddress (uint32_t interface, Ipv4InterfaceAddress address) override
  {
  }
  ...
  void
  PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const override
  {
  }
};

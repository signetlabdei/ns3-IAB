          ...
          if (m_senderSent == 1)
            {
              ...
              NS_TEST_ASSERT_MSG_EQ (unsigned (ipTosTag.GetTos ()), 0x1,
          ...
          if (m_senderSent == 1)
            {
              NS_TEST_ASSERT_MSG_EQ (
                  foundTag, false, "IP TOS should not have ECT1 for SYN packet for DCTCP traffic");
            }
          if (m_senderSent == 3)
            {
              NS_TEST_ASSERT_MSG_EQ (foundTag, true, "Tag not found");
              NS_TEST_ASSERT_MSG_EQ (unsigned (ipTosTag.GetTos ()), 0x2,
  ...
  else if (who == RECEIVER && (m_testCase == 1 || m_testCase == 2))
    {
      ...
      if (m_testCase == 1)
      ...
      else
        {
          if (m_receiverSent == 1)
            {
              NS_TEST_ASSERT_MSG_EQ (foundTag, false,
...
public:
  ...
  TcpDctcpCongestedRouter () : TcpSocketMsgBase ()
  {
    m_dataPacketSent = 0;
  }
  ...
  /**
     ...
     */
  TcpDctcpCongestedRouter (const TcpDctcpCongestedRouter &other) : TcpSocketMsgBase (other)
  {
  }
...
TcpDctcpCodePointsTest::CreateSenderSocket (Ptr<Node> node)
{
  if (m_testCase == 2)
    ...
    {
      return TcpGeneralTest::CreateSocket (node, TcpSocketMsgBase::GetTypeId (),
                                           TcpDctcp::GetTypeId ());
    }
}
...
public:
  ...
  {
    AddTestCase (new TcpDctcpToLinuxReno (2 * 1446, 1446, 4 * 1446, 2, SequenceNumber32 (4753),
                                          SequenceNumber32 (3216), MilliSeconds (100),
                                          "DCTCP falls to New Reno for slowstart"),
                 TestCase::QUICK);
    AddTestCase (new TcpDctcpCodePointsTest (1, "ECT Test : Check if ECT is set on Syn, Syn+Ack, "
                                                "Ack and Data packets for DCTCP packets"),
                 TestCase::QUICK);
    AddTestCase (new TcpDctcpCodePointsTest (
                     2, "ECT Test : Check if ECT is not set on Syn, Syn+Ack and Ack but set on "

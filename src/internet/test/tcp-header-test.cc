  ...
  for (uint32_t i = 0; i < 1000; ++i)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (header.GetLength (), 5,
...
{
  {
    ...
    NS_TEST_ASSERT_MSG_EQ (header.GetLength (), 6,
                           "Four byte added as option "
                           "are not a word");
    NS_TEST_ASSERT_MSG_EQ (header.GetSerializedSize (), 24,
                           "Four byte added as option "
                           "are not a word");
...
{
  {
    ...
    NS_TEST_ASSERT_MSG_EQ (header.GetLength (), 5,
                           ...
                           " END option");
    NS_TEST_ASSERT_MSG_EQ (header.GetSerializedSize (), 20,
                           "Length has changed also for"
                           " END option");
...

  {
    ...
    NS_TEST_ASSERT_MSG_EQ (header.HasOption (TcpOption::MSS), true,
...
TcpHeaderFlagsToString::TcpHeaderFlagsToString (std::string name) : TestCase (name)
{
}

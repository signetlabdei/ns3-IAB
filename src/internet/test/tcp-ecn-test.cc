...
public:
  ...
  TcpSocketCongestedRouter () : TcpSocketMsgBase ()
  {
    m_dataPacketSent = 0;
  }
  ...
  /**
     ...
     */
  TcpSocketCongestedRouter (const TcpSocketCongestedRouter &other) : TcpSocketMsgBase (other)
  {
  }
  ...
  if (m_testcase == 6)
    {
      if (newValue < oldValue)
        {
          m_cwndChangeCount++;
          NS_TEST_ASSERT_MSG_EQ (m_cwndChangeCount, 1,
                                 "Congestion window should be reduced once per every window");
          NS_TEST_ASSERT_MSG_EQ (newValue, 1000,
  ...
  if (who == RECEIVER)
    {
      if (m_receiverReceived == 0)
        {
          NS_TEST_ASSERT_MSG_NE (((h.GetFlags ()) & TcpHeader::SYN), 0,
                                 "SYN should be received as first message at the receiver");
          if (m_testcase == 2 || m_testcase == 4 || m_testcase == 5 || m_testcase == 6)
            {
              NS_TEST_ASSERT_MSG_NE (
                  ((h.GetFlags ()) & TcpHeader::ECE) && ((h.GetFlags ()) & TcpHeader::CWR), 0,
        ...
        {
          NS_TEST_ASSERT_MSG_NE (((h.GetFlags ()) & TcpHeader::ACK), 0,
  ...
  else if (who == SENDER)
    {
      ...
      if (m_testcase == 5 && m_receiverReceived > 12)
        {
          NS_TEST_ASSERT_MSG_EQ (((h.GetFlags ()) & TcpHeader::ECE), 0,
      ...
      if (m_senderSent == 3)
        {
          ...
          if (m_testcase == 4 || m_testcase == 6)
            {
              NS_TEST_ASSERT_MSG_EQ (ipTos, 0x2,
            ...
            {
              NS_TEST_ASSERT_MSG_NE (ipTos, 0x2,
...
public:
  TcpEcnTestSuite () : TestSuite ("tcp-ecn-test", UNIT)
  {
    AddTestCase (new TcpEcnTest (
                     1, "ECN Negotiation Test : ECN incapable sender and ECN incapable receiver"),
                 TestCase::QUICK);
    AddTestCase (
        new TcpEcnTest (2, "ECN Negotiation Test : ECN capable sender and ECN incapable receiver"),
        TestCase::QUICK);
    AddTestCase (
        new TcpEcnTest (3, "ECN Negotiation Test : ECN incapable sender and ECN capable receiver"),
        TestCase::QUICK);
    AddTestCase (
    ...
    AddTestCase (
        new TcpEcnTest (
            5, "ECE and CWR Functionality Test: ECN capable sender and ECN capable receiver"),
        TestCase::QUICK);
    AddTestCase (
        new TcpEcnTest (
            6, "Congestion Window Reduction Test :ECN capable sender and ECN capable receiver"),
        TestCase::QUICK);
  }
};

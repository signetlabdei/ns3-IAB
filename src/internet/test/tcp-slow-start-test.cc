...
public:
  /**
     ...
     */
  TcpSlowStartNormalTest (uint32_t segmentSize, uint32_t packetSize, uint32_t initSsTh,
                          uint32_t packets, const TypeId &congControl, const std::string &desc);
...
TcpSlowStartNormalTest::TcpSlowStartNormalTest (uint32_t segmentSize, uint32_t packetSize,
...
public:
  /**
     ...
     * \param desc Test description.
     */
  TcpSlowStartAttackerTest (uint32_t segmentSize, uint32_t packetSize, uint32_t initSsTh,
                            uint32_t packets, const TypeId &congControl, const std::string &desc);
...
TcpSlowStartAttackerTest::TcpSlowStartAttackerTest (uint32_t segmentSize, uint32_t packetSize,
                                                    uint32_t initSsTh, uint32_t packets,
                                                    const TypeId &typeId, const std::string &msg)
    ...
    for (const auto &t : types)
      {
        ...
        AddTestCase (
            new TcpSlowStartNormalTest (500, 500, 10000, 10, t, "slow start 500 byte, " + typeName),
            TestCase::QUICK);
        ...
        AddTestCase (new TcpSlowStartAttackerTest (
                         500, 500, 10000, 10, t, "slow start ack attacker, 500 byte, " + typeName),
                     TestCase::QUICK);
        AddTestCase (
            new TcpSlowStartAttackerTest (1000, 1000, 10000, 9, t,

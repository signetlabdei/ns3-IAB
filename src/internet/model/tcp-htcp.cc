...
TcpHtcp::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::TcpHtcp")
          ...
          .AddAttribute ("DefaultBackoff", "The default AIMD backoff factor", DoubleValue (0.5),
                         MakeDoubleAccessor (&TcpHtcp::m_defaultBackoff),
                         MakeDoubleChecker<double> (0, 1))
          .AddAttribute ("ThroughputRatio", "Threshold value for updating beta", DoubleValue (0.2),
                         MakeDoubleAccessor (&TcpHtcp::m_throughputRatio),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("DeltaL", "Delta_L parameter in increase function",
                         TimeValue (Seconds (1)), MakeTimeAccessor (&TcpHtcp::m_deltaL),
                         MakeTimeChecker ());
  return tid;
}
...
TcpHtcp::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time &rtt)
{
  ...
  m_throughput = static_cast<uint32_t> (
      m_dataSent / (Simulator::Now ().GetSeconds () - m_lastCon.GetSeconds ()));

...
TcpPrrRecovery::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::TcpPrrRecovery")
          ...
          .AddAttribute ("ReductionBound", "Type of Reduction Bound", EnumValue (SSRB),
                         MakeEnumAccessor (&TcpPrrRecovery::m_reductionBoundMode),
                         MakeEnumChecker (CRB, "CRB", SSRB, "SSRB"));
  return tid;
}

TcpPrrRecovery::TcpPrrRecovery () : TcpClassicRecovery ()
{
...
void
TcpPrrRecovery::EnterRecovery (Ptr<TcpSocketState> tcb, uint32_t dupAckCount [[maybe_unused]],

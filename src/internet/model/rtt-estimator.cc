...
RttEstimator::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::RttEstimator")
          ...
          .AddAttribute ("InitialEstimation", "Initial RTT estimate", TimeValue (Seconds (1.0)),
...
RttMeanDeviation::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::RttMeanDeviation")
          ...
          .AddAttribute ("Alpha", "Gain used in estimating the RTT, must be 0 <= alpha <= 1",
                         DoubleValue (0.125), MakeDoubleAccessor (&RttMeanDeviation::m_alpha),
                         MakeDoubleChecker<double> (0, 1))
          .AddAttribute ("Beta",
                         "Gain used in estimating the RTT variation, must be 0 <= beta <= 1",
                         DoubleValue (0.25), MakeDoubleAccessor (&RttMeanDeviation::m_beta),
                         MakeDoubleChecker<double> (0, 1));
  return tid;
}

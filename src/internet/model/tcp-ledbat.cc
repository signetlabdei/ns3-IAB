...
TcpLedbat::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::TcpLedbat")
          ...
          .AddAttribute ("TargetDelay", "Targeted Queue Delay", TimeValue (MilliSeconds (100)),
                         MakeTimeAccessor (&TcpLedbat::m_target), MakeTimeChecker ())
          .AddAttribute ("BaseHistoryLen", "Number of Base delay samples", UintegerValue (10),
                         MakeUintegerAccessor (&TcpLedbat::m_baseHistoLen),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("NoiseFilterLen", "Number of Current delay samples", UintegerValue (4),
                         MakeUintegerAccessor (&TcpLedbat::m_noiseFilterLen),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Gain", "Offset Gain", DoubleValue (1.0),
                         MakeDoubleAccessor (&TcpLedbat::m_gain), MakeDoubleChecker<double> ())
          .AddAttribute ("SSParam", "Possibility of Slow Start", EnumValue (DO_SLOWSTART),
                         MakeEnumAccessor (&TcpLedbat::SetDoSs),
                         MakeEnumChecker (DO_SLOWSTART, "yes", DO_NOT_SLOWSTART, "no"))
          .AddAttribute ("MinCwnd", "Minimum cWnd for Ledbat", UintegerValue (2),
...
TcpLedbat::TcpLedbat () : TcpNewReno ()
{
...
TcpLedbat::TcpLedbat (const TcpLedbat &sock) : TcpNewReno (sock)
{
  NS_LOG_FUNCTION (this);
...
TcpLedbat::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time &rtt)
{
  ...
  if (rtt.IsPositive ())
    {
      AddDelay (m_noiseFilter, tcb->m_rcvTimestampValue - tcb->m_rcvTimestampEchoReply,
                m_noiseFilterLen);

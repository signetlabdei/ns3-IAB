...
void
NdiscCache::SetDevice (Ptr<NetDevice> device, Ptr<Ipv6Interface> interface,
  ...
  if (m_nsRetransmit < m_ndCache->m_icmpv6->GetMaxMulticastSolicit ())
    {
      ...
      m_ndCache->m_icmpv6->SendNS (addr, Ipv6Address::MakeSolicitedAddress (m_ipv6Address),
  ...
  else
    {
      ...
      m_ndCache->m_icmpv6->SendErrorDestinationUnreachable (malformedPacket.first, addr,
...
NdiscCache::Entry::FunctionDelayTimeout ()
{
  ...
  Ipv6PayloadHeaderPair p = m_ndCache->m_icmpv6->ForgeNS (addr, m_ipv6Address, m_ipv6Address,
  ...
  if (m_nsRetransmit < m_ndCache->m_icmpv6->GetMaxUnicastSolicit ())
    {
      ...
      /* icmpv6->SendNS (m_ndCache->GetInterface ()->GetLinkLocalAddress (), m_ipv6Address,
         * m_ipv6Address, m_ndCache->GetDevice ()->GetAddress ()); */
      Ipv6PayloadHeaderPair p = m_ndCache->m_icmpv6->ForgeNS (
          addr, m_ipv6Address, m_ipv6Address, m_ndCache->GetDevice ()->GetAddress ());
      p.first->AddHeader (p.second);
      m_ndCache->GetDevice ()->Send (p.first, this->GetMacAddress (), Ipv6L3Protocol::PROT_NUMBER);

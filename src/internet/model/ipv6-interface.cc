...
Ipv6Interface::DoSetup ()
{
  ...
  Ptr<Icmpv6L4Protocol> icmpv6 =
      DynamicCast<Icmpv6L4Protocol> (m_node->GetObject<Ipv6> ()->GetProtocol (
          Icmpv6L4Protocol::GetStaticProtocolNumber (), interfaceId));
              ...
              if (icmpv6->IsAlwaysDad ())
                {
                  ...
                  Simulator::Schedule (icmpv6->GetDadTimeout (),
                                       &Icmpv6L4Protocol::FunctionDadTimeout, icmpv6, this, addr);
                }
              else
                {
                  Simulator::Schedule (Seconds (0.), &Icmpv6L4Protocol::FunctionDadTimeout, icmpv6,
                                       this, addr);
                }
            }
        }
  ...
  for (Ipv6InterfaceAddressListCI it = m_addresses.begin (); it != m_addresses.end (); ++it)
    {
      if (dest == it->first.GetAddress ())
        {
          p->AddHeader (hdr);
          m_tc->Receive (m_device, p, Ipv6L3Protocol::PROT_NUMBER, m_device->GetBroadcast (),
                         m_device->GetBroadcast (), NetDevice::PACKET_HOST);
          return;
        }
    }
      ...
      if (found)
        {
          ...
          m_tc->Send (m_device, Create<Ipv6QueueDiscItem> (p, hardwareDestination,
                                                           Ipv6L3Protocol::PROT_NUMBER, hdr));
        }
    }
  else
    {
      NS_LOG_LOGIC ("Doesn't need NDISC");
      m_tc->Send (m_device, Create<Ipv6QueueDiscItem> (p, m_device->GetBroadcast (),
                                                       Ipv6L3Protocol::PROT_NUMBER, hdr));
    }
}

...
{
  static TypeId tid =
      TypeId ("ns3::TcpScalable")
          .SetParent<TcpNewReno> ()
          .AddConstructor<TcpScalable> ()
          .SetGroupName ("Internet")
          .AddAttribute ("AIFactor", "Additive Increase Factor", UintegerValue (50),
                         MakeUintegerAccessor (&TcpScalable::m_aiFactor),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("MDFactor", "Multiplicative Decrease Factor", DoubleValue (0.125),
                         MakeDoubleAccessor (&TcpScalable::m_mdFactor),
                         MakeDoubleChecker<double> ());
  return tid;
}

TcpScalable::TcpScalable () : TcpNewReno (), m_ackCnt (0), m_aiFactor (50), m_mdFactor (0.125)
{

...
public:
  ...
  /**
     ...
     */
  inline uint8_t
  ClearEcnBits (uint8_t tos) const
  {
    return tos & 0xfc;
  }
  ...
  /**
     ...
     */
  inline bool
  CheckEcnEct0 (uint8_t tos) const
  {
  ...
  /**
     ...
     */
  inline bool
  CheckEcnEct1 (uint8_t tos) const
  {
  ...
  /**
     ...
     */
  inline uint8_t
  MarkEcnCodePoint (const uint8_t tos, const TcpSocketState::EcnCodePoint_t codePoint) const
  {
  ...
  int
  Connect (const Address &address) override; // Setup endpoint and call ProcessAction() to connect
  int
  Listen () override; // Verify the socket is in a correct state and call ProcessAction() to listen
  ...
  RecvFrom (uint32_t maxSize, uint32_t flags,
            Address &fromAddress) override; // ... and write the remote address at fromAddress
  uint32_t GetTxAvailable () const override; // Available Tx buffer size
  uint32_t
  GetRxAvailable () const override; // Available-to-read data size, i.e. value of m_rxAvailable

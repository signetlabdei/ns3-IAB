...
TcpHybla::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::TcpHybla")
                          ...
                          .AddAttribute ("RRTT", "Reference RTT", TimeValue (MilliSeconds (50)),
                                         MakeTimeAccessor (&TcpHybla::m_rRtt), MakeTimeChecker ())
                          .AddTraceSource ("Rho", "Rho parameter of Hybla",
                                           MakeTraceSourceAccessor (&TcpHybla::m_rho),
                                           "ns3::TracedValueCallback::Double");
  return tid;
}

TcpHybla::TcpHybla () : TcpNewReno (), m_rho (1.0), m_cWndCnt (0)
{
  NS_LOG_FUNCTION (this);
}

TcpHybla::TcpHybla (const TcpHybla &sock)
    : TcpNewReno (sock), m_rho (sock.m_rho), m_cWndCnt (sock.m_cWndCnt)
{
  NS_LOG_FUNCTION (this);
}

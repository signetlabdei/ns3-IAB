...
void
TcpHeader::InitializeChecksum (const Ipv4Address &source, const Ipv4Address &destination,
                               uint8_t protocol)
{
  m_source = source;
...
void
TcpHeader::InitializeChecksum (const Ipv6Address &source, const Ipv6Address &destination,
                               uint8_t protocol)
{
  m_source = source;
  m_destination = destination;
  ...
  if (m_optionsLen + option->GetSerializedSize () <= m_maxOptionsLen)
    {
      if (!TcpOption::IsKindKnown (option->GetKind ()))
        {
          NS_LOG_WARN ("The option kind " << static_cast<int> (option->GetKind ())
                                          << " is unknown");
          return false;
        }
...
{
  return (lhs.m_sourcePort == rhs.m_sourcePort && lhs.m_destinationPort == rhs.m_destinationPort &&

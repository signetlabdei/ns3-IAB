  ...
  static TypeId tid =
      TypeId ("ns3::TcpYeah")
          ...
          .AddAttribute ("Alpha", "Maximum backlog allowed at the bottleneck queue",
                         UintegerValue (80), MakeUintegerAccessor (&TcpYeah::m_alpha),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Gamma", "Fraction of queue to be removed per RTT", UintegerValue (1),
                         MakeUintegerAccessor (&TcpYeah::m_gamma), MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Delta", "Log minimum fraction of cwnd to be removed on loss",
                         UintegerValue (3), MakeUintegerAccessor (&TcpYeah::m_delta),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Epsilon", "Log maximum fraction to be removed on early decongestion",
                         UintegerValue (1), MakeUintegerAccessor (&TcpYeah::m_epsilon),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Phy", "Maximum delta from base", UintegerValue (8),
                         MakeUintegerAccessor (&TcpYeah::m_phy), MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Rho", "Minimum # of consecutive RTT to consider competition on loss",
                         UintegerValue (16), MakeUintegerAccessor (&TcpYeah::m_rho),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Zeta", "Minimum # of state switches to reset m_renoCount",
                         UintegerValue (50), MakeUintegerAccessor (&TcpYeah::m_zeta),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("StcpAiFactor", "STCP additive increase factor", UintegerValue (100),

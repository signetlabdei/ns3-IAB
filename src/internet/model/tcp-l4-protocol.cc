...
TcpL4Protocol::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::TcpL4Protocol")
          ...
          .AddAttribute ("RttEstimatorType", "Type of RttEstimator objects.",
                         TypeIdValue (RttMeanDeviation::GetTypeId ()),
                         MakeTypeIdAccessor (&TcpL4Protocol::m_rttTypeId), MakeTypeIdChecker ())
          .AddAttribute (
              "SocketType", "Socket type of TCP objects.", TypeIdValue (TcpCubic::GetTypeId ()),
              MakeTypeIdAccessor (&TcpL4Protocol::m_congestionTypeId), MakeTypeIdChecker ())
          .AddAttribute ("RecoveryType", "Recovery type of TCP objects.",
                         TypeIdValue (TcpPrrRecovery::GetTypeId ()),
                         MakeTypeIdAccessor (&TcpL4Protocol::m_recoveryTypeId),
                         MakeTypeIdChecker ())
          .AddAttribute ("SocketList", "The list of sockets associated to this protocol.",
                         ObjectVectorValue (), MakeObjectVectorAccessor (&TcpL4Protocol::m_sockets),
...
Ipv4EndPoint *
TcpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, Ipv4Address localAddress,
...
Ipv6EndPoint *
TcpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice, Ipv6Address localAddress,
...
void
TcpL4Protocol::ReceiveIcmp (Ipv4Address icmpSource, uint8_t icmpTtl, uint8_t icmpType,
...
void
TcpL4Protocol::ReceiveIcmp (Ipv6Address icmpSource, uint8_t icmpTtl, uint8_t icmpType,
...
void
TcpL4Protocol::NoEndPointsFound (const TcpHeader &incomingHeader, const Address &incomingSAddr,
...
enum IpL4Protocol::RxStatus
TcpL4Protocol::Receive (Ptr<Packet> packet, const Ipv4Header &incomingIpHeader,
                        Ptr<Ipv4Interface> incomingInterface)
{
  ...
  checksumControl = PacketReceived (packet, incomingTcpHeader, incomingIpHeader.GetSource (),
  ...
  endPoints = m_endPoints->Lookup (
      incomingIpHeader.GetDestination (), incomingTcpHeader.GetDestinationPort (),
      incomingIpHeader.GetSource (), incomingTcpHeader.GetSourcePort (), incomingInterface);
...
enum IpL4Protocol::RxStatus
TcpL4Protocol::Receive (Ptr<Packet> packet, const Ipv6Header &incomingIpHeader,
                        Ptr<Ipv6Interface> interface)
{
  ...
  Ipv6EndPointDemux::EndPoints endPoints = m_endPoints6->Lookup (
      incomingIpHeader.GetDestination (), incomingTcpHeader.GetDestinationPort (),
      incomingIpHeader.GetSource (), incomingTcpHeader.GetSourcePort (), interface);
  if (endPoints.empty ())
    {
      NS_LOG_LOGIC ("TcpL4Protocol "
                    << this
...
void
TcpL4Protocol::SendPacket (Ptr<Packet> pkt, const TcpHeader &outgoing, const Address &saddr,
                           const Address &daddr, Ptr<NetDevice> oif) const
{

...
Icmpv6L4Protocol::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::Icmpv6L4Protocol")
          ...
          .AddAttribute ("DAD", "Always do DAD check.", BooleanValue (true),
          ...
          .AddAttribute (
              ...
              IntegerValue (3), MakeIntegerAccessor (&Icmpv6L4Protocol::m_maxUnicastSolicit),
              MakeIntegerChecker<uint8_t> ())
          .AddAttribute ("ReachableTime", "Neighbor Discovery node constants: reachable time.",
                         TimeValue (Seconds (30)),
                         MakeTimeAccessor (&Icmpv6L4Protocol::m_reachableTime), MakeTimeChecker ())
          .AddAttribute (
              "RetransmissionTime", "Neighbor Discovery node constants: retransmission timer.",
              TimeValue (Seconds (1)), MakeTimeAccessor (&Icmpv6L4Protocol::m_retransmissionTime),
              MakeTimeChecker ())
          .AddAttribute (
              "DelayFirstProbe", "Neighbor Discovery node constants: delay for the first probe.",
              TimeValue (Seconds (5)), MakeTimeAccessor (&Icmpv6L4Protocol::m_delayFirstProbe),
              MakeTimeChecker ())
          .AddAttribute ("DadTimeout", "Duplicate Address Detection (DAD) timeout",
                         TimeValue (Seconds (1)),
                         MakeTimeAccessor (&Icmpv6L4Protocol::m_dadTimeout), MakeTimeChecker ())
...
Icmpv6L4Protocol::Icmpv6L4Protocol () : m_node (nullptr)
{
  NS_LOG_FUNCTION (this);
}
...
Icmpv6L4Protocol::DoDAD (Ipv6Address target, Ptr<Ipv6Interface> interface)
{
  ...
  NdiscCache::Ipv6PayloadHeaderPair p = ForgeNS ("::", Ipv6Address::MakeSolicitedAddress (target),
                                                 target, interface->GetDevice ()->GetAddress ());
...
Icmpv6L4Protocol::Receive (Ptr<Packet> packet, const Ipv4Header &header,
...
enum IpL4Protocol::RxStatus
Icmpv6L4Protocol::Receive (Ptr<Packet> packet, const Ipv6Header &header,
...
void
Icmpv6L4Protocol::Forward (Ipv6Address source, Icmpv6Header icmp, uint32_t info,
  ...
  if (nextHeader != Icmpv6L4Protocol::PROT_NUMBER)
    {
      ...
      if (l4)
        {
          l4->ReceiveIcmp (source, ipHeader.GetHopLimit (), icmp.GetType (), icmp.GetCode (), info,
                           ipHeader.GetSource (), ipHeader.GetDestination (), payload);
        }
    }
}

void
Icmpv6L4Protocol::HandleEchoRequest (Ptr<Packet> packet, const Ipv6Address &src,
                                     const Ipv6Address &dst, Ptr<Ipv6Interface> interface)
{
  ...
  SendEchoReply (dst.IsMulticast () ? interface->GetLinkLocalAddress ().GetAddress () : dst, src,
                 request.GetId (), request.GetSeq (), p);
  delete[] buf;
}

void
Icmpv6L4Protocol::HandleRA (Ptr<Packet> packet, const Ipv6Address &src, const Ipv6Address &dst,
                            Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
...
void
Icmpv6L4Protocol::ReceiveLLA (Icmpv6OptionLinkLayerAddress lla, const Ipv6Address &src,
                              const Ipv6Address &dst, Ptr<Ipv6Interface> interface)
{
...
void
Icmpv6L4Protocol::HandleRS (Ptr<Packet> packet, const Ipv6Address &src, const Ipv6Address &dst,
                            Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  Ptr<Ipv6L3Protocol> ipv6 = m_node->GetObject<Ipv6L3Protocol> ();
...
void
Icmpv6L4Protocol::HandleNS (Ptr<Packet> packet, const Ipv6Address &src, const Ipv6Address &dst,
                            Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  Icmpv6NS nsHeader ("::");
  ...
  if (!src.IsAny ())
    {
      Icmpv6OptionLinkLayerAddress llOption (
          1, hardwareAddress); /* we give our mac address in response */
...
NdiscCache::Ipv6PayloadHeaderPair
Icmpv6L4Protocol::ForgeEchoRequest (Ipv6Address src, Ipv6Address dst, uint16_t id, uint16_t seq,
                                    Ptr<Packet> data)
{
...
void
Icmpv6L4Protocol::HandleNA (Ptr<Packet> packet, const Ipv6Address &src, const Ipv6Address &dst,
                            Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  Icmpv6NA naHeader;
...
void
Icmpv6L4Protocol::HandleRedirection (Ptr<Packet> packet, const Ipv6Address &src,
                                     const Ipv6Address &dst, Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << packet << src << dst << interface);
  bool hasLla = false;
  ...
  if (redirTarget == redirDestination)
    {
      ipv6->GetRoutingProtocol ()->NotifyAddRoute (redirDestination, Ipv6Prefix (128),
                                                   Ipv6Address ("::"),
  ...
  else
    {
      ...
      ipv6->GetRoutingProtocol ()->NotifyAddRoute (redirDestination, Ipv6Prefix (128), redirTarget,
                                                   ifIndex);
    }
}

void
Icmpv6L4Protocol::HandleDestinationUnreachable (Ptr<Packet> p, const Ipv6Address &src,
...
void
Icmpv6L4Protocol::HandleTimeExceeded (Ptr<Packet> p, const Ipv6Address &src, const Ipv6Address &dst,
                                      Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << *p << src << dst << interface);
  Ptr<Packet> pkt = p->Copy ();
...
void
Icmpv6L4Protocol::HandlePacketTooBig (Ptr<Packet> p, const Ipv6Address &src, const Ipv6Address &dst,
                                      Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << *p << src << dst << interface);
  Ptr<Packet> pkt = p->Copy ();
...
void
Icmpv6L4Protocol::HandleParameterError (Ptr<Packet> p, const Ipv6Address &src,
                                        const Ipv6Address &dst, Ptr<Ipv6Interface> interface)
{
  NS_LOG_FUNCTION (this << *p << src << dst << interface);
  Ptr<Packet> pkt = p->Copy ();
...
void
Icmpv6L4Protocol::DelayedSendMessage (Ptr<Packet> packet, Ipv6Address src, Ipv6Address dst,
                                      uint8_t ttl)
{
  NS_LOG_FUNCTION (this << packet << src << dst << (uint32_t) ttl);
...
void
Icmpv6L4Protocol::SendMessage (Ptr<Packet> packet, Ipv6Address dst, Icmpv6Header &icmpv6Hdr,
                               uint8_t ttl)
{
  ...
  if (route)
    {
      ...
      icmpv6Hdr.CalculatePseudoHeaderChecksum (
          src, dst, packet->GetSize () + icmpv6Hdr.GetSerializedSize (), PROT_NUMBER);
...
void
Icmpv6L4Protocol::SendEchoReply (Ipv6Address src, Ipv6Address dst, uint16_t id, uint16_t seq,
                                 Ptr<Packet> data)
{
  NS_LOG_FUNCTION (this << src << dst << id << seq << data);
  Ptr<Packet> p = data->Copy ();
...
void
Icmpv6L4Protocol::SendNS (Ipv6Address src, Ipv6Address dst, Ipv6Address target,
                          Address hardwareAddress)
{
  ...
  /* Ipv6Header ipHeader; */
  Icmpv6NS ns (target);
  Icmpv6OptionLinkLayerAddress llOption (1,
  ...
  if (!dst.IsMulticast ())
  ...
  else
    {
      ...
      Simulator::Schedule (Time (MilliSeconds (m_solicitationJitter->GetValue ())),
                           &Icmpv6L4Protocol::DelayedSendMessage, this, p, src, dst, 255);
    }
}
  ...
  else
    {
      ...
      m_handleRsTimeoutEvent =
          Simulator::Schedule (rsDelay + m_rsPrevRetransmissionTimeout,
                               &Icmpv6L4Protocol::HandleRsTimeout, this, src, dst, hardwareAddress);
    }
}
...
void
Icmpv6L4Protocol::SendErrorDestinationUnreachable (Ptr<Packet> malformedPacket, Ipv6Address dst,
                                                   uint8_t code)
{
...
void
Icmpv6L4Protocol::SendErrorParameterError (Ptr<Packet> malformedPacket, Ipv6Address dst,
...
void
Icmpv6L4Protocol::SendRedirection (Ptr<Packet> redirectedPacket, Ipv6Address src, Ipv6Address dst,
                                   Ipv6Address redirTarget, Ipv6Address redirDestination,
                                   Address redirHardwareTarget)
{
  ...
  redirectionHeader.CalculatePseudoHeaderChecksum (
      src, dst, p->GetSize () + redirectionHeader.GetSerializedSize (), PROT_NUMBER);
...
NdiscCache::Ipv6PayloadHeaderPair
Icmpv6L4Protocol::ForgeNA (Ipv6Address src, Ipv6Address dst, Address *hardwareAddress,
                           uint8_t flags)
{
  ...
  Icmpv6NA na;
  Icmpv6OptionLinkLayerAddress llOption (
      0, *hardwareAddress); /* we give our mac address in response */
...
NdiscCache::Ipv6PayloadHeaderPair
Icmpv6L4Protocol::ForgeNS (Ipv6Address src, Ipv6Address dst, Ipv6Address target,
                           Address hardwareAddress)
{
  NS_LOG_FUNCTION (this << src << dst << target << hardwareAddress);
  Ptr<Packet> p = Create<Packet> ();
...
bool
Icmpv6L4Protocol::Lookup (Ipv6Address dst, Ptr<NetDevice> device, Ptr<NdiscCache> cache,
...
bool
Icmpv6L4Protocol::Lookup (Ptr<Packet> p, const Ipv6Header &ipHeader, Ipv6Address dst,
                          Ptr<NetDevice> device, Ptr<NdiscCache> cache,
                          Address *hardwareDestination)
{
  ...
  if (found && ifaddr.GetState () != Ipv6InterfaceAddress::INVALID)
    {
      ...
      if (!ipv6->IsForwarding (ipv6->GetInterfaceForDevice (interface->GetDevice ())) &&
          addr.IsLinkLocal ())
        {
          ...
          m_rsRetransmissionCount = 0;
          Simulator::Schedule (Seconds (0.0), &Icmpv6L4Protocol::SendRS, this, ifaddr.GetAddress (),

...
GlobalRoutingLinkRecord::GlobalRoutingLinkRecord ()
    : m_linkId ("0.0.0.0"), m_linkData ("0.0.0.0"), m_linkType (Unknown), m_metric (0)
{
  NS_LOG_FUNCTION (this);
}

GlobalRoutingLinkRecord::GlobalRoutingLinkRecord (LinkType linkType, Ipv4Address linkId,
                                                  Ipv4Address linkData, uint16_t metric)
    : m_linkId (linkId), m_linkData (linkData), m_linkType (linkType), m_metric (metric)
{
...
GlobalRoutingLSA::GlobalRoutingLSA (GlobalRoutingLSA::SPFStatus status, Ipv4Address linkStateId,
...
void
GlobalRouter::ProcessSingleBroadcastLink (Ptr<NetDevice> nd, GlobalRoutingLSA *pLSA,
                                          NetDeviceContainer &c)
{
  NS_LOG_FUNCTION (this << nd << pLSA << &c);
...
void
GlobalRouter::ProcessBridgedBroadcastLink (Ptr<NetDevice> nd, GlobalRoutingLSA *pLSA,
                                           NetDeviceContainer &c)
{
  NS_LOG_FUNCTION (this << nd << pLSA << &c);
...
GlobalRouter::ProcessPointToPointLink (Ptr<NetDevice> ndLocal, GlobalRoutingLSA *pLSA)
{
  ...
  NS_ABORT_MSG_UNLESS (
      ipv4Local, "GlobalRouter::ProcessPointToPointLink (): GetObject for <Ipv4> interface failed");
  ...
  NS_ABORT_MSG_UNLESS (
      ipv4Remote, "GlobalRouter::ProcessPointToPointLink(): GetObject for remote <Ipv4> failed");
      ...
      if (ndTest->IsBridge ())
        {
          ...
          NS_ABORT_MSG_UNLESS (
              bnd, "GlobalRouter::DiscoverLSAs (): GetObject for <BridgeNetDevice> failed");

  ...
  static TypeId tid =
      TypeId ("ns3::UdpSocketImpl")
          ...
          .AddAttribute ("IcmpCallback",
                         ...
                         CallbackValue (), MakeCallbackAccessor (&UdpSocketImpl::m_icmpCallback),
                         MakeCallbackChecker ())
          .AddAttribute ("IcmpCallback6",
                         "Callback invoked whenever an icmpv6 error is received on this socket.",
                         CallbackValue (), MakeCallbackAccessor (&UdpSocketImpl::m_icmpCallback6),
                         MakeCallbackChecker ());
  return tid;
}
...
UdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv4Address dest, uint16_t port, uint8_t tos)
{
  ...
  if (dest.IsBroadcast ())
    ...
    {
      m_udp->Send (p->Copy (), m_endPoint->GetLocalAddress (), dest, m_endPoint->GetLocalPort (),
                   port, nullptr);
      NotifyDataSent (p->GetSize ());
      NotifySend (GetTxAvailable ());
      return p->GetSize ();
...
UdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv6Address dest, uint16_t port)
{
  ...
  if (m_endPoint6->GetLocalAddress () != Ipv6Address::GetAny ())
    {
      m_udp->Send (p->Copy (), m_endPoint6->GetLocalAddress (), dest, m_endPoint6->GetLocalPort (),
                   port, nullptr);
      NotifyDataSent (p->GetSize ());
      NotifySend (GetTxAvailable ());
      return p->GetSize ();
...
void
UdpSocketImpl::ForwardIcmp (Ipv4Address icmpSource, uint8_t icmpTtl, uint8_t icmpType,
...
void
UdpSocketImpl::ForwardIcmp6 (Ipv6Address icmpSource, uint8_t icmpTtl, uint8_t icmpType,
                             uint8_t icmpCode, uint32_t icmpInfo)
{
  NS_LOG_FUNCTION (this << icmpSource << (uint32_t) icmpTtl << (uint32_t) icmpType
                        << (uint32_t) icmpCode << icmpInfo);
...
void
UdpSocketImpl::Ipv6JoinGroup (Ipv6Address address, Socket::Ipv6MulticastFilterMode filterMode,

  ...
  static TypeId tid =
      TypeId ("ns3::Rip")
          ...
          .AddAttribute ("UnsolicitedRoutingUpdate",
                         ...
                         TimeValue (Seconds (30)), MakeTimeAccessor (&Rip::m_unsolicitedUpdate),
                         MakeTimeChecker ())
          .AddAttribute (
              "StartupDelay", "Maximum random delay for protocol startup (send route requests).",
              TimeValue (Seconds (1)), MakeTimeAccessor (&Rip::m_startupDelay), MakeTimeChecker ())
          .AddAttribute ("TimeoutDelay", "The delay to invalidate a route.",
                         TimeValue (Seconds (180)), MakeTimeAccessor (&Rip::m_timeoutDelay),
                         MakeTimeChecker ())
          .AddAttribute ("GarbageCollectionDelay", "The delay to delete an expired route.",
                         TimeValue (Seconds (120)),
                         MakeTimeAccessor (&Rip::m_garbageCollectionDelay), MakeTimeChecker ())
          .AddAttribute ("MinTriggeredCooldown", "Min cooldown delay after a Triggered Update.",
                         TimeValue (Seconds (1)),
                         MakeTimeAccessor (&Rip::m_minTriggeredUpdateDelay), MakeTimeChecker ())
          .AddAttribute ("MaxTriggeredCooldown", "Max cooldown delay after a Triggered Update.",
                         TimeValue (Seconds (5)),
                         MakeTimeAccessor (&Rip::m_maxTriggeredUpdateDelay), MakeTimeChecker ())
          .AddAttribute ("SplitHorizon", "Split Horizon strategy.", EnumValue (Rip::POISON_REVERSE),
                         MakeEnumAccessor (&Rip::m_splitHorizonStrategy),
                         MakeEnumChecker (Rip::NO_SPLIT_HORIZON, "NoSplitHorizon",
                                          Rip::SPLIT_HORIZON, "SplitHorizon", Rip::POISON_REVERSE,
                                          "PoisonReverse"))
          .AddAttribute ("LinkDownValue", "Value for link down in count to infinity.",
                         UintegerValue (16), MakeUintegerAccessor (&Rip::m_linkDown),
...
Ptr<Ipv4Route>
Rip::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif,
...
bool
Rip::RouteInput (Ptr<const Packet> p, const Ipv4Header &header, Ptr<const NetDevice> idev,
...
void
Rip::AddNetworkRouteTo (Ipv4Address network, Ipv4Mask networkPrefix, Ipv4Address nextHop,
                        uint32_t interface)
{
...
void
Rip::HandleRequests (RipHeader requestHdr, Ipv4Address senderAddress, uint16_t senderPort,
              ...
              for (RoutesI rtIter = m_routes.begin (); rtIter != m_routes.end (); rtIter++)
                {
                  ...
                  Ipv4InterfaceAddress rtDestAddr = Ipv4InterfaceAddress (
                      rtIter->first->GetDestNetwork (), rtIter->first->GetDestNetworkMask ());
...
void
Rip::HandleResponses (RipHeader hdr, Ipv4Address senderAddress, uint32_t incomingInterface,
                      uint8_t hopLimit)
{
              ...
              if (rteMetric < it->first->GetRouteMetric ())
              ...
              else if (rteMetric == it->first->GetRouteMetric ())
                {
                  if (senderAddress == it->first->GetGateway ())
                    {
                      it->second.Cancel ();
                      it->second = Simulator::Schedule (m_timeoutDelay, &Rip::InvalidateRoute, this,
                                                        it->first);
                    }
                  else
                    {
                      if (Simulator::GetDelayLeft (it->second) < m_timeoutDelay / 2)
                        {
                          ...
                          it->first = route;
                          it->second.Cancel ();
                          it->second = Simulator::Schedule (m_timeoutDelay, &Rip::InvalidateRoute,
                                                            this, route);
                          changed = true;
                        }
                    }
...
RipRoutingTableEntry::RipRoutingTableEntry (Ipv4Address network, Ipv4Mask networkPrefix,

...
GlobalRouteManagerLSDB::GlobalRouteManagerLSDB () : m_database (), m_extdatabase ()
{
  NS_LOG_FUNCTION (this);
}
...
//
int
GlobalRouteManagerImpl::SPFNexthopCalculation (SPFVertex *v, SPFVertex *w,
              ...
              if (lr->GetLinkId () == myRouterId)
                {
                  ...
                  Ptr<Ipv4GlobalRouting> gr = router->GetRoutingProtocol ();
                  NS_ASSERT (gr);
                  gr->AddNetworkRouteTo (Ipv4Address ("0.0.0.0"), Ipv4Mask ("0.0.0.0"),
                                         lr->GetLinkData (),
      ...
      if (rtr->GetRouterId () == routerId)
        {
          ...
          //
          NS_ASSERT_MSG (v->GetLSA (), "GlobalRouteManagerImpl::SPFIntraAddRouter (): "

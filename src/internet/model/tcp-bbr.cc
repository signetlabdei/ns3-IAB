  ...
  static TypeId tid =
      TypeId ("ns3::TcpBbr")
          ...
          .AddAttribute ("Stream",
                         ...
                         UintegerValue (4), MakeUintegerAccessor (&TcpBbr::SetStream),
          ...
          .AddAttribute ("BwWindowLength", "Length of bandwidth windowed filter",
                         ...
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("RttWindowLength", "Length of RTT windowed filter",
                         TimeValue (Seconds (10)), MakeTimeAccessor (&TcpBbr::m_rtPropFilterLen),
                         MakeTimeChecker ())
          .AddAttribute ("ProbeRttDuration", "Time to be spent in PROBE_RTT phase",
                         TimeValue (MilliSeconds (200)),
                         MakeTimeAccessor (&TcpBbr::m_probeRttDuration), MakeTimeChecker ())
          .AddAttribute ("ExtraAckedRttWindowLength", "Window length of extra acked window",
                         UintegerValue (5),
...
const char *const TcpBbr::BbrModeName[BBR_PROBE_RTT + 1] = {"BBR_STARTUP", "BBR_DRAIN",
...
TcpBbr::InitPacingRate (Ptr<TcpSocketState> tcb)
{
  ...
  m_maxBwFilter = MaxBandwidthFilter_t (m_bandwidthWindowLength,
                                        DataRate (tcb->m_cWnd * 8 / rtt.GetSeconds ()), 0);
}

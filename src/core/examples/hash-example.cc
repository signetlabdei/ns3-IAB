...
private:
   ...
   */
  uint64_t
  GetHash (const std::string phrase)
  {
    m_hash.clear ();
    uint64_t h = 0;
...
public:
   ...
   */
  void
  ReportExpectedCollisions () const
  {
   ...
   */
  void
  TimeOne (const Collider &collider)
  {
    // Hashing speed
    uint32_t reps = 100;
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("dict", "Dictionary file to hash", MakeCallback (&DictFiles::Add, &files),

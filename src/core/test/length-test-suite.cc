...
protected:
   ...
   */
  void
  AssertFalse (bool condition, std::string msg)
  {
   ...
   */
  void
  AssertTrue (bool condition, std::string msg)
  {
...
private:
   ...
   */
  void TestConstructLengthFromString (double unitValue, double meterValue, double tolerance,
   ...
   */
  template <class T>
  void TestLengthSerialization (const Length &l, const T &unit, const std::string &expectedOutput,
...
void
LengthTestCase::TestConstructLengthFromString (double unitValue, double meterValue,
                                               double tolerance,
                                               const std::initializer_list<std::string> &symbols)
{
...
template <class T>
void
LengthTestCase::TestLengthSerialization (const Length &l, const T &unit,
...
{
  static TypeId tid =
      TypeId ("LengthValueTestCase::TestObject")
          .SetParent<Object> ()
          .SetGroupName ("Test")
          .AddConstructor<TestObject> ()
          .AddAttribute ("Length", "Length value", LengthValue (),
                         MakeLengthAccessor (&TestObject::m_length), MakeLengthChecker ());
...
LengthTestSuite::LengthTestSuite () : TestSuite ("length")
{

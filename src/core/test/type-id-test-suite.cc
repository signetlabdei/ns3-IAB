...
UniqueTypeIdTestCase::UniqueTypeIdTestCase () : TestCase ("Check uniqueness of all TypeIds")
{
}
  ...
  for (uint16_t i = 0; i < nids; ++i)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (tid.GetUid (), TypeId::LookupByName (tid.GetName ()).GetUid (),
      ...
      NS_TEST_ASSERT_MSG_EQ (tid.GetUid (), TypeId::LookupByHash (tid.GetHash ()).GetUid (),
...
CollisionTestCase::CollisionTestCase () : TestCase ("Check behavior when type names collide")
{
}
  ...
  GetTypeId (void)
  {
    static TypeId tid =
        TypeId ("DeprecatedAttribute")
            ...
            .AddAttribute ("obsoleteAttribute", "the obsolete attribute", EmptyAttributeValue (),
                           MakeEmptyAttributeAccessor (), MakeEmptyAttributeChecker (),
                           TypeId::OBSOLETE, "refactor to use 'attribute'")

            // The new trace source
            .AddTraceSource ("trace", "the TraceSource",
                             MakeTraceSourceAccessor (&DeprecatedAttribute::m_trace),
                             "ns3::TracedValueCallback::Double")
            // The old trace source
            .AddTraceSource ("oldTrace", "the old trace source",
                             MakeTraceSourceAccessor (&DeprecatedAttribute::m_trace),
            ...
            .AddTraceSource ("obsoleteTraceSource", "the obsolete trace source",
                             MakeEmptyTraceSourceAccessor (), "ns3::TracedValueCallback::Void",
                             TypeId::OBSOLETE, "refactor to use 'trace'");
...
LookupTimeTestCase::LookupTimeTestCase () : TestCase ("Measure average lookup time")
{
}
...
TypeIdPerformanceSuite::TypeIdPerformanceSuite () : TestSuite ("type-id-perf", PERFORMANCE)
{

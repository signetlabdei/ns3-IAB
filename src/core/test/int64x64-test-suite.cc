...
public:
  ...
  Printer (const int64x64_t value)
      : m_haveInt (true), m_value (value), m_high (value.GetHigh ()), m_low (value.GetLow ())
  {
  }
...
Int64x64IntRoundTestCase::Int64x64IntRoundTestCase (void) : TestCase ("Check GetInt and Round")
{
}

void
Int64x64IntRoundTestCase::Check (const int64x64_t value, const int64_t expectInt,
...
Int64x64InputTestCase::Int64x64InputTestCase () : TestCase ("Parse int64x64_t numbers as strings")
{
}
void
Int64x64InputTestCase::Check (const std::string &str, const int64_t hi, const uint64_t lo,
                              const int64_t tolerance /* = 0 */)
...
Int64x64ArithmeticTestCase::Int64x64ArithmeticTestCase () : TestCase ("Basic arithmetic operations")
{
}
void
Int64x64ArithmeticTestCase::Check (const int test, const int64x64_t value, const int64x64_t expect,
...
Int64x64Bug455TestCase::Int64x64Bug455TestCase () : TestCase ("Test case for bug 455")
{
}
void
...
Int64x64Bug863TestCase::Int64x64Bug863TestCase () : TestCase ("Test case for bug 863")
{
}
void
...
Int64x64Bug1786TestCase::Int64x64Bug1786TestCase () : TestCase ("Test case for bug 1786")
{
}
void
...
Int64x64CompareTestCase::Int64x64CompareTestCase () : TestCase ("Basic compare operations")
{
}
void
...
public:
   ...
   * \param tolerance The allowed tolerance.
   */
  void CheckCase (const uint64_t factor, const int64x64_t result, const int64x64_t expect,
                  const std::string &msg, const double tolerance = 0);
};

Int64x64InvertTestCase::Int64x64InvertTestCase () : TestCase ("Invert and MulByInvert")
{
}
void
Int64x64InvertTestCase::CheckCase (const uint64_t factor, const int64x64_t result,
...
public:
   ...
   */
  void Check (const long double dec, const long double frac, const int64_t intPart,
              const uint64_t lo);
...
Int64x64DoubleTestCase::Int64x64DoubleTestCase ()
    : TestCase ("Construct from floating point."), m_last{0}, m_deltaMax{0}, m_deltaCount{0}
{
}

void
Int64x64DoubleTestCase::Check (const long double dec, const long double frac, const int64_t intPart,
                               const uint64_t lo)
{
  ...
  */
  ...
  std::cout << GetParent ()->GetName ()
            << " Double: " << (skip ? "skip " : (pass ? "pass " : "FAIL ")) << std::showpos << value
            << " == " << Printer (result) << (under ? " (underflow)" : "") << std::endl;
...
Int64x64ImplTestCase::Int64x64ImplTestCase () : TestCase ("Print the implementation")
{
}

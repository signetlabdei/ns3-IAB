...
TimeSimpleTestCase::TimeSimpleTestCase () : TestCase ("Sanity check of common time operations")
{
}
...
void
TimeSimpleTestCase::DoTimeOperations (void)
{
  // Test Multiplication
...
TimeWithSignTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL (timePositive.GetSeconds (), +1000.0, 1.0e-8,
                             "Positive time not parsed correctly.");

  NS_TEST_ASSERT_MSG_EQ_TOL (timePositiveWithUnits.GetSeconds (), +1.0, 1.0e-8,
                             "Positive time with units not parsed correctly.");

  NS_TEST_ASSERT_MSG_EQ_TOL (timeNegative.GetSeconds (), -1000.0, 1.0e-8,
                             "Negative time not parsed correctly.");

  NS_TEST_ASSERT_MSG_EQ_TOL (timeNegativeWithUnits.GetSeconds (), -1.0, 1.0e-8,
...
TimeInputOutputTestCase::TimeInputOutputTestCase () : TestCase ("Input,output from,to strings")
{
}

...
Time::ConvertTimes (const enum Unit unit)
{
  ...
  NS_ASSERT_MSG (g_markingTimes != 0, "No MarkedTimes registry. "
...
MakeTimeChecker (const Time min, const Time max)
{
  ...
  struct Checker : public AttributeChecker
  {
    ...
    virtual bool
    Check (const AttributeValue &value) const
    {
    ...
    virtual std::string
    GetValueTypeName (void) const
    {
      NS_LOG_FUNCTION_NOARGS ();
      return "ns3::TimeValue";
    }
    virtual bool
    HasUnderlyingTypeInformation (void) const
    {
      NS_LOG_FUNCTION_NOARGS ();
      return true;
    }
    virtual std::string
    GetUnderlyingTypeInformation (void) const
    {
      NS_LOG_FUNCTION_NOARGS ();
    ...
    virtual bool
    Copy (const AttributeValue &source, AttributeValue &destination) const
    {

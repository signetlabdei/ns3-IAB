...
template <typename T>
Ptr<T>::Ptr (T *ptr, bool ref) : m_ptr (ptr)
{
  if (ref)
    {
      Acquire ();
...
template <typename T>
Ptr<T>::Ptr (Ptr const &o) : m_ptr (nullptr)
{
  T *ptr = PeekPointer (o);
  if (ptr != 0)
    {
...
template <typename T>
template <typename U>
Ptr<T>::Ptr (Ptr<U> const &o) : m_ptr (PeekPointer (o))
{
  Acquire ();
}

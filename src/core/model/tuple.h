  ...
  while (iss >> value)
    {
      ...
      values.push_back (
          tupleChecker->GetCheckers ().at (i++)->CreateValidValue (StringValue (value)));
      if (!values.back ())
        {
          return false;
        }
...
public:
  ...
  const std::vector<Ptr<const AttributeChecker>> &
  GetCheckers (void) const override
  {
    return m_checkers;
  }
  bool
...
struct TupleHelper
{
   ...
   */
  template <class... Ts>
  static Ptr<const AttributeChecker>

...
NameNode::NameNode () : m_parent (0), m_name (""), m_object (0)
{
}
...
NamesPriv::Clear (void)
{
  ...
  //
  for (std::map<Ptr<Object>, NameNode *>::iterator i = m_objectMap.begin ();
       i != m_objectMap.end (); ++i)
    {
      delete i->second;
      i->second = 0;
...
Names::Rename (std::string path, std::string oldname, std::string newname)
{
  ...
  NS_ABORT_MSG_UNLESS (result, "Names::Rename (): Error renaming " << path << " " << oldname
                                                                   << " to " << newname);
}
...
Names::Add (Ptr<Object> context, std::string name, Ptr<Object> object)
{
  ...
  NS_ABORT_MSG_UNLESS (result,
                       "Names::Add(): Error adding name " << name << " under context " << &context);
}
...
Names::Rename (Ptr<Object> context, std::string oldname, std::string newname)
{
  ...
  NS_ABORT_MSG_UNLESS (result, "Names::Rename (): Error renaming " << oldname << " to " << newname
                                                                   << " under context "
                                                                   << &context);
}

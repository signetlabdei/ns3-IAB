...
MatchContainer::MatchContainer (const std::vector<Ptr<Object>> &objects,
    ...
    : m_objects (objects), m_contexts (contexts), m_path (path)
{
...
ArrayMatcher::ArrayMatcher (std::string element) : m_element (element)
{
...
Resolver::Resolver (std::string path) : m_path (path)
{
...
ConfigImpl::LookupMatches (std::string path)
{
  ...
  {
  public:
    LookupMatchesResolver (std::string path) : Resolver (path)
    {
    }
    virtual void
    DoOne (Ptr<Object> object, std::string path)
    {

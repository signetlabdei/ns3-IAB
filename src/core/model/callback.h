...
public:
   ...
   */
  bool
  IsEqual (std::shared_ptr<const CallbackComponentBase> other) const
  {
...
public:
  ...
  virtual bool
  IsEqual (Ptr<const CallbackImplBase> other) const
  {
...
public:
   ...
   */
  Callback (Ptr<CallbackImpl<R, UArgs...>> const &impl) : CallbackBase (impl)
  {
  }
  ...
  {
    auto cbDerived =
        static_cast<CallbackImpl<R, BArgs..., UArgs...> const *> (PeekPointer (cb.GetImpl ()));
    ...
    m_impl = Create<CallbackImpl<R, UArgs...>> (
        [f, bargs...] (UArgs... uargs) -> R { return f (bargs..., uargs...); }, components);
  }
   ...
   */
  template <typename T, std::enable_if_t<!std::is_base_of_v<CallbackBase, T>, int> = 0,
            typename... BArgs>
...
private:
  ...
  auto
  BindImpl (std::index_sequence<INDEX...> seq, BoundArgs... bargs)
  {
...
public:
   ...
   */
  bool
  IsEqual (const CallbackBase &other) const
  {
   ...
   */
  bool
  CheckType (const CallbackBase &other) const
  {
   ...
   */
  bool
  Assign (const CallbackBase &other)
  {
...
private:
   ...
   */
  bool
  DoCheckType (Ptr<const CallbackImplBase> other) const
  {
    if (other && dynamic_cast<const CallbackImpl<R, UArgs...> *> (PeekPointer (other)) != 0)
      {
        return true;
      }
    else if (!other)

...
bool
ObjectBase::DoSet (Ptr<const AttributeAccessor> accessor, Ptr<const AttributeChecker> checker,
...
ObjectBase::SetAttribute (std::string name, const AttributeValue &value)
{
  ...
  if (!DoSet (info.accessor, info.checker, value))
    {
      NS_FATAL_ERROR (
          "Attribute name=" << name << " could not be set for this object: tid=" << tid.GetName ());
    }
}
bool

...
struct TestCaseFailure
{
   ...
   */
  TestCaseFailure (std::string _cond, std::string _actual, std::string _limit, std::string _message,
...
private:
   ...
   */
  std::list<TestCase *> FilterTests (std::string testName, enum TestSuite::Type testType,
...
TestCaseFailure::TestCaseFailure (std::string _cond, std::string _actual, std::string _limit,
                                  std::string _message, std::string _file, int32_t _line)
    : cond (_cond), actual (_actual), limit (_limit), message (_message), file (_file), line (_line)
{
  NS_LOG_FUNCTION (this << _cond << _actual << _limit << _message << _file << _line);
}
TestCase::Result::Result () : childrenFailed (false)
{
...
void
TestCase::ReportTestFailure (std::string cond, std::string actual, std::string limit,
                             std::string message, std::string file, int32_t line)
{
  NS_LOG_FUNCTION (this << cond << actual << limit << message << file << line);
  m_result->failure.push_back (TestCaseFailure (cond, actual, limit, message, file, line));
...
TestSuite::TestSuite (std::string name, TestSuite::Type type) : TestCase (name), m_type (type)
{
...
TestRunnerImpl::TestRunnerImpl ()
    : m_tempDir (""), m_assertOnFailure (false), m_continueOnFailure (true), m_updateData (false)
{
  NS_LOG_FUNCTION (this);
}
...
Indent::Indent (int _level) : level (_level)
{
  ...
  if (xml)
    {
      ...
      *os << Indent (level + 1) << "<Time real=\"" << real << "\" user=\"" << user << "\" system=\""
          << system << "\"/>" << std::endl;
...
std::list<TestCase *>
TestRunnerImpl::FilterTests (std::string testName, enum TestSuite::Type testType,
                             enum TestCase::TestDuration maximumTestDuration)
{

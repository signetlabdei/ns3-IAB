...
private:
   ...
   */
  void StateChangeNotificationDev0 (std::string context, Time now, LrWpanPhyEnumeration oldState,
   ...
   * \param newState new PHY state
   */
  void StateChangeNotificationDev2 (std::string context, Time now, LrWpanPhyEnumeration oldState,
                                    LrWpanPhyEnumeration newState);
...
void
TestRxOffWhenIdleAfterCsmaFailure::StateChangeNotificationDev0 (std::string context, Time now,
...
void
TestRxOffWhenIdleAfterCsmaFailure::StateChangeNotificationDev2 (std::string context, Time now,
                                                                LrWpanPhyEnumeration oldState,
                                                                LrWpanPhyEnumeration newState)
{
  NS_LOG_DEBUG (Simulator::Now ().As (Time::S)
...
TestRxOffWhenIdleAfterCsmaFailure::DoRun ()
{
  ...
  Simulator::ScheduleWithContext (1, Seconds (0.00033), &LrWpanMac::McpsDataRequest,
  ...
  Simulator::ScheduleWithContext (2, Seconds (0.0), &LrWpanMac::McpsDataRequest, dev2->GetMac (),
                                  params, p2);
...
TestActiveScanPanDescriptors::DoRun ()
{
  ...
  */
  ...
  NS_TEST_EXPECT_MSG_EQ (m_panDescriptorList.size (), 2,
                         "Error, Beacons not received or PAN descriptors not found");
  NS_TEST_ASSERT_MSG_LT (m_panDescriptorList[0].m_linkQuality, 255,
                         "Error, Coordinator 1 (PAN 5) LQI value should be less than 255.");
  NS_TEST_ASSERT_MSG_LT (m_panDescriptorList[1].m_linkQuality, 255,
                         "Error, Coordinator 2 (PAN 7) LQI value should be less than 255.");
  NS_TEST_ASSERT_MSG_GT (m_panDescriptorList[0].m_linkQuality, 0,
                         "Error, Coordinator 1 (PAN 5) LQI value should be greater than 0.");
  NS_TEST_ASSERT_MSG_GT (m_panDescriptorList[1].m_linkQuality, 0,
                         "Error, Coordinator 2 (PAN 7) LQI value should be greater than 0.");

  NS_TEST_ASSERT_MSG_LT (
      m_panDescriptorList[1].m_linkQuality, m_panDescriptorList[0].m_linkQuality,

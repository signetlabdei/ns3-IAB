...
{
  LrWpanAddressMode
      m_coorAddrMode; //!< The coordinator addressing mode corresponding to the received beacon frame.
  uint16_t
      m_coorPanId; //!< The PAN ID of the coordinator as specified in the received beacon frame.
  Mac16Address
      m_coorShortAddr; //!< The coordinator short address as specified in the coordinator address mode.
  Mac64Address
      m_coorExtAddr; //!< The coordinator extended address as specified in the coordinator address mode.
  ...
  SuperframeField
      m_superframeSpec; //!< The superframe specification as specified in the received beacon frame.
  bool
      m_gtsPermit; //!< TRUE if the beacon is from the PAN coordinator that is accepting GTS requests.
  uint8_t
      m_linkQuality; //!< The LQI at which the network beacon was received. Lower values represent lower LQI.
  Time
      m_timeStamp; //!< Beacon frame reception time. Used as Time data type in ns-3 to avoid precision problems.
};
...
struct MlmeAssociateResponseParams
{
  Mac64Address m_extDevAddr; //!< The extended address of the device requesting association
  Mac16Address
      m_assocShortAddr; //!< The short address allocated by the coordinator on succesful assoc. FF:FF = Unsuccessful
  LrWpanAssociationStatus
      m_status; //!< The status of the association attempt (As defined on Table 83 IEEE 802.15.4-2006)
};
...
struct MlmeStartRequestParams
{
  ...
  uint32_t m_startTime{
      0}; //!< Time at which to begin transmitting beacons (Used by Coordinator not PAN Coordinators). The time is specified in symbols.
  uint8_t m_bcnOrd{
      15}; //!< Beacon Order, Used to calculate the beacon interval, a value of 15 indicates no periodic beacons will be transmitted.
  ...
  bool m_battLifeExt{
      false}; //!< Flag indicating whether or not the Battery life extension (BLE) features are used.
  bool m_coorRealgn{
      false}; //!< True if a realignment request command is to be transmitted prior changing the superframe.
};
...
struct MlmeSyncRequestParams
{
  ...
  bool m_trackBcn{
      false}; //!< True if the mlme sync with the next beacon and attempts to track future beacons. False if mlme sync only the next beacon.
};
...
{
  LrWpanMlmeScanType
      m_scanType{}; //!< Indicates the type of scan performed as described in IEEE 802.15.4-2011 (5.1.2.1).
  uint32_t m_scanChannels{0x7FFFFFF}; //!< The channel numbers to be scanned.
  uint8_t m_scanDuration{
      14}; //!< A value used to calculate the length of time to spend scanning [aBaseSuperframeDuration * (2^m_scanDuration +)].
...
struct MlmeScanConfirmParams
{
  ...
  LrWpanMlmeScanType
      m_scanType; //!< Indicates the type of scan performed (ED,ACTIVE,PASSIVE,ORPHAN).
  uint32_t m_chPage; //!< The channel page on which the scan was performed.
  std::vector<uint8_t>
      m_unscannedCh; //!< A list of channels given in the request which were not scanned (Not valid for ED scans).
  std::vector<uint8_t>
      m_energyDetList; //!< A list of energy measurements, one for each channel searched during ED scan (Not valid for Active, Passive or Orphan Scans)
  std::vector<PanDescriptor>
      m_panDescList; //!< A list of PAN descriptor, one for each beacon found (Not valid for ED and Orphan scans).
};
...
struct MlmeSyncLossIndicationParams
{
  ...
  uint16_t
      m_panId; //!< The PAN identifier with which the device lost synchronization or to which it was realigned.
  uint8_t
      m_logCh; //!< The channel number on which the device lost synchronization or to which it was realigned.
};
...
{
  uint16_t
      m_panId; //!< The PAN identifier of the device from which the frame was received or to which the frame was being sent.
  uint8_t m_srcAddrMode; //!< The source addressing mode for this primitive
  Mac16Address
      m_srcShortAddr; //!< The short address of the entity from which the frame causing the error originated.
  Mac64Address
      m_srcExtAddr; //!< The extended address of the entity from which the frame causing the error originated.
...
{
  LrWpanMlmePollConfirmStatus
      m_status; //!< The confirmation status resulting from a MLME-poll.request.
};
...
public:
   ...
   */
  void PlmeGetAttributeConfirm (LrWpanPhyEnumeration status, LrWpanPibAttributeIdentifier id,

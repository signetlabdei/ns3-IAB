...
LrWpanMac::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::LrWpanMac")
          ...
          .AddAttribute ("PanId", "16-bit identifier of the associated PAN", UintegerValue (),
          ...
          .AddTraceSource ("MacIncSuperframeStatus", "The period status of the incoming superframe",
                           MakeTraceSourceAccessor (&LrWpanMac::m_incSuperframeStatus),
                           "ns3::TracedValueCallback::SuperframeState")
          .AddTraceSource ("MacOutSuperframeStatus", "The period status of the outgoing superframe",
  ...
  if (superframeType == INCOMING)
    {
      ...
      m_incCfpEvent = Simulator::Schedule (endCfpTime, &LrWpanMac::StartInactivePeriod, this,
  ...
  else
    {
      ...
      m_cfpEvent = Simulator::Schedule (endCfpTime, &LrWpanMac::StartInactivePeriod, this,
      ...
      if (m_macPromiscuousMode)
      ...
      else
        {
          ...
          if (acceptFrame &&
            ...
            {
              acceptFrame =
                  receivedMacHdr.GetSrcPanId () == m_macPanId; // TODO: need to check if PAN coord
            }
          ...
          if (macHdr.IsBeacon ())
            {
              ...
              if (m_csmaCa->IsSlottedCsmaCa ())
                {
                  ...
                  m_macBeaconTxTime =
                      Simulator::Now () - Seconds (double (beaconSymbols) / symbolRate);

                  m_txPkt = 0;
                  m_capEvent =
                      Simulator::ScheduleNow (&LrWpanMac::StartCAP, this, SuperframeType::OUTGOING);
              ...
              macHdr
                  .IsAckReq ()) // We have sent a regular data packet, check if we have to wait  for an ACK.
            {
...
void
LrWpanMac::PlmeGetAttributeConfirm (LrWpanPhyEnumeration status, LrWpanPibAttributeIdentifier id,
...
{
  NS_LOG_LOGIC (this << " change lrwpan mac state from " << m_lrWpanMacState << " to " << newState);

...
protected:
  ...
  AssociationSet
      m_associationSet; //!<    Association Set (\RFC{3626}, section12.2). Associations obtained from HNA messages generated by other nodes.
  Associations
      m_associations; //!< The node's local Host Network Associations that will be advertised using HNA messages.
...
public:
   ...
   */
  const AssociationSet &
  GetAssociationSet () const // Associations known to the node
  {
   ...
   */
  const Associations &
  GetAssociations () const // Set of associations that the node has
  {
  ...
  AssociationTuple *FindAssociationTuple (const Ipv4Address &gatewayAddr,
                                          const Ipv4Address &networkAddr, const Ipv4Mask &netmask);
  /**

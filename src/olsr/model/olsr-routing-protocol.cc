          ...
          for (LinkSet::const_iterator it2 = linkSet.begin (); it2 != linkSet.end (); it2++)
            {
              ...
              if ((GetMainAddress (link_tuple.neighborIfaceAddr) == nb_tuple.neighborMainAddr) &&
                  link_tuple.time >= Simulator::Now ())
                {
                  ...
                  lt = &link_tuple;
                  AddEntry (link_tuple.neighborIfaceAddr, link_tuple.neighborIfaceAddr,
          ...
          if (!have_destAddrEntry && have_lastAddrEntry && lastAddrEntry.distance == h)
            {
              ...
              //                                    route entry where:
              //                                       R_dest_addr == T_last_addr.
              AddEntry (topology_tuple.destAddr, lastAddrEntry.nextAddr, lastAddrEntry.interface,
                        h + 1);
              added = true;
            }
          else
  ...
  for (std::vector<olsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage != hello.linkMessages.end (); linkMessage++)
    {
      ...
      for (std::vector<Ipv4Address>::const_iterator neighIfaceAddr =
           ...
           neighIfaceAddr != linkMessage->neighborInterfaceAddresses.end (); neighIfaceAddr++)
        {
...
RoutingProtocol::AddEntry (Ipv4Address const &dest, Ipv4Address const &next,
                           Ipv4Address const &interfaceAddress, uint32_t distance)
{

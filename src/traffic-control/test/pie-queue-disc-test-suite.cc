...
PieQueueDiscTestCase::RunPieTest (QueueSizeUnit mode)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxSize", QueueSizeValue (QueueSize (mode, qSize))), true,
  ...
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("QueueDelayReference", TimeValue (Seconds (0.02))), true,
      "Verify that we can actually set the attribute QueueDelayReference");
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxBurstAllowance", TimeValue (Seconds (0.1))), true,
  ...
  NS_TEST_ASSERT_MSG_EQ (queue->SetAttributeFailSafe ("DequeueThreshold", UintegerValue (10000)),
                         true, "Verify that we can actually set the attribute DequeueThreshold");
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("QueueDelayReference", TimeValue (Seconds (0.08))), true,
      "Verify that we can actually set the attribute QueueDelayReference");
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxBurstAllowance", TimeValue (Seconds (0.1))), true,
      "Verify that we can actually set the attribute MaxBurstAllowance");
  ...
  NS_TEST_ASSERT_MSG_EQ (
      ...
      "Verify that we can actually set the attribute MaxSize");
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("UseDequeueRateEstimator", BooleanValue (true)), true,
  ...
  NS_TEST_ASSERT_MSG_LT (
      testAttributes->m_maxDropProbDiff, 0.0200000000000001,
  ...
  NS_TEST_ASSERT_MSG_EQ (
      ...
      "Verify that we can actually set the attribute MaxSize");
  NS_TEST_ASSERT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxBurstAllowance", TimeValue (Seconds (0.0))), true,
      "Verify that we can actually set the attribute MaxBurstAllowance");
  NS_TEST_ASSERT_MSG_EQ (queue->SetAttributeFailSafe ("UseDerandomization", BooleanValue (true)),
                         true, "Verify that we can actually set the attribute UseDerandomization");
  queue->Initialize ();
  ...
  NS_TEST_ASSERT_MSG_EQ (
      test14, testAttributes->m_expectedDrops,
                                        ...
                                        Ptr<PieQueueDiscTestItem> testAttributes)
{
  ...
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &PieQueueDiscTestCase::Enqueue, this,
                           queue, size, 1, testAttributes);
    }
}
...
PieQueueDiscTestCase::DequeueWithDelay (Ptr<PieQueueDisc> queue, double delay, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &PieQueueDiscTestCase::Dequeue, this,
                           queue, 1);
    }
}

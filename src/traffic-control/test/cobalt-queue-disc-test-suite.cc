...
CobaltQueueDiscTestItem::CobaltQueueDiscTestItem (Ptr<Packet> p, const Address &addr,
                                                  uint16_t protocol, bool ecnCapable)
    : QueueDiscItem (p, addr, ecnCapable), m_ecnCapablePacket (ecnCapable)
{
}
...
CobaltQueueDiscBasicEnqueueDequeue::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (queue->GetStats ().GetNDroppedPackets (CobaltQueueDisc::OVERLIMIT_DROP), 0,
  ...
  NS_TEST_ASSERT_MSG_EQ (
      queue->GetStats ().GetNDroppedPackets (CobaltQueueDisc::TARGET_EXCEEDED_DROP), 0,
...

{
  ...
  NS_TEST_ASSERT_MSG_NE (st.GetNDroppedPackets (CobaltQueueDisc::OVERLIMIT_DROP), 0,
...
CobaltQueueDiscDropTest::EnqueueWithDelay (Ptr<CobaltQueueDisc> queue, uint32_t size, uint32_t nPkt)
{
  ...
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &CobaltQueueDiscDropTest::Enqueue,
                           this, queue, size, 1);
    }
}
...
CobaltQueueDiscMarkTest::DoRun (void)
{
  ...
  Simulator::Schedule (waitUntilFirstDequeue, &CobaltQueueDiscMarkTest::Dequeue, this, queue,
                       modeSize, 1);

  // This dequeue should cause a packet to be dropped
  Time waitUntilSecondDequeue = waitUntilFirstDequeue + 2 * queue->GetInterval ();
  Simulator::Schedule (waitUntilSecondDequeue, &CobaltQueueDiscMarkTest::Dequeue, this, queue,
                       modeSize, 1);
  ...
  Simulator::Schedule (waitUntilSecondDequeue * 2, &CobaltQueueDiscMarkTest::Dequeue, this, queue,
                       modeSize, 2);
...
void
CobaltQueueDiscMarkTest::Enqueue (Ptr<CobaltQueueDisc> queue, uint32_t size, uint32_t nPkt,
                                  bool ecnCapable)
{
  Address dest;
  for (uint32_t i = 0; i < nPkt; i++)
  ...
  if (initialMarkCount > 0 && currentTime.GetNanoSeconds () > initialDropNext && testCase == 3)
    {
      queue->TraceConnectWithoutContext (
          "DropNext", MakeCallback (&CobaltQueueDiscMarkTest::DropNextTracer, this));
    }
              ...
              if (currentTime < queue->GetInterval ())
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentMarkCount, 0,
              ...
              if (currentTime.GetNanoSeconds () <= initialDropNext)
              ...
              else if (currentTime.GetNanoSeconds () > initialDropNext)
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (
                      ...
                      "The number of packets dequeued equals to the number of times m_dropNext is "
                      "updated plus initial dequeue");
                  NS_TEST_EXPECT_MSG_EQ (
                      currentDropCount, m_dropNextCount,
...
void
CobaltQueueDiscCeThresholdTest::EnqueueWithDelay (Ptr<CobaltQueueDisc> queue, uint32_t size,
                                                  uint32_t nPkt, Time delay)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay.GetSeconds ())),
                           &CobaltQueueDiscCeThresholdTest::Enqueue, this, queue, size, 1);
    }
}
...
CobaltQueueDiscCeThresholdTest::Dequeue (Ptr<CobaltQueueDisc> queue, uint32_t modeSize)
{
  ...
  if (Simulator::Now () > MilliSeconds (11) && Simulator::Now () < MilliSeconds (28))
    {
      NS_TEST_EXPECT_MSG_EQ (
          queue->GetStats ().GetNMarkedPackets (CobaltQueueDisc::CE_THRESHOLD_EXCEEDED_MARK), 1,
  ...
  if (Simulator::Now () > MilliSeconds (31))
    {
      NS_TEST_EXPECT_MSG_EQ (
          queue->GetStats ().GetNMarkedPackets (CobaltQueueDisc::CE_THRESHOLD_EXCEEDED_MARK), 3,
...
void
CobaltQueueDiscCeThresholdTest::DequeueWithDelay (Ptr<CobaltQueueDisc> queue, uint32_t modeSize,
                                                  uint32_t nPkt, Time delay)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay.GetSeconds ())),
                           &CobaltQueueDiscCeThresholdTest::Dequeue, this, queue, modeSize);
    }
}
...
CobaltQueueDiscCeThresholdTest::DoRun (void)
{
  ...
  Simulator::Schedule (waitUntilFirstMark, &CobaltQueueDiscCeThresholdTest::EnqueueWithDelay, this,
                       queue, pktSize, 9, MicroSeconds (1200));

  // Keep dequeueing with the same delay
  Simulator::Schedule (waitUntilFirstMark, &CobaltQueueDiscCeThresholdTest::DequeueWithDelay, this,
  ...
  Simulator::Schedule (waitUntilDecreasingEnqueueDelay,
                       &CobaltQueueDiscCeThresholdTest::EnqueueWithDelay, this, queue, pktSize, 10,
                       MilliSeconds (1));
...

{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      ...
      " From the 41st dequeue until the last one, sojourn time is above 400ms, so 60 packets have "
      "sojourn time above 400ms"
      "hence Pdrop should be increased 60*(1/256) which is 0.234375");
  NS_TEST_ASSERT_MSG_EQ (st.GetNDroppedPackets (CobaltQueueDisc::TARGET_EXCEEDED_DROP), 49,
  ...
  NS_TEST_ASSERT_MSG_EQ (st.GetNDroppedPackets (CobaltQueueDisc::TARGET_EXCEEDED_DROP), 0,
...
void
CobaltQueueDiscEnhancedBlueTest::DequeueWithDelay (Ptr<CobaltQueueDisc> queue, uint32_t nPkt,
                                                   Time delay)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {

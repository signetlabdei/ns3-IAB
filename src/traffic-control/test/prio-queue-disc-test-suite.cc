...
PrioQueueDiscTestFilter::PrioQueueDiscTestFilter (bool cls) : m_cls (cls), m_ret (0)
{
}
...
PrioQueueDiscTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (qdisc->SetAttributeFailSafe ("Priomap", StringValue (priomap)), true,
                         "Verify that we can actually set the attribute Priomap");

  StringValue sv;
  NS_TEST_ASSERT_MSG_EQ (qdisc->GetAttributeFailSafe ("Priomap", sv), true,
  ...
  for (uint16_t i = 0; i < 4; i++)
    {
      NS_TEST_ASSERT_MSG_EQ (qdisc->GetQueueDiscClass (i)->GetQueueDisc ()->GetNPackets (), 0,
      ...
      NS_TEST_ASSERT_MSG_EQ (qdisc->GetQueueDiscClass (i)->GetQueueDisc ()->GetNPackets (), 1,
  ...
  for (uint16_t i = 0; i < 4; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (qdisc->GetQueueDiscClass (i)->GetQueueDisc ()->GetNPackets (), 2,
  ...
  for (uint16_t i = 0; i < 4; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (qdisc->GetQueueDiscClass (i)->GetQueueDisc ()->GetNPackets (), 3,

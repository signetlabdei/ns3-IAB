...
TbfQueueDiscTestCase::RunTbfTest (QueueSizeUnit mode)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (queue->GetFirstBucketTokens (), burst,
  ...
  NS_TEST_ASSERT_MSG_EQ (
      queue->GetFirstBucketTokens (), burst - (3 * pktSize),
  ...
  for (uint32_t i = 1; i <= nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &TbfQueueDiscTestCase::Enqueue, this,
                           queue, dest, pktSize);
    }
  delay = 0.1;
  for (uint32_t i = 1; i <= nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &TbfQueueDiscTestCase::DequeueAndCheck,
                           this, queue, true, "No packet should be blocked");
    }
      ...
      if (i == 10)
        {
          Simulator::Schedule (Time (Seconds ((i + 1) * delay)),
                               &TbfQueueDiscTestCase::DequeueAndCheck, this, queue, false,
                               "10th packet should be blocked");
        }
      else
        {
          Simulator::Schedule (Time (Seconds ((i + 1) * delay)),
                               &TbfQueueDiscTestCase::DequeueAndCheck, this, queue, true,
  ...
  for (uint32_t i = 1; i <= nPkt; i++)
    {
      if (i % 2 == 1)
      ...
      else
        {
          ...
          nextDelay +=
              0.06; // Need 0.04 + 0.06 seconds to allow the next packet to be dequeued without block
        }
    }
...
void
TbfQueueDiscTestCase::DequeueAndCheck (Ptr<TbfQueueDisc> queue, bool flag,

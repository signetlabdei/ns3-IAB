...
FifoQueueDiscTestCase::DoRunFifoTest (Ptr<FifoQueueDisc> q, uint32_t qSize, uint32_t pktSize)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (q->Enqueue (Create<FifoQueueDiscTestItem> (p, dest)), false,
  ...
  for (uint32_t i = 1; i <= numPackets; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (q->GetCurrentSize ().GetValue (), (numPackets - i) * modeSize,
  ...
  if (mode == QueueSizeUnit::PACKETS)
    {
      NS_TEST_ASSERT_MSG_EQ (
          iq->SetAttributeFailSafe (
              "MaxSize", QueueSizeValue (QueueSize (QueueSizeUnit::PACKETS, numPackets))),
  ...
  else
    {
      NS_TEST_ASSERT_MSG_EQ (
          iq->SetAttributeFailSafe (
              "MaxSize", QueueSizeValue (QueueSize (QueueSizeUnit::BYTES, numPackets * pktSize))),
          true, "Verify that we can actually set the attribute MaxSize on the internal queue");
    }

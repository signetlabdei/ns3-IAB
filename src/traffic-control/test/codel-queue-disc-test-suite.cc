...
CoDelQueueDiscBasicEnqueueDequeue::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (queue->GetStats ().GetNDroppedPackets (CoDelQueueDisc::OVERLIMIT_DROP), 0,
  ...
  NS_TEST_ASSERT_MSG_EQ (
      queue->GetStats ().GetNDroppedPackets (CoDelQueueDisc::TARGET_EXCEEDED_DROP), 0,
...
CoDelQueueDiscBasicOverflow::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (queue->GetStats ().GetNDroppedPackets (CoDelQueueDisc::OVERLIMIT_DROP), 3,
...
uint32_t
CoDelQueueDiscControlLawTest::_codel_control_law (uint32_t t, uint32_t interval,
                                                  uint32_t recInvSqrt)
{
      ...
      for (uint16_t recInvSqrt = 0xff; recInvSqrt > 0; recInvSqrt /= 2)
        {
          ...
          NS_TEST_ASSERT_MSG_EQ (ns3Result, linuxResult,
...
CoDelQueueDiscBasicDrop::DoRun (void)
{
  ...
  Simulator::Schedule (waitUntilFirstDequeue, &CoDelQueueDiscBasicDrop::Dequeue, this, queue,
                       modeSize);

  // This dequeue should cause a drop
  Time waitUntilSecondDequeue = waitUntilFirstDequeue + 2 * queue->GetInterval ();
  Simulator::Schedule (waitUntilSecondDequeue, &CoDelQueueDiscBasicDrop::Dequeue, this, queue,
                       modeSize);
  ...
  Simulator::Schedule (waitUntilSecondDequeue * 2, &CoDelQueueDiscBasicDrop::Dequeue, this, queue,
                       modeSize);
  ...
  if (initialDropCount > 0 && currentTime.GetMicroSeconds () >= initialDropNext)
    {
      queue->TraceConnectWithoutContext (
          "DropNext", MakeCallback (&CoDelQueueDiscBasicDrop::DropNextTracer, this));
    }
          ...
          if (currentTime.GetMicroSeconds () < initialDropNext)
          ...
          else if (currentTime.GetMicroSeconds () >= initialDropNext)
            {
              ...
              NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (),
                                     ...
                                     "m_dropNext is updated plus initial dequeue");
              NS_TEST_EXPECT_MSG_EQ (currentDropCount, 1 + m_dropNextCount,
                                     "The number of drops equals to the number of times m_dropNext "
                                     "is updated plus 1 from last dequeue");
            }
        }
    }
}
...
CoDelQueueDiscBasicMark::DoRun (void)
{
  ...
  Time waitUntilFirstDequeue = 2 * queue->GetTarget ();
  Simulator::Schedule (waitUntilFirstDequeue, &CoDelQueueDiscBasicMark::Dequeue, this, queue,
                       modeSize, 1);

  // This dequeue should cause a packet to be dropped
  Time waitUntilSecondDequeue = waitUntilFirstDequeue + 2 * queue->GetInterval ();
  Simulator::Schedule (waitUntilSecondDequeue, &CoDelQueueDiscBasicMark::Dequeue, this, queue,
                       modeSize, 1);
  ...
  Simulator::Schedule (waitUntilSecondDequeue * 2, &CoDelQueueDiscBasicMark::Dequeue, this, queue,
                       modeSize, 2);
  ...
  Simulator::Schedule (MilliSeconds (1), &CoDelQueueDiscBasicMark::Dequeue, this, queue, modeSize,
                       4);

  // Sojourn time above CE threshold so this dequeue should cause a packet to be CE thershold exceeded marked
  Simulator::Schedule (MilliSeconds (3), &CoDelQueueDiscBasicMark::Dequeue, this, queue, modeSize,
                       4);
...
void
CoDelQueueDiscBasicMark::Enqueue (Ptr<CoDelQueueDisc> queue, uint32_t size, uint32_t nPkt,
                                  bool ecnCapable)
{
  Address dest;
  for (uint32_t i = 0; i < nPkt; i++)
...
CoDelQueueDiscBasicMark::Dequeue (Ptr<CoDelQueueDisc> queue, uint32_t modeSize, uint32_t testCase)
{
  ...
  if (initialTargetMarkCount > 0 && currentTime.GetMicroSeconds () >= initialDropNext &&
    ...
    {
      queue->TraceConnectWithoutContext (
          "DropNext", MakeCallback (&CoDelQueueDiscBasicMark::DropNextTracer, this));
    }
          ...
          if (initialTargetMarkCount == 0 && currentTime > queue->GetTarget ())
            {
              if (currentTime < queue->GetInterval ())
              ...
              else if (currentTime >= queue->GetInterval ())
                {
                  ...
                  NS_TEST_ASSERT_MSG_EQ (currentDropCount, 0,
                                         "There should not be any packet drops");
                  NS_TEST_ASSERT_MSG_EQ (currentTargetMarkCount, 1,
              ...
              if (currentTime.GetMicroSeconds () < initialDropNext)
              ...
              else if (currentTime.GetMicroSeconds () >= initialDropNext)
                {
                  ...
                  NS_TEST_ASSERT_MSG_EQ (
                      ...
                      "to number of packets in the queue before first mark as the behavior untill "
          ...
          if (initialTargetMarkCount == 0 && currentTime > queue->GetTarget ())
            {
              if (currentTime < queue->GetInterval ())
              ...
              else if (currentTime >= queue->GetInterval ())
                {
                  ...
                  NS_TEST_ASSERT_MSG_EQ (currentTargetMarkCount, 1,
                                         "There should be 1 target exceeded marked packet");
                  NS_TEST_ASSERT_MSG_EQ (currentCeThreshMarkCount, 1,
              ...
              if (currentTime.GetMicroSeconds () < initialDropNext)
                {
                  ...
                  NS_TEST_ASSERT_MSG_EQ (currentTargetMarkCount, 1,
                                         "There should still be only 1 target exceeded marked "
                                         "packet from the last dequeue");
                  NS_TEST_ASSERT_MSG_EQ (currentCeThreshMarkCount, 2,
          ...
          else if (initialCeThreshMarkCount > 0 && currentTime < queue->GetInterval ())
            {
              if (initialCeThreshMarkCount < 2)
                ...
                { // In dropping state
                  ...
                  NS_TEST_ASSERT_MSG_EQ (currentDropCount, 0,
                                         "There should not be any packet drops");
                  NS_TEST_ASSERT_MSG_EQ (currentCeThreshMarkCount, 3,

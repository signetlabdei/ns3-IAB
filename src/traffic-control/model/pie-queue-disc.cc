  ...
  static TypeId tid =
      TypeId ("ns3::PieQueueDisc")
          ...
          .AddAttribute ("A", "Value of alpha", DoubleValue (0.125),
                         MakeDoubleAccessor (&PieQueueDisc::m_a), MakeDoubleChecker<double> ())
          .AddAttribute ("B", "Value of beta", DoubleValue (1.25),
                         MakeDoubleAccessor (&PieQueueDisc::m_b), MakeDoubleChecker<double> ())
          .AddAttribute ("Tupdate", "Time period to calculate drop probability",
                         TimeValue (MilliSeconds (15)), MakeTimeAccessor (&PieQueueDisc::m_tUpdate),
                         MakeTimeChecker ())
          .AddAttribute ("Supdate", "Start time of the update timer", TimeValue (Seconds (0)),
                         MakeTimeAccessor (&PieQueueDisc::m_sUpdate), MakeTimeChecker ())
          ...
          .AddAttribute ("DequeueThreshold",
                         ...
                         UintegerValue (16384), MakeUintegerAccessor (&PieQueueDisc::m_dqThreshold),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("QueueDelayReference", "Desired queue delay",
                         TimeValue (MilliSeconds (15)),
                         MakeTimeAccessor (&PieQueueDisc::m_qDelayRef), MakeTimeChecker ())
          .AddAttribute ("MaxBurstAllowance", "Current max burst allowance before random drop",
                         TimeValue (MilliSeconds (150)),
                         MakeTimeAccessor (&PieQueueDisc::m_maxBurst), MakeTimeChecker ())
          ...
          .AddAttribute ("MarkEcnThreshold",
                         ...
                         DoubleValue (0.1), MakeDoubleAccessor (&PieQueueDisc::m_markEcnTh),
                         MakeDoubleChecker<double> (0, 1))
          .AddAttribute (
              "UseDerandomization", "Enable/Disable Derandomization feature mentioned in RFC 8033",
              BooleanValue (false), MakeBooleanAccessor (&PieQueueDisc::m_useDerandomization),
              MakeBooleanChecker ())
          .AddAttribute ("ActiveThreshold", "Threshold for activating PIE (disabled by default)",
                         TimeValue (Time::Max ()),
                         MakeTimeAccessor (&PieQueueDisc::m_activeThreshold), MakeTimeChecker ())
          .AddAttribute ("CeThreshold", "The FqPie CE threshold for marking packets",
                         TimeValue (Time::Max ()), MakeTimeAccessor (&PieQueueDisc::m_ceThreshold),
                         MakeTimeChecker ())
...
PieQueueDisc::PieQueueDisc () : QueueDisc (QueueDiscSizePolicy::SINGLE_INTERNAL_QUEUE)
{

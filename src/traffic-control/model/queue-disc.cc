...
QueueDiscClass::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::QueueDiscClass")
          ...
          .AddAttribute ("QueueDisc", "The queue disc attached to the class", PointerValue (),
...
QueueDisc::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::QueueDisc")
          ...
          .AddAttribute ("InternalQueueList", "The list of internal queues.", ObjectVectorValue (),
                         MakeObjectVectorAccessor (&QueueDisc::m_queues),
                         MakeObjectVectorChecker<InternalQueue> ())
          .AddAttribute ("PacketFilterList", "The list of packet filters.", ObjectVectorValue (),
          ...
          .AddAttribute ("QueueDiscClassList", "The list of queue disc classes.",
                         ObjectVectorValue (), MakeObjectVectorAccessor (&QueueDisc::m_classes),
          ...
          .AddTraceSource ("BytesInQueue", "Number of bytes currently stored in the queue disc",
                           MakeTraceSourceAccessor (&QueueDisc::m_nBytes),
                           "ns3::TracedValueCallback::Uint32")
          .AddTraceSource (
              "SojournTime", "Sojourn time of the last packet dequeued from the queue disc",
              MakeTraceSourceAccessor (&QueueDisc::m_sojourn), "ns3::Time::TracedCallback");
  return tid;
}
...
QueueDisc::QueueDisc (QueueDiscSizePolicy policy)
    : m_nPackets (0),
      m_nBytes (0),
      m_maxSize (
          QueueSize ("1p")), // to avoid that setting the mode at construction time is ignored
      m_running (false),
      m_peeked (false),
      m_sizePolicy (policy),
      m_prohibitChangeMode (false)
{
  ...
  m_childQueueDiscDbeFunctor = [this] (Ptr<const QueueDiscItem> item, const char *r) {
    return DropBeforeEnqueue (
        item, m_childQueueDiscDropMsg.assign (CHILD_QUEUE_DISC_DROP).append (r).data ());
  };
...
QueueDisc::QueueDisc (QueueDiscSizePolicy policy, QueueSizeUnit unit) : QueueDisc (policy)
{
...
QueueDisc::AddQueueDiscClass (Ptr<QueueDiscClass> qdClass)
{
  ...
  qdClass->GetQueueDisc ()->TraceConnectWithoutContext (
      "Mark", MakeCallback (&ChildQueueDiscMarkFunctor::operator(), &m_childQueueDiscMarkFunctor));

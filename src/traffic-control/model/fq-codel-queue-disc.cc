...
FqCoDelFlow::FqCoDelFlow () : m_deficit (0), m_status (INACTIVE), m_index (0)
{
  ...
  static TypeId tid =
      TypeId ("ns3::FqCoDelQueueDisc")
          ...
          .AddAttribute ("UseEcn", "True to use ECN (packets are marked instead of being dropped)",
                         BooleanValue (true), MakeBooleanAccessor (&FqCoDelQueueDisc::m_useEcn),
                         MakeBooleanChecker ())
          ...
          .AddAttribute (
              ...
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("CeThreshold", "The FqCoDel CE threshold for marking packets",
                         TimeValue (Time::Max ()),
                         MakeTimeAccessor (&FqCoDelQueueDisc::m_ceThreshold), MakeTimeChecker ())
          ...
          .AddAttribute ("SetWays", "The size of a set of queues (used by set associative hash)",
                         UintegerValue (8), MakeUintegerAccessor (&FqCoDelQueueDisc::m_setWays),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("UseL4s", "True to use L4S (only ECT1 packets are marked at CE threshold)",
                         BooleanValue (false), MakeBooleanAccessor (&FqCoDelQueueDisc::m_useL4s),
                         MakeBooleanChecker ());
  return tid;
}

  ...
  for (int i = 0; i < 16; i++)
    {
      if (!(is >> priomap[i]))
        {
          NS_FATAL_ERROR ("Incomplete priomap specification (" << i
  ...
  static TypeId tid =
      TypeId ("ns3::PrioQueueDisc")
          ...
          .AddAttribute ("Priomap", "The priority to band mapping.",
                         ...
                         MakePriomapAccessor (&PrioQueueDisc::m_prio2band), MakePriomapChecker ());
  return tid;
}

PrioQueueDisc::PrioQueueDisc () : QueueDisc (QueueDiscSizePolicy::NO_LIMITS)
{

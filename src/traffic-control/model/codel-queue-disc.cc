  ...
  static TypeId tid =
      TypeId ("ns3::CoDelQueueDisc")
          ...
          .AddAttribute ("UseEcn", "True to use ECN (packets are marked instead of being dropped)",
                         BooleanValue (false), MakeBooleanAccessor (&CoDelQueueDisc::m_useEcn),
                         MakeBooleanChecker ())
          .AddAttribute ("UseL4s", "True to use L4S (only ECT1 packets are marked at CE threshold)",
                         BooleanValue (false), MakeBooleanAccessor (&CoDelQueueDisc::m_useL4s),
                         MakeBooleanChecker ())
          .AddAttribute (
          ...
          .AddAttribute ("MinBytes", "The CoDel algorithm minbytes parameter.",
                         UintegerValue (1500), MakeUintegerAccessor (&CoDelQueueDisc::m_minBytes),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("Interval", "The CoDel algorithm interval", StringValue ("100ms"),
                         MakeTimeAccessor (&CoDelQueueDisc::m_interval), MakeTimeChecker ())
          .AddAttribute ("Target", "The CoDel algorithm target queue delay", StringValue ("5ms"),
                         MakeTimeAccessor (&CoDelQueueDisc::m_target), MakeTimeChecker ())
          .AddAttribute ("CeThreshold", "The CoDel CE threshold for marking packets",
                         TimeValue (Time::Max ()),
                         MakeTimeAccessor (&CoDelQueueDisc::m_ceThreshold), MakeTimeChecker ())
          .AddTraceSource ("Count", "CoDel count",
                           MakeTraceSourceAccessor (&CoDelQueueDisc::m_count),
                           "ns3::TracedValueCallback::Uint32")
          .AddTraceSource ("LastCount", "CoDel lastcount",
  ...
  if (m_firstAboveTime == 0)
    {
       ...
       */
      NS_LOG_LOGIC ("Sojourn time has just gone above target from below, need to stay above for at "
                    "least q->interval before packet can be dropped. ");
          ...
          while (m_dropping && CoDelTimeAfterEq (now, m_dropNext))
            {
              ...
              if (m_useEcn && Mark (item, TARGET_EXCEEDED_MARK))
                {
                  isMarked = true;
                  NS_LOG_LOGIC ("Sojourn time is still above target and it's time for next drop or "
                                "mark; marking "
                                << item);

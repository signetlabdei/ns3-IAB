...
FqCobaltFlow::FqCobaltFlow () : m_deficit (0), m_status (INACTIVE), m_index (0)
{
...
TypeId
FqCobaltQueueDisc::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::FqCobaltQueueDisc")
          ...
          .AddAttribute ("UseEcn", "True to use ECN (packets are marked instead of being dropped)",
                         BooleanValue (true), MakeBooleanAccessor (&FqCobaltQueueDisc::m_useEcn),
                         MakeBooleanChecker ())
          .AddAttribute ("Interval", "The CoDel algorithm interval for each FqCobalt queue",
                         StringValue ("100ms"), MakeStringAccessor (&FqCobaltQueueDisc::m_interval),
                         MakeStringChecker ())
          .AddAttribute ("Target", "The CoDel algorithm target queue delay for each FqCobalt queue",
                         StringValue ("5ms"), MakeStringAccessor (&FqCobaltQueueDisc::m_target),
                         MakeStringChecker ())
          ...
          .AddAttribute ("Flows",
                         ...
                         UintegerValue (1024), MakeUintegerAccessor (&FqCobaltQueueDisc::m_flows),
          ...
          .AddAttribute (
              ...
              UintegerValue (0), MakeUintegerAccessor (&FqCobaltQueueDisc::m_perturbation),
              MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("CeThreshold", "The FqCobalt CE threshold for marking packets",
                         TimeValue (Time::Max ()),
                         MakeTimeAccessor (&FqCobaltQueueDisc::m_ceThreshold), MakeTimeChecker ())
          ...
          .AddAttribute ("SetWays", "The size of a set of queues (used by set associative hash)",
                         UintegerValue (8), MakeUintegerAccessor (&FqCobaltQueueDisc::m_setWays),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("UseL4s", "True to use L4S (only ECT1 packets are marked at CE threshold)",
                         BooleanValue (false), MakeBooleanAccessor (&FqCobaltQueueDisc::m_useL4s),
                         MakeBooleanChecker ())
          .AddAttribute ("Pdrop", "Marking Probability", DoubleValue (0),
                         MakeDoubleAccessor (&FqCobaltQueueDisc::m_Pdrop),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("Increment", "Pdrop increment value", DoubleValue (1. / 256),
                         MakeDoubleAccessor (&FqCobaltQueueDisc::m_increment),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("Decrement", "Pdrop decrement Value", DoubleValue (1. / 4096),
                         MakeDoubleAccessor (&FqCobaltQueueDisc::m_decrement),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("BlueThreshold", "The Threshold after which Blue is enabled",

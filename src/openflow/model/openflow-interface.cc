...
Stats::Stats (ofp_stats_types _type, size_t body_len)
{
  ...
  switch (type)
    ...
    case OFPST_PORT:
      min_body = 0;
      max_body = std::numeric_limits<size_t>::
          max (); // Not sure about this one. This would guarantee that the body_len is always acceptable.
      break;
...
Stats::FlowStatsDump (Ptr<OpenFlowSwitchNetDevice> swtch, FlowStatsState *s, ofpbuf *buffer)
{
  ...
  while (s->table_idx < swtch->GetChain ()->n_tables &&
         (s->rq.table_id == 0xff || s->rq.table_id == s->table_idx))
    {
      ...
      if (table->iterate (table, &match_key, s->rq.out_port, &s->position, Stats::FlowDumpCallback,
                          s))
        {
          break;
        }
...
int
Stats::AggregateStatsDump (Ptr<OpenFlowSwitchNetDevice> swtch, ofp_aggregate_stats_request *s,
                           ofpbuf *buffer)
{
...
uint16_t
Action::Validate (ofp_action_type type, size_t len, const sw_flow_key *key,
...
void
Action::Execute (ofp_action_type type, ofpbuf *buffer, sw_flow_key *key,
                 const ofp_action_header *ah)
{
  switch (type)
    {
...
void
VPortAction::Execute (ofp_vport_action_type type, ofpbuf *buffer, const sw_flow_key *key,
                      const ofp_action_header *ah)
{
  switch (type)
    {
...
void
EricssonAction::Execute (er_action_type type, ofpbuf *buffer, const sw_flow_key *key,
...
ofp_flow_mod *
Controller::BuildFlow (sw_flow_key key, uint32_t buffer_id, uint16_t command, void *acts,
                       size_t actions_len, int idle_timeout, int hard_timeout)
{
...
DropController::ReceiveFromSwitch (Ptr<OpenFlowSwitchNetDevice> swtch, ofpbuf *buffer)
{
  ...
  if (type ==
      OFPT_PACKET_IN) // The switch didn't understand the packet it received, so it forwarded it to the controller.
    {
      ofp_packet_in *opi =
...
LearningController::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::ofi::LearningController")
          ...
          .AddAttribute (
              "ExpirationTime", "Time it takes for learned MAC state entry/created flow to expire.",
              TimeValue (Seconds (0)), MakeTimeAccessor (&LearningController::m_expirationTime),
              MakeTimeChecker ());
  return tid;
}
      ...
      if (st == m_learnState.end ()) // We haven't learned our source MAC yet.
        {
          ...
          ofp_flow_mod *ofm2 = BuildFlow (
              key, -1, OFPFC_MODIFY, x2, sizeof (x2), OFP_FLOW_PERMANENT,
              m_expirationTime.IsZero () ? OFP_FLOW_PERMANENT : m_expirationTime.GetSeconds ());
...
void
ExecuteActions (Ptr<OpenFlowSwitchNetDevice> swtch, uint64_t packet_uid, ofpbuf *buffer,
                sw_flow_key *key, const ofp_action_header *actions, size_t actions_len,
                int ignore_no_fwd)
{
...
void
ExecuteVPortActions (Ptr<OpenFlowSwitchNetDevice> swtch, uint64_t packet_uid, ofpbuf *buffer,
                     sw_flow_key *key, const ofp_action_header *actions, size_t actions_len)
{
  /* Every output action needs a separate clone of 'buffer', but the common
   * case is just a single output action, so that doing a clone and then

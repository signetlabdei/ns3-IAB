...
private:
  ...
  int AggregateStatsDump (Ptr<OpenFlowSwitchNetDevice> dp, ofp_aggregate_stats_request *state,
                          ofpbuf *buffer);
...
struct Action
{
   ...
   */
  static uint16_t Validate (ofp_action_type type, size_t len, const sw_flow_key *key,
...
struct VPortAction
{
   ...
   * \param ah Action's data header.
   */
  static void Execute (ofp_vport_action_type type, ofpbuf *buffer, const sw_flow_key *key,
                       const ofp_action_header *ah);
};
...
struct EricssonAction
{
   ...
   * \param ah Action's data header.
   */
  static void Execute (er_action_type type, ofpbuf *buffer, const sw_flow_key *key,
...
public:
   ...
   */
  virtual void
  ReceiveFromSwitch (Ptr<OpenFlowSwitchNetDevice> swtch, ofpbuf *buffer)
  {
  }
...
protected:
   ...
   */
  ofp_flow_mod *BuildFlow (sw_flow_key key, uint32_t buffer_id, uint16_t command, void *acts,
 ...
 */
void ExecuteActions (Ptr<OpenFlowSwitchNetDevice> swtch, uint64_t packet_uid, ofpbuf *buffer,
 ...
 */
uint16_t ValidateActions (const sw_flow_key *key, const ofp_action_header *actions,
                          size_t actions_len);
 ...
 * \param actions_len Length of actions buffer.
 */
void ExecuteVPortActions (Ptr<OpenFlowSwitchNetDevice> swtch, uint64_t packet_uid, ofpbuf *buffer,
                          sw_flow_key *key, const ofp_action_header *actions, size_t actions_len);

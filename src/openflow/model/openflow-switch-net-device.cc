  ...
  static TypeId tid =
      TypeId ("ns3::OpenFlowSwitchNetDevice")
          ...
          .AddAttribute ("FlowTableLookupDelay",
                         ...
                         MakeTimeChecker ())
          .AddAttribute (
              "Flags", // Note: The Controller can configure this value, overriding the user's setting.
              "Flags to turn different functionality on/off, such as whether to inform the "
              "controller when a flow expires, or how to handle fragments.",
              UintegerValue (
                  0), // Look at the ofp_config_flags enum in openflow/include/openflow.h for options.
              MakeUintegerAccessor (&OpenFlowSwitchNetDevice::m_flags),
              MakeUintegerChecker<uint16_t> ())
          .AddAttribute (
              "FlowTableMissSendLength", // Note: The Controller can configure this value, overriding the user's setting.
              "When forwarding a packet the switch didn't match up to the controller, it can be "
              "more efficient to forward only the first x bytes.",
...
OpenFlowSwitchNetDevice::OpenFlowSwitchNetDevice () : m_node (0), m_ifIndex (0), m_mtu (0xffff)
{
  ...
  if (m_ports.size () < DP_MAX_PORTS)
    {
      ...
      m_node->RegisterProtocolHandler (
          MakeCallback (&OpenFlowSwitchNetDevice::ReceiveFromDevice, this), 0, switchPort, true);
...
void
OpenFlowSwitchNetDevice::DoOutput (uint32_t packet_uid, int in_port, size_t max_len, int out_port,
                                   bool ignore_no_fwd)
{
...
bool
OpenFlowSwitchNetDevice::SendFrom (Ptr<Packet> packet, const Address &src, const Address &dest,
...
ofpbuf *
OpenFlowSwitchNetDevice::BufferFromPacket (Ptr<const Packet> constPacket, Address src, Address dst,
...
void
OpenFlowSwitchNetDevice::ReceiveFromDevice (Ptr<NetDevice> netdev, Ptr<const Packet> packet,
...
void
OpenFlowSwitchNetDevice::OutputPort (uint32_t packet_uid, int in_port, int out_port,
                                     bool ignore_no_fwd)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
OpenFlowSwitchNetDevice::OutputControl (uint32_t packet_uid, int in_port, size_t max_len,
                                        int reason)
{
  ...
  ofp_packet_in *opi =
      (ofp_packet_in *) ofpbuf_push_uninit (buffer, offsetof (ofp_packet_in, data));
...
{
  ofpbuf *buffer;
  ofp_vport_table_features *ovtfr = (ofp_vport_table_features *) MakeOpenflowReply (
...
OpenFlowSwitchNetDevice::SendErrorMsg (uint16_t type, uint16_t code, const void *data, size_t len)
{
  ofpbuf *buffer;
  ofp_error_msg *oem =
      (ofp_error_msg *) MakeOpenflowReply (sizeof (*oem) + len, OFPT_ERROR, &buffer);
  oem->type = htons (type);
  oem->code = htons (code);
...
void
OpenFlowSwitchNetDevice::FlowTableLookup (sw_flow_key key, ofpbuf *buffer, uint32_t packet_uid,
...
void
OpenFlowSwitchNetDevice::RunThroughFlowTable (uint32_t packet_uid, int port,
                                              bool send_to_controller)
{
  ...
  Simulator::Schedule (m_lookupDelay, &OpenFlowSwitchNetDevice::FlowTableLookup, this, key, buffer,
                       packet_uid, port, send_to_controller);
}
...
OpenFlowSwitchNetDevice::RunThroughVPortTable (uint32_t packet_uid, int port, uint32_t vport)
{
  ...
  while (vpe != 0)
    {
      ofi::ExecuteVPortActions (this, packet_uid, m_packetData.find (packet_uid)->second.buffer,
                                &key, vpe->port_acts->actions, vpe->port_acts->actions_len);
      ...
      if (buffer)
        {
          ...
          ofi::ExecuteActions (this, ofm->buffer_id, buffer, &key, ofm->actions, actions_len,
                               false);
          ofpbuf_delete (buffer);
        }
      else
      ...
      if (buffer)
        {
          ...
          ofi::ExecuteActions (this, ofm->buffer_id, buffer, &skb_key, ofm->actions, actions_len,
                               false);
          ofpbuf_delete (buffer);
        }
      else
...
OpenFlowSwitchNetDevice::ForwardControlInput (const void *msg, size_t length)
{
  ...
  switch (oh->type)
    ...
    case OFPT_VPORT_TABLE_FEATURES_REQUEST:
      ...
      break;
    default:
      SendErrorMsg ((ofp_error_type) OFPET_BAD_REQUEST, (ofp_bad_request_code) OFPBRC_BAD_TYPE, msg,
                    length);
      error = -EINVAL;
    }

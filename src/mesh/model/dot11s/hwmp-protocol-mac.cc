...
HwmpProtocolMac::ReceiveAction (Ptr<Packet> packet, const WifiMacHeader &header)
{
  ...
  if (failedDestinations.size () > 0)
    {
      m_protocol->ReceivePerr (failedDestinations, header.GetAddr2 (), m_ifIndex,
                               header.GetAddr3 ());
    }
...
void
HwmpProtocolMac::RequestDestination (Mac48Address dst, uint32_t originator_seqno,
                                     uint32_t dst_seqno)
{
  ...
  for (std::vector<IePreq>::iterator i = m_myPreq.begin (); i != m_myPreq.end (); i++)
    {
      ...
      i->AddDestinationAddressElement (m_protocol->GetDoFlag (), m_protocol->GetRfFlag (), dst,
                                       dst_seqno);
    }
  IePreq preq;
  preq.SetHopcount (0);
  ...
  preq.AddDestinationAddressElement (m_protocol->GetDoFlag (), m_protocol->GetRfFlag (), dst,
                                     dst_seqno);
                              ...
                              std::vector<Mac48Address> receivers)
{
  ...
  MeshInformationElementVector elements;
  for (std::vector<HwmpProtocol::FailedDestination>::const_iterator i = failedDestinations.begin ();
       i != failedDestinations.end (); i++)
    {
      if (!perr->IsFull ())
        {

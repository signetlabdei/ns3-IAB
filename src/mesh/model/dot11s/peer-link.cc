  ...
  static TypeId tid =
      TypeId ("ns3::dot11s::PeerLink")
          ...
          .AddAttribute ("RetryTimeout", "Retry timeout",
                         ...
                         MakeTimeAccessor (&PeerLink::m_dot11MeshRetryTimeout), MakeTimeChecker ())
          .AddAttribute (
              "HoldingTimeout", "Holding timeout", TimeValue (TimeValue (MicroSeconds (40 * 1024))),
              MakeTimeAccessor (&PeerLink::m_dot11MeshHoldingTimeout), MakeTimeChecker ())
          .AddAttribute (
              "ConfirmTimeout", "Confirm timeout", TimeValue (TimeValue (MicroSeconds (40 * 1024))),
              MakeTimeAccessor (&PeerLink::m_dot11MeshConfirmTimeout), MakeTimeChecker ())
          .AddAttribute ("MaxRetries", "Maximum number of retries", UintegerValue (4),
          ...
          .AddAttribute ("MaxBeaconLoss",
                         ...
                         UintegerValue (2), MakeUintegerAccessor (&PeerLink::m_maxBeaconLoss),
                         MakeUintegerChecker<uint16_t> (1))
          .AddAttribute ("MaxPacketFailure",
                         "Maximum number of failed packets before link will be closed",
                         UintegerValue (2), MakeUintegerAccessor (&PeerLink::m_maxPacketFail),
                         MakeUintegerChecker<uint16_t> (1));
  return tid;
}

const char *const PeerLink::PeerStateNames[6] = {"IDLE",     "OPN_SNT", "CNF_RCVD",
                                                 "OPN_RCVD", "ESTAB",   "HOLDING"};
...
void
PeerLink::OpenReject (uint16_t localLinkId, IeConfiguration conf, Mac48Address peerMp,
...
void
PeerLink::ConfirmAccept (uint16_t localLinkId, uint16_t peerLinkId, uint16_t peerAid,
  ...
  switch (m_state)
    ...
    case OPN_SNT:
      switch (event)
        ...
        case CNF_ACPT:
          m_state = CNF_RCVD;
          m_linkStatusCallback (m_interface, m_peerAddress, m_peerMeshPointAddress, OPN_SNT,
                                CNF_RCVD);
          ClearRetryTimer ();
          SetConfirmTimer ();
          break;
    ...
    case CNF_RCVD:
      switch (event)
        ...
        case OPN_ACPT:
          m_state = ESTAB;
          m_linkStatusCallback (m_interface, m_peerAddress, m_peerMeshPointAddress, CNF_RCVD,
                                ESTAB);
          ClearConfirmTimer ();
          SendPeerLinkConfirm ();
    ...
    case OPN_RCVD:
      switch (event)
        ...
        case CNF_ACPT:
          m_state = ESTAB;
          m_linkStatusCallback (m_interface, m_peerAddress, m_peerMeshPointAddress, OPN_RCVD,
                                ESTAB);
          ClearRetryTimer ();
          NS_ASSERT (m_peerMeshPointAddress != Mac48Address::GetBroadcast ());
          break;
    ...
    case HOLDING:
      switch (event)
        ...
        case CNF_ACPT:
          m_state = HOLDING;
          m_linkStatusCallback (m_interface, m_peerAddress, m_peerMeshPointAddress, HOLDING,
                                HOLDING);
...
PeerLink::SendPeerLinkClose (PmpReasonCode reasoncode)
{
  ...
  m_macPlugin->SendPeerLinkManagementFrame (m_peerAddress, m_peerMeshPointAddress, m_assocId,
                                            peerElement, m_configuration);
}
void

...
public:
  ...
  typedef Callback<void, uint32_t, Mac48Address, Mac48Address, PeerLink::PeerState,
                   PeerLink::PeerState>
...
private:
   ...
   * \param reason the reason to close
   */
  void OpenReject (uint16_t localLinkId, IeConfiguration conf, Mac48Address peerMp,
   ...
   * \param peerMp the peer MP
   */
  void ConfirmAccept (uint16_t localLinkId, uint16_t peerLinkId, uint16_t peerAid,
   ...
   * \param reason the reason to close
   */
  void ConfirmReject (uint16_t localLinkId, uint16_t peerLinkId, IeConfiguration conf,

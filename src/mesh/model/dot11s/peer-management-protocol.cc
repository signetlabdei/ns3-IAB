...
void
PeerManagementProtocol::ReceiveBeacon (uint32_t interface, Mac48Address peerAddress,
                                       Time beaconInterval, Ptr<IeBeaconTiming> timingElement)
{
  //PM STATE Machine
  ...
  if (peerManagementElement.SubtypeIsOpen ())
    {
      ...
      if (!reject)
        ...
        {
          peerLink->OpenReject (peerManagementElement.GetLocalLinkId (), meshConfig,
                                peerMeshPointAddress, reasonCode);
        }
    }
  if (!peerLink)
    {
...
void
PeerManagementProtocol::NotifyLinkOpen (Mac48Address peerMp, Mac48Address peerIface,
...
void
PeerManagementProtocol::NotifyLinkClose (Mac48Address peerMp, Mac48Address peerIface,
                                         Mac48Address myIface, uint32_t interface)
{
...
PeerManagementProtocol::NotifyBeaconSent (uint32_t interface, Time beaconInterval)
{
  ...
  Simulator::Schedule (beaconInterval - TuToTime (m_maxBeaconShift + 1),
                       &PeerManagementProtocol::CheckBeaconCollisions, this, interface);

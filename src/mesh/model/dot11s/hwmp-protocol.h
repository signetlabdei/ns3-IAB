...
public:
   ...
   */
  void PeerLinkStatus (Mac48Address meshPontAddress, Mac48Address peerAddress, uint32_t interface,
                       bool status);
  /**
...
private:
  ...
  struct QueuedPacket
  {
    ...
    uint32_t
        inInterface; ///< incoming device interface ID. (if packet has come from upper layers, this is Mesh point ID)
  ...
  bool ForwardUnicast (uint32_t sourceIface, const Mac48Address source,
                       ...
                       RouteReplyCallback routeReply, uint32_t ttl);
   ...
   */
  void ReceivePreq (IePreq preq, Mac48Address from, uint32_t interface, Mac48Address fromMp,
                    uint32_t metric);
  /**
   ...
   * \param metric the metric
   */
  void ReceivePrep (IePrep prep, Mac48Address from, uint32_t interface, Mac48Address fromMp,
                    uint32_t metric);
  /**
   ...
   * \param fromMp the from MP address
   */
  void ReceivePerr (std::vector<FailedDestination> destinations, Mac48Address from,
...
private:
  ...
  uint8_t
      m_dot11MeshHWMPmaxPREQretries; //!< Maximum number of retries before we suppose the destination to be unreachable
  Time
      m_dot11MeshHWMPnetDiameterTraversalTime; //!< Time we suppose the packet to go from one edge of the network to another
  ...
  uint8_t
      m_unicastPerrThreshold; //!< Maximum number of PERR receivers, when we send a PERR as a chain of unicasts
  uint8_t
      m_unicastPreqThreshold; //!< Maximum number of PREQ receivers, when we send a PREQ as a chain of unicasts
  uint8_t
      m_unicastDataThreshold; //!< Maximum number of broadcast receivers, when we send a broadcast as a chain of unicasts

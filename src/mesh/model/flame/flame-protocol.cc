  ...
  static TypeId tid =
      TypeId ("ns3::flame::FlameProtocol")
          ...
          .AddAttribute ("BroadcastInterval", "How often we must send broadcast packets",
                         TimeValue (Seconds (5)),
                         MakeTimeAccessor (&FlameProtocol::m_broadcastInterval), MakeTimeChecker ())
          .AddAttribute ("MaxCost", "Cost threshold after which packet will be dropped",
                         UintegerValue (32), MakeUintegerAccessor (&FlameProtocol::m_maxCost),
...
bool
FlameProtocol::RequestRoute (uint32_t sourceIface, const Mac48Address source,
                             const Mac48Address destination, Ptr<const Packet> const_packet,
      ...
      if (destination == Mac48Address::GetBroadcast ())
        {
          ...
          NS_ASSERT (HandleDataFrame (flameHdr.GetSeqno (), source, flameHdr, tag.transmitter,
                                      sourceIface));
          ...
          packet->AddHeader (flameHdr);
          packet->AddPacketTag (tag);
          routeReply (true, packet, source, destination, FLAME_PROTOCOL,
      ...
      else
        {
          ...
          if (HandleDataFrame (flameHdr.GetSeqno (), source, flameHdr, tag.transmitter,
                               sourceIface))
            {
              return false;
            }

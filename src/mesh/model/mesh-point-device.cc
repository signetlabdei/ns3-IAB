...
MeshPointDevice::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::MeshPointDevice")
          ...
          .AddAttribute ("Mtu", "The MAC-level Maximum Transmission Unit", UintegerValue (0xffff),
...
MeshPointDevice::MeshPointDevice () : m_ifIndex (0)
{
...
void
MeshPointDevice::ReceiveFromDevice (Ptr<NetDevice> incomingPort, Ptr<const Packet> packet,
  ...
  if (dst48.IsGroup ())
    {
      ...
      if (m_routingProtocol->RemoveRoutingStuff (incomingPort->GetIfIndex (), src48, dst48,
                                                 packet_copy, realProtocol))
        {
          ...
          Simulator::Schedule (forwardingDelay, &MeshPointDevice::Forward, this, incomingPort,
                               packet, protocol, src48, dst48);
        }
      return;
    }
  if (dst48 == m_address)
  ...
  else
    {
      Time forwardingDelay = GetForwardingDelay ();
      Simulator::Schedule (forwardingDelay, &MeshPointDevice::Forward, this, incomingPort,
                           packet->Copy (), protocol, src48, dst48);
                          ...
                          const Mac48Address src, const Mac48Address dst)
{
  ...
  bool result = m_routingProtocol->RequestRoute (inport->GetIfIndex (), src, dst, packet, protocol,
...
MeshPointDevice::Send (Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
  ...
  return m_routingProtocol->RequestRoute (m_ifIndex, m_address, dst48, packet, protocolNumber,
                                          MakeCallback (&MeshPointDevice::DoSend, this));
}
                           ...
                           uint16_t protocolNumber)
{
  ...
  const Mac48Address src48 = Mac48Address::ConvertFrom (src);
  const Mac48Address dst48 = Mac48Address::ConvertFrom (dest);
  return m_routingProtocol->RequestRoute (m_ifIndex, src48, dst48, packet, protocolNumber,
                                          MakeCallback (&MeshPointDevice::DoSend, this));
}
...
MeshPointDevice::AddInterface (Ptr<NetDevice> iface)
{
  ...
  m_node->RegisterProtocolHandler (MakeCallback (&MeshPointDevice::ReceiveFromDevice, this), 0,
                                   iface, /*promiscuous = */
                                   true);

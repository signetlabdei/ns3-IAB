...
DsrNodeStab::DsrNodeStab (Time nodeStab) : m_nodeStability (nodeStab + Simulator::Now ())
{
}
...
DsrLinkStab::DsrLinkStab (Time linkStab) : m_linkStability (linkStab + Simulator::Now ())
{
}
  ...
  if (i == m_sortedRoutes.end ())
  ...
  else
    {
      ...
      if (rtVector.size () >= m_maxEntriesEachDst)
        {
          RemoveLastEntry (
              rtVector); // Drop the last entry for the sorted route cache, the route has already been sorted
        }
...
void
DsrRouteCache::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode,
                                           Ipv4Address node)
{
...
struct CloseNeighbor
{
   ...
   */
  bool
  operator() (const DsrRouteCache::Neighbor &nb) const
  {

...
public:
   ...
   */
  void PacketNewRoute (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
                       uint8_t protocol);
  /**
   ...
   * \param protocol number
   */
  void SendUnreachError (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst,
   ...
   */
  void ForwardErrPacket (DsrOptionRerrUnreachHeader &rerr, DsrOptionSRHeader &sourceRoute,
   ...
   * \param protocol number
   */
  void SendPacketFromBuffer (DsrOptionSRHeader const &sourceRoute, Ipv4Address nextHop,
                             uint8_t protocol);
  /**
   ...
   */
  bool PassiveEntryCheck (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
  ...
  */
   ...
   * \return
   */
  bool CancelPassiveTimer (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
                           uint8_t segsLeft);
  /**
   ...
   */
  void CallCancelPacketTimer (uint16_t ackId, Ipv4Header const &ipv4Header, Ipv4Address realSrc,
                              Ipv4Address realDst);
  /**
   ...
   * \param protocol number
   */
  void SalvagePacket (Ptr<const Packet> packet, Ipv4Address source, Ipv4Address dst,
                      uint8_t protocol);
  /**
  ...
  void ForwardPacket (Ptr<const Packet> packet, DsrOptionSRHeader &sourceRoute,
                      Ipv4Header const &ipv4Header, Ipv4Address source, Ipv4Address destination,
   ...
   * \param route Route
   */
  void ScheduleInitialReply (Ptr<Packet> packet, Ipv4Address source, Ipv4Address nextHop,
                             Ptr<Ipv4Route> route);
  /**
   ...
   */
  void ScheduleCachedReply (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
   ...
   * \param route Route
   */
  void SendAck (uint16_t ackId, Ipv4Address destination, Ipv4Address realSrc, Ipv4Address realDst,
   ...
   * in the stack.
   */
  virtual enum IpL4Protocol::RxStatus Receive (Ptr<Packet> p, Ipv4Header const &header,
   ...
   */
  virtual enum IpL4Protocol::RxStatus Receive (Ptr<Packet> p, Ipv6Header const &header,
   ...
   */
  uint8_t Process (Ptr<Packet> &packet, Ipv4Header const &ipv4Header, Ipv4Address dst,
                   uint8_t *nextHeader, uint8_t protocol, bool &isDropped);
  /**
   ...
   * \param protocol the protocol number
   */
  void ScheduleRreqRetry (Ptr<Packet> packet, std::vector<Ipv4Address> address, bool nonProp,
   ...
   * \param protocol the protocol number
   */
  void RouteRequestTimerExpire (Ptr<Packet> packet, std::vector<Ipv4Address> address,
                                uint32_t requestId, uint8_t protocol);
...
private:
   ...
   */
  bool PromiscReceive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,
  ...
  uint32_t
      m_rreqRetries; ///< Maximum number of retransmissions of RREQ with TTL = NetDiameter to discover a route
  ...
  uint32_t
      m_maxSendBuffLen; ///< The maximum number of packets that we allow a routing protocol to buffer.

  Time
      m_sendBufferTimeout; ///< The maximum period of time that a routing protocol is allowed to buffer a packet for.
  ...
  Ptr<dsr::DsrRreqTable>
      m_rreqTable; ///< A "drop-front" queue used by the routing layer to cache route request sent.

  Ptr<dsr::DsrPassiveBuffer>
      m_passiveBuffer; ///< A "drop-front" queue used by the routing layer to cache route request sent.
  ...
  std::vector<Ipv4Address>
      m_addresses; ///< The bind ipv4 addresses with next hop, src, destination address in sequence

...
public:
   ...
   * \param r Route
   */
  DsrNetworkQueueEntry (Ptr<const Packet> pa = 0, Ipv4Address s = Ipv4Address (),
                        Ipv4Address n = Ipv4Address (), Time exp = Simulator::Now (),
                        Ptr<Ipv4Route> r = 0)
      : m_packet (pa), m_srcAddr (s), m_nextHopAddr (n), tstamp (exp), m_ipv4Route (r)
  {
  }
  /**
   ...
   */
  bool
  operator== (DsrNetworkQueueEntry const &o) const
  {
   ...
   */
  void
  SetIpv4Route (Ptr<Ipv4Route> route)
  {
    m_ipv4Route = route;
  }
  /**
   ...
   */
  void
  SetSourceAddress (Ipv4Address addr)
  {
    m_srcAddr = addr;
  }
  /**
   ...
   */
  void
  SetNextHopAddress (Ipv4Address addr)
  {
    m_nextHopAddr = addr;
  }
  /**
   ...
   */
  Time
  GetInsertedTimeStamp (void) const
  {
    return tstamp;
  }
  /**
   ...
   */
  void
  SetInsertedTimeStamp (Time time)
  {
    tstamp = time;
  }

...
public:
   ...
   */
  Ptr<const ChannelMatrix> GetChannel (Ptr<const MobilityModel> aMob, Ptr<const MobilityModel> bMob,
...
protected:
  ...
  struct ThreeGppChannelParams : public MatrixBasedChannelModel::ChannelParams
  {
    ...
    MatrixBasedChannelModel::Double2DVector
        m_norRvAngles; //!< stores the normal variable for random angles angle[cluster][id] generated for equation (7.6-11)-(7.6-14), where id = 0(aoa),1(zoa),2(aod),3(zod)
  ...
  struct ParamsTable : public SimpleRefCount<ParamsTable>
  {
    ...
    double m_sqrtC
        [7][7]; //!< The square root matrix and follows the order of [SF, K, DS, ASD, ASA, ZSD, ZSA]
  };
  ...
  Ptr<ThreeGppChannelParams> GenerateChannelParameters (
      const Ptr<const ChannelCondition> channelCondition, const Ptr<const ParamsTable> table3gpp,
      const Ptr<const MobilityModel> aMob, const Ptr<const MobilityModel> bMob) const;
  ...
  GetNewChannel (Ptr<const ThreeGppChannelParams> channelParams, Ptr<const ParamsTable> table3gpp,
                 const Ptr<const MobilityModel> sMob, const Ptr<const MobilityModel> uMob,
                 Ptr<const PhasedArrayModel> sAntenna, Ptr<const PhasedArrayModel> uAntenna) const;
  /**
   ...
   */
  DoubleVector
  CalcAttenuationOfBlockage (const Ptr<ThreeGppChannelModel::ThreeGppChannelParams> channelParams,
  ...
  std::unordered_map<uint64_t, Ptr<ChannelMatrix>>
      m_channelMatrixMap; //!< map containing the channel realizations per pair of PhasedAntennaArray instances, the key of this map is reciprocal uniquely identifies a pair of PhasedAntennaArrays
  std::unordered_map<uint64_t, Ptr<ThreeGppChannelParams>>
      m_channelParamsMap; //!< map containing the common channel parameters per pair of nodes, the key of this map is reciprocal and uniquely identifies a pair of nodes
  ...
  double
      m_vScatt; //!< value used to compute the additional Doppler contribution for the delayed paths
  Ptr<UniformRandomVariable>
      m_uniformRvDoppler; //!< uniform random variable, used to compute the additional Doppler contribution

...
ThreeGppSpectrumPropagationLossModel::CalcLongTerm (
    ...
    const PhasedArrayModel::ComplexVector &sW, const PhasedArrayModel::ComplexVector &uW) const
{
  NS_LOG_FUNCTION (this);
...
ThreeGppSpectrumPropagationLossModel::CalcBeamformingGain (
    ...
    Ptr<const MatrixBasedChannelModel::ChannelParams> channelParams, const ns3::Vector &sSpeed,
    ...
    Ptr<const PhasedArrayModel> bPhasedArrayModel) const
{
  ...
  rxPsd = CalcBeamformingGain (rxPsd, longTerm, channelMatrix, channelParams, a->GetVelocity (),
                               b->GetVelocity ());

...
public:
  ...
  struct ChannelMatrix : public SimpleRefCount<ChannelMatrix>
  {
    ...
    std::pair<uint32_t, uint32_t>
        m_antennaPair; //!< the first element is the ID of the antenna of the s-node (the antenna of the transmitter when the channel was generated), the second element is ID of the antenna of the u-node antenna (the antenna of the receiver when the channel was generated)
    std::pair<uint32_t, uint32_t>
        m_nodeIds; //!< the first element is the s-node ID (the transmitter when the channel was generated), the second element is the u-node ID (the receiver when the channel was generated)
     ...
     */
    bool
    IsReverse (uint32_t aAntennaId, uint32_t bAntennaId) const
    {
      ...
      NS_ASSERT_MSG ((sAntennaId == aAntennaId && uAntennaId == bAntennaId) ||
                     ...
                     "This channel matrix does not represent the channel among the antenna arrays "
                     "for which are provided IDs.");
  ...
  struct ChannelParams : public SimpleRefCount<ChannelParams>
  {
    ...
    Double2DVector
        m_angle; //!< cluster angle angle[direction][n], where direction = 0(AOA), 1(ZOA), 2(AOD), 3(ZOD) in degree.
    DoubleVector
        m_alpha; //!< alpha term per cluster as described in 3GPP TR 37.885 v15.3.0, Sec. 6.2.3 for calculating doppler
    DoubleVector
        m_D; //!< D term per cluster as described in 3GPP TR 37.885 v15.3.0, Sec. 6.2.3 for calculating doppler
    std::pair<uint32_t, uint32_t>
        m_nodeIds; //!< the first element is the s-node ID (the transmitter when the channel params were generated), the second element is the u-node ID (the receiver when the channel params were generated generated)
    /**

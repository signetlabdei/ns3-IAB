...
SingleModelSpectrumChannel::StartTx (Ptr<SpectrumSignalParameters> txParams)
{
  ...
  for (PhyList::const_iterator rxPhyIterator = m_phyList.begin ();
       rxPhyIterator != m_phyList.end (); ++rxPhyIterator)
    {
          ...
          if (senderMobility && receiverMobility)
            {
              ...
              if (rxAntenna)
                {
                  Angles rxAngles (senderMobility->GetPosition (),
              ...
              // Gain trace
              m_gainTrace (senderMobility, receiverMobility, txAntennaGain, rxAntennaGain,
          ...
          if (rxNetDevice)
            {
              ...
              Simulator::ScheduleWithContext (dstNode, delay, &SingleModelSpectrumChannel::StartRx,
                                              this, rxParams, *rxPhyIterator);
            }
          else
            {
              // the receiver is not attached to a NetDevice, so we cannot assume that it is attached to a node
              Simulator::Schedule (delay, &SingleModelSpectrumChannel::StartRx, this, rxParams,
                                   *rxPhyIterator);
            }
        }
    }
...
void
SingleModelSpectrumChannel::StartRx (Ptr<SpectrumSignalParameters> params,

...
public:
   ...
   */
  SpectrumIdealPhyTestCase (double snrLinear, uint64_t phyRate, bool rateIsAchievable,
...
SpectrumIdealPhyTestCase::SpectrumIdealPhyTestCase (double snrLinear, uint64_t phyRate,
...
SpectrumIdealPhyTestCase::DoRun (void)
{
  ...
  propLoss->SetLoss (c.Get (0)->GetObject<MobilityModel> (), c.Get (1)->GetObject<MobilityModel> (),
                     lossDb, true);
  ...
  if (m_rateIsAchievable)
    {
      NS_TEST_ASSERT_MSG_EQ_TOL (throughputBps, m_phyRate, m_phyRate * 0.01,
                                 "throughput does not match PHY rate");
    }
  else
    {
      NS_TEST_ASSERT_MSG_EQ (throughputBps, 0.0,
...
SpectrumIdealPhyTestSuite::SpectrumIdealPhyTestSuite () : TestSuite ("spectrum-ideal-phy", SYSTEM)
{
  ...
  for (double snr = 0.01; snr <= 10; snr *= 2)
    {
      ...
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.1),
                                                 true, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.5),
                                                 true, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.95),
                                                 true, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 1.05),
                                                 false, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 2),
                                                 false, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 4),
                                                 false, "ns3::SingleModelSpectrumChannel"),
                   TestCase::QUICK);
    }
  for (double snr = 0.01; snr <= 10; snr *= 10)
    {
      double achievableRate = g_bandwidth * log2 (1 + snr);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.1),
                                                 true, "ns3::MultiModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.5),
                                                 true, "ns3::MultiModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 0.95),
                   ...
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 1.05),
                                                 false, "ns3::MultiModelSpectrumChannel"),
                   TestCase::QUICK);
      AddTestCase (new SpectrumIdealPhyTestCase (snr, static_cast<uint64_t> (achievableRate * 2),
                                                 false, "ns3::MultiModelSpectrumChannel"),
                   TestCase::QUICK);

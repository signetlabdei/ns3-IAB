...
private:
   ...
   */
  void DoComputeNorm (Ptr<ThreeGppChannelModel> channelModel, Ptr<MobilityModel> txMob,
      ...
      for (uint64_t sIndex = 0; sIndex < txAntennaElements; sIndex++)
        {
          for (uint32_t uIndex = 0; uIndex < rxAntennaElements; uIndex++)
            {
              clusterNorm += std::pow (
                  std::abs (channelMatrix->m_channel.at (uIndex).at (sIndex).at (cIndex)), 2);
            }
        }
...
ThreeGppChannelMatrixComputationTest::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (
      channelMatrix->m_channel.at (0).size (), txAntennaElements[0] * txAntennaElements[1],
      "The second dimension of H should be equal to the number of tx antenna elements");
  NS_TEST_ASSERT_MSG_EQ (
      channelMatrix->m_channel.size (), rxAntennaElements[0] * rxAntennaElements[1],
  ...
  for (uint16_t i = 0; i < numIt; i++)
    {
      Simulator::Schedule (MilliSeconds (updatePeriodMs * i),
                           &ThreeGppChannelMatrixComputationTest::DoComputeNorm, this, channelModel,
                           txMob, rxMob, txAntenna, rxAntenna);
    }
...
private:
   ...
   */
  void DoGetChannel (Ptr<ThreeGppChannelModel> channelModel, Ptr<MobilityModel> txMob,
                     Ptr<MobilityModel> rxMob, Ptr<PhasedArrayModel> txAntenna,
                                               ...
                                               Ptr<PhasedArrayModel> rxAntenna, bool update)
{
  ...
  if (!m_currentChannel)
  ...
  else
    {
      ...
      NS_TEST_ASSERT_MSG_EQ ((m_currentChannel != channelMatrix), update,
...
ThreeGppChannelMatrixUpdateTest::DoRun (void)
{
  ...
  Simulator::Schedule (MilliSeconds (firstTimeMs + updatePeriodMs / 2),
                       &ThreeGppChannelMatrixUpdateTest::DoGetChannel, this, channelModel, txMob,
                       rxMob, txAntenna, rxAntenna, false);
  ...
  Simulator::Schedule (MilliSeconds (firstTimeMs + updatePeriodMs + 1),
                       &ThreeGppChannelMatrixUpdateTest::DoGetChannel, this, channelModel, txMob,
                       rxMob, txAntenna, rxAntenna, true);
...
private:
   ...
   */
  void DoBeamforming (Ptr<NetDevice> thisDevice, Ptr<PhasedArrayModel> thisAntenna,
    ...
    const CheckLongTermUpdateParams &params)
{
  Ptr<SpectrumValue> rxPsdNew = params.lossModel->DoCalcRxPowerSpectralDensity (
      params.txParams, params.txMob, params.rxMob, params.txAntenna, params.rxAntenna);
  NS_TEST_ASSERT_MSG_EQ (ArePsdEqual (params.rxPsdOld, rxPsdNew), false,
...
ThreeGppSpectrumPropagationLossModelTest::DoRun ()
{
  ...
  lossModel->SetChannelModelAttribute (
      ...
      PointerValue (
          condModel)); // create the ThreeGppChannelModel object used to generate the channel matrix
  ...
  NS_TEST_ASSERT_MSG_EQ (ArePsdEqual (rxPsdOld, rxPsdNew), true,
...
ThreeGppChannelTestSuite::ThreeGppChannelTestSuite () : TestSuite ("three-gpp-channel", UNIT)
{

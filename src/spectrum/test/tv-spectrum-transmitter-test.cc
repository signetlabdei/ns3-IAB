...
public:
   ...
   */
  TvSpectrumTransmitterTestCase (double startFrequency, double channelBandwidth, double basePsd,
...
private:
   ...
   */
  static std::string Name (TvSpectrumTransmitter::TvType tvType, double startFrequency,
...
std::string
TvSpectrumTransmitterTestCase::Name (TvSpectrumTransmitter::TvType tvType, double startFrequency,
                                     double channelBandwidth, double basePsd)
{
...
TvSpectrumTransmitterTestCase::DoRun (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL ((*(bandEnd - 1)).fc, m_startFrequency + m_channelBandwidth, epsilon,
      ...
      for (double bandwidth = 100; bandwidth < 1e15; bandwidth *= 10)
        {
          for (double psd = -100; psd <= 100; psd += 20)
            {
              AddTestCase (new TvSpectrumTransmitterTestCase (startFreq, bandwidth, psd,

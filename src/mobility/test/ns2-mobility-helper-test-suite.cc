...
public:
  ...
  struct ReferencePoint
  {
    ...
    /**
         ...
         */
    bool
    operator<(const ReferencePoint &o) const
    {
      return (time < o.time);
    }
  };
  ...
  Ns2MobilityHelperTest (const std::string &name, Time timeLimit, uint32_t nodes = 1)
      : TestCase (name), m_timeLimit (timeLimit), m_nodeCount (nodes), m_nextRefPoint (0)
  {
  }
  ...
  /**
     ...
     */
  void
  SetTrace (const std::string &trace)
  {
    m_trace = trace;
  }
  ...
  /**
     ...
     */
  void
  AddReferencePoint (const ReferencePoint &r)
  {
  ...
  /**
     ...
     */
  void
  AddReferencePoint (const char *id, double sec, const Vector &p, const Vector &v)
  {
...
private:
    ...
    while (m_nextRefPoint < m_reference.size () && m_reference[m_nextRefPoint].time == Seconds (0))
      {
        ...
        NS_TEST_ASSERT_MSG_NE_RETURNS_BOOL (mob, nullptr,
                                            "Can't find mobility for node " << rp.node);

        double tol = 0.001;
        NS_TEST_EXPECT_MSG_EQ (AreVectorsEqual (mob->GetPosition (), rp.pos, tol), true,
                               "Initial position mismatch for node " << rp.node);
        NS_TEST_EXPECT_MSG_EQ (AreVectorsEqual (mob->GetVelocity (), rp.vel, tol), true,
  ...
  CourseChange (std::string context, Ptr<const MobilityModel> mobility)
  {
    ...
    double tol = 0.001;
    NS_TEST_EXPECT_MSG_EQ (AreVectorsEqual (pos, ref.pos, tol), true,
                           "Position mismatch at time " << time.GetSeconds () << " s for node "
                                                        << id);
    NS_TEST_EXPECT_MSG_EQ (AreVectorsEqual (vel, ref.vel, tol), true,
...
public:
  Ns2MobilityHelperTestSuite () : TestSuite ("mobility-ns2-trace-helper", UNIT)
  {
    ...
    t->AddReferencePoint ("0", 0.000, Vector (350.000, 50.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 50.000, Vector (350.000, 50.000, 0.000),
                          Vector (1.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 100.000, Vector (400.000, 50.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 150.000, Vector (400.000, 50.000, 0.000),
                          Vector (0.000, 4.000, 0.000));
    t->AddReferencePoint ("0", 175.000, Vector (400.000, 150.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 300.000, Vector (400.000, 150.000, 0.000),
                          Vector (-3.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 350.000, Vector (250.000, 150.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 350.000, Vector (250.000, 150.000, 0.000),
                          Vector (0.000, -1.000, 0.000));
    t->AddReferencePoint ("0", 450.000, Vector (250.000, 50.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    t->AddReferencePoint ("0", 600.000, Vector (250.000, 50.000, 0.000),
                          Vector (0.000, 2.000, 0.000));
    t->AddReferencePoint ("0", 900.000, Vector (250.000, 650.000, 0.000),
                          Vector (2.500, 0.000, 0.000));
    t->AddReferencePoint ("0", 920.000, Vector (300.000, 650.000, 0.000),
                          Vector (0.000, 0.000, 0.000));
    AddTestCase (t, TestCase::QUICK);
  }

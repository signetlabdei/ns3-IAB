...
WaypointMobilityModelNotifyTest::DoRun ()
{
  ...
  for (double updateTime = 0.5; updateTime <= ((double) waypointCount + 1.5); updateTime += 1.0)
    {
      Simulator::Schedule (Seconds (updateTime), &WaypointMobilityModelNotifyTest::ForceUpdates,
                           this);
    }
...
{
public:
  WaypointMobilityModelAddWaypointTest () : TestCase ("Check Waypoint Mobility Model waypoint add")
  {
  }

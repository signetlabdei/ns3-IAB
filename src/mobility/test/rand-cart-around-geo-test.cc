...
public:
  /**
     ...
     */
  RandCartAroundGeoTestCase (double originLatitude, double originLongitude, double maxAltitude,
...
std::string
RandCartAroundGeoTestCase::Name (double originLatitude, double originLongitude,
...
RandCartAroundGeoTestCase::RandCartAroundGeoTestCase (double originLatitude, double originLongitude,
...
{
  std::list<Vector> points = GeographicPositions::RandCartesianPointsAroundGeographicPoint (
      m_originLatitude, m_originLongitude, m_maxAltitude, m_numPoints, m_maxDistFromOrigin,
      m_uniRand);
  Vector origin = GeographicPositions::GeographicToCartesianCoordinates (
      m_originLatitude, m_originLongitude, m_maxAltitude, GeographicPositions::SPHERE);
  Vector randPoint;
  while (!points.empty ())
    {
...
RandCartAroundGeoTestSuite::RandCartAroundGeoTestSuite () : TestSuite ("rand-cart-around-geo", UNIT)
{
      ...
      for (double originLongitude = 0; originLongitude <= 360; originLongitude += 72)
        {
          for (double maxDistFromOrigin = 1000; maxDistFromOrigin <= 1000000;
            ...
            {
              AddTestCase (new RandCartAroundGeoTestCase (originLatitude, originLongitude,

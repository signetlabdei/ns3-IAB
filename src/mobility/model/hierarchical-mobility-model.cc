...
HierarchicalMobilityModel::HierarchicalMobilityModel () : m_child (nullptr), m_parent (nullptr)
{
  ...
  if (m_child)
    {
      ...
      m_child->TraceDisconnectWithoutContext (
          "CourseChange", MakeCallback (&HierarchicalMobilityModel::ChildChanged, this));
    }
  m_child = model;
...
HierarchicalMobilityModel::DoGetPosition () const
{
  ...
  return Vector (parentPosition.x + childPosition.x, parentPosition.y + childPosition.y,
...
HierarchicalMobilityModel::DoSetPosition (const Vector &position)
{
  ...
  if (m_parent)
    {
      Vector parentPosition = m_parent->GetPosition ();
      Vector childPosition (position.x - parentPosition.x, position.y - parentPosition.y,
  ...
  if (m_parent)
    {
      ...
      Vector speed (parentSpeed.x + childSpeed.x, parentSpeed.y + childSpeed.y,

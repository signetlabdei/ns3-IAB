  ...
  if (!model)
    {
      ...
      if (m_mobilityStack.empty ())
      ...
      else
        {
          ...
          Ptr<MobilityModel> hierarchical = CreateObjectWithAttributes<HierarchicalMobilityModel> (
              "Child", PointerValue (model), "Parent", PointerValue (parent));
...
MobilityHelper::EnableAscii (Ptr<OutputStreamWrapper> stream, uint32_t nodeid)
{
  ...
  Config::ConnectWithoutContextFailSafe (
      oss.str (), MakeBoundCallback (&MobilityHelper::CourseChanged, stream));
}

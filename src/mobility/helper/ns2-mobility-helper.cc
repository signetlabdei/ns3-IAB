 ...
 */
static DestinationPoint SetMovement (Ptr<ConstantVelocityMobilityModel> model, Vector lastPos,
                                     double at, double xFinalPosition, double yFinalPosition,
                                     double speed);
 ...
 * \return The vector of the position
 */
static Vector SetInitialPosition (Ptr<ConstantVelocityMobilityModel> model, std::string coord,
                                  double coordVal);
 ...
 */
static Vector SetSchedPosition (Ptr<ConstantVelocityMobilityModel> model, double at,
                                std::string coord, double coordVal);

Ns2MobilityHelper::Ns2MobilityHelper (std::string filename) : m_filename (filename)
{
              ...
              if (IsSchedMobilityPos (pr))
                {
                  ...
                  //                                     coord      velocity
                  last_pos[iNodeId] = SetMovement (model, last_pos[iNodeId].m_finalPosition, at,
  ...
  if (speed == 0)
    {
      ...
      retval.m_stopEvent = Simulator::Schedule (
          Seconds (at), &ConstantVelocityMobilityModel::SetVelocity, model, Vector (0, 0, 0));
      return retval;
    }
  if (speed > 0)
    {
...
// Schedule a set of position for a node
Vector
SetSchedPosition (Ptr<ConstantVelocityMobilityModel> model, double at, std::string coord,
                  double coordVal)
{
  // update position
  model->SetPosition (SetOneInitialCoord (model->GetPosition (), coord, coordVal));

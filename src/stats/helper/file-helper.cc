...
void
FileHelper::WriteProbe (const std::string &typeId, const std::string &path,
  ...
  if (matchCount == 1 && pathHasNoWildcards)
    {
      ...
      ConnectProbeToAggregator (typeId, matchIdentifier, path, probeTraceSource,
                                m_outputFileNameWithoutExtension, onlyOneAggregator);
    }
      ...
      for (uint32_t i = 0; i < matchCount; i++)
        {
          ...
          ConnectProbeToAggregator (typeId, matchIdentifier, matchedPath, probeTraceSource,
...
void
FileHelper::AddProbe (const std::string &typeId, const std::string &probeName,
...
void
FileHelper::AddAggregator (const std::string &aggregatorName, const std::string &outputFileName,
                           bool onlyOneAggregator)
{
...
void
FileHelper::ConnectProbeToAggregator (const std::string &typeId, const std::string &matchIdentifier,
  ...
  if (m_probeMap[probeName].second == "ns3::DoubleProbe")
    {
      m_probeMap[probeName].first->TraceConnectWithoutContext (
          probeTraceSource,
          MakeCallback (&TimeSeriesAdaptor::TraceSinkDouble, m_timeSeriesAdaptorMap[probeContext]));
    }

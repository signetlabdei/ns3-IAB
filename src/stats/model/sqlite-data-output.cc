...
SqliteDataOutput::SqliteDataOutput () : DataOutputInterface ()
{
...
SqliteDataOutput::Output (DataCollector &dc)
{
  ...
  res = m_sqliteOut->SpinExec ("CREATE TABLE IF NOT EXISTS Experiments (run, experiment, strategy, "
                               "input, description text)");
  NS_ASSERT (res);

  sqlite3_stmt *stmt;
  res = m_sqliteOut->WaitPrepare (&stmt, "INSERT INTO Experiments "
  ...
  res = m_sqliteOut->WaitPrepare (&stmt, "INSERT INTO Metadata "
                                         "(run, key, value)"
                                         "values (?, ?, ?)");
  NS_ASSERT (res);
...
void
SqliteDataOutput::SqliteOutputCallback::OutputStatistic (std::string key, std::string variable,
...
void
SqliteDataOutput::SqliteOutputCallback::OutputSingleton (std::string key, std::string variable,
                                                         int val)
{

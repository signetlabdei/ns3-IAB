...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("tapNetwork",
                ...
                "Note that the network address must be that from the vsys_vnet tag which must "
                "exist in your PlanetLab slice.",
                network);
  cmd.AddValue ("tapMask", "Network mask for configure the TAP device (dotted decimal only please)",
                mask);
  cmd.Parse (argc, argv);

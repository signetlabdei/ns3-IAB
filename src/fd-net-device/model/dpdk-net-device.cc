...
DpdkNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::DpdkNetDevice")
          ...
          .AddAttribute ("TxTimeout", "The time to wait before transmitting burst from Tx buffer.",
                         TimeValue (MicroSeconds (2000)),
                         MakeTimeAccessor (&DpdkNetDevice::m_txTimeout), MakeTimeChecker ())
          .AddAttribute ("MaxRxBurst", "Size of Rx Burst.", UintegerValue (64),
                         MakeUintegerAccessor (&DpdkNetDevice::m_maxRxPktBurst),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("MaxTxBurst", "Size of Tx Burst.", UintegerValue (64),
                         MakeUintegerAccessor (&DpdkNetDevice::m_maxTxPktBurst),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("MempoolCacheSize", "Size of mempool cache.", UintegerValue (256),
                         MakeUintegerAccessor (&DpdkNetDevice::m_mempoolCacheSize),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("NbRxDesc", "Number of Rx descriptors.", UintegerValue (1024),
                         MakeUintegerAccessor (&DpdkNetDevice::m_nbRxDesc),
                         MakeUintegerChecker<uint16_t> ())
          .AddAttribute ("NbTxDesc", "Number of Tx descriptors.", UintegerValue (1024),
                         MakeUintegerAccessor (&DpdkNetDevice::m_nbTxDesc),
                         MakeUintegerChecker<uint16_t> ());
  return tid;
}

DpdkNetDevice::DpdkNetDevice () : m_mempool (NULL)
{
...
DpdkNetDevice::InitDpdk (int argc, char **argv, std::string dpdkDriver)
{
  ...
  m_mempool = rte_pktmbuf_pool_create ("mbuf_pool", nbMbufs, m_mempoolCacheSize, 0,
  ...
  ret = rte_eth_rx_queue_setup (m_portId, 0, m_nbRxDesc, rte_eth_dev_socket_id (m_portId), &reqConf,
                                m_mempool);
  if (ret < 0)
    {
  ...
  m_txBuffer = (rte_eth_dev_tx_buffer *) rte_zmalloc_socket (
      "tx_buffer", RTE_ETH_TX_BUFFER_SIZE (m_maxTxPktBurst), 0, rte_eth_dev_socket_id (m_portId));
  NS_LOG_INFO ("Initialize Rx buffers");
  m_rxBuffer = (rte_eth_dev_tx_buffer *) rte_zmalloc_socket (
      "rx_buffer", RTE_ETH_TX_BUFFER_SIZE (m_maxRxPktBurst), 0, rte_eth_dev_socket_id (m_portId));
...
DpdkNetDevice::Write (uint8_t *buffer, size_t length)
{
  ...
  pkt[0] =
      (struct rte_mbuf *) RTE_PTR_SUB (buffer, sizeof (struct rte_mbuf) + RTE_PKTMBUF_HEADROOM);

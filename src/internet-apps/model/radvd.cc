  ...
  static TypeId tid =
      TypeId ("ns3::Radvd")
          ...
          .AddAttribute (
              ...
              StringValue ("ns3::UniformRandomVariable"), MakePointerAccessor (&Radvd::m_jitter),
  ...
  for (RadvdInterfaceListCI it = m_configurations.begin (); it != m_configurations.end (); it++)
    {
      if ((*it)->IsSendAdvert ())
        {
          m_unsolicitedEventIds[(*it)->GetInterface ()] = Simulator::Schedule (
              Seconds (0.), &Radvd::Send, this, (*it), Ipv6Address::GetAllNodesMulticast (), true);
        }
  ...
  if (reschedule)
    {
      ...
      m_unsolicitedEventIds[config->GetInterface ()] = Simulator::Schedule (
          t, &Radvd::Send, this, config, Ipv6Address::GetAllNodesMulticast (), true);
    }
}
                      ...
                      if (scheduleSingle)
                        {
                          ...
                          m_solicitedEventIds[(*it)->GetInterface ()] =
                              Simulator::Schedule (MilliSeconds (delay), &Radvd::Send, this, (*it),
                                                   Ipv6Address::GetAllNodesMulticast (), false);
                        }
                    }
                }
              break;

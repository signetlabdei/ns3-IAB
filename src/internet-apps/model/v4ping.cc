  ...
  static TypeId tid =
      TypeId ("ns3::V4Ping")
          ...
          .AddAttribute ("Remote", "The address of the machine we want to ping.",
                         Ipv4AddressValue (), MakeIpv4AddressAccessor (&V4Ping::m_remote),
                         MakeIpv4AddressChecker ())
          .AddAttribute ("Verbose", "Produce usual output.", BooleanValue (false),
                         MakeBooleanAccessor (&V4Ping::m_verbose), MakeBooleanChecker ())
          .AddAttribute ("Interval", "Wait  interval  seconds between sending each packet.",
                         TimeValue (Seconds (1)), MakeTimeAccessor (&V4Ping::m_interval),
                         MakeTimeChecker ())
          .AddAttribute ("Size",
                         "The number of data bytes to be sent, real packet will be 8 (ICMP) + 20 "
                         "(IP) bytes longer.",
                         UintegerValue (56), MakeUintegerAccessor (&V4Ping::m_size),
...
V4Ping::V4Ping ()
    : m_interval (Seconds (1)), m_size (56), m_socket (0), m_seq (0), m_verbose (false), m_recv (0)
{

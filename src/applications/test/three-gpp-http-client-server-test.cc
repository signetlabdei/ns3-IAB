...
public:
   ...
   */
  ThreeGppHttpObjectTestCase (const std::string &name, uint32_t rngRun, const TypeId &tcpType,
...
ThreeGppHttpObjectTestCase::ThreeGppHttpObjectTestCase (const std::string &name, uint32_t rngRun,
                                                      ...
                                                      Address &assignedAddress)
{
  ...
  dev->TraceConnectWithoutContext (
      "PhyRxDrop", MakeCallback (&ThreeGppHttpObjectTestCase::DeviceDropCallback, this));
...
ThreeGppHttpObjectTestCase::DoRun ()
{
  ...
  traceSourceConnected = httpServer->TraceConnectWithoutContext (
      "MainObject", MakeCallback (&ThreeGppHttpObjectTestCase::ServerMainObjectCallback, this));
  NS_ASSERT (traceSourceConnected);
  traceSourceConnected = httpClient->TraceConnectWithoutContext (
      "RxMainObjectPacket",
  ...
  NS_ASSERT (traceSourceConnected);
  traceSourceConnected = httpClient->TraceConnectWithoutContext (
      "RxMainObject", MakeCallback (&ThreeGppHttpObjectTestCase::ClientRxMainObjectCallback, this));
  NS_ASSERT (traceSourceConnected);
  ...
  NS_ASSERT (traceSourceConnected);
  traceSourceConnected = httpClient->TraceConnectWithoutContext (
      "RxDelay", MakeCallback (&ThreeGppHttpObjectTestCase::ClientRxDelayCallback, this));
  NS_ASSERT (traceSourceConnected);
  traceSourceConnected = httpClient->TraceConnectWithoutContext (
      "RxRtt", MakeCallback (&ThreeGppHttpObjectTestCase::ClientRxRttCallback, this));
  NS_ASSERT (traceSourceConnected);
...
void
ThreeGppHttpObjectTestCase::ClientRxEmbeddedObjectCallback (
            ...
            for (uint32_t i2 = 0; i2 < 2; i2++)
              {
                for (uint32_t i3 = 0; i3 < 2; i3++)
                  {
                    AddHttpObjectTestCase (run++, channelDelay[i1], bitErrorRate[i2], mtuSize[i3],
                                           false);
                    AddHttpObjectTestCase (run++, channelDelay[i1], bitErrorRate[i2], mtuSize[i3],
                                           true);
                  }
...
private:
   ...
   */
  void
  AddHttpObjectTestCase (uint32_t rngRun, const Time &channelDelay, double bitErrorRate,
                         uint32_t mtuSize, bool useIpv6)
  {
    ...
    AddTestCase (new ThreeGppHttpObjectTestCase (name.str (), rngRun, TcpNewReno::GetTypeId (),
                                                 channelDelay, bitErrorRate, mtuSize, useIpv6),
                 testDuration);
  }

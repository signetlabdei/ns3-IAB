...
CsmaMulticastTestCase::DoRun (void)
{
  ...
  multicast.AddMulticastRoute (multicastRouter, multicastSource, multicastGroup, inputIf,
                               outputDevices);
...
CsmaPacketSocketTestCase::DoRun (void)
{
  ...
  Ptr<CsmaChannel> channel = CreateObjectWithAttributes<CsmaChannel> (
      "DataRate", DataRateValue (DataRate (5000000)), "Delay", TimeValue (MilliSeconds (2)));
...
CsmaStarTestCase::DoRun (void)
{
  ...
  // nodes on each spoke.
  NS_TEST_ASSERT_MSG_EQ (m_count, 10 * (nSpokes * (nFill + 1)),

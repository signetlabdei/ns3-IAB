  ...
  if (m_writeVectors)
    {
      ...
      m_pcapFile.Write (uint32_t (tMicroSeconds / 1000000), uint32_t (tMicroSeconds % 1000000),
                        received);
    }
  else
    {
      ...
      uint32_t origLen;
      uint32_t readLen;
      m_pcapFile.Read (expectedBuffer, sizeof (expectedBuffer), tsSec, tsUsec, inclLen, origLen,
                       readLen);
      ...
      if (IsStatusSuccess ())
        {
          NS_TEST_EXPECT_MSG_EQ (
              result, 0, "Expected data comparison error: " << m_tcpModel << "-" << m_testCase);
        }
    }
}
...
Ns3TcpLossTestCase::DoRun ()
{
  ...
  localSocket->Bind ();
  Simulator::ScheduleNow (&Ns3TcpLossTestCase::StartFlow, this, localSocket,

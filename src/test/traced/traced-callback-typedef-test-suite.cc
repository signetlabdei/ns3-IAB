...
#define DUPE(U, T1)                                                                     \
  ...
  if (TypeName<U> (0) == TypeName<T1> (0))                                              \
    ...
    NS_TEST_ASSERT_MSG_EQ (TypeName<U> (0), TypeName<T1> (0),                           \
                           "the typedef " << #U << " used to match the typedef " << #T1 \
...
TracedCallbackTypedefTestCase::DoRun ()
{
  ...
  CHECK (Ipv4L3Protocol::DropTracedCallback, const Ipv4Header &, Ptr<const Packet>,
  ...
  CHECK (Ipv6L3Protocol::DropTracedCallback, const Ipv6Header &, Ptr<const Packet>,
  ...
  CHECK (LteEnbMac::DlSchedulingTracedCallback, uint32_t, uint32_t, uint16_t, uint8_t, uint16_t,
  ...
  CHECK (LteUeRrc::StateTracedCallback, uint64_t, uint16_t, uint16_t, LteUeRrc::State,
         LteUeRrc::State);
  ...
  CHECK (olsr::RoutingProtocol::PacketTxRxTracedCallback, const olsr::PacketHeader &,
  ...
  CHECK (SixLowPanNetDevice::DropTracedCallback, SixLowPanNetDevice::DropReason, Ptr<const Packet>,
         Ptr<SixLowPanNetDevice>, uint32_t);

  CHECK (SixLowPanNetDevice::RxTxTracedCallback, Ptr<const Packet>, Ptr<SixLowPanNetDevice>,
         uint32_t);

  CHECK (SpectrumChannel::LossTracedCallback, Ptr<const SpectrumPhy>, Ptr<const SpectrumPhy>,
         double);
  ...
  CHECK (UeManager::StateTracedCallback, uint64_t, uint16_t, uint16_t, UeManager::State,
         UeManager::State);
  ...
  CHECK (WifiPhyStateHelper::RxOkTracedCallback, Ptr<const Packet>, double, WifiMode, WifiPreamble);

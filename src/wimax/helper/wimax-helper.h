...
public:
   ...
   */
  static void EnableAsciiForConnection (Ptr<OutputStreamWrapper> oss, uint32_t nodeid,
   ...
   */
  NetDeviceContainer Install (NodeContainer c, NetDeviceType type, PhyType phyType,
   ...
   * depending on the type parameter) is attached to the shared input channel.
   */
  NetDeviceContainer Install (NodeContainer c, NetDeviceType deviceType, PhyType phyType,
   ...
   * depending on the type parameter) is attached to the shared input channel.
   */
  Ptr<WimaxNetDevice> Install (Ptr<Node> node, NetDeviceType deviceType, PhyType phyType,
                               Ptr<WimaxChannel> channel, SchedulerType schedulerType);
...
private:
   ...
   */
  virtual void EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool explicitFilename,
                                   bool promiscuous);

...
enum ServiceFlow::SchedulingType
ServiceFlow::GetServiceSchedulingType (void) const
{
  return m_schedulingType;
}
uint32_t
...
ServiceFlow::GetCsSpecification (void) const
{
...
ServiceFlow::ToTlv (void) const
{
  ...
  tmpSfVector.Add (Tlv (SfVectorTlvValue::Maximum_Sustained_Traffic_Rate, 4,
                        U32TlvValue (m_maxSustainedTrafficRate)));
  tmpSfVector.Add (
      Tlv (SfVectorTlvValue::Maximum_Traffic_Burst, 4, U32TlvValue (m_maxTrafficBurst)));
  tmpSfVector.Add (Tlv (SfVectorTlvValue::Minimum_Reserved_Traffic_Rate, 4,
                        U32TlvValue (m_minReservedTrafficRate)));
  tmpSfVector.Add (Tlv (SfVectorTlvValue::Minimum_Tolerable_Traffic_Rate, 4,
                        U32TlvValue (m_minTolerableTrafficRate)));
  tmpSfVector.Add (
      Tlv (SfVectorTlvValue::Service_Flow_Scheduling_Type, 1, U8TlvValue (m_schedulingType)));
  tmpSfVector.Add (Tlv (SfVectorTlvValue::Request_Transmission_Policy, 4,
  ...
  tmpSfVector.Add (Tlv (SfVectorTlvValue::Fixed_length_versus_Variable_length_SDU_Indicator, 1,
...
bool
ServiceFlow::CheckClassifierMatch (Ipv4Address srcAddress, Ipv4Address dstAddress, uint16_t srcPort,
                                   uint16_t dstPort, uint8_t proto) const
{
  return m_convergenceSublayerParam.GetPacketClassifierRule ().CheckMatch (srcAddress, dstAddress,

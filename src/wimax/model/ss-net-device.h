...
public:
   ...
   */
  void SendBurst (uint8_t uiuc, uint16_t nrSymbols, Ptr<WimaxConnection> connection,
...
private:
   ...
   * \returns true if successful
   */
  bool DoSend (Ptr<Packet> packet, const Mac48Address &source, const Mac48Address &dest,
  ...
  Time
      m_lostDlMapInterval; ///< in milliseconds, time since last received DL-MAP before downlink synchronization is considered lost, maximum is 600
  Time
      m_lostUlMapInterval; ///< in milliseconds, time since last received UL-MAP before uplink synchronization is considered lost, maximum is 600
  ...
  Time
      m_intervalT2; ///< in seconds, wait for broadcast ranging timeout, i.e., wait for initial ranging opportunity
  Time
      m_intervalT3; ///< in milliseconds, ranging Response reception timeout following the transmission of a ranging request
  ...
  Time
      m_intervalT21; ///< in seconds, time the SS searches for (decodable) DL-MAP on a given channel

...
TypeId
BaseStationNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::BaseStationNetDevice")
          ...
          .AddAttribute ("IntervalT8", "Wait for DSA/DSC Acknowledge timeout. Maximum 300ms.",
                         TimeValue (Seconds (0.05)),
                         MakeTimeAccessor (&BaseStationNetDevice::GetIntervalT8,
                                           &BaseStationNetDevice::SetIntervalT8),
                         MakeTimeChecker ())

          .AddAttribute ("RangReqOppSize", "The ranging opportunity size in symbols",
                         UintegerValue (8),
          ...
          .AddAttribute ("BwReqOppSize", "The bandwidth request opportunity size in symbols",
                         UintegerValue (2),
          ...
          .AddAttribute ("SSManager", "The ss manager attached to this device.", PointerValue (),
                         MakePointerAccessor (&BaseStationNetDevice::GetSSManager,
                                              &BaseStationNetDevice::SetSSManager),
                         MakePointerChecker<SSManager> ())

          .AddAttribute ("Scheduler", "The BS scheduler attached to this device.", PointerValue (),
                         MakePointerAccessor (&BaseStationNetDevice::GetBSScheduler,
                                              &BaseStationNetDevice::SetBSScheduler),
                         MakePointerChecker<BSScheduler> ())

          .AddAttribute ("LinkManager", "The link manager attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("UplinkScheduler", "The uplink scheduler attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("ServiceFlowManager", "The service flow manager attached to this device.",
                         PointerValue (),
...
BaseStationNetDevice::StartDlSubFrame (void)
{
  ...
  Simulator::Schedule (Seconds (m_nrDlSymbols * m_symbolDuration.GetSeconds ()),
                       &BaseStationNetDevice::EndDlSubFrame, this);
}
...
BaseStationNetDevice::EndDlSubFrame (void)
{
  ...
  Simulator::Schedule (Seconds (GetTtg () * m_psDuration.GetSeconds ()),
                       &BaseStationNetDevice::StartUlSubFrame, this);
}
...
BaseStationNetDevice::StartUlSubFrame (void)
{
  ...
  Simulator::Schedule (Seconds (m_nrUlSymbols * m_symbolDuration.GetSeconds ()),
                       &BaseStationNetDevice::EndUlSubFrame, this);
}
...
BaseStationNetDevice::EndUlSubFrame (void)
{
  ...
  Simulator::Schedule (Seconds (GetRtg () * m_psDuration.GetSeconds ()),
                       &BaseStationNetDevice::EndFrame, this);
}
...
bool
BaseStationNetDevice::DoSend (Ptr<Packet> packet, const Mac48Address &source,
                              const Mac48Address &dest, uint16_t protocolNumber)
{
  ...
  if (protocolNumber == 2048)
    {
      serviceFlow = m_bsClassifier->Classify (packet, GetServiceFlowManager (),
...
bool
BaseStationNetDevice::Enqueue (Ptr<Packet> packet, const MacHeaderType &hdrType,
      ...
      if (cid != GetInitialRangingConnection ()->GetCid () &&
          cid != GetBroadcastConnection ()->GetCid ())
        {
          if (m_serviceFlowManager->GetServiceFlow (cid) != 0)
            {
              modulationType = GetBurstProfileManager ()->GetModulationType (
                  diuc, WimaxNetDevice::DIRECTION_DOWNLINK);
            }
          else
            {
              modulationType = GetBurstProfileManager ()->GetModulationType (
...
BaseStationNetDevice::CreateUlMap (void)
{
  ...
  for (std::list<OfdmUlMapIe>::iterator iter = uplinkAllocations.begin ();
       iter != uplinkAllocations.end (); ++iter)
    {
  ...
  for (std::list<OfdmUlMapIe>::iterator iter = uplinkAllocations.begin ();
       iter != uplinkAllocations.end (); ++iter)
    {
      ...
      MarkUplinkAllocationEnd ((symbolsToAllocation + uplinkAllocation.GetDuration ()) *
                                   m_symbolDuration,
                               uplinkAllocation.GetCid (), uplinkAllocation.GetUiuc ());
    }
}
...
{
  Simulator::Schedule (allocationEndTime, &BaseStationNetDevice::UplinkAllocationEnd, this, cid,
                       uiuc);
}

...
WimaxNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::WimaxNetDevice")
          ...
          .AddAttribute ("Phy", "The PHY layer attached to this device.", PointerValue (),
          ...
          .AddAttribute ("RTG", "receive/transmit transition gap.", UintegerValue (0),
                         MakeUintegerAccessor (&WimaxNetDevice::GetRtg, &WimaxNetDevice::SetRtg),
                         MakeUintegerChecker<uint16_t> (0, 120))

          .AddAttribute ("TTG", "transmit/receive transition gap.", UintegerValue (0),
                         MakeUintegerAccessor (&WimaxNetDevice::GetTtg, &WimaxNetDevice::SetTtg),
                         MakeUintegerChecker<uint16_t> (0, 120))

          .AddAttribute ("ConnectionManager", "The connection manager attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("BurstProfileManager",
                         "The burst profile manager attached to this device.", PointerValue (),
          ...
          .AddAttribute ("BandwidthManager", "The bandwidth manager attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("InitialRangingConnection", "Initial ranging connection", PointerValue (),
                         MakePointerAccessor (&WimaxNetDevice::m_initialRangingConnection),
                         MakePointerChecker<WimaxConnection> ())

          .AddAttribute ("BroadcastConnection", "Broadcast connection", PointerValue (),
...
WimaxNetDevice::WimaxNetDevice (void) : m_state (0), m_symbolIndex (0), m_ttg (0), m_rtg (0)
{
  InitializeChannels ();
...
uint32_t
WimaxNetDevice::GetNrFrames (void) const
{
...
bool
WimaxNetDevice::SendFrom (Ptr<Packet> packet, const Address &source, const Address &dest,

...
SubscriberStationNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::SubscriberStationNetDevice")
          ...
          .AddAttribute ("BasicConnection", "Basic connection", PointerValue (),
                         MakePointerAccessor (&SubscriberStationNetDevice::m_basicConnection),
                         MakePointerChecker<WimaxConnection> ())

          .AddAttribute ("PrimaryConnection", "Primary connection", PointerValue (),
          ...
          .AddAttribute ("IntervalT1", "Wait for DCD timeout. Maximum is 5*maxDcdInterval",
          ...
          .AddAttribute ("IntervalT7", "wait for DSA/DSC/DSD Response timeout. Maximum is 1s",
                         TimeValue (Seconds (0.1)),
                         MakeTimeAccessor (&SubscriberStationNetDevice::GetIntervalT7,
                                           &SubscriberStationNetDevice::SetIntervalT7),
                         MakeTimeChecker ())

          .AddAttribute ("IntervalT12", "Wait for UCD descriptor.Maximum is 5*MaxUcdInterval",
                         TimeValue (Seconds (10)),
          ...
          .AddAttribute ("SSScheduler", "The ss scheduler attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("LinkManager", "The ss link manager attached to this device.",
                         PointerValue (),
          ...
          .AddAttribute ("Classifier", "The ss classifier attached to this device.",
                         PointerValue (),
...
SubscriberStationNetDevice::InitSubscriberStationNetDevice (void)
{
  ...
  m_intervalT2 = Seconds (
      5 *
      2); // shall be 5 * RangingInterval, if ranging interval=see T2 at page 638) means Initial Ranging Interval=see page 637)
...
bool
SubscriberStationNetDevice::DoSend (Ptr<Packet> packet, const Mac48Address &source,
...
bool
SubscriberStationNetDevice::Enqueue (Ptr<Packet> packet, const MacHeaderType &hdrType,
...
void
SubscriberStationNetDevice::SendBurst (uint8_t uiuc, uint16_t nrSymbols,
                                       Ptr<WimaxConnection> connection,
      ...
      if (cid == GetBroadcastConnection ()->GetCid () && !fragmentation)
          ...
          switch (msgType.GetType ())
            {
              case ManagementMessageType::MESSAGE_TYPE_DL_MAP: {
                ...
                m_linkManager->ScheduleScanningRestart (m_lostDlMapInterval, EVENT_LOST_DL_MAP,
                ...
                m_linkManager->ScheduleScanningRestart (m_intervalT1, EVENT_DCD_WAIT_TIMEOUT, false,
                ...
                m_linkManager->ScheduleScanningRestart (m_intervalT12, EVENT_UCD_WAIT_TIMEOUT, true,
              ...
              case ManagementMessageType::MESSAGE_TYPE_DCD: {
                ...
                if (m_dcdWaitTimeoutEvent.IsRunning ())
                  {
                    Simulator::Cancel (m_dcdWaitTimeoutEvent);
                    m_linkManager->ScheduleScanningRestart (m_intervalT1, EVENT_DCD_WAIT_TIMEOUT,
                                                            false, m_dcdWaitTimeoutEvent);
                  }
              ...
              case ManagementMessageType::MESSAGE_TYPE_UCD: {
                ...
                if (m_ucdWaitTimeoutEvent.IsRunning ())
                  {
                    Simulator::Cancel (m_ucdWaitTimeoutEvent);
                    m_linkManager->ScheduleScanningRestart (m_intervalT12, EVENT_UCD_WAIT_TIMEOUT,
                                                            true, m_ucdWaitTimeoutEvent);
                  }
                ...
                if (GetState () == SS_STATE_ACQUIRING_PARAMETERS)
                  {
                    ...
                    m_linkManager->ScheduleScanningRestart (m_intervalT2,
                                                            EVENT_RANG_OPP_WAIT_TIMEOUT, false,
      ...
      else if (GetConnectionManager ()->GetConnection (cid)) // transport connection
        {
          ...
          if (!fragmentation)
            {
              m_ssRxTrace (packet);
              ForwardUp (packet, m_baseStationId,
              ...
              if (fc == 2)
                {
                  ...
                  ForwardUp (fullPacket, m_baseStationId,
                             GetMacAddress ()); // source shall be BS's address or sender SS's?
                }
              else
                {
          ...
          if (ulMapIe.GetUiuc () ==
...

            {
              ...
              Simulator::Schedule (timeToAllocation, &SSLinkManager::SendRangingRequest,
                                   m_linkManager, ulMapIe.GetUiuc (), ulMapIe.GetDuration ());
            }
          else if (ulMapIe.GetUiuc () == OfdmUlBurstProfile::UIUC_REQ_REGION_FULL) // unicast poll

            {

              Simulator::Schedule (timeToAllocation, &BandwidthManager::SendBandwidthRequest,
                                   GetBandwidthManager (), ulMapIe.GetUiuc (),
                                   ulMapIe.GetDuration ());
            }
...

            {
              ...
              Simulator::Schedule (timeToAllocation, &SubscriberStationNetDevice::SendBurst, this,
                                   ulMapIe.GetUiuc (), ulMapIe.GetDuration (), connection,
...
SubscriberStationNetDevice::ProcessUcd (const Ucd &ucd)
{
  ...
  m_linkManager->SetRangingCW (
      (uint8_t) std::pow ((double) 2, (double) ucd.GetRangingBackoffStart ()) -

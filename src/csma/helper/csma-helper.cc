...
void
CsmaHelper::EnableAsciiInternal (Ptr<OutputStreamWrapper> stream, std::string prefix,
  ...
  if (!stream)
    {
      ...
      //
      asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<CsmaNetDevice> (device, "MacRx",
                                                                            theStream);

...
RandomWalk2dOutdoorMobilityModel::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::RandomWalk2dOutdoorMobilityModel")
          ...
          .AddAttribute ("Time", "Change current direction and speed after moving for this delay.",
          ...
          .AddAttribute ("Mode",
                         ...
                         MakeEnumChecker (RandomWalk2dOutdoorMobilityModel::MODE_DISTANCE,
                                          "Distance", RandomWalk2dOutdoorMobilityModel::MODE_TIME,
                                          "Time"))
          .AddAttribute ("Direction", "A random variable used to pick the direction (radians).",
      ...
      if (outdoor)
        {
          m_event = Simulator::Schedule (
              delayLeft, &RandomWalk2dOutdoorMobilityModel::DoInitializePrivate, this);
        }
      else
        {
          ...
          m_event = Simulator::Schedule (delay, &RandomWalk2dOutdoorMobilityModel::AvoidBuilding,
                                         this, delayLeft - delay, nextPosition);
        }
    }
  else
    {
      ...
      if ((*bit)->IsIntersect (currentPosition, nextPosition))
        {
          ...
          auto intersection = CalculateIntersectionFromOutside (currentPosition, nextPosition,
                                                                (*bit)->GetBoundaries ());
  ...
  while (nextWouldBeInside && iter < m_maxIter)
    {
      ...
      if (!outdoor)
        {
          NS_LOG_LOGIC ("inside loop intersect "
                        << intersectPosition << " nextPosition " << nextPosition << " " << outdoor
      ...
      if (!outdoor)
        {
          ...
          NS_FATAL_ERROR ("Not able to find an outdoor position. Try to increase the attribute "
                          "MaxIterations and check the position of the buildings in the scenario.");
        }
      else
        {
          NS_LOG_LOGIC ("inside loop intersect " << intersectPosition << " nextPosition "

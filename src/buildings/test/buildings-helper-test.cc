...
BuildingData::BuildingData ()
    : xmin (0), xmax (0), ymin (0), ymax (0), zmin (0), zmax (0), nrx (0), nry (0), nf (0)
{
}
...
BuildingsHelperOneTestCase::BuildingsHelperOneTestCase (PositionInBuilding pib, BuildingData bd)
    : TestCase (BuildNameString (pib, bd)), m_pib (pib), m_bd (bd)
{
}
  ...
  if (m_pib.indoor)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ ((uint32_t) buildingInfo->GetRoomNumberX (), m_pib.rx,
                             "x room number mismatch");
      NS_TEST_ASSERT_MSG_EQ ((uint32_t) buildingInfo->GetRoomNumberY (), m_pib.ry,
...
BuildingsHelperTestSuite::BuildingsHelperTestSuite () : TestSuite ("buildings-helper", UNIT)
{

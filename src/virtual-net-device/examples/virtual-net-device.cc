...
class Tunnel
{
   ...
   */
  bool
  N0VirtualSend (Ptr<Packet> packet, const Address &source, const Address &dest,
   ...
   */
  bool
  N1VirtualSend (Ptr<Packet> packet, const Address &source, const Address &dest,
                 uint16_t protocolNumber)
  {
    NS_LOG_DEBUG ("Send to " << m_n3Address << ": " << *packet);
   ...
   */
  bool
  N3VirtualSend (Ptr<Packet> packet, const Address &source, const Address &dest,
                 uint16_t protocolNumber)
  {
   ...
   */
  void
  N3SocketRecv (Ptr<Socket> socket)
  {
   ...
   */
  void
  N0SocketRecv (Ptr<Socket> socket)
  {
    Ptr<Packet> packet = socket->Recv (65535, 0);
   ...
   */
  void
  N1SocketRecv (Ptr<Socket> socket)
  {
    Ptr<Packet> packet = socket->Recv (65535, 0);
...
public:
   ...
   */
  Tunnel (Ptr<Node> n3, Ptr<Node> n0, Ptr<Node> n1, Ipv4Address n3Addr, Ipv4Address n0Addr,
          Ipv4Address n1Addr)
      : m_n3Address (n3Addr), m_n0Address (n0Addr), m_n1Address (n1Addr)
  {
    ...
    ipv4->AddAddress (
        i, Ipv4InterfaceAddress (Ipv4Address ("11.0.0.254"), Ipv4Mask ("255.255.255.0")));
    ipv4->SetUp (i);
  }
};
...
main (int argc, char *argv[])
{
  ...
  Tunnel tunnel (c.Get (3), c.Get (0), c.Get (1), i3i2.GetAddress (0), i0i2.GetAddress (0),
                 i1i2.GetAddress (0));

  ...
  for (uint32_t i = 0; i < test.m_loadProfiles.size (); i++)
    {
      ...
      if (test.VariableLoadTest (test.m_loadProfiles[i].loads, test.m_loadProfiles[i].timeStamps,
...
bool
BatteryLifetimeTest::VariableLoadTest (std::vector<double> loads, std::vector<Time> timeStamps,
                                       Time expLifetime)
{
  ...
  for (uint32_t i = 1; i < loads.size (); i++)
    {
      Simulator::Schedule (timeStamps[i], &WifiRadioEnergyModel::SetIdleCurrentA, wifiDevicePtr,
                           loads[i]);
    }

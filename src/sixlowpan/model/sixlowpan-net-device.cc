...
TypeId
SixLowPanNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::SixLowPanNetDevice")
          ...
          .AddAttribute ("Rfc6282", "Use RFC6282 (IPHC) if true, RFC4944 (HC1) otherwise.",
                         BooleanValue (true), MakeBooleanAccessor (&SixLowPanNetDevice::m_useIphc),
                         MakeBooleanChecker ())
          .AddAttribute (
              "OmitUdpChecksum", "Omit the UDP checksum in IPHC compression.", BooleanValue (true),
              MakeBooleanAccessor (&SixLowPanNetDevice::m_omitUdpChecksum), MakeBooleanChecker ())
          .AddAttribute (
          ...
          .AddAttribute (
              ...
              MakeTimeChecker ())
          .AddAttribute ("CompressionThreshold", "The minimum MAC layer payload size.",
                         UintegerValue (0x0),
                         MakeUintegerAccessor (&SixLowPanNetDevice::m_compressionThreshold),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute (
              "ForceEtherType", "Force a specific EtherType in L2 frames.", BooleanValue (false),
              MakeBooleanAccessor (&SixLowPanNetDevice::m_forceEtherType), MakeBooleanChecker ())
          .AddAttribute ("EtherType", "The specific EtherType to be used in L2 frames.",
                         UintegerValue (0xFFFF),
                         MakeUintegerAccessor (&SixLowPanNetDevice::m_etherType),
                         MakeUintegerChecker<uint16_t> ())
          .AddAttribute ("UseMeshUnder", "Use a mesh-under routing protocol.", BooleanValue (false),
                         MakeBooleanAccessor (&SixLowPanNetDevice::m_meshUnder),
                         MakeBooleanChecker ())
          .AddAttribute ("MeshUnderRadius", "Hops Left to use in mesh-under.", UintegerValue (10),
                         MakeUintegerAccessor (&SixLowPanNetDevice::m_meshUnderHopsLeft),
                         MakeUintegerChecker<uint8_t> ())
          .AddAttribute ("MeshCacheLength", "Length of the cache for each source.",
                         UintegerValue (10),
...
SixLowPanNetDevice::SixLowPanNetDevice () : m_node (0), m_netDevice (0), m_ifIndex (0)
{
...
void
SixLowPanNetDevice::ReceiveFromDevice (Ptr<NetDevice> incomingPort, Ptr<const Packet> packet,
                                       ...
                                       PacketType packetType)
{
          ...
          if (hopsLeft == 0)
          ...
          else
            {
              ...
              Simulator::Schedule (Time (MilliSeconds (m_meshUnderJitter->GetValue ())),
                                   &NetDevice::Send, m_netDevice, sendPkt,
                                   m_netDevice->GetBroadcast (), protocol);
            }
  ...
  if (!m_promiscRxCallback.IsNull ())
    {
      m_promiscRxCallback (this, copyPkt, Ipv6L3Protocol::PROT_NUMBER, realSrc, realDst,
                           packetType);
    }
...
uint32_t
SixLowPanNetDevice::GetIfIndex (void) const
{
  NS_LOG_FUNCTION (this);
  return m_ifIndex;
}
...
Ptr<Channel>
SixLowPanNetDevice::GetChannel (void) const
{
  NS_LOG_FUNCTION (this);
...
Address
SixLowPanNetDevice::GetAddress (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
uint16_t
SixLowPanNetDevice::GetMtu (void) const
{
  NS_LOG_FUNCTION (this);
...
bool
SixLowPanNetDevice::IsLinkUp (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::IsBroadcast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
Address
SixLowPanNetDevice::GetBroadcast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::IsMulticast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::IsPointToPoint (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::IsBridge (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::SendFrom (Ptr<Packet> packet, const Address &src, const Address &dest,
...
bool
SixLowPanNetDevice::DoSend (Ptr<Packet> packet, const Address &src, const Address &dest,
...
Ptr<Node>
SixLowPanNetDevice::GetNode (void) const
{
  NS_LOG_FUNCTION (this);
  return m_node;
}
...
bool
SixLowPanNetDevice::NeedsArp (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_netDevice, "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
...
bool
SixLowPanNetDevice::DecompressLowPanIphc (Ptr<Packet> packet, Address const &src,
                                          Address const &dst)
{
  NS_LOG_FUNCTION (this << *packet << src << dst);
  ...
  if (encoding.GetNh ())
    {
      ...
      if (dispatchVal == SixLowPanDispatch::LOWPAN_UDPNHC)
        ...
        {
          std::pair<uint8_t, bool> retval = DecompressLowPanNhc (
              packet, src, dst, ipHeader.GetSource (), ipHeader.GetDestination ());
...
uint32_t
SixLowPanNetDevice::CompressLowPanNhc (Ptr<Packet> packet, uint8_t headerType, Address const &src,
                                       Address const &dst)
{
...
SixLowPanNetDevice::DecompressLowPanNhc (Ptr<Packet> packet, Address const &src, Address const &dst,
                                         Ipv6Address srcAddress, Ipv6Address dstAddress)
{
...
void
SixLowPanNetDevice::DecompressLowPanUdpNhc (Ptr<Packet> packet, Ipv6Address saddr,
                                            Ipv6Address daddr)
{
  NS_LOG_FUNCTION (this << *packet);
...
void
SixLowPanNetDevice::DoFragmentation (Ptr<Packet> packet, uint32_t origPacketSize,
...
bool
SixLowPanNetDevice::ProcessFragment (Ptr<Packet> &packet, Address const &src, Address const &dst,
                                     bool isFirst)
{
  NS_LOG_FUNCTION (this << *packet);
  ...
  if (m_timeoutEventList.empty ())
    {
      m_timeoutEvent = Simulator::Schedule (m_fragmentExpirationTimeout,
                                            &SixLowPanNetDevice::HandleTimeout, this);
    }
...
void
SixLowPanNetDevice::HandleTimeout (void)
{
...
void
SixLowPanNetDevice::AddContext (uint8_t contextId, Ipv6Prefix contextPrefix,
...
bool
SixLowPanNetDevice::GetContext (uint8_t contextId, Ipv6Prefix &contextPrefix,
                                bool &compressionAllowed, Time &validLifetime)
{

...
BridgeNetDevice::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::BridgeNetDevice")
          ...
          .AddAttribute ("Mtu", "The MAC-level Maximum Transmission Unit", UintegerValue (1500),
                         MakeUintegerAccessor (&BridgeNetDevice::SetMtu, &BridgeNetDevice::GetMtu),
                         MakeUintegerChecker<uint16_t> ())
          .AddAttribute ("EnableLearning", "Enable the learning mode of the Learning Bridge",
                         BooleanValue (true),
                         MakeBooleanAccessor (&BridgeNetDevice::m_enableLearning),
                         MakeBooleanChecker ())
          .AddAttribute ("ExpirationTime", "Time it takes for learned MAC state entry to expire.",
                         TimeValue (Seconds (300)),
                         MakeTimeAccessor (&BridgeNetDevice::m_expirationTime), MakeTimeChecker ());
  return tid;
}

BridgeNetDevice::BridgeNetDevice () : m_node (0), m_ifIndex (0)
{
...
void
BridgeNetDevice::ReceiveFromDevice (Ptr<NetDevice> incomingPort, Ptr<const Packet> packet,
...
BridgeNetDevice::AddBridgePort (Ptr<NetDevice> bridgePort)
{
  ...
  m_node->RegisterProtocolHandler (MakeCallback (&BridgeNetDevice::ReceiveFromDevice, this), 0,
                                   bridgePort, true);
...
bool
BridgeNetDevice::SendFrom (Ptr<Packet> packet, const Address &src, const Address &dest,

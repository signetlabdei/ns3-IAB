...
PrintPosition (Ptr<Node> client, Ptr<Node> server)
{
  ...
  oss << "Set up Server Device " << (server->GetDevice (0))->GetAddress () << " with ip "
      << ipv4AddrServer << " position (" << serverPos.x << "," << serverPos.y << "," << serverPos.z
      << ")";

  oss << "Set up Client Device " << (client->GetDevice (0))->GetAddress () << " with ip "
      << ipv4AddrClient << " position (" << clientPos.x << "," << clientPos.y << "," << clientPos.z
      << ")"
      << "\n";
  return oss.str ();
}
                 ...
                 const MobilityHelper &mobility)
{
  ...
  mobil.SetPositionAllocator ("ns3::GridPositionAllocator", "MinX", DoubleValue (0.0), "MinY",
  ...
  if (m_enableMobility && m_enableRouting)
    {
      ...
      mobil.SetMobilityModel ("ns3::RandomDirection2dMobilityModel", "Bounds",

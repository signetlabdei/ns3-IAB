...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("downlink", "Generate downlink flows if set to 1, uplink flows otherwise",
                downlink);
  ...
  cmd.AddValue ("dlAckType",
                ...
                dlAckSeqType);
  cmd.AddValue ("enableUlOfdma", "Enable UL OFDMA (useful if DL OFDMA is enabled and TCP is used)",
                enableUlOfdma);
  cmd.AddValue ("enableBsrp", "Enable BSRP (useful if DL and UL OFDMA are enabled and TCP is used)",
                enableBsrp);
  cmd.AddValue (
          ...
          for (int gi = 3200; gi >= 800;) //Nanoseconds
            {
              ...
              wifi.ConfigHeOptions ("GuardInterval", TimeValue (NanoSeconds (gi)), "MpduBufferSize",
              ...
              if (phyModel == "Spectrum")
                {
                  ...
                  */
                  ...
                  if (dlAckSeqType != "NO-OFDMA")
                    {
                      mac.SetMultiUserScheduler ("ns3::RrMultiUserScheduler", "EnableUlOfdma",

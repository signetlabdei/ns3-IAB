...
main (int argc, char *argv[])
{
  ...
  iabPositionAlloc->Add (Vector (10.0, 0.0, 0.0));
  iabPositionAlloc->Add (Vector (20.0, 0.0, 0.0));
  ...
  ulAppParams.startTime = MilliSeconds (100);
  ulAppParams.endTime = Seconds (1); // Some cooldown time
  ulAppParams.IPI = MilliSeconds (20); // On average, a packet every 200 ms
  ...
  auto iabMmw0 = DynamicCast<MmWaveIabNetDevice> (iabMmWaveDev.Get (0));
  iabMmw0->GetMtRrc ()->TraceConnectWithoutContext (
      "ConnectionEstablished", MakeBoundCallback (&ConnectionEstablishedCallback));
  
  auto iabMmw1 = DynamicCast<MmWaveIabNetDevice> (iabMmWaveDev.Get (1));
  iabMmw1->GetMtRrc ()->TraceConnectWithoutContext (
      "ConnectionEstablished", MakeBoundCallback (&ConnectionEstablishedCallback));

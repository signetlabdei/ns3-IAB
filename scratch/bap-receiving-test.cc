...
static void
BapReceptionCallback (BapHeader bapHeader, bool isPdu)
{
  NS_LOG_FUNCTION ("Received: "<<isPdu);
...
{
  NS_ABORT_MSG_IF (m_receivedPacketCounter!=2 , "Received "<< (uint32_t) m_receivedPacketCounter<<" packets, instead of 2 packets.");
}
...
main (int argc, char *argv[])
{
  ...
  enbMmw->GetBap ()->TraceConnectWithoutContext ("RxPacket", MakeBoundCallback (&BapReceptionCallback));

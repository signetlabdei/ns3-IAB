...
title: |    
       ...
       | **a brief User Guide**
author: |    
        | SIGNET Lab., Department of Information Engineering 
...
The script `parse_results_vs_scale.py` (P) illustrates how to parse and analyze the simulation results. If you wish to generate the same plots as per-configured, i.e., same x-axis parameter and y-axis metric, just with different parameters (for instance, additional scenario scaling values), it is enough to modify the corresponding parameters in the R script. The P script should parse all the values automatically.

On the other hand, if you wish to change the x- and/or y-axis parameter/metric, first iterate through the values of the chosen input parameter. Then, compute a metric for each such value ([here](#metric_list) you can find a list of the available functions). Finally, generate the plot you are interested in using the collection of metric and parameter values. 
For instance, you can iterate through the `scalingFactor` values and load the corresponding throughput via:
```python
thr = [] 
  for scale in aListOfScalingFactors:
  key = {
    'scalingFactor' : scale
  }
  results = campaign_relays.db.get_results(key)
  y = np.array(get_e2e_avg_throughput(campaign_relays, results, \
...
Please note that with the above code *all the results* referring to any simulation run with `scalingFactor` = `scale` are loaded. If you wish to fix any other parameters, specify its value in `key` as a parameter name and value pair.

## List of metric functions {#metric_list}

| Name | Description |
| --- | --- |
| `get_e2e_avg_throughput` | Returns a `List` which contains the average APP layer throughput achieved by each UE for each each simulation run |
| `get_e2e_system_throughput` | Returns a `List` which contains the average APP layer system throughput of each simulation run |
| `get_phy_system_throughput` | Returns the average system throughput measured at the PHY layer |
| `get_e2e_avg_latency` | Returns a `List` which contains the average APP layer latency of each simulation run |
| `get_e2e_latency_samples` | Returns a `List` which contains the APP layer latency of each received packet of each simulation run |
| `getSinrSamples` | Returns a `List` of SINRs experienced in the various simulation runs |
